...
- `internal/platform/postgres` owns its narrow `DB` port, settings and binding resolution, connection construction, and PostgreSQL telemetry. It does not own a generic transaction lifecycle facade; Network Flow owns its private transaction helper over `postgres.DB`.
