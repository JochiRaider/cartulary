...
- `make agent-finalize` is the required first end-of-run maintenance command before broader verification. It runs duration baseline maintenance when `RESULTS_DIR=<successful run dir>` is supplied, then runs `phase-schedules`, `phase-schedule-drift`, and `json-shape-check`; report whether it ran unchanged, updated generated artifacts, skipped duration baseline refresh because `RESULTS_DIR` was unset, failed at a subtarget, or was explicitly skipped.
