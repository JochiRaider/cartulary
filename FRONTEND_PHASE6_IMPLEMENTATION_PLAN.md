...
- `tools/frontend_phase_registry.json` marks `FE-P6` as `planned` with `row_rollup_state=activation_ready` and activation blocker `frontend_phase_not_active`.
- `tools/frontend_phase_maps/fe_p6_test_map.json` contains exactly five FE-P6 rows. `FE-U-P6-01`, `FE-I-P6-01`, `FE-E-P6-01`, `FE-V-P6-01`, and `FE-A11Y-P6-01` have `claim_status=implemented`.
...
- Latest accessibility preflight retained evidence lists `FE-A11Y-P6-01` as blocked-row smoke with `required_for_closure=false`. It did not close FE-P6 accessibility readiness.
- Fresh Sprint 6 audit accessibility evidence at `.cartulary/test-results/20260607T171427Z-p2892902/browser-e2e-a11y/accessibility/frontend-accessibility-summary.json` has schema `cartulary.frontend_accessibility_summary.v2`, includes `FE-A11Y-P6-01`, status `pass`, the exact mapped scenario status `pass`, passing keyboard/state/contrast checks, and no violations.
- Fresh Sprint 6 audit row-accounting evidence at `.cartulary/test-results/20260607T171427Z-p2892902/browser-e2e-a11y/frontend-row-accounting.json` has schema `cartulary.frontend_row_accounting.v3`, includes `FE-A11Y-P6-01`, matches the current registry, guide, and FE-P6 map digests, and records `closure_status=closed`.
...
Current FE-P6 row status is activation-ready while the phase remains planned. The table records the live map status, closure posture, and direct evidence posture.
...
| `FE-V-P6-01` | `implemented`; closed by Sprint 5 mapped visual evidence | `design_direction` | `make browser-e2e-visual` | UI/UX design guide evidence affordance direction; `docs/design.md`; visual golden maintenance guide; `FE-VFIX-05` fixture identity | `FE-V-P6-01 Capture evidence count, affordance, available, requested, pending, blocked, failed, inconsistent, preview, and download-handle state fixtures.` | none | current row-owned visual accounting tied to `FE-VFIX-05`, with evidence count, access affordance, available, requested, pending, blocked, failed, inconsistent, preview, and download-handle state fixtures |
| `FE-A11Y-P6-01` | `implemented`; closed by Sprint 6 mapped accessibility evidence | `design_direction` | `make browser-e2e-a11y` | UI/UX design guide accessibility direction; `docs/design.md`; frontend accessibility harness mechanics | `FE-A11Y-P6-01 Verify evidence icon buttons, blocked states, error states, preview controls, and download controls have names, focus, contrast, and non-color-only distinctions.` | none | current row-owned accessibility summary and v3 row accounting from direct `browser-e2e-a11y`; preflight smoke remains non-closure evidence |
...
- [x] Sprint 6: Implement accessibility readiness for `FE-A11Y-P6-01`, treating preflight as blocked smoke unless the row is promoted or remapped.
...
Sprint 5 close-out, 2026-06-07:

- Binary recommendation and accepted result: `PROMOTE FE-V-P6-01`.
- `FE-V-P6-01` is promoted to `claim_status=implemented` in `tools/frontend_phase_maps/fe_p6_test_map.json`, with `evidence_class=design_direction`, `claim_publication_intent=none`, `closure_scope=scenario`, `required_for_closure=true`, no Core IDs, and no blockers.
- `FE-VFIX-05` remains the single visual fixture owned by `FE-P6` and `FE-V-P6-01`. It now points to the exact mapped FE-P6 scenario title and the two row-owned FE-P6 visual goldens: `fe-v-p6-01-evidence-affordance-states-linux.png` and `fe-v-p6-01-timeline-evidence-count-linux.png`.
- The FE-P6 visual scenario in `apps/web/e2e/workbook.visual.spec.ts` uses real workbook Evidence and Timeline UI. It exercises requested, pending, quarantined/blocked, available preview, available download, unsupported preview, failed-handle, inconsistent-handle, and Timeline evidence-count states.
- The failed-handle and inconsistent-handle states are induced through the token-guarded public-error fault route for deterministic `blob_failed` and `evidence_inconsistent` handle messages. This remains visual readiness and does not create product-conformance or Core 05 claim-publication evidence.
- Visual golden maintenance was followed for recapture. `docs/guides/cartulary_visual_golden_maintenance.md` now records the FE-P6 fixture seed, viewport, masks, screenshot scopes, and golden names. A separate current-profile visual maintenance citation records the required `V-5-GRID-01` managed-object golden refresh that was needed for the canonical visual target.
- `docs/testing/frontend_phase_coverage_ledgers/fe_p6_coverage_ledger.md` was regenerated through `make phase-ledgers`; `tools/frontend_phase_registry.json` digests were refreshed while FE-P6 remained `planned` and `partially_implemented` because `FE-A11Y-P6-01` remains blocked.
- Fresh `make browser-e2e-visual` passed with run root `.cartulary/test-results/20260607T153029Z-p2765006`. `browser-e2e-visual/frontend-row-accounting.json` records `FE-V-P6-01` with `claim_status_at_run=implemented`, `evidence_class=design_direction`, `target_mapping_status=mapped`, `closure_status=closed`, and the exact mapped FE-V scenario title.
...
`CLOSED: FE-V-P6-01 visual row is promoted and closed by current mapped visual evidence tied to FE-VFIX-05. Remaining FE-P6 blocker is FE-A11Y-P6-01.`
...
Sprint 6 close-out, 2026-06-07:

- Binary result: `FE-A11Y-P6-01` is promoted and closed by fresh current row-owned accessibility evidence from mapped `browser-e2e-a11y`.
- `tools/frontend_phase_maps/fe_p6_test_map.json` now records `FE-A11Y-P6-01` as `claim_status=implemented`, `evidence_class=design_direction`, `claim_publication_intent=none`, `closure_scope=scenario`, `required_for_closure=true`, mapped to `make browser-e2e-a11y`, and with no blockers.
- The exact mapped scenario in `apps/web/e2e/workbook.a11y.spec.ts` covers evidence requested, pending, blocked/quarantined, available preview, available download, unsupported preview, failed-handle, and inconsistent-handle states.
- Production DOM semantics were limited to accessibility readiness: evidence access messages now expose live-region semantics, using `role=status` with polite announcements for normal progress and success and `role=alert` with assertive announcements for failed, unavailable, inconsistent, blocked, unsupported preview, or public-error messages. Generic mutation value controls now expose selected-field labels.
- `docs/testing/frontend_phase_coverage_ledgers/fe_p6_coverage_ledger.md` was regenerated through `make phase-ledgers`; `tools/frontend_phase_registry.json` digests were refreshed while FE-P6 remained `planned`, moved to `row_rollup_state=activation_ready`, and kept activation blocker `frontend_phase_not_active`.
- Fresh `make browser-e2e-a11y` passed with run root `.cartulary/test-results/20260607T164746Z-p2862083`. `browser-e2e-a11y/accessibility/frontend-accessibility-summary.json` records schema `cartulary.frontend_accessibility_summary.v2`, status `pass`, row `FE-A11Y-P6-01`, the exact mapped scenario status `pass`, and no violations.
- The same run's `browser-e2e-a11y/frontend-row-accounting.json` records schema `cartulary.frontend_row_accounting.v3`, target `browser-e2e-a11y`, target status `pass`, `FE-A11Y-P6-01` with `claim_status_at_run=implemented`, `evidence_class=design_direction`, `target_mapping_status=mapped`, `closure_status=closed`, and the exact mapped scenario title.
- Sprint 6 validation passed: `make phase-ledgers` at `.cartulary/test-results/20260607T163053Z-p2829757`, `make phase-map-check` at `.cartulary/test-results/20260607T165437Z-p2876204`, `make json-shape-check` at `.cartulary/test-results/20260607T165436Z-p2875741`, `make phase-ledger-drift` at `.cartulary/test-results/20260607T165436Z-p2875730`, `make frontend-typecheck` at `.cartulary/test-results/20260607T164736Z-p2861678`, `make browser-e2e-a11y` at `.cartulary/test-results/20260607T164746Z-p2862083`, `git diff --check`, and `env -u RESULTS_DIR make agent-finalize` at `.cartulary/test-results/20260607T165445Z-p2876723`.
- Retained-run maintenance was skipped by `agent-finalize` because `RESULTS_DIR` was unset. No successful full warm `make check` run root was supplied for retained evidence maintenance.
- `browser-e2e-a11y-preflight` was not used as closure evidence. Broad `make check`, generated ledgers, stale retained artifacts, and preflight success were not counted as closure evidence.
- `FE-A11Y-P6-01` remains design-direction evidence only. It does not close product conformance, implementation support, FE-P6 activation, Core 05 claim-publication readiness, or broad repository readiness.

Sprint 6 row-owned evidence audit, 2026-06-07:

- Binary recommendation: `ACCEPT FE-A11Y-P6-01`.
- Audit scope was limited to `FE-A11Y-P6-01` accessibility-readiness evidence under the current FE-P6 map. The audit did not count product-conformance evidence, implementation-support evidence, Core 05 claim-publication evidence, generated ledgers, broad `make check`, fixture or scenario names by themselves, stale retained artifacts, or preflight success as closure.
- Source truth was aligned: `tools/frontend_phase_maps/fe_p6_test_map.json` records `FE-A11Y-P6-01` as `claim_status=implemented`, `evidence_class=design_direction`, `claim_publication_intent=none`, `closure_scope=scenario`, no blockers, required target `browser-e2e-a11y`, `frontend_row_accounting_required=true`, and the exact mapped scenario title.
- `docs/guides/cartulary_frontend_implementation_testing_guide.md` and `docs/testing-harness-nlspec.md` still define `browser-e2e-a11y` as implemented accessibility closure evidence and `browser-e2e-a11y-preflight` as blocked-row smoke only.
- Fresh `make explain-target TARGET=browser-e2e-a11y DETAIL=summary` passed before the evidence run and confirmed `browser-e2e-a11y` is a public full-gate target with expected retained artifacts.
- Fresh `make browser-e2e-a11y` passed with run root `.cartulary/test-results/20260607T171427Z-p2892902`, 14 tests, 0 failures, and 0 missing scenarios.
- The current source digests used for freshness were `tools/frontend_phase_registry.json` digest `93192aaa5bd3df4638f9ba824b8a78a3afde0120bee8a3ae73e8094321d5c8b0`, frontend guide digest `76431c802578d17038f2867e0ad0822caeacbdb70a8bc59fe27831088c5bbd74`, and FE-P6 map digest `a5e22761102ee798aa7a855aea2480200e278c85f6715bc572e9e876ad70af24`.
- `.cartulary/test-results/20260607T171427Z-p2892902/browser-e2e-a11y/accessibility/frontend-accessibility-summary.json` records schema `cartulary.frontend_accessibility_summary.v2`, status `pass`, row `FE-A11Y-P6-01` as implemented `design_direction`, exact mapped scenario status `pass`, passing keyboard, state-communication, and contrast checks, and no blocking violations for the row.
- `.cartulary/test-results/20260607T171427Z-p2892902/browser-e2e-a11y/frontend-row-accounting.json` records schema `cartulary.frontend_row_accounting.v3`, target `browser-e2e-a11y`, command `cartulary.harness.command.browser_e2e_a11y.v1`, target status `pass`, phase namespace `frontend`, matching current registry/guide/FE-P6 map digests, row `FE-A11Y-P6-01` with `claim_status_at_run=implemented`, `evidence_class=design_direction`, `target_mapping_status=mapped`, `closure_status=closed`, empty `failure_reason`, and the exact mapped closing scenario title.
- `browser-e2e-a11y-preflight` remained excluded from closure and was mentioned only as blocked-row smoke. Broad `make check`, generated ledgers, retained stale artifacts, fixture names, scenario-title text alone, and claim-publication evidence were also excluded from closure.
- No conflict was found between the live FE-P6 map, guide, direct accessibility summary, or row-accounting artifact. No blocker was recorded.
...
Commands not required for Sprint 6 accessibility promotion:

- `make browser-e2e-a11y-preflight`, because preflight is blocked-row smoke only and was not closure evidence for `FE-A11Y-P6-01`.
- `make check`, because Sprint 6 closure came from direct row-owned `browser-e2e-a11y` evidence and the required completion gate was `env -u RESULTS_DIR make agent-finalize`.
- `make generate-drift`, because Sprint 6 did not touch generated or contract surfaces.
- `make generated-artifact-policy-check`, because Sprint 6 did not touch generated roots or generated policy inputs.
- `make phase-schedule-drift`, because Sprint 6 did not affect schedules.
- Product-conformance targets beyond the requested drift/typecheck gates, because Sprint 6 changed only design-direction accessibility readiness and did not alter the already closed product-conformance rows.
...
- `FE-V-P6-01` has Sprint 5 row-owned visual accounting tied to `FE-VFIX-05`; registry status and golden filenames alone remain insufficient.
...
- `FE-A11Y-P6-01` is mapped to implemented accessibility evidence in `make browser-e2e-a11y`.
- Row-owned evidence from the implemented accessibility target exists.
