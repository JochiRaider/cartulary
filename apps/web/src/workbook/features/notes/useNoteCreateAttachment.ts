...
import { noteCreateFeature, noteCreateView } from "./noteCreateModel";

const noSubscribe = () => () => {};
const noSnapshot = () => null;
export function useNoteCreateAttachment(
  subject: WorkbookInspectorLiveRowBinding | null,
  open = true,
) {
  const context = useContext(NoteCreateContext);
  const token = useRef(Symbol("note-inspector")).current;
  ...
  const workflow: InspectorRelatedRecordWorkflowState | null =
    draft?.origin.subject && draft.origin.feature && state?.attachment === token
      ? {
          ...
          draft: { ...draft.values },
          targetContract: requireViewContract(noteCreateView),
  ...
  const begin = useCallback(
    (feature: InspectorFeatureGroup) => {
      if (feature.featureGroupKey !== noteCreateFeature) return false;
