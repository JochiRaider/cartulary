...
function batchPlan(recordId = "row", clear = false): WorkbookBatchPlan {
  return {
    ...
    request: {
      ...(clear
        ? {
            kind: "clear_cells_v1" as const,
            field_keys: ["timeline.activity_synopsis_text"] as [
              "timeline.activity_synopsis_text",
            ],
          }
        : {
            kind: "fill_down_v1" as const,
...
describe("Workbook batch operation ownership", () => {
  it("projects one logical batch through uncertainty conflicts refresh and completion", async () => {
    for (const clear of [false, true]) {
      const plan = (recordId = "row") => batchPlan(recordId, clear);
  ...
  it("captures once, distinguishes delivery from repetition, and preserves original targets", async () => {
    for (const clear of [false, true]) {
      ...
      const attempt = required(
        required(owner.getSnapshot().entries[0]).attempt,
      );
  ...
  it("retains overlapping actions and explicitly replays the identical uncertain attempt", async () => {
    for (const clear of [false, true]) {
      ...
      owner.retry(first);
      await settle();
      expect(required(vi.mocked(transport.send).mock.calls[2])[0]).toBe(
        captured,
      );
