...
describe("WorkbookCollaborationCoordinator", () => {
  ...
  it("accounts for detached Evidence and Task Party versions before transaction suppression in either HTTP order", () => {
    for (const view of [
      ...
      for (const order of ["http_first", "socket_first"]) {
        const fixture = projectionFixture();
        const owner = fixture.mutationRuntime.explicitPatches;
        owner.setAuthority({
          actorId: "actor",
          incidentId: fixture.mutationRuntime.scope.incidentId,
          sessionIdentity: "session",
        ...
        const resolved = vi.fn(() => true);
        fixture.projection.registerClientTxnResolver(resolved);
        if (order === "http_first") owner.acceptVersion("indicator", 5);
        fixture.emit({
          kind: "message",
          message: {
            ...serverEnvelope,
        ...
        expect(owner.latestVersion("indicator")).toBe(
          order === "http_first" ? 5 : 4,
        );
        expect(observed.mock.invocationCallOrder.at(-1)).toBeLessThan(
          resolved.mock.invocationCallOrder[0] ?? 0,
        );
        if (order === "socket_first") owner.acceptVersion("indicator", 5);
        expect(owner.latestVersion("indicator")).toBe(5);
        expect(owner.getSnapshot().entries).toEqual([]);
        fixture.projection.dispose();
      }
  });
  it("accounts for Indicator socket versions before transaction suppression in either HTTP order", () => {
    for (const order of ["http_first", "socket_first"]) {
      const fixture = projectionFixture();
      const owner = fixture.mutationRuntime.indicatorLifecycle;
      const observed = vi.spyOn(owner, "acceptVersion");
      const resolved = vi.fn(() => true);
      fixture.projection.registerClientTxnResolver(resolved);
      if (order === "http_first") owner.acceptVersion("indicator", 5);
  ...
  it("accounts for observation affected record versions before transaction suppression in either HTTP order", () => {
    for (const order of ["http_first", "socket_first"]) {
      const fixture = projectionFixture();
      const owner = fixture.mutationRuntime.indicatorObservations;
      owner.setAuthority({
        actorId: "actor",
        incidentId: fixture.mutationRuntime.scope.incidentId,
        sessionIdentity: "session",
