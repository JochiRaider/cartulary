...
test("Verify saved-view create/update/select/default UI uses active surface scope and public saved-view/workbook-preference contracts.", async ({
  page,
}) => {
  const incidentId = await createIncident(
    page,
...
test("workbook startup falls back to Timeline for an unsupported explicit sheet", async ({
  page,
}) => {
  const incidentId = await createIncident(
    page,
...
test("browser Timeline sort, filter, and group controls submit stable query keys", async ({
  page,
}) => {
  const incidentId = await createIncident(
    page,
