...
test("pastes a representative 20x5 Timeline clipboard range", async ({
  page,
}) => {
...
test("groups paste conflicts and preserves selection continuity", async ({
  page,
}) => {
  const incidentId = await createIncident(
    page,
...
test("Workbook inspector fills the shell work area across row counts", async ({
  page,
}) => {
...
test("Party create and link preserve raw text on the workbook surface", async ({
  page,
}) => {
  const incidentId = await createIncident(
    page,
...
test("assessment workflow keeps invalid timestamp drafts local", async ({
  page,
}) => {
  const incidentId = await createIncident(
    page,
...
test("Verify Timeline inspector Workflow create-related actions stay in the workbook shell.", async ({
  page,
  workerAdmin,
}) => {
  const incidentId = await createIncident(
...
test("coordination workbook workflows stay native", async ({
  page,
  workerAdmin,
}) => {
  const incidentId = await createIncident(
...
test("optional standardized surfaces are workbook-native when exposed", async ({
  page,
}) => {
  const incidentId = await createIncident(
    page,
  ...
  await setGenericCreateField(page, "finding.kind", "hypothesis");
  await setGenericCreateField(page, "finding.confidence_score", "72");
  await page
  ...
  await setGenericCreateField(page, "investigative_query.platform", "Kusto");
  await setGenericCreateField(
    page,
  ...
  await setGenericCreateField(page, "forensic_keyword.pattern", "7z\\.exe");
  await setGenericCreateField(
    page,
    "forensic_keyword.reason",
    "Archive utility execution",
  );
  await setGenericCreateField(page, "forensic_keyword.match_mode", "regex");
  await setGenericCreateField(page, "forensic_keyword.case_sensitive", "true");
  await page
...
test("required registry identities stay canonical with optional additions", async ({
  page,
}) => {
  const incidentId = await createIncident(
    page,
...
test("Verify Task Requests, Decisions, Parties, Communications Log, Handoff, Status Review, and Lesson open inside the same workbook shell and retain view controls.", async ({
  page,
}) => {
  const incidentId = await createIncident(
    page,
