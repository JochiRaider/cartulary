...
test("Verify history and rollback preview/action use public route contracts, preserve retained history, and render public error envelopes.", async ({
  page,
}) => {
...
test("Verify inspector Details, Relationships, Evidence, History, rollback, and destructive-action authorization through public browser routes.", async ({
  browser,
  page,
  sessionTracker,
}) => {
...
test("Verify default-closed inspector state, no-row state, surface switch config changes, saved-view switch over the same view_schema_id keeps the same config, closed incident read-only behavior, server-denied action behavior, and Timeline create/edit/paste without opening the inspector.", async ({
  browser,
  page,
  sessionTracker,
}) => {
...
async function disableWorkbookSockets(page: Page) {
  await page.addInitScript(() => {
    class InspectorClosedWebSocket {
      ...
      readonly url: string;
      readyState = InspectorClosedWebSocket.CONNECTING;
      ...
      close() {
        this.readyState = InspectorClosedWebSocket.CLOSED;
    ...
    Object.defineProperty(window, "WebSocket", {
      configurable: true,
      value: InspectorClosedWebSocket,
    });
  });
}
