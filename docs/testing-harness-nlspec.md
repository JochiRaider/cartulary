...
| `cartulary.scheduler_pressure_summary.v2`       | `tools/schemas/cartulary.scheduler_pressure_summary.v2.schema.json`       | present           | Scheduler reporter       | Before scheduler target success.          |
...
| Scheduler pressure summary                           | Scheduler reporter                              | `<target>/pressure-summary.json`                                | `cartulary.scheduler_pressure_summary.v2`                      | Closed current-profile fields are defined below; ordering is target, lane, resource, fixture-class, row fixture pressure, execution-family fixture pressure, fixture proofs, and slowest-work lexical order after producer timestamps are normalized | Retained.                                                    |
...
Fixture-tier lowering MUST be backed by a closed proof model before selected evidence is relabeled. A `transaction` proof MUST show the symbol uses a rollback-scoped store fixture or another bounded transaction boundary and has no required cross-connection committed-state visibility. A `package_reset` proof MUST include an FK-closed `dirty_tables[]`, no schema mutation, no process restart dependency, and no object-store residue. A `group_clone` proof MUST name the shared seeded state and prove that grouped symbols do not require per-test isolation. A retained `template_clone` proof MUST name the clone-only boundary being preserved, such as process lifecycle, schema mutation, database identity, destructive residue, unsafe object-store effects, or unproven cleanup. Proof diagnostics are harness evidence only; they MUST NOT reduce selected product rows, enable scheduler reuse for product tests, or make old retained artifacts satisfy current validation.
...
Warm scheduler health checks MAY consume retained timing artifacts from a successful warm-ready run. Such checks MUST remain harness-maintenance evidence and MUST NOT be described as claim-bearing product benchmark evidence. When a warm `check` artifact is evaluated, the check MUST fail if default local `check-service-backed` includes ordinary browser measurement work, if hidden provisioning prevents warm eligibility, if `check-service-backed` exceeds the configured warm budget, or if non-isolated backend/browser peer lanes exceed the configured balance ratio by more than the bounded materiality floor. Unless the caller supplies a different value, the supported WSL2 hard warm-maintenance budget for `check-service-backed` is `240000ms`, the balance ratio is `1.25`, and the materiality floor is `5000ms`.
...
| TH-HARNESS-AC-031 | Section 8         | Scheduler diagnostic artifact closure | Current scheduler artifact families plus positive and negative pressure-summary fixtures | Schema/artifact policy checker and schema fixture validation | Success only when `pressure-summary.json` is schema-owned by present `cartulary.scheduler_pressure_summary.v2`, validates before scheduler success, and rejects unknown or missing required fields | Bounded report | Empty on success; bounded diagnostic on mismatch | Artifact policy report naming pressure-summary status and schema fixture results | Missing `cartulary.scheduler_pressure_summary.v2` schema, open top-level pressure-summary fields, or unvalidated scheduler success passes | none |
