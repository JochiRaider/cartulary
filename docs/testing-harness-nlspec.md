...
Default `check` placement MUST be selected by evidence ownership rather than target history. Phase-map rows MUST use the v2 default-check metadata set: `default_check_required`, `default_check_kind`, `default_check_reason_code`, `primary_evidence_owner`, `duplicate_of`, `evidence_delta`, and `warm_local_cost_class`. Rows whose default inclusion is not self-evident MUST also declare `default_check_reason`. `default_check_required=true` is only valid for implemented, executable rows in the current harness. Planned or blocked rows that describe future local-check placement MUST use `future_default_check_candidate=true` instead of current default-check membership.
