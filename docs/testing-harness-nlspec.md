...
| `scripts/harness-contract.sh`                            | yes for wrapper behavior, no for implementation path | Stable Make-owned compatibility wrapper for harness contract subcommands; reusable CLI implementation belongs under `tools/harness/core`. |
...
Browser shard manifest verification MUST carry exact selected scenario identity for each selected Playwright shard entry. The shard-selected artifact MUST use `cartulary.playwright_manifest_selection.v1` and MUST identify every selected executable scenario by row ID, normalized Playwright file, and exact title. Per-shard run verification MUST compare only the selected scenario titles for that shard and phase. `CARTULARY_MANIFEST_SELECTED_IDS` MAY remain as a compatibility fallback for older non-sharded or artifact-less paths, but row ID alone MUST NOT be the primary browser-shard selection contract when exact selected-test artifacts are available. Row ID remains the evidence owner, and frontend row accounting remains aggregate/title-based under `cartulary.frontend_row_accounting.v3`.
Verified by: TH-HARNESS-AC-001, TH-HARNESS-AC-016
