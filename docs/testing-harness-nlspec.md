...
Frontend readiness mechanics introduced by `browser-e2e-visual`, `browser-e2e-a11y`, `browser-e2e-a11y-preflight`, `frontend-evidence-audit`, `tools/frontend_phase_registry.json`, `tools/frontend_phase_maps/*.json`, `tools/frontend_visual_fixture_registry.json`, `docs/testing/frontend_phase_coverage_ledgers/*.md`, `cartulary.frontend_phase_registry.v3`, `cartulary.frontend_phase_test_map.v3`, `cartulary.frontend_row_accounting.v3`, `cartulary.frontend_visual_fixture_registry.v3`, `cartulary.frontend_accessibility_summary.v2`, `cartulary.frontend_accessibility_preflight_summary.v1`, and `cartulary.frontend_claim_publication_review.v1` are harness and implementation-readiness mechanics only. They MUST NOT define Core product behavior, MUST NOT promote visual or accessibility evidence into product-conformance evidence, and MUST NOT activate Core 05 claim-publication review unless a claim-bearing publication predicate is active.
...
| Frontend row-accounting artifact generation and validation. | `frontend_row_accounting` | Phase-accounting frontend row-accounting sub-boundary. | `owner_facade` | `cartulary.frontend_row_accounting.v3`, row closure semantics, selected-row scope validation, retained artifact path `frontend-row-accounting.json`, and target-summary failure injection. |
...
The following legacy scheduler catch-all helper paths are `unsupported_private` in the current profile. They MUST NOT be recreated as stable compatibility shims:

| Path | Owner facade or successor | Required removal condition |
| --- | --- | --- |
| `tools/harness/scheduler/adapters/backend.mjs` | `backend_shard_plan` | Remove when scheduler callers import the backend shard-plan facade directly and phase-slice/service-backed scheduler characterization passes. |
| `tools/harness/scheduler/adapters/schedule-context.mjs` | Execution summary topology and backend target-plan facades. | Remove when scheduler callers import the owning execution/backend facades directly and scheduler summary/context characterization passes. |
| `tools/harness/scheduler/check-service-backed-expansion.mjs` | `service_backed_schedule_planning` | Remove when service-backed expansion callers use `tools/harness/execution/service-backed/schedule-planning.mjs` and service-backed scheduler characterization passes. |
| `tools/harness/scheduler/service-backed-schedule-manifest.mjs` | `service_backed_schedule_planning` | Remove when manifest callers use the service-backed planning facade and schema/topology fixtures pass. |
| `tools/harness/scheduler/service-backed-schedule-topology.mjs` | `service_backed_schedule_planning` | Remove when topology callers use the service-backed planning facade and generated topology drift passes. |
| `tools/harness/scheduler/phase-slice-plan.mjs` | `phase_slice_planning` | Remove when phase-slice callers use the phase-accounting facade and phase-slice JSON characterization passes. |
| `tools/harness/scheduler/phase-slice-cli.mjs` | `phase_slice_planning` | Remove when Make/task-surface inputs use the phase-accounting CLI path and phase-slice public target behavior is preserved. |
| `tools/harness/scheduler/execution-dependencies.mjs` | Execution topology helper facade. | Remove when phase-accounting, diagnostics, generated-artifact, and service-backed callers import `tools/harness/execution/execution-dependencies.mjs`. |
| `tools/harness/scheduler/scheduler/process-executor.mjs` | `scheduler_process_execution` | Remove when scheduler engine imports `tools/harness/scheduler/process-executor.mjs` and process-execution characterization passes. |
| `tools/harness/scheduler/duration-baseline-cli.mjs` | `scheduler_duration_accounting` | Remove when duration-accounting CLIs import the duration-accounting helper path. |
| `tools/harness/scheduler/duration-drift.mjs` | `scheduler_duration_accounting` | Remove when backend, browser, finalization, and duration-accounting callers use the duration-accounting facade/helper path. |
| `tools/harness/scheduler/target-duration-baselines.mjs` | `scheduler_duration_accounting` | Remove when duration baseline readers use the duration-accounting helper path and duration baseline fixtures pass. |
| `tools/harness/scheduler/service-backed-make-target-durations-cli.mjs` | `scheduler_duration_accounting` | Remove when Make/task-surface inputs use the duration-accounting CLI path and service-backed duration baseline behavior is preserved. |
| `tools/harness/scheduler/harness-smoke-durations-cli.mjs` | `scheduler_duration_accounting` | Remove when Make/task-surface inputs use the duration-accounting CLI path and harness-smoke duration baseline behavior is preserved. |
| `tools/harness/scheduler/duration-baseline-drift-suite.sh` | `scheduler_duration_accounting` | Remove when Make/task-surface inputs use the duration-accounting suite path and retained-run duration drift behavior is preserved. |
| `tools/harness/scheduler/scheduler-event-order-drift-cli.mjs` | `scheduler_evidence_drift` | Remove when Make/task-surface inputs use the diagnostics CLI path and event-order drift characterization passes. |
| `tools/harness/scheduler/scheduler-summary-timing-drift-cli.mjs` | `scheduler_evidence_drift` | Remove when Make/task-surface inputs use the diagnostics CLI path and summary timing drift characterization passes. |

The following legacy frontend catch-all helper paths are `unsupported_private` in the current profile. They MUST NOT be recreated as stable compatibility shims:

| Path | Owner facade or successor | Required removal condition |
| --- | --- | --- |
| `tools/harness/frontend/accessibility-summary-cli.mjs` | `browser_accessibility_evidence` | Remove when browser a11y callers use the browser owner path and accessibility summary characterization passes. |
| `tools/harness/frontend/build-web-artifact.sh` | `web_build_artifact` | Remove when Make/cache inputs use the readiness owner path and build-web characterization passes. |
| `tools/harness/frontend/design-token-cli.mjs` | `design_token_generation` | Remove when generated-artifact generation uses the design-token owner path and generated drift passes. |
| `tools/harness/frontend/design-tokens.mjs` | `design_token_generation` | Remove when design-token loader callers use the generated-artifact owner facade and generated drift passes. |
| `tools/harness/frontend/design/index.mjs` | `design_token_generation` | Remove when static-analysis design contracts use the generated-artifact owner facade. |
| `tools/harness/frontend/embed-web-assets.sh` | `web_build_artifact` | Remove when embedded web asset Make/cache inputs use the readiness owner path and build characterization passes. |
| `tools/harness/frontend/evidence/index.mjs` | `frontend_phase_accounting`, `frontend_row_accounting`, `frontend_evidence_audit` | Remove when diagnostics, generated-artifact, browser, phase-accounting, and release-evidence callers use owner facades. |
| `tools/harness/frontend/evidence/test-output-indexes.mjs` | `frontend_evidence_audit` | Remove when test-output Vitest and Playwright runners use the test-output owner path. |
| `tools/harness/frontend/font-bundle-check-cli.mjs` | `font_asset_validation` | Remove when font validation callers use the static-analysis owner path. |
| `tools/harness/frontend/frontend-evidence-audit-cli.mjs` | `frontend_evidence_audit` | Remove when `frontend-evidence-audit` uses the phase-accounting owner path and retained-root audit characterization passes. |
| `tools/harness/frontend/frontend-install.sh` | `frontend_toolchain_readiness` | Remove when Make/cache inputs use the readiness owner path and install characterization passes. |
| `tools/harness/frontend/frontend-phase-manifest.mjs` | `frontend_phase_accounting` | Remove when phase-map, diagnostics, browser, generated-artifact, and release-evidence callers use the phase-accounting owner path. |
| `tools/harness/frontend/frontend-phase-slice-plan.mjs` | `frontend_phase_accounting` | Remove when scheduler slice planning uses the phase-accounting owner path. |
| `tools/harness/frontend/frontend-row-accounting.mjs` | `frontend_row_accounting` | Remove when test-output target-summary callers use the phase-accounting owner path and row-accounting characterization passes. |
| `tools/harness/frontend/frontend-toolchain.sh` | `frontend_toolchain_readiness` | Remove when Make/cache inputs use the readiness owner path and toolchain characterization passes. |
| `tools/harness/frontend/readiness/index.mjs` | `frontend_phase_accounting` | Remove when scheduler callers use the phase-accounting readiness facade. |
| `tools/harness/frontend/run-frontend-unit.sh` | `frontend_target_execution` | Remove when Make/task-surface/scheduler callers use the execution owner path and frontend-unit characterization passes. |
| `tools/harness/frontend/run-vitest-manifest-phase.sh` | `vitest_execution_diagnostics` | Remove when Vitest manifest-phase callers use the execution owner path. |
| `tools/harness/frontend/run-vitest-phase.sh` | `vitest_execution_diagnostics` | Remove when Vitest phase callers use the execution owner path. |
| `tools/harness/frontend/vitest-failure-details.mjs` | `vitest_execution_diagnostics` | Remove when frontend-unit uses the diagnostics owner path and sidecar characterization passes. |
...
| `frontend-fallow-static` | `cartulary.harness.command.frontend_fallow_static.v1` | `static_analysis_security` | `helper_only` | `summary_with_artifacts` | `cartulary.tool_run_summary.v3` | `evidence_normalization` (Section 8), `failure_normalization` (Section 9) | `retained_artifacts` | `public_active` | Phase A helper-only Fallow open static analysis. Emits `cartulary.fallow_static_summary.v1`; Fallow Runtime and Fallow security scans are not selected. |
...
| `browser-e2e-a11y-preflight` | `cartulary.harness.command.browser_e2e_a11y_preflight.v1` | `browser_e2e` | `release-check` | `summary_with_artifacts` | `cartulary.tool_run_summary.v3` | `service_lifecycle` (Section 11), `evidence_normalization` (Section 8), `failure_normalization` (Section 9) | `retained_artifacts`, `service_start`, `service_resource_mutation`, `runtime_reset` | `public_active` | Emits `cartulary.frontend_accessibility_preflight_summary.v1` for blocked future-row smoke and explicitly mapped preflight row-accounting scenarios. Release-check consumes it only where mapped by frontend row accounting. |
...
`frontend-fallow-static` accepts no target-local Make variables in the current Phase A profile. A future changed-code audit base such as `FALLOW_CHANGED_SINCE` MUST be added to this registry before it becomes public input.
...
| `frontend-evidence-audit` | `CHECK_RESULTS_DIR` | `result_selector` | yes | Make command line, environment, Makefile default | none | missing required input | invalid | `path_token` | existing retained full warm `check` run root | `usage_error`, exit `2` | value | runtime env |
| `frontend-evidence-audit` | `BROWSER_SUPPORT_RESULTS_DIR` | `result_selector` | yes | Make command line, environment, Makefile default | none | missing required input | invalid | `path_token` | existing retained `browser-e2e-support` run root | `usage_error`, exit `2` | value | runtime env |
| `frontend-evidence-audit` | `BROWSER_VISUAL_RESULTS_DIR` | `result_selector` | yes | Make command line, environment, Makefile default | none | missing required input | invalid | `path_token` | existing retained `browser-e2e-visual` run root | `usage_error`, exit `2` | value | runtime env |
| `frontend-evidence-audit` | `BROWSER_A11Y_RESULTS_DIR` | `result_selector` | yes | Make command line, environment, Makefile default | none | missing required input | invalid | `path_token` | existing retained `browser-e2e-a11y` run root | `usage_error`, exit `2` | value | runtime env |
| `frontend-evidence-audit` | `BROWSER_A11Y_PREFLIGHT_RESULTS_DIR` | `result_selector` | no | Make command line, environment, Makefile default | none | omitted unless selected phase requires preflight closure | omitted | `path_token` | existing retained `browser-e2e-a11y-preflight` run root | `usage_error`, exit `2` when required and missing | value | runtime env |
...
| `cartulary.scheduler_pressure_summary.v1`       | `tools/schemas/cartulary.scheduler_pressure_summary.v1.schema.json`       | present           | Scheduler reporter       | Before scheduler target success.          |
...
| `cartulary.frontend_phase_registry.v3`          | `tools/schemas/cartulary.frontend_phase_registry.v3.schema.json`          | present           | Frontend phase registry validation | During JSON shape checks, phase-map checks, phase-schedule generation, and frontend freshness validation. |
...
Retained-run readers for historical artifact shapes are diagnostic-only compatibility aids. A historical reader MAY remain when it has a named investigation value and fixture coverage, but it MUST be isolated from current-conformance validation and MUST identify the legacy shape when surfaced to users. Unsupported fallback readers MAY be removed instead of carried forward; current conformance MUST be proven from current schema-owned artifacts, not from best-effort legacy inference.
...
`browser-e2e-a11y-preflight` MAY emit blocked future-row smoke evidence and explicitly mapped preflight row-accounting scenarios as `cartulary.frontend_accessibility_preflight_summary.v1`. That preflight artifact is implementation-support readiness evidence only and MUST NOT mark blocked rows complete, satisfy frontend phase completion by itself, or replace `cartulary.frontend_accessibility_summary.v2` for rows mapped to `browser-e2e-a11y`. Axe-style reports, Playwright traces, DOM snapshots, screenshots, browser console logs, and preflight smoke summaries MAY be retained as diagnostic inputs, but they MUST NOT replace the normalized accessibility summary schema or required `cartulary.frontend_row_accounting.v3`.
...
The retained `cartulary.frontend_visual_fixture_registry.v1` and `cartulary.frontend_visual_fixture_registry.v2` schemas are historical/diagnostic only. Current registry validation and row-closure freshness use `cartulary.frontend_visual_fixture_registry.v3`.
...
| Scheduler pressure summary                           | Scheduler reporter                              | `<target>/pressure-summary.json`                                | `cartulary.scheduler_pressure_summary.v1`                      | Closed current-profile fields are defined below; ordering is target, lane, resource, fixture-class, and slowest-work lexical order after producer timestamps are normalized | Retained.                                                    |
...
| Frontend accessibility preflight summary             | Browser accessibility preflight target          | `browser-e2e-a11y-preflight/accessibility-preflight/frontend-accessibility-preflight-summary.json` | `cartulary.frontend_accessibility_preflight_summary.v1`        | Blocked or explicitly mapped implemented `phase_rows[]`, smoke `scenarios[]`, `violations[]`, and `artifact_refs[]` in schema-defined order | Retained for explicit preflight target only.                 |
| Frontend row accounting                              | Frontend-aware target summaries                 | `<target>/frontend-row-accounting.json`                             | `cartulary.frontend_row_accounting.v3`                         | Accounting scope, command ID, map and registry digests, run root, scenario results, row results, required-target closure, stale/not-closed reasons, and compatibility row/count copies | Retained for target; target/tool-run summaries reference this artifact instead of duplicating row details. |
...
In the current profile, `pressure-summary.json` is a required retained diagnostic artifact with schema-owned field closure. It MUST be a JSON object that validates against `cartulary.scheduler_pressure_summary.v1` before scheduler target success. Current-profile scheduler work reuse is not adopted: `reused_accounting_counts.reused` MUST be `0`, and the scheduler MUST emit truthful nonnegative `executed` and `skipped` counts where those values are derivable from scheduler records. `readiness_attribution_counts` MUST remain an empty object unless a later owner revision adds scheduler-readable readiness source metadata. The schema validates harness diagnostics only; the artifact MUST NOT be cited as product-conformance or Core 05 claim-publication evidence.

| Field | Required type | Meaning | Omission/null rule |
| --- | --- | --- | --- |
| `schema_id` | string | Schema-owned producer marker; value is `cartulary.scheduler_pressure_summary.v1`. | MUST be present and non-null. |
...
| `readiness_attribution_counts` | object | Readiness attribution counts by readiness source key. Keys are non-empty strings and values are nonnegative integers. | MUST be present; an empty object means the current scheduler has no readiness source metadata to attribute. |
...
**TH-HARNESS-REQ-351**
Phase selection MUST include a `phase_namespace` value. The default namespace is `base`. `PHASE=phaseN` without an explicit namespace MUST remain base-only and MUST resolve only through `tools/phase_registry.json`. Frontend phase selection MUST use `PHASE_NAMESPACE=frontend PHASE=FE-P<N>` and MUST resolve only through the contiguous declared phase IDs in `tools/frontend_phase_registry.json`. `task-guide`, `explain-phase`, `phase-slice`, and `service-backed-slice` MUST reject ambiguous or cross-namespace phase identifiers with a bounded usage diagnostic instead of guessing. Executable slice commands MUST reject whole `planned` or `retired` frontend phases before child work. Executable slice commands MAY run explicit selected `ROWS` from a planned frontend phase when every selected row is implemented or stale and belongs to the selected phase or an earlier frontend phase. Explanation commands MAY display `planned` frontend phase metadata when they clearly report the planned/non-executable status.

Current browser readiness, browser duration, and base Playwright selection joins intentionally use the numeric compatibility bridge `FE-P<N>` -> `phase<N>` only to attach frontend readiness rows to existing base-phase browser selection surfaces. That bridge is not namespace equivalence and MUST be revised with targeted tests before any future frontend phase numbering diverges from the current numeric base-phase join.
...
Runtime binary injection is scheduler-owned. The current runtime-binary registry contains exactly one entry:

| ID | Producer target | Producer output variable | Consumer env |
| --- | --- | --- | --- |
| `operator` | `build-operator` | `OPERATOR_BIN` | `CARTULARY_OPERATOR_BIN` |

Phase-map Go rows MAY declare `runtime_binaries=["operator"]`. A scheduler-selected Go shard or process unit that includes such a row MUST depend on the registry producer target before starting the consumer process, and MUST pass only the registry consumer env to that Go process. The public producer input `OPERATOR_BIN` MUST NOT be forwarded to arbitrary child processes. `CARTULARY_OPERATOR_BIN` MUST be forwarded only to Go shard/process units selected from rows that declare `runtime_binaries=["operator"]`.
...
| `browser_stage_*` | `check`, `service_backed`, `phase_slice` | `1` | none | manifest positive integer only | 1 | 8 | display order `135`, then resource name lexical order | use generated default `1` unless manifest declares another positive value |
...
| `service_backed_browser_stack` | Count distinct `browser_stage_*` resource lanes in the normalized provisional work-unit set. If the count is `0`, return `1`. Otherwise return `max(1, min(lane_count, stack_claiming_unit_count when nonzero, process limit when set, max selected CPU limit when set))`, where the selected CPU limit is `host_cpu` for `check` and `go_cpu` for `service_backed` or `phase_slice`. |
...
| TH-HARNESS-AC-031 | Section 8         | Scheduler diagnostic artifact closure | Current scheduler artifact families plus positive and negative pressure-summary fixtures | Schema/artifact policy checker and schema fixture validation | Success only when `pressure-summary.json` is schema-owned by present `cartulary.scheduler_pressure_summary.v1`, validates before scheduler success, and rejects unknown or missing required fields | Bounded report | Empty on success; bounded diagnostic on mismatch | Artifact policy report naming pressure-summary status and schema fixture results | Missing `cartulary.scheduler_pressure_summary.v1` schema, open top-level pressure-summary fields, or unvalidated scheduler success passes | none |
