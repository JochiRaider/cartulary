...
The full-chain fixture MUST apply exactly versions `1..40`, preserving the
immutable `1..29` boundary, from a pristine database. Contamination fixtures
MUST cover pre-existing Cartulary objects,
...
| Vitest execution diagnostics and sidecars. | `vitest_execution_diagnostics` | Execution, test-output, and diagnostics helper boundaries. | `owner_facade` | Vitest row wrappers, exact-title filtering, watchdog handling, `cartulary.vitest_failure_details.v2`, and retained `vitest-failure-details.json` sidecar paths. |
...
| Task-surface or public-target metadata | `make task-surface-report TASK_SURFACE_REPORT_ARGS='--check --all'` and public-target parity checks. |
...
Every target MUST declare in `backing_scripts` each target-level repo-local
script referenced by its rendered Make binding, including a dispatcher added by
a `work_graph` profile or `graph_entry` expansion. A shared runtime helper that
is referenced only through the factored generated runtime MAY be declared once
by its owning surface and omitted from individual target rows. Generation MUST
validate these authored declarations and MUST NOT infer, synthesize, or silently
...
| `postgres-baseline-reset` | `cartulary.harness.command.postgres_baseline_reset.v1` | `local_services_dev` | `helper_only` | `service_summary` | `cartulary.tool_run_summary.v5` | `destructive_safety` (Section 13), `service_lifecycle` (Section 11), `evidence_normalization` (Section 8), `failure_normalization` (Section 9) | `retained_artifacts`, `service_start`, `destructive_cleanup` | `public_active` | Accepts only `POSTGRES_BASELINE_PROFILE=dev\|mvp`; requires `CARTULARY_DESTRUCTIVE_CONFIRM=postgres-baseline-reset` unless dry-run; removes only exact Compose-labeled legacy/current PostgreSQL volumes. |
...
For scheduler work graphs, `command.executable="node"` is a logical command
identity, not a request to search the caller's `PATH`. Before managed service
startup, the graph runner MUST normalize and validate the selected `NODE_BIN`
as an executable file. At process launch, the scheduler executor MUST run that
binary for every logical `node` command. Managed-suite startup and all graph
children MUST receive an environment with the selected binary's directory first
on `PATH`, including after unit and fixture environment overlays, so descendant
Node invocations select the same runtime. The remaining `PATH` entries retain
their order. Missing, invalid, or unlaunchable selected Node MUST produce
`failure_class=config`, `failure_reason=configuration_error`, and public exit
code `2` without falling back to a different Node or starting managed services.
Runtime-specific absolute paths MUST NOT replace logical commands in the work
graph or its semantic digest.
...
| `cartulary.vitest_failure_details.v2`           | `tools/schemas/cartulary.vitest_failure_details.v2.schema.json`           | present           | Vitest wrappers          | Before owner summaries consume failure diagnostics. |
...
Runner-confirmed Vitest test and hook timeouts MUST use `failure_class=timing`, `failure_reason=timeout_failure`, and normalized exit `13`. Playwright per-test timeouts retain `product/test_assertion_failure`; this amendment does not change the Playwright adapter contract. Harness-owned watchdogs, command deadlines, lock deadlines, service readiness deadlines, and cleanup deadlines remain operational failures and MUST use `failure_reason=timeout_failure` or `failure_reason=service_readiness_timeout` according to the failure-reason table.
...
The `browser-unit` Vitest project MUST declare a 15-second per-test timeout. This project exercises jsdom React composition, virtualization, and bounded multi-case interaction matrices while scheduled `frontend-unit` shares the declared host capacity with other graph work; the runner's generic five-second default is not a stable product-failure boundary for that workload. The harness-owned Vitest watchdog remains the outer hung-process boundary, and a runner-confirmed test or hook timeout uses `timing/timeout_failure` with normalized exit `13`. An assertion mismatch remains `product/test_assertion_failure` with normalized exit `10`. The `harness-node` project retains the runner default unless a separately adopted owner rule requires another bound. Individual browser-unit tests MUST NOT add a longer timeout merely to mask an unbounded wait or missing readiness condition.

Canonical Vitest invocations MUST install the harness-owned collector through
supported reporter lifecycle APIs. The collector preserves original serialized
error messages separately from stacks before JSON reporter formatting can lose
the message. A shared parent-side normalizer MUST correlate observations by
project, repository-relative file, and exact full catalog title, validate the
current invocation identity, and retain schema-valid
`cartulary.vitest_failure_details.v2` evidence before accepting completed runner
results. This applies to selected rows and full frontend execution. Parent
writers remain the sole canonical row, unit, and target result writers.

The v2 diagnostic records original errors, failure kind, effective test deadline
when known, actual runner duration, and current-run runner/log references.
Executed row `duration_ms` is the sum of exact selected runner durations rounded
once to integer milliseconds; process wall duration remains separate. Error
kinds distinguish assertion, test timeout, hook timeout, suite/setup failure,
and unknown failure. Timeout recognition uses the pinned runner's original
error signature, never elapsed duration or `STACK_TRACE_ERROR` alone. A marker
without a preserved cause is `unknown/unknown_failure`. Ordinary suite/setup
failure uses `infra/preflight_error`; genuine assertions remain product
failures. Parent watchdog and cancellation classifications are unchanged.
Concurrent failure records MUST all survive normalization and Section 9.1
primary-failure ordering.

Collectors write only invocation-private observations. Before retaining them,
parents MUST validate schema, identity, bounded regular-file inputs, and exact
selector accounting (an exact leaf selector is admitted only when unambiguous), and apply the existing redaction and private-file
permission rules. Missing or malformed required diagnostics are
`artifact/artifact_error`; contradictory or duplicate terminal observations are
`harness/scheduler_accounting_error`. Diagnostic stacks are supporting context,
not evidence that an assertion failed at the test registration site.

V2 replaces v1 atomically for current producers and consumers. Current code MUST
NOT emit, read, translate, or fall back to v1. Required sidecars are retained as
`vitest-failure-details.json` under the frontend raw artifact directory or
`unit-logs/<unit-log-id>/`; selected-row runner JSON is retained beside the
sidecar. Collector, schema, and normalization dependencies participate in cache
identity. Historical artifacts remain historical investigation evidence. Cache hits start no
Vitest invocation and need no newly collected sidecar; execution artifacts MUST
NOT be synthesized from cached success.
...
application MUST NOT expose a debug route as a measurement compatibility path.
...
`make postgres-baseline-reset` MUST accept only the closed `dev` and `mvp`
profiles, resolve only their exact Compose project and legacy/current PostgreSQL
volume labels, and reject ambiguous or unproved resources before mutation. Its
dry-run MUST be inert and list exact targets. A real reset MUST reject before
service or volume mutation unless
`CARTULARY_DESTRUCTIVE_CONFIRM=postgres-baseline-reset` was supplied on the
Make command line. It MUST preserve every non-PostgreSQL volume, create the
exact PostgreSQL 18.6 baseline, and apply the current migration head.
...
Private child-output capture MUST allocate exclusive, bounded storage for each
invocation inside the validated suite-private runtime. The capture runtime owns
this allocation; callers supply semantic execution identity only through existing
execution metadata. Allocation MUST be independent of semantic-name length and
MUST keep concurrent or repeated invocations distinct, including invocations with
identical semantic names. Private allocation names are not public identities.
Existing capture directories MUST be validated without permission repair.

Capture acquisition MUST be guarded from its first owned resource. Partial
setup, spawn and output-read failures MUST attempt closure and removal of every
acquired resource. Files MUST be validated before bounded diagnostic reads.
Cleanup ownership transfers to the consumer only when the complete capture
result is ready. Successful cleanup MUST be idempotent; failed cleanup MUST
attempt remaining releases and remain visible. Invocation cleanup MUST remove
only its owned files and directory, never a borrowed suite root or shared capture
parent. Capture/artifact failures use `artifact_error`; cleanup-only failures use
`cleanup_error`. An earlier primary failure MUST remain primary under Section 9.1.
Scheduler cancellation MUST reap child processes before suite-private cleanup.
...
| TH-HARNESS-AC-001 | Sections 1, 4      | Command registry                 | Current tree                                                                 | `make task-surface-report TASK_SURFACE_REPORT_ARGS='--check --all'` plus registry parity checker    | `0` when registry matches exactly                              | Bounded report                                         | Empty on success                                             | Public target registry parity report                                                               | Extra/missing public target fails                                            | none                                                    |
...
| TH-HARNESS-AC-021 | Sections 5, 8, 10 | Frontend-unit harness stability | Current graph plus delayed jsdom workbook-row, inspector-subject, row-history selector, controlled-input helper, worker-limit, and Vitest failure-sidecar fixtures | Topology contract tests; `make frontend-unit`; capacity-override graph fixture | Success when Vitest workers and graph claims match, shared helpers tolerate bounded hydration, inspector readiness uses stable identity, and required v2 observations retain original messages separately from stacks, exact identity, deadlines, and actual test durations | Bounded summary | Empty on success | Work graph records the worker effect in CPU claims; frontend helper and selector-policy tests pass; both Vitest execution paths retain and validate v2 diagnostics, original errors, redaction, exact title accounting, and current-run references; v1 is rejected | Runner workers exceed claims, helper waits are unbounded, identity is unstable, or v2 evidence is missing, malformed, misattributed, discarded, or replaced by marker/duration inference | none |
...
| TH-HARNESS-AC-058 | Sections 4, 5, 8 | Task-surface ownership and generated Make density | Authored task-surface owner, execution topology, generated task-surface projection, shared Make runtime, thin Make bindings, a graph-entry fixture with its required dispatcher declaration removed, and synthetic-growth fixtures | Task-surface owner/schema validation, public registry and backing-script parity, generation drift, density validation, and public wrapper characterization | Success only when public command metadata has one authored machine owner, topology cannot redefine it, every target-level rendered script is explicitly owner-declared, generated v15 parity holds, unsupported `alias` profiles and per-variable origin transport are rejected, every size/line/growth budget passes, and public target behavior is unchanged | Bounded ownership and density report | Bounded configuration or generated-drift diagnostic | Owner/projection digest relationship plus byte, line-length, repeated-expansion, and synthetic-growth metrics | A generated projection becomes an owner, topology embeds task metadata, a graph-backed target omits its rendered dispatcher from `backing_scripts`, dense global input plumbing returns, budgets regress, or public behavior changes | generated scratch outputs removed |
