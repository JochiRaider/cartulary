...
Legacy retained `service-scope.json` artifacts without `schema_id` MAY be read for investigation, but MUST NOT close current conformance, satisfy current scheduler failure propagation, or replace lifecycle events and leases.
