...
Frontend readiness mechanics introduced by `browser-e2e-a11y`, `browser-e2e-a11y-preflight`, `tools/frontend_phase_registry.json`, `tools/frontend_phase_maps/*.json`, `docs/testing/frontend_phase_coverage_ledgers/*.md`, `cartulary.frontend_accessibility_summary.v2`, and `cartulary.frontend_accessibility_preflight_summary.v1` are harness and implementation-readiness mechanics only. They MUST NOT define Core product behavior, MUST NOT promote visual or accessibility evidence into product-conformance evidence, and MUST NOT activate Core 05 claim-publication review unless a claim-bearing publication predicate is active.
...
Input-stamped scheduler units MUST emit an explicit hit, miss, or bypass outcome in the retained unit log. Scheduler summaries and aggregate run summaries MUST classify stamp hits as `reused` accounting instead of zero-duration `actual` work, and MUST retain enough per-unit cache evidence to diagnose late static work without comparing separate runs. In the current local profile, stamp eligibility is limited to static or drift units whose profiles close over their meaningful inputs, including `lint-shell`, `generate-drift`, `lint-biome`, and `frontend-import-boundary-check`.
Verified by: TH-HARNESS-AC-001, TH-HARNESS-AC-006
...
Backend integration and integration-support target-plan defaults use `template_clone` because ordinary route/runtime coverage needs committed state without coupling unrelated packages through a shared mutable database. Store-layer backend defaults use `transaction` when rollback safety is available. `package_reset` is a compatibility path only: new reusable reset rows SHOULD declare a targeted table set closed over relevant foreign-key dependencies when the touched surface is small and stable, and rows that intentionally depend on broad mutable reset MUST declare an explicit reset reason. Future work SHOULD move safe committed route groups from per-test clones to `group_clone` when grouping reduces cost without reducing fixture clarity.
