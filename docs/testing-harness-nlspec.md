...
Task-surface default inclusion MUST agree with scheduler reachability. A target that lists `check` in `default_inclusion_sets` MUST either run as a direct `check` work unit, be an explicit public aggregate summarized by `check`, or declare a named default-check projection. A projection MUST name its owning schedule and stage, MUST label the evidence class it supplies, and MUST declare when it is not equivalent to the full direct target. `explain-target`, task-surface diagnostics, generated task-surface manifests, and scheduler manifests MUST render the same projection metadata. Support-only internal targets MUST NOT advertise `check` membership unless the check scheduler actually selects them.
...
Default local `make check` MAY satisfy the `browser-e2e-visual` child through a bounded scheduler-only visual smoke group when the full visual stage would exceed the local browser lane budget. Direct `make browser-e2e-visual`, explicit browser aggregates, and CI/release profiles that select full visual evidence MUST retain the full visual target behavior. Default-check accessibility readiness MAY likewise use a bounded scheduler projection only when the direct accessibility target remains available as full-fidelity evidence.
...
| `browser-e2e-a11y` | `cartulary.harness.command.browser_e2e_a11y.v1` | `browser_e2e` | `test`, `check`, `ci` | `summary_with_artifacts` | `cartulary.tool_run_summary.v3` | `service_lifecycle` (Section 11), `evidence_normalization` (Section 8), `failure_normalization` (Section 9) | `retained_artifacts`, `service_start`, `service_resource_mutation`, `runtime_reset` | `public_active` | Emits `cartulary.frontend_accessibility_summary.v2` for implemented accessibility rows only. Default `check` MAY use the bounded accessibility-readiness projection from Section 4; direct target evidence remains full-fidelity. |
...
| `browser-e2e-visual` | `cartulary.harness.command.browser_e2e_visual.v1` | `browser_e2e` | `test`, `check`, `ci` | `summary_with_artifacts` | `cartulary.tool_run_summary.v3` | `service_lifecycle` (Section 11), `evidence_normalization` (Section 8), `failure_normalization` (Section 9) | `retained_artifacts`, `service_start`, `service_resource_mutation`, `runtime_reset` | `public_active` | Runs current-profile `V-*` visual manifest rows and frontend visual readiness rows selected from frontend phase-map titles. Default `check` MAY use the bounded `visual-smoke` projection from Section 4; direct target evidence remains full visual evidence. |
