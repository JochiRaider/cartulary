...
| `browser_stage_*` | `check`, `service_backed`, `phase_slice` | `1` | none | manifest positive integer only | 1 | 8 | display order `135`, then resource name lexical order | use generated default `1` unless manifest declares another positive value; current default-check stateful partitions declare `browser_stage_stateful=2` |
