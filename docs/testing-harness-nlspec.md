...
| `cartulary.scheduler_pressure_summary.v3`       | `tools/schemas/cartulary.scheduler_pressure_summary.v3.schema.json`       | present           | Scheduler reporter       | Before scheduler target success.          |
...
| Scheduler pressure summary                           | Scheduler reporter                              | `<target>/pressure-summary.json`                                | `cartulary.scheduler_pressure_summary.v3`                      | Closed current-profile fields are defined below; ordering is target, lane, resource, fixture-class, row fixture pressure, execution-family fixture pressure, fixture proofs, fixture-tier proof artifacts, and slowest-work lexical order after producer timestamps are normalized | Retained.                                                    |
...
| TH-HARNESS-AC-031 | Section 8         | Scheduler diagnostic artifact closure | Current scheduler artifact families plus positive and negative pressure-summary and fixture-tier proof fixtures | Schema/artifact policy checker and schema fixture validation | Success only when `pressure-summary.json` is schema-owned by present `cartulary.scheduler_pressure_summary.v3`, embeds retained proof objects that validate as `cartulary.fixture_tier_proof.v1`, validates before scheduler success, and rejects unknown or missing required fields | Bounded report | Empty on success; bounded diagnostic on mismatch | Artifact policy report naming pressure-summary status and schema fixture results | Missing `cartulary.scheduler_pressure_summary.v3` or `cartulary.fixture_tier_proof.v1` schema, open top-level pressure-summary fields, open fixture proof fields, or unvalidated scheduler success passes | none |
