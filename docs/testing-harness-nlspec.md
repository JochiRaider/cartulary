...
| static analysis and security | `static_analysis_security` | Toolchain and source roots | Canonical target-owned rule, flag, and package profiles. Shell lint is blocking for public Make targets. `GOVULNCHECK_DB` is the only public security-profile override in the current profile. | `summary_with_artifacts` | Scheduled inside `check` where declared. | None | Tool summary and logs. | Findings are gate failures for scheduled local correctness targets. Advisory targets MUST be explicitly selected outside local `check`. |
...
| `LINT_SHELL_STRICT`                                                                             | lint                    | exact `1`; public Make lint targets force strict blocking behavior                                                    | `1` for public Make targets; raw script fallback may default false                       | target-owned Make recipe                         | false outside public Make                            | exact string compare                                                                                          | public Make target overrides ignored by recipe-owned strict value                   | boolean when true                                  |
| `STATICCHECK_CHECKS`                                                                             | static analysis          | closed; not a public Make target input                                                                                | target-owned Staticcheck default                                                          | raw script only                                  | not applicable                                           | none                                                                                                          | Make command-line use is `usage_error`, exit `2`                                  | none                                               |
| Gosec rule, flag, and pattern variables (`GOSEC_*`, `GOSEC_TARGETED_*`, `GOSEC_AUDIT_*`)         | security                | closed; not public Make target inputs                                                                                 | target-owned curated profiles from this NLSpec and task-surface owner inputs             | raw script only                                  | not applicable                                           | none                                                                                                          | Make command-line use is `usage_error`, exit `2`                                  | retained security profile metadata                 |
...
| `GOVULNCHECK_FLAGS`, `GOVULNCHECK_PATTERNS`                                                      | security                | closed; not public Make target inputs                                                                                 | target-owned `-test` flags and authored package roots                                    | raw script only                                  | not applicable                                           | none                                                                                                          | Make command-line use is `usage_error`, exit `2`                                  | none                                               |
...
| `service_summary`                  | `db-up`, `db-reset`, `services-up`, `services-down`, `db-down`, `minio-init`, `object-store-reset` |                 yes | One `cartulary.tool_run_summary.v3` JSON object plus LF | Empty after wrapper starts                                                            | Tool-run summary and service diagnostics where applicable | Same schema with service failure fields                                |
...
| `make services-down`  |                                no |                                           no |                          no | no; stops only this repo's local Compose services and preserves named volumes |
| `make db-down`        |                                no |                                           no |                          no | no; deprecated alias for `services-down` |
...
`make object-store-reset` MUST clear only objects in the configured local object-store bucket and MUST leave the bucket present afterward. In the current implementation profile the local object store is MinIO, but the public command and command ID are provider-neutral. A real `object-store-reset` MUST reject before Compose, MinIO, or object-store commands unless `CARTULARY_DESTRUCTIVE_CONFIRM=object-store-reset` was supplied on the Make command line.
