...
A future reusable artifact cache MAY classify work as `reused` only when the cache record validates the relevant tracked and untracked inputs, tool versions or binary digests, configuration inputs, expected outputs, summary schema, and artifact references needed to diagnose the reused work from a single retained run. Until that contract exists, local input stamp files under `tmp/check-stamps/` are diagnostic or obsolete implementation artifacts and MUST NOT cause default `check` child commands to be skipped.
Verified by: TH-HARNESS-AC-001, TH-HARNESS-AC-006
...
| static analysis and security | `static_analysis_security` | Toolchain and source roots | Canonical target-owned rule, flag, and package profiles; `LINT_SHELL_STRICT=1` makes shell lint blocking where a target declares it. | `summary_with_artifacts` | Scheduled inside `check` where declared. | None | Tool summary and logs. | Findings are gate failures unless target definition makes them warning-only. |
