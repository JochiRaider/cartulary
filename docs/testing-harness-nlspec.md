...
Browser rows selected by default `check` MUST declare `default_check_required=true` plus a reason that identifies the cross-stack browser value that lower layers cannot provide. Default-check browser rows MUST be classified as `default_local_cross_stack_conformance`, `explicit_full_browser_conformance`, or `duplicate_regression`; default local `check` MAY select only implemented executable rows with lower-layer-gap reasons or bounded readiness reasons. Direct public browser targets MUST remain full-fidelity for their declared target, including explicit measurement, full visual, accessibility, and isolated stack behavior where the target owns it. A default-check browser projection MUST NOT be treated as proof that the direct target's full evidence ran.
...
| `browser-e2e-a11y` | `cartulary.harness.command.browser_e2e_a11y.v1` | `browser_e2e` | `test`, `ci` | `summary_with_artifacts` | `cartulary.tool_run_summary.v3` | `service_lifecycle` (Section 11), `evidence_normalization` (Section 8), `failure_normalization` (Section 9) | `retained_artifacts`, `service_start`, `service_resource_mutation`, `runtime_reset` | `public_active` | Emits `cartulary.frontend_accessibility_summary.v2` for implemented accessibility rows only. Default `check` uses the bounded accessibility-readiness projection from Section 4 with `full_target_equivalent=false`; direct target evidence remains full-fidelity. |
...
| `browser-e2e-visual` | `cartulary.harness.command.browser_e2e_visual.v1` | `browser_e2e` | `test`, `ci` | `summary_with_artifacts` | `cartulary.tool_run_summary.v3` | `service_lifecycle` (Section 11), `evidence_normalization` (Section 8), `failure_normalization` (Section 9) | `retained_artifacts`, `service_start`, `service_resource_mutation`, `runtime_reset` | `public_active` | Runs current-profile `V-*` visual manifest rows and frontend visual readiness rows selected from frontend phase-map titles. Default `check` uses the bounded `visual-smoke` projection from Section 4 with `full_target_equivalent=false`; direct target evidence remains full visual evidence. |
