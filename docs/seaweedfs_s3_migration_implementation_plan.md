...
| `SWFS-OWNER-STORAGEREF-001` | `01_architecture_storage_and_view_contracts.md` and any generated contracts | Object blob storage-ref owner | `normative_core` | Define or cite the current storage-ref grammar, canonicalization, key generation, maximum length, and invalid-state behavior used by new server-generated object keys. | §7.3, §8.2, §13.4, `SWFS-AC-018` | `blocks_completion` | Owner diff or verified existing owner citation. |
...
| New server-generated object keys | MUST follow the Core-owned storage-ref/key generator. Until cited, `SWFS-OWNER-STORAGEREF-001` blocks claimability. | Reject before backend call. |
...
| `server_generated_key` | Core-owned storage-ref/key grammar; max length blocked by `SWFS-OWNER-STORAGEREF-001`. | Invalid when required. | Invalid. | `object_store_invalid_request`. |
...
| `object_store_invalid_request` | Adapter caller supplied invalid bucket, key, range, metadata, purpose, or unsupported profile operation. | `false` | Product bug in tests; public route must fail earlier with owner-owned validation where possible. |
...
| `SWFS-AC-015` | The SeaweedFS compatibility suite passes every `SWFS-COMP-*` case and contains no multipart or presigned-GET skip row. | `repo_or_external_fact_required` | none | `TODO:seaweedfs-compat-command` | compatibility report | every case pass; no forbidden skip | `blocks_phase` |
