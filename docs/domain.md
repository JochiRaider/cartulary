...
| Workbook projection versus graph projection | Workbook projections are Core-owned workbook read models. Graph projection is a draft graph-oriented derivation vocabulary until separately adopted. | Applying draft graph-projection lifecycle or query rules to workbook-grid projection tables or `view_row_v1`. | Core 01 §8; `docs/graph_projection_nlspec.md` status note |
...
| Projection | Workbook projection table or equivalent denormalized read model. | Projection state is disposable and derived, not authoritative source state; draft graph projection language is not workbook authority. |
