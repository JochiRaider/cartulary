...
| Extensions Subsystem NLSpec | Shared recognition imports, immutable resolved claims, generated descriptors, dependency and binding admission, publication coordination, state-presence vocabulary, and cross-owner extension protocols. | Supplies current extension-coordination vocabulary without taking ownership from Core 00 through Core 04 or profile/shared owners. Its status is `adopted/current`; exact behavior remains allocated to the named primary owners. |
...
| Graph Projection NLSpec | Pure graph derivation, immutable projection-result identity, output ordering, result publication invariants, traversal, leases, and deterministic reconstruction. | Supplies supporting graph-computation vocabulary and MUST NOT acquire source-owner saved-resource, job, route, authorization, or visualization meaning. |
| Network Flow Activity NLSpec | Adopted analytical-table, normalized flow-row, saved graph declaration, graph-adapter, indicator-binding, and Network Analysis workspace owner under the Core 00 `network_flow_activity` boundary. | Supplies current extension vocabulary when claimed; MUST NOT redefine Core records, view schemas, Core saved views, import sessions/units, or Base Profile surfaces. |
...
| `docs/design.md` | Sole normative design-direction owner within its stated boundary. | Supplies presentation direction without redefining Core-owned runtime contracts. |
| UI/UX design guide | Non-normative design-support guide. | Restates owners and explains rationale, implementation consequences, and examples; defines no product behavior, defaults, interfaces, acceptance claims, or release evidence. |
...
| Core 04 | User, session, role, deployment-admin, trust-boundary, release, authorization, supported performance-fixture, and product-visible measurement-predicate vocabulary. | Authentication, authorization, credential lifecycle, incident role behavior, deployment, evidence access security, implementation performance fixtures and predicates, release gate behavior, acceptance criteria. | `party`, `identity`, `user`, and incident-role language MUST NOT imply authorization behavior beyond Core 04. |
| Core 05 | Publication and benchmark claim vocabulary. | Benchmark hardware/browser/network profiles, publication manifests, predicate and fixture bindings, retention, and claim-bearing publication conditions. | Core 05 MUST reference rather than redefine Core 04 product fixtures, predicates, thresholds, or implementation sampling. Document-readiness criteria in §19 are not Core 05 publication evidence. |
| Extensions Subsystem NLSpec | Recognized/claimable/claimed distinctions, resolved claim-set identity, descriptor and dependency vocabulary, extension state presence, workspace availability, and shared participant terminology. | Generated schemas, admission algorithms, configuration/process lifecycle, state coordination, transaction participation, publication, conformance accounting, and browser lifecycle remain delegated to the exact Core/profile/shared owners imported by that NLSpec. | The subsystem coordinates owner facts; it MUST NOT redefine recognition, routes, storage semantics, profile behavior, reporting, portability, backup, telemetry, or browser interaction ownership. The atomic companion adoption is current. |
...
| Saved graph versus projection result versus materialization job | The saved graph records user intent; the projection result is immutable derived graph content; the Common Job records one operational materialization attempt. | Treating attempt state as graph lifecycle, using a job ID as result identity, or storing mutable job facts in the semantic result. | Network Flow Activity NLSpec §19.1; Graph Projection NLSpec §§2, 6; Core/Common Jobs owner |
| Projection result versus Reporting diagram | A Graph projection result is exact selected source-derived content. A Reporting diagram is disclosure-filtered, redacted, presentation-oriented output derived from an exact leased result. | Treating Graph labels as release-safe, mutating Graph output to author a diagram, or treating a diagram as a reusable graph result. | Graph Projection NLSpec §6; Reporting Subsystem NLSpec §15 |
...
| Recognized versus claimable versus claimed profile | Recognition is a Core 00 fact, claimability is a recognized-profile property, and claimed means admission succeeded and the profile is present in the immutable resolved claim set. | Inferring recognition from packaged code, treating a configured claim request as admission, or letting runtime failure silently rewrite the claim set. |
| Authoritative extension state versus extension metadata | State presence comes only from members of the profile owner's declared authoritative state families. Generic metadata, migration ledgers, jobs, caches, projections, staged objects, and conformance records are coordination/support state and never establish profile state presence. | Skipping initialization or blocking portability because a generic metadata row exists. |
| Discovery versus client support versus workspace availability | Discovery describes recognized server profiles; packaged client support describes what this browser build can render; workspace availability is the current authorized intersection for one response and epoch/generation. | Rendering from discovery alone, treating support as authorization, or retaining stale availability across an epoch change. |
| Capability wire reservation versus capability activation | Empty `capabilities[]` is a reserved future-compatible wire member. The current contract defines no capability fact or activation behavior. | Treating an empty array as an extensibility callback or accepting a nonempty capability from server, browser, configuration, or profile code. |
...
| `client_txn_id` | Client-supplied idempotency key where the owner route requires it. Browser-authored values are cryptographically collision-resistant and remain stable across uncertain replay of one logical mutation. | Replay-safe mutation boundaries within the route-owned server scope. | Record ID, operation type, globally coordinated transaction object, or cross-route identity; collision resistance does not broaden server scope. | Core 01, Core 03 | `base` | `current-required` |
...
| `target_kind` | Exact persisted Revisions mutation-target token. | Selects one source-owned history facet and row/non-row rollback dispatch entry. | Display label, record type by implication, SQL relation name, import target kind, or administrative-audit target kind. | Core 02 REQ-02-265 | `base` | `current-required` |
| `snapshot_schema_id` | Exact schema identity carried by a canonical Revisions row snapshot. | Binds `{record, source}` to one admitted record type and source-owner validator. | Snapshot/Reporting `snapshot_id`, inferred JSON shape, view schema, record type alias, or compatibility hint. | Core 02 REQ-02-265 | `base` | `current-required` |
...
| `profile_id` | Recognized extension-profile identity. | Joins Core recognition, claim admission, generated descriptors, discovery, owner manifests, support rows, and conformance accounting. | Package name, configuration key, visible label, route family, workspace key, or capability. | Core 00; Core 01; Extensions Subsystem NLSpec after adoption | `base` | `current-required` |
| `contract_major` | Public compatibility major for one recognized profile. | Server/client compatibility and discovery identity for the profile's current contract line. | Document version, stored state version, migration lineage, implementation package version, or generated-artifact revision. | Core 00; Core 01; named profile owner | `base` | `current-required` |
| `workspace_key` | Stable extension-workspace identity within one profile. | Generated client support, discovery, availability, and extension `sheet_ref` selection after authorization. | Visible workspace label, `view_schema_id`, route family, React component, or browser module path. | Core 01; Core 03; named profile owner | `extension-profile` | `current-extension-when-claimed` |
...
| Revisions | `supporting_context` | Change sets, mutation entries, record revisions, row history, rollback, merge history. | Live presence transport, credential secrets, or deployment-local administrative audit. | §11 history rows; §13.10. | Core 02 for history substrate; Core 01 for public routes. | Incident record history resolves through Core 02; administrative audit belongs to Authentication and Administration under Core 04. | `base` |
...
| Graph Projection | `supporting_context` | Pure graph derivation, stable result/object identity, ordered reads, traversal, leases, and deterministic reconstruction. | Saved graph declarations, source authorization, jobs, public routes, Reporting redaction, or visualization. | §11 Graph projection result row. | Graph Projection NLSpec. | Source-owner behavior stays with the producer; release behavior stays with Reporting. | `extension-profile` |
...
| Authentication and Administration | `generic_context` | Local users, current account profile, account preferences, sessions, MFA, deployment admin, credential lifecycle, enterprise-auth bindings, incident roles, administrative audit. | Incident-scoped party identity, incident-scoped workbook preferences, workbook row mutation, or incident revision history. | §11 User, Current account profile, Account preference, Incident membership, Deployment admin rows; Core 04 administrative-audit vocabulary. | Core 04 primary; Core 01 for public auth/admin routes. | Security, authorization, and administrative-audit disputes resolve through Core 04. | `base` |
...
| Network Flow Analysis | `supporting_context` | Network Flow analytical tables, immutable normalized flow rows, saved graph declarations, graph adaptation, contributor queries, and explicit indicator bindings. | Core record envelopes, `view_schema` resources, Core saved views, raw SIEM/EDR identity, live collection, or whole-incident removal. | §11 Network Flow rows; §13.12; §16. | Network Flow Activity NLSpec; Core 01–04 for imported interfaces, interaction, identity, and security. | Core owners govern shared interfaces; the adopted extension NLSpec governs only extension-local analytical semantics. | `extension-profile` |
...
| Revisions | `supporting` | Enables incident-history recoverability and traceability across the core domain. | Use domain-specific terms at incident-history boundaries. | Generic audit-log terms cannot replace record revision or rollback semantics, and administrative audit cannot enter this context. |
...
| Graph Projection | `supporting` | Provides reusable deterministic graph derivation and stable immutable result identity. | Keep computation and narrow result capabilities independent of source-owner applications. | Saved resources, jobs, routes, authorization, and report presentation cannot leak into the pure engine. |
...
| Revisions | Workbook Interaction | `published_language` | change_set, mutation entry, record revision, rollback. | Core 02/Core 03 | Workbook history UI may expose row-history and rollback vocabulary. | Live presence, save state, and administrative audit MUST NOT become incident-history authority. | Any change to history, rollback, change-set, or revision vocabulary MUST update workbook history terminology without converting live presence, save state, or administrative audit into revision state. |
...
| Network Flow Analysis | Graph Projection | `customer_supplier` | trusted graph view/source-owner context, semantic input, immutable result binding. | Network Flow Activity NLSpec; Graph Projection NLSpec | Network Flow owns source selection and declarations; Graph Projection supplies deterministic derivation. | Graph Projection MUST NOT query Network Flow authority or acquire declaration/job semantics. | Any input or identity change MUST update both owners and generated contracts before runtime changes. |
| Network Flow Analysis | Reporting and Snapshots | `published_language` | exact saved graph declaration and selected-result binding. | Network Flow Activity NLSpec; Reporting Subsystem NLSpec | Network Flow validates one immutable alternate source boundary; Reporting owns leases, selection, redaction, and diagrams. | Raw Network Flow rows are not copied into Core snapshots merely to support graph diagrams. | Any source-ref change MUST update both owner schemas and same-transaction tests. |
| Graph Projection | Backup and Restore | `published_language` | exact rebuild request/result, source registry, deterministic result identity. | Graph Projection NLSpec; Core 01/Core 04 Recovery owners | Recovery coordinates; Graph Projection reconstructs derived results from restored source-owner declarations. | Derived Graph rows MUST NOT become authoritative backup input. | Any table or algorithm change requires catalog, binding, isolated-restore, and readiness updates together. |
...
| Collaboration stream recovery | Core 01/Core 03/Core 04 owner sections. | Any change to stream quarantine, repaired payload, requeue, pending intent, local Collaboration operator command, or private operator journal vocabulary. |
...
| Revisions | Core 01/Core 02 owner sections. | Any change to change set, mutation entry, record revision, rollback, or merge-history language. |
...
| Graph Projection | Graph Projection NLSpec. | Any change to semantic graph input, result/object identity, ordering, traversal, leases, publication, or deterministic reconstruction. |
...
| Authentication and Administration | Core 01/Core 04 owner sections. | Any change to user, session, credential, account profile, account preference, deployment admin, enterprise-auth, or administrative-audit language. |
...
| Incident lifecycle | The public active/closed state of an incident and the owner-defined close/reopen transitions. | Collaboration stream quarantine, evidence quarantine, dispatch pause, poison-event state, archive, delete, or purge. | incident status; close/reopen routes | §11 | Core 01/Core 02/Core 04 by contract family | `base` | `current-required` |
...
| Evidence quarantine | An Evidence-owned lifecycle/access condition for an evidence record or its linked object blob that blocks ordinary access until owner-defined repair or release. | Collaboration stream dispatch quarantine, incident closure, incident lifecycle, operator requeue, or generic error handling. | evidence lifecycle `quarantined`; blob upload state `quarantined` | §11 | Core 01/Core 02/Core 04 by contract family | `base` | `current-required` |
| Collaboration stream quarantine | An internal incident-scoped safety stop on durable Collaboration sequencing after a deterministic pending-intent failure. | Incident lifecycle state, evidence quarantine, record quarantine, deletion, event selection, skipped event, public workflow, or analyst-facing incident status. | quarantined stream cursor; pending event intent | §11 | Core 03 §4.3.3; Core 01/Core 04 for local operator transport/security | `base` | `current-required` |
| Collaboration requeue | Deployment-local, repair-first release of one Collaboration stream quarantine after all pending canonical payloads validate. | Event deletion, event skip, incident reopen, evidence release, idempotent retry alias, public action, browser action, or generic job requeue. | `operator collaboration requeue`; `operation='collaboration_requeue'` | §11 | Core 03 §4.3.3; Core 01 §12.2.2; Core 04 §2 | `base` | `current-required` |
...
| Target kind | Exact persisted token identifying one mutation semantic family. | Record type inferred from a target ID, display grouping, or physical source relation. | `target_kind`, source-owner target-semantics contribution | §11 | Core 02 | `base` | `current-required` |
| Canonical revision snapshot | Schema-identified authoritative `{record, source}` state captured in the caller transaction for row history and restore. | Projection row, visible workbook row, live Collaboration event material, or Snapshot/Reporting export model. | `snapshot_schema_id` | §11 | Core 02/Core 01 | `base` | `current-required` |
...
| Network Flow saved graph | Incident-shared authoritative declaration of a normalized Network Flow semantic graph query, refresh intent, and selected exact Graph result. | Core saved view, graph layout, job attempt, Graph result row, or browser-local exploration state. | `graph_view_id`, `graph_view_version`, `materialization_generation` | §11 | Network Flow Activity NLSpec | `extension-profile` | `current-extension-when-claimed` |
| Network Flow semantic graph query | Closed Network Flow-owned declaration of table scope, time range, aggregation mode, and lower resource overrides supplied to the pure Graph Projection adapter. | Graph Projection configuration, SQL query text, browser layout state, or generic temporal-graph protocol. | `cartulary.network_flow.graph_semantic_query.v2` | §11 | Network Flow Activity NLSpec | `extension-profile` | `current-extension-when-claimed` |
| Network Flow time bucket | One Unix-epoch-aligned UTC half-open interval used by the Network Flow temporal aggregation mode and response summary. | Calendar period, local-time window, persisted Graph object, database table, or generic Graph Projection time-series concept. | canonical `[bucket_start_utc,bucket_end_utc)` and fixed width | §11 | Network Flow Activity NLSpec | `extension-profile` | `current-extension-when-claimed` |
| Bucketed flow edge | Network Flow source relationship aggregating accepted rows by one time bucket and the default flow-edge key. | Generic temporal edge, duplicated/prorated counter, Graph engine temporal policy, or Reporting label. | `nfbe_…`, `network_flow.bucketed_flow_edge.v1` | §11 | Network Flow Activity NLSpec | `extension-profile` | `current-extension-when-claimed` |
| Network Flow bucket summary | Response-owned metadata for every bucket intersecting a temporal query, including empty buckets and bounded counts. | Graph Projection result state, persisted index, authoritative source row, or new database table. | `time_buckets[]` on the temporal result variant | §11 | Network Flow Activity NLSpec | `extension-profile` | `current-extension-when-claimed` |
| Graph projection result | Immutable derived graph content with deterministic result, vertex, edge, and digest identity. | Saved graph declaration, Graph lifecycle run, Common Job, report diagram, or authoritative source state. | `projection_result_id`, `graph_projection.v2` | §11 | Graph Projection NLSpec | `extension-profile` | `current-extension-when-claimed` |
| Graph materialization job | Common Job attempt that computes and conditionally publishes one saved graph generation. | Saved graph state, projection result identity, retention policy, or hidden Graph worker. | `network_flow_activity.graph_view_materialize_v1` | §11 | Common Jobs owner; Network Flow Activity NLSpec | `extension-profile` | `current-extension-when-claimed` |
| Network Flow indicator binding | Explicit incident-scoped relationship between one normalized IP endpoint and one canonical family-matching indicator, committed through the owner transaction boundary. | Indicator observation, automatic host/identity creation, record link inferred from graph adjacency, or cross-incident identity. | Network Flow binding identity plus canonical indicator `record_id` | §11 | Core 02; Network Flow Activity NLSpec | `extension-profile` | `current-extension-when-claimed` |
...
| Change set | Revisions | Primary language. |
| Mutation entry | Revisions | Primary language. |
| Record revision | Revisions | Primary language. |
| Rollback | Revisions | Primary language. |
...
| Network Flow saved graph | Network Flow Analysis | Current claimed-profile saved-declaration language distinct from Core saved views. |
| Graph projection result | Graph Projection | Immutable derived-content language. |
| Graph materialization job | Network Flow Analysis | Operational attempt language imported from Common Jobs. |
...
`Find in loaded rows` names the local Timeline, Hosts and Identities cell-navigation interaction owned
...
| Saved graph materialization | A Network Flow declaration remains authoritative while Common Jobs owns attempts and Graph Projection owns immutable content; omission of a selected result is not a hidden Graph lifecycle state. | Network Flow Activity NLSpec; Graph Projection NLSpec; Core/Common Jobs owner |
...
| Graph Projection | `summarized here` | Result/declaration/job/diagram distinctions are summarized; exact input, identity, limit, lease, and rebuild behavior remains owner-owned. | Graph Projection NLSpec; Network Flow Activity NLSpec; Reporting Subsystem NLSpec |
