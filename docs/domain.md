...
| Network Flow Activity NLSpec | Draft analytical-table, normalized flow-row, graph-adapter, indicator-binding, and Network Analysis workspace proposal under the Core 00 recognized `network_flow_activity` boundary. | Supplies draft extension vocabulary only until coordinated adoption; MUST NOT redefine Core records, view schemas, saved views, import sessions/units, or Base Profile surfaces. |
...
| flow table | Yes, extension-specific after adoption | Network Flow table | Network Flow Activity analytical-resource discussion. | SQL table, import unit, worksheet, `view_schema`, saved view, or raw telemetry stream. | Core 00 profile posture; Network Flow Activity NLSpec after adoption |
| Network Analysis | Limited visible label | Network Analysis workspace | User-facing extension-workspace label after canonical identity is stated. | Stable identifier, Base built-in tab, system view, saved view, or module identity. | Core 01/Core 03; Network Flow Activity NLSpec after adoption |
...
| `network_flow_table_id` | Network Flow analytical table resource. | Stable incident-scoped identity for one successfully published normalized flow table after profile adoption. | `record_id`, `view_schema_id`, `saved_view_id`, import unit, worksheet, filename, or visible tab label. | Network Flow Activity NLSpec after adoption; Core 01 for public reference unions | `extension-profile` | `not-current` |
...
| Network Flow Analysis | `supporting_context` | Network Flow analytical tables, immutable normalized flow rows, graph adaptation, contributor queries, and explicit indicator bindings after profile adoption. | Core record envelopes, `view_schema` resources, saved views, raw SIEM/EDR identity, live collection, or whole-incident removal. | §11 Network Flow rows; §13.12; §16. | Network Flow Activity NLSpec after adoption; Core 01–04 for imported interfaces, interaction, identity, and security. | Core owners govern shared interfaces; the adopted extension NLSpec governs only extension-local analytical semantics. | `extension-profile` |
...
| Network Flow Analysis | Core 00 profile posture; Core 01–04 shared interfaces; Network Flow Activity NLSpec after adoption. | Any change to Network Flow table/row identity, extension workspace, import target/result, graph boundary, indicator binding, authorization, or retention vocabulary. |
...
| Network Flow table | Incident-scoped analytical resource containing one atomically published set of normalized immutable flow rows after the Network Flow Activity profile is adopted and claimed. | Core record, view schema, saved view, import unit, worksheet, filename, raw telemetry stream, or physical SQL table. | `network_flow_table_id` | §11 | Network Flow Activity NLSpec after adoption; Core 01 for import/public reference interfaces | `extension-profile` | `not-current` |
| Network Flow row | Immutable normalized analytical row owned by one Network Flow table. | `view_row_v1`, Core record revision, indicator observation, graph node, raw SIEM/EDR event, or mutable spreadsheet row. | table-scoped Network Flow row identity after owner adoption | §11 | Network Flow Activity NLSpec after adoption | `extension-profile` | `not-current` |
| Network Analysis workspace | Claimed extension workspace for working with active Network Flow tables and derived graph/query results. | Base built-in tab, system view, saved view, `view_schema_id`, full application module, or identity inferred from the `Network Analysis` label. | owner-adopted extension workspace identity and `sheet_ref` variant | §11 | Core 01/Core 03; Network Flow Activity NLSpec after adoption | `extension-profile` | `not-current` |
| Network Flow indicator binding | Explicit incident-scoped relationship between one normalized IP endpoint and one canonical family-matching indicator, committed through the owner transaction boundary. | Indicator observation, automatic host/identity creation, record link inferred from graph adjacency, or cross-incident identity. | Network Flow binding identity plus canonical indicator `record_id` after owner adoption | §11 | Core 02; Network Flow Activity NLSpec after adoption | `extension-profile` | `not-current` |
...
| Network Flow Analysis | `extension-only` | Network Flow terms remain draft vocabulary only until the profile is adopted and claimed; omission adds no route, workspace, resource, or Base surface. | Core 00; Core 01–04; Network Flow Activity NLSpec after adoption |
...
| --- | --- | --- | --- | --- |
| Network Flow Activity claim and runtime behavior | Recognized bounded extension identity, but not a current claimable profile while its NLSpec is draft. | Permit unclaimed discovery identity and vocabulary review only; reject route, workspace, storage, or conformance claims until coordinated adoption. | Draft NLSpec, adoption tracker, owner-amendment text, and implementation hidden behind an unclaimed gate. | Close owner interfaces, contracts, implementation, fixtures, and evidence; then coordinate Core 00 and NLSpec adoption status. |
