...
Current repo status after Sprint 9 remediation on 2026-05-16: Phase 8 is listed as `active` in `tools/phase_registry.json`; `tools/phase8_test_map.json` and `docs/testing/phase8_coverage_ledger.md` exist and cover all 19 authoritative rows; generated contract, ledger, schedule, and duration-maintenance drift checks pass; all Sprint 0 placeholder tests have been replaced by direct row evidence or by an explicit product-gap sentinel where the saved-view browser lifecycle remains unimplemented. `AC-184` and `AC-185` now have direct Phase 8 evidence. `P8S8-AUD-001` was resolved by tightening the Core 03 group-header boundary, keeping recordless draft/create affordances outside grouped committed result buckets, and scoping `E-8-03` browser proof to the active Timeline grid. The earlier Sprint 9 traceability blocker was resolved by renaming row-wire evidence to `TestPhase8_RowWireFamilies_U_8_10`, tightening authoritative row-name validation, and refreshing ledgers, schedules, and duration baselines. The final Sprint 9 direct-command audit blocker `P8S9-AUD-001` is remediated by hardening service-suite stale Docker cleanup so a proven prior container already being removed cannot fail new-suite startup preflight. The stale full-gate blocker prose was replaced with the passing `make check` root `.cartulary/test-results/20260516T220043Z-p916905`. `make phase-slice PHASE=phase8`, `make service-backed-slice PHASE=phase8`, `make test-fast`, `make agent-finalize RESULTS_DIR=.cartulary/test-results/20260516T220043Z-p916905`, and `make check` now pass after Sprint 9 remediation.
...
| [x] | 9. Phase gate, ledgers, schedules, baselines, and exit | [x] Phase 8 authoritative selections, public wrappers, generated drift, ledger drift, schedule drift, duration baseline coverage/drift, `test-fast`, `agent-finalize`, and `make check` passed | None for Sprint 9 after remediation. | `U-8-10` row-wire evidence now carries its own row identity; Phase 9 handoff remains compatibility-only and `E-8-01` remains a saved-view lifecycle UI sentinel. |
...
- Remediated: `P8S9-AUD-001`; `U-8-10` now maps to `internal/modules/workbook/phase8_row_wire_test.go::TestPhase8_RowWireFamilies_U_8_10`, and stale wrong-row row-wire references were removed from manifests, ledgers, schedules, and Go duration baselines.
- Remediated: authoritative phase evidence naming now fails when a manifest row points at another row's symbol/title. The guard also exposed and resolved one legacy Phase 4 alias by adding `TestPhase4_GenericCollectionPatch_I_4_COLLECTION_01` for `I-4-COLLECTION-01`.
- Passed after Sprint 9 traceability remediation: `make phase-test-name-check`, `make phase-map-check`, `make phase-ledger-drift`, `make phase-schedule-drift`, `make backend-integration CARTULARY_MANIFEST_PHASE=phase8 CARTULARY_MANIFEST_COVERAGE=authoritative` (120 tests, retained root `.cartulary/test-results/20260516T222404Z-p1012701`), `make lint-scripts`, `make harness-contract-tests`, `make service-backed-unit-check`, `go test ./tools/gotestservicecheck`, `make go-test-duration-baseline-coverage`, `make go-test-duration-baseline-drift RESULTS_DIR=.cartulary/test-results/20260516T220043Z-p916905`, and `git diff --check`.
- Passed after Sprint 9 traceability remediation: `make check`; retained root `.cartulary/test-results/20260516T220043Z-p916905`; 94/94 work units, 554 tests, 0 failed, wall duration 89417ms.
- Superseded historical blocker: earlier `make check` root `.cartulary/test-results/20260516T185011Z-p459690` failed outside Phase 8 at Phase 3 visual row `V-3-GRID-03`; that failure was superseded by passing full-gate evidence.
...
- Generated phase ledgers and schedules were refreshed through `make phase-ledgers`, `make phase-schedules`, and `make agent-finalize RESULTS_DIR=.cartulary/test-results/20260516T220043Z-p916905`; ledger and schedule drift checks pass after the remediation.
...
- Duration baselines were refreshed from retained evidence. `make agent-finalize RESULTS_DIR=.cartulary/test-results/20260516T220043Z-p916905` passed at `.cartulary/test-results/20260516T222148Z-p1007337` with `generated=updated files=3 duration=refreshed run_checks=pass`; it updated `tools/browser_e2e_duration_baselines.json`, `tools/harness_smoke_duration_baselines.json`, and `tools/service_backed_make_target_duration_baselines.json`. Go test duration baselines contain `TestPhase8_RowWireFamilies_U_8_10` and no stale wrong-row row-wire key.
...
- Standalone `make test-service-backed` and `make check-service-backed` runs attempted for prune-mode duration maintenance failed after tests with harness message `functional-shard index must be a non-negative integer`; the full `make check` service-backed stage passed, so this is not a Phase 8 product blocker.
- Final audit remediation: a later direct `make backend-integration CARTULARY_MANIFEST_PHASE=phase8 CARTULARY_MANIFEST_COVERAGE=authoritative` attempt failed before tests at `.cartulary/test-results/20260516T225353Z-p1093086` because Docker reported stale suite container removal was already in progress. This is classified as harness infrastructure cleanup behavior, not Phase 8 product behavior. The durable remediation is in `tools/testservices` stale-container cleanup classification and the testing harness NLSpec. After the remediation, the exact direct command passed at `.cartulary/test-results/20260516T234209Z-p1240243` with 120 tests, and the retained service scope recorded `stale_containers_scanned=2`, `stale_containers_removed=0`, `stale_containers_deferred=0`, and no startup-preflight failure.
- Final audit validation after stale-cleanup remediation passed: `go test ./tools/testservices`, `go test ./internal/testutil/suiteservices`, `make service-backed-slice PHASE=phase8` at `.cartulary/test-results/20260516T234738Z-p1251283`, `make test-fast` at `.cartulary/test-results/20260516T235306Z-p1263064`, `make agent-finalize RESULTS_DIR=.cartulary/test-results/20260516T220043Z-p916905` at `.cartulary/test-results/20260516T235922Z-p1281540`, selected drift checks, `git diff --check`, and `make check` at `.cartulary/test-results/20260517T000024Z-p1285329`.
- Final audit finalizer accounting: `make agent-finalize RESULTS_DIR=.cartulary/test-results/20260516T220043Z-p916905` at `.cartulary/test-results/20260516T225701Z-p1123219` and again at `.cartulary/test-results/20260516T235922Z-p1281540` reported `generated=updated files=5`. The audit-caused generated/baseline outputs are `tools/browser_e2e_duration_baselines.json`, `tools/execution_topology_render_index.json`, `tools/go_test_duration_baselines.json`, `tools/harness_smoke_duration_baselines.json`, and `tools/service_backed_make_target_duration_baselines.json`. These are accepted only as explicit finalizer outputs from the retained successful run, not silent remediation.
- Final audit provenance: 18 tracked files were already staged before the final audit execution. Those staged files remain attributed to earlier Sprint 9 remediation. New final-audit remediation is limited to the harness stale-cleanup contract/implementation/tests, suite-service diagnostic field, explicit finalizer baseline refresh outputs, and this provenance note.

Sprint 9 changed files:
- `PHASE8_IMPLEMENTATION_PLAN.md` records observed Sprint 9 results, the traceability remediation, the passing full-check root, and Phase 9 handoff notes.
- Harness traceability and guard files were updated so authoritative row evidence carries exact row identity and service-backed U-tests are governed by manifest execution dependency.
- Generated ledger, schedule, and duration-maintenance artifacts were refreshed from the updated manifests and retained evidence. No lockfile or visual-golden tracked diff was produced.
- Harness stale-cleanup remediation updates `docs/testing-harness-nlspec.md`, `tools/testservices`, and suite-service diagnostics so proven prior-suite containers that Docker is already removing are retained as deferred cleanup diagnostics instead of blocking new direct service-backed evidence.
...
- Phase 8 compatibility for those Phase 9 surfaces is limited to the shared contracts named above. Future Phase 9 implementation should add owner-driven workflow and browser evidence rather than carrying forward partial Phase 8 behavior as legacy support.
