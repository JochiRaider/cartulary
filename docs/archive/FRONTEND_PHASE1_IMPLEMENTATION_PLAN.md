...
- `/packages/protocol-ts`, `/packages/ui-contracts`, and `/packages/test-utils` exist. `@cartulary/ui-contracts` contains FE-P1 auth, account, admin, landing, route, error, and incident landing selector builders. FE-P1 helper-level selector consumers now require stable/shared selector IDs or explicitly documented app-local IDs, and `apps/web/src/selectorContractPolicy.test.ts` classifies FE-P1 shared selector families as shared-builder owned.
...
- FE-P1 row-owned closure evidence was not collected during Sprint 1. Sprint 4 later collected direct product-conformance evidence for `FE-E-P1-01` from `make browser-e2e-webserver-backed`; Sprint 5 later collected and audit-refreshed direct design-direction evidence for `FE-A11Y-P1-01` from `make browser-e2e-a11y`; Sprint 6 later collected direct implementation-support evidence for `FE-S-P1-01` from `make frontend-unit` and `make browser-e2e-support`.
...
| [x] | 6. Stable bootstrap and error-state selector builders; owns `FE-S-P1-01` | `make frontend-unit`; `make browser-e2e-support` | None for Sprint 6 selector-contract scope | Helper-level raw FE-P1 selector bypass closed with branded stable IDs and policy coverage |
...
- Status: Complete for `FE-S-P1-01` after selector-policy remediation and row-owned support evidence.
...
- Audit finding remediated: The Sprint 6 audit found that raw FE-P1 literals could bypass policy enforcement through helper-level selector consumers such as `Phase1Page.requireText()`, `Phase1Page.setCheckbox()`, and FE-P1 accessibility `focusTestId`/`tabStops` options.
- Test-first sequence: Shared-builder tests already cover FE-P1 auth, account, admin, landing, route, bootstrap, session, incident landing, and error selectors. Sprint 6 remediation added policy regression coverage that rejects raw FE-P1 literals at helper-level selector sinks.
- Implementation tasks: FE-P1 selector builders now return a branded `StableTestId` type; FE-P1 E2E helper methods require stable IDs; FE-P1 accessibility helper options accept stable IDs or explicitly documented app-local IDs; app-local selector ownership remains metadata-backed.
- Validation commands: `make frontend-unit`; `make browser-e2e-support`; supporting `make frontend-typecheck`.
- Deliverables: Shared selector/test-id builders, migrated FE-P1 selector consumers, typed helper boundaries, and policy coverage that keeps FE-P1 selector ownership explicit.
- Risks and assumptions: Selector/test-id strings are internal test contracts, not public API compatibility commitments.
- Completion evidence: `make frontend-typecheck` passed with `.cartulary/test-results/20260530T160012Z-p3768252/frontend-typecheck/tool-run-summary.json`; `make frontend-unit` passed with `.cartulary/test-results/20260530T160023Z-p3768647/frontend-unit/tool-run-summary.json`; `make browser-e2e-support` passed with `.cartulary/test-results/20260530T160043Z-p3770601/browser-e2e-support/tool-run-summary.json`.
- Supporting evidence: `make agent-finalize` passed with `.cartulary/test-results/20260530T155959Z-p3767132/agent-finalize/tool-run-summary.json` and reported generated artifacts unchanged while retained-run maintenance was skipped because `RESULTS_DIR` was unset; `make phase-ledger-drift` passed with `.cartulary/test-results/20260530T160108Z-p3773627/phase-ledger-drift/tool-run-summary.json`; `make generated-artifact-policy-check` passed with `.cartulary/test-results/20260530T160111Z-p3773861/generated-artifact-policy-check/tool-run-summary.json`; `make generate-drift` passed with `.cartulary/test-results/20260530T160114Z-p3774082/generate-drift/tool-run-summary.json`; `make frontend-import-boundary-check` passed with `.cartulary/test-results/20260530T160119Z-p3774781/frontend-import-boundary-check/tool-run-summary.json`; `git diff --check` passed; `git diff --cached --check` passed.
- Phase metadata: No phase-map metadata changed during this remediation, so `make phase-ledgers` was skipped; `make phase-ledger-drift` verified the existing generated ledger remained consistent.
- Blockers and follow-up notes: None for `FE-S-P1-01`.
...
| Generated ledgers are produced by generator, not hand-edited | Satisfied through Sprint 5 metadata generation and Sprint 6 drift validation; no phase-map metadata changed during Sprint 6, so `make phase-ledgers` was skipped and `make phase-ledger-drift` passed with `.cartulary/test-results/20260530T160108Z-p3773627/phase-ledger-drift/tool-run-summary.json` |
...
| `FE-S-P1-01` has selector-builder support evidence or a blocker | Satisfied for Sprint 6 selector-contract scope: `make frontend-unit` passed with `.cartulary/test-results/20260530T160023Z-p3768647/frontend-unit/tool-run-summary.json`, `make browser-e2e-support` passed with `.cartulary/test-results/20260530T160043Z-p3770601/browser-e2e-support/tool-run-summary.json`, and helper-level raw FE-P1 selector bypass coverage is enforced by `apps/web/src/selectorContractPolicy.test.ts` |
...
| P0 regression checks remain green or exact blockers are recorded | Satisfied for Sprint 6 selector-contract remediation: `make frontend-typecheck`, `make frontend-unit`, `make generated-artifact-policy-check`, `make generate-drift`, `make frontend-import-boundary-check`, and `make phase-ledger-drift` passed |
...
| Whitespace checks pass | Satisfied for Sprint 6 remediation: `git diff --check` and `git diff --cached --check` passed |
