...
- DONE: linked-note implementation ownership now delegates to `internal/modules/artifacts/linkednotes`; workbook keeps route-compatible request/result adaptation.
...
| S-02 | S-01 | Move linked-note orchestration behind artifacts plus links facade while preserving current route and wire response | `internal/modules/workbook/mutation_store.go`; `internal/modules/artifacts/linkednotes`; `internal/modules/links`; revisions/projections helpers | Linked artifact ID, reference writes, projection refresh, revisions, status codes, `record_changed` | Linked-notes route characterization; phase9 notes/indicator; workbook mutation integration | `make backend-unit`; `make backend-integration`; `make phase-slice PHASE=phase9`; `make service-backed-slice PHASE=phase9` | Retain workbook wrapper that calls old path until facade proves equivalent | DONE: workbook now delegates linked-note source lookup and create orchestration to `artifacts/linkednotes`; route/wire wrapper is unchanged. |
| S-03 | S-01 | Move decision supersede orchestration behind tasksdecisions plus links plus revisions facade while preserving route | `mutation_store.go`; `internal/modules/tasksdecisions`; `internal/modules/links`; `internal/modules/revisions` | Decision lifecycle, supersede status, link refs, revisions, projection refresh, status codes | Phase9 task decisions; workbook mutation integration; revisions tests | `make backend-unit`; `make backend-integration`; `make phase-slice PHASE=phase9` | Keep workbook wrapper delegating to prior implementation | DONE: workbook delegates decision supersede orchestration to `tasksdecisions.SupersedeFacade`; route and result/error wrapper remain workbook-compatible. |
| S-04 | S-01 | Reduce workbook mutation DTO ownership by adding owner-specific adapters without changing wire hashes | `mutation_api.go`; owner adapter packages; route tests | Hash drift, normalization drift, validation errors, generated schema mismatch | Mutation API hash fixtures; phase6 conflict; phase9 timestamp; OpenAPI drift checks | `make backend-unit`; `make generate-drift`; `make json-shape-check` | Keep exported workbook DTO wrappers that call new adapters | IN_PROGRESS: owner-specific create validation delegates to evidence, entities, artifacts, and tasksdecisions; direct patch token validation now delegates to evidence/artifacts/tasksdecisions; collection reference/tag/risk-ref mutation and reference validation delegate to links. Workbook still owns route decode/hash compatibility wrappers. |
| S-05 | S-01, S-04 | Replace imports-to-workbook production dependency with import-owned dispatcher for non-timeline targets | `internal/modules/imports`; owner create facades | Import request behavior, idempotency, target routing, validation errors | Import route/store tests plus owner create tests | `make backend-unit`; `make phase-slice PHASE=phase11`; targeted import tests if available | Retain unsupported targets as fail-closed until owner facade exists | IN_PROGRESS: required non-timeline/non-entity supported targets now have owner-create facades and a journaled unit transaction; legacy timeline/entity import paths still use existing row-level owner transactions and need owner support before the apply journal can be fully unit-atomic across every supported target. |
...
| WB2-06 | Track mutation-store orchestration split | WF-05 | IN_PROGRESS | WB2-02, WB2-03 | Findings and slices S-02/S-03/S-04 | Linked-note and decision-supersede orchestration moved to owner facades; create and direct patch validation plus collection side effects now live in owner modules; route decode/hash wrappers and generic patch transaction coordination remain. |
...
| WB2-12 | Move linked-note orchestration behind owner facade | WF-05 | DONE | WB2-11 | `internal/modules/artifacts/linkednotes/facade.go`; workbook linked-note wrapper; focused workbook package validation | Workbook no longer directly creates note artifacts or linked-note references; route behavior and result shape remain delegated through the compatibility wrapper. |
...
| WB2-14 | Add remaining owner create facades for non-timeline/non-entity import targets | WF-05 | DONE | WB2-13 | `internal/modules/imports/targets.go`; `internal/modules/imports/owner_apply.go`; `internal/modules/entities/assessment_mutations.go`; focused import integration evidence | Evidence, artifacts/notes/coordination artifacts, tasks/decisions, parties, and assessments have owner create facades; optional findings/investigative-query/forensic-keyword surfaces remain `supported_when_implemented`. |
| WB2-15 | Add explicit import apply journal | WF-05 | IN_PROGRESS | WB2-13 | `db/migrations/00042_import_apply_journal.sql`; `internal/modules/imports/store.go`; `TestPhase11_I_11_IMPORT_02_EvidenceImportUsesOwnerFacadeAndJournal` | Journal table and store writes exist for the new generic owner-facade unit transaction. TODO: timeline/host/identity/indicator imports still need owner support for a single selected-unit transaction and journal insertion. |
...
| 2026-07-01 | Codex remediation continuation | S-05R advanced: required non-timeline/non-entity import targets have concrete owner-create facades and write `import_apply_journal` rows inside the new generic unit transaction | Touched `internal/modules/imports/targets.go`, `routes.go`, `store.go`, `owner_apply.go`, `tabularingest/owner_create.go`, `internal/modules/entities/assessment_mutations.go`, `db/migrations/00042_import_apply_journal.sql`, `internal/modules/imports/imports_integration_test.go` | `gofmt`; `go test ./internal/modules/imports ./internal/modules/entities` | Focused import/entities tests passed; evidence import now applies through the owner facade, creates a projected row, writes one journal row, and records one unit-level change set | Legacy timeline/host/identity/indicator import paths still use pre-existing row-level owner create transactions, so full Core 03 unit-atomic journaling remains incomplete for those targets | Continue S-02 linked-note owner facade; return to timeline/entity import transaction support before marking WB2-15 DONE. |
| 2026-07-01 | Codex remediation continuation | S-02 completed: linked-note orchestration moved behind an artifacts/links owner facade subpackage | Touched `internal/modules/artifacts/linkednotes/facade.go`, `internal/modules/workbook/store.go`, `internal/modules/workbook/mutation_store.go` | `gofmt`; `go test ./internal/modules/artifacts ./internal/modules/artifacts/linkednotes ./internal/modules/workbook` | Focused artifacts/workbook validation passed; workbook `CreateLinkedNote` now performs request/result conversion and delegates source lookup/create orchestration | Broader phase9 validation still pending for final gate | Continue S-03 decision supersede owner facade. |
| 2026-07-01 | Codex remediation continuation | S-03 completed: decision supersede orchestration moved behind tasks/decisions owner facade | Touched `internal/modules/tasksdecisions/supersede_facade.go`, `internal/modules/workbook/store.go`, `internal/modules/workbook/mutation_store.go` | `gofmt`; `go test ./internal/modules/tasksdecisions ./internal/modules/workbook` | Focused tasksdecisions/workbook validation passed; workbook `SupersedeDecision` now delegates transaction, decision machine checks, supersedes link, revisions, projections, and multi-record change results to the owner facade | Broader phase9 validation still pending for final gate | Continue S-04 DTO/patch adapter split. |
| 2026-07-01 | Codex remediation continuation | S-04 advanced: owner-specific create validation moved to owner packages | Touched `internal/modules/evidence/create_validation.go`, `internal/modules/entities/party_validation.go`, `internal/modules/artifacts/create_validation.go`, `internal/modules/tasksdecisions/create_validation.go`, `internal/modules/workbook/mutation_store.go` | `gofmt`; `go test ./internal/modules/artifacts ./internal/modules/evidence ./internal/modules/entities ./internal/modules/tasksdecisions ./internal/modules/workbook` | Focused owner/workbook validation passed; workbook create request hashes and decoding remain unchanged while owner packages now own create validation rules | Patch adapters, collection normalization, and owner-owned route hash wrappers remain incomplete | Continue S-04 patch adapter work or record explicit deferral before S-06/final handoff. |
| 2026-07-01 | Codex remediation continuation | S-04 advanced again: direct patch token validation and collection side effects moved behind owner modules | Touched `internal/modules/links/collection_actions.go`, `internal/modules/artifacts/linkednotes/facade.go`, `internal/modules/workbook/mutation_store.go`, `internal/modules/artifacts/{create_validation.go,mutations.go}`, `internal/modules/evidence/create_validation.go`, `internal/modules/tasksdecisions/create_validation.go` | `gofmt`; `go test ./internal/modules/artifacts ./internal/modules/evidence ./internal/modules/tasksdecisions ./internal/modules/links ./internal/modules/workbook` | Focused owner/workbook validation passed; links now owns collection reference/tag/risk-ref validation and mutation; evidence/artifacts/tasksdecisions own direct patch token validation; workbook keeps wire DTOs and hash fixtures stable | Workbook still owns generic patch transaction/conflict coordination and route decode/hash compatibility wrappers | Continue only with owner transaction/query facade work that can preserve route contracts; otherwise record deliberate deferrals before final handoff. |
...
| 2026-07-01 | Codex remediation continuation | S-04 links/direct-patch owner split tracker validation completed | Touched this tracker after owner-split implementation | `make lint-markdown`; `make generated-artifact-policy-check`; `make json-shape-check` | All passed. Artifact policy run root: `.cartulary/test-results/20260701T010632Z-p299081`; JSON shape run root: `.cartulary/test-results/20260701T010632Z-p299123` | None for this tracker/static gate | Continue only after rerunning static validation for this evidence row. |
| 2026-07-01 | Codex remediation continuation | Owner-facade implementation backend unit validation completed | Touched owner/workbook/import files listed in this handoff | `make backend-unit` | Passed with run root `.cartulary/test-results/20260701T010754Z-p302096` | Phase-specific and full final gates still pending | Run final tracker/static validation after this handoff row, then decide whether to escalate to phase slices or final gate. |
| 2026-07-01 | Codex remediation continuation | Phase and final validation completed for the implemented owner-facade slices | Touched owner/workbook/import files listed in this handoff | `make phase-slice PHASE=phase9`; `make phase-slice PHASE=phase11`; `make agent-finalize`; `make test-fast` | All passed. Phase 9 run root `.cartulary/test-results/20260701T010906Z-p309840`; Phase 11 run root `.cartulary/test-results/20260701T011002Z-p323946`; agent-finalize root `.cartulary/test-results/20260701T011041Z-p337196`; test-fast root `.cartulary/test-results/20260701T011056Z-p338612` | Retained-run reuse in `make agent-finalize` was skipped because `RESULTS_DIR` was unset; OQ-03 and OQ-05 remain unresolved | Do not mark S-04/S-05/S-06 complete until route-wire/hash ownership and legacy import unit journaling are either implemented or explicitly deferred by owner decision. |
| 2026-07-01 | Codex remediation continuation | Generated drift remediated for the new import apply journal table | Touched generated sqlc model through Make-owned generation | `make generate-drift`; `make generate`; `make generate-drift` | First drift run failed with generated sqlc model drift at `.cartulary/test-results/20260701T011611Z-p387779`; `make generate` passed at `.cartulary/test-results/20260701T011639Z-p389087`; rerun `make generate-drift` passed at `.cartulary/test-results/20260701T011646Z-p389891` | Generated SQL model changed because migration `00042_import_apply_journal.sql` adds a table; no hand edit was made to generated roots | Rerun tracker/static gates after this handoff row. |
| 2026-07-01 | Codex remediation continuation | Post-generation backend unit validation completed | Touched generated sqlc model through Make-owned generation | `make backend-unit` | Passed with run root `.cartulary/test-results/20260701T011735Z-p392073` | OQ-03 and OQ-05 remain unresolved | Final response should state the unresolved work rather than claiming S-06 completion. |
| 2026-07-01 | Codex remediation continuation | Full-check staticcheck failure remediated after owner-facade cleanup | Touched `internal/modules/artifacts/linkednotes/facade.go`, `internal/modules/imports/owner_apply.go`, and `internal/modules/workbook/mutation_store.go` | User-run `make check`; `make lint-go-staticcheck` | User-run `make check` failed at `.cartulary/test-results/20260701T011925Z-p399231` on unused linked-notes helper; first targeted rerun failed at `.cartulary/test-results/20260701T012110Z-p439052` on two additional unused helpers; after deleting dead refactor helpers, `make lint-go-staticcheck` passed at `.cartulary/test-results/20260701T012259Z-p441185` | OQ-03 and OQ-05 remain unresolved; full `make check` rerun still pending after this tracker update | Rerun tracker/static gates for this evidence row, then rerun `make check`. |
| 2026-07-01 | Codex remediation continuation | Tracker/static gates and full-check rerun completed after staticcheck cleanup | Touched this tracker after the dead-code cleanup | `make lint-markdown`; `make generated-artifact-policy-check`; `make json-shape-check`; `make check` | Tracker/static gates passed before full check: lint-markdown root `.cartulary/test-results/20260701T012404Z-p443498`, artifact policy root `.cartulary/test-results/20260701T012404Z-p443529`, JSON shape root `.cartulary/test-results/20260701T012404Z-p443533`; full `make check` passed with 276/276 work units and 973 tests at `.cartulary/test-results/20260701T012423Z-p444444`; final tracker/static gates passed after the check evidence edit: lint-markdown root `.cartulary/test-results/20260701T012730Z-p525518`, artifact policy root `.cartulary/test-results/20260701T012730Z-p525539`, JSON shape root `.cartulary/test-results/20260701T012730Z-p525564` | OQ-03 and OQ-05 remain unresolved; S-04/S-05/S-06 are not complete | Hand off with unresolved items explicit. |
...
| 2026-07-01 | Codex remediation continuation | Main remaining risks are route-wire/hash adapter ownership and unit-atomic apply journaling for timeline/host/identity/indicator import paths | This tracker plus owner/workbook/import implementation files | Validation commands listed above, including final `make test-fast` | Implemented owner facades for linked notes and decision supersede; import generic owner path and apply journal for required non-timeline/non-entity targets; owner validation and links collection mutation split | OQ-03 and OQ-05 remain; S-06 cannot be marked DONE without resolving or formally deferring them | Next work should add transaction-scoped import create hooks for legacy timeline/entity targets or owner-downgrade those targets before mapping approval, then extract the remaining generic patch/query coordination. |
...
| OQ-03 | `TODO: determine whether route-facing workbook DTOs remain a stable public adapter or split into owner adapters after characterization` | DTO moves risk hash, validation, and generated-contract drift | Core route contract evidence, mutation API tests, OpenAPI schemas | IN_PROGRESS: create validation, direct patch validation, and collection side effects delegate to owners; route-facing decode/hash wrappers remain workbook-compatible pending a broader route-wire adapter split. |
...
| OQ-05 | `TODO: complete unit-atomic apply journal for legacy timeline/host/identity/indicator import paths` | Required non-timeline/non-entity targets now have owner create facades and journal rows, but existing timeline/entity facades commit one row per owner call and do not expose a selected-unit transaction hook | Timeline/entity owner facade designs, import replay/idempotency tests, Phase 11 import validation | IN_PROGRESS |
