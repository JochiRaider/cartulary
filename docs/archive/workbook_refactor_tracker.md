...
| WB-WS-03 | WorkbookShell responsibility decomposition analysis. | done | WB-WS-02 | `WorkbookShell.tsx`, shell components/hooks | `make frontend-unit`; `make frontend-typecheck` | Initial behavior-preserving shell seam extracted: selected grid-row materialization and edit-target selection moved to models, and pending grid-focus retry moved to a shell hook. Further component slimming remains under specific grid/query/pending/inspector/generic workstreams. |
| WB-WS-04 | Grid facade usage and vendor-boundary cleanup planning. | done | WB-WS-02 | Grid-related components/utils/models | `make frontend-import-boundary-check`; `make frontend-unit`; `make frontend-typecheck` | Generic saved-row grid materialization lives in `workbookContractRows.ts`; selected-row materialization is centralized there; timeline grid-row metadata now lives in `timelineRowsModel.ts`; vendor mechanics remain in `@cartulary/grid-adapter`. |
...
| WB-WS-06 | Test characterization and fixture ownership planning. | done | WB-WS-02 | `WorkbookShell.*.test.*`, model tests, testing helpers | Narrow `make frontend-unit VITEST_FLAGS=...`; full `make frontend-unit` | Shell-export cleanup landed; repeated default Timeline render setup now routes through `timelineWorkbookRenderTestSupport.tsx` for selected phase files while phase filenames and characterization assertions remain intact. |
| WB-WS-07 | Autosave, pending, save-state, and action-sequencing seam planning. | done | WB-WS-03, WB-WS-06 | Pending queue, timeline runtime, save-state components | Phase3/4 tests and pending queue tests; `make frontend-unit`; `make frontend-typecheck` | Timeline record-action, delete/restore, rollback, and same-field conflict-resolution payloads now route through `timelineMutationRequests.ts`; pending queue behavior remains model-owned. |
| WB-WS-08 | Inspector, history, sentinel, focus, and continuity seam planning. | done | WB-WS-03, WB-WS-06 | Timeline inspector/history/focus utilities | Phase7/9 tests and model tests; `make frontend-unit`; `make frontend-typecheck` | History subject selection now lives in `timelineHistoryModel.ts`; shell pending grid focus is hook-owned; default-closed inspector and focus behavior remain characterized. |
| WB-WS-09 | Saved-view/query and multi-surface seam planning. | done | WB-WS-03, WB-WS-06 | Runtime/query/surface registry | Phase8/surfaces/model tests; `make frontend-unit`; `make frontend-typecheck` | Query surface-slot and contract lookup now live in `workbookSurfaceQueryRuntime.ts`; `useWorkbookShellRuntime.ts` uses the seam for query-state selection, saved-view application, and query clearing while preserving `view_schema_id`, `query_json`, and startup semantics. |
...
## 12. Remediation execution tracker

Current remediation posture: behavior-preserving implementation remediation is authorized for `apps/web/src/workbook` plus app-owned services and test helpers. Core specs, generated artifacts, backend workbook routes, authorization behavior, persistence behavior, and public wire contracts are out of scope unless a later slice discovers that an implementation change cannot remain behavior-preserving. In that case, stop the slice and update the owning Core spec or contract inputs before implementation.

Controlling rule for this remediation: update this tracker after each completed workstream and before starting the next one. Each update must record changed files, validation commands, run roots when available, residual risks, and the next active workstream.

| Remediation ID | Workstream | Status | Depends on | Scope | Required validation | Exit condition |
| --- | --- | --- | --- | --- | --- | --- |
| WB-REM-01 | Spec/tracker preflight | done | None | This tracker, owner-doc posture | Tracker update only; final markdown validation in WB-REM-10 | Tracker records the remediation plan, scope, and next active workstream. |
| WB-REM-02 | Baseline characterization | done | WB-REM-01 | Make target discovery and first-slice baseline tests | Target explanations passed; narrow surfaces run passed tests but failed harness artifact completeness as expected for partial shell selection; full `make frontend-unit` passed; `make frontend-import-boundary-check` passed | Baseline pass/fail and run roots recorded before source edits. |
| WB-REM-03 | Shell responsibility seam | done | WB-REM-02 | `WorkbookShell.tsx`, shell hooks/models/components | Targeted model test passed; `make frontend-unit` passed; `make frontend-typecheck` passed | Shell keeps composition responsibility while non-rendering derivation moves to hooks/models. |
| WB-REM-04 | Grid facade seam | done | WB-REM-03 | Grid rows, provenance, focus helpers, grid surfaces | Targeted timeline row model test passed; full `make frontend-unit` passed; `make frontend-import-boundary-check` passed; `make frontend-typecheck` passed | Grid mechanics stay behind `@cartulary/grid-adapter`; row/focus/provenance logic is centralized. |
| WB-REM-05 | Saved-view/query/multi-surface seam | done | WB-REM-03, WB-REM-04 | `useWorkbookShellRuntime.ts`, `workbookQuery.ts`, saved-view models, surface registry | Query model, phase8 query, phase4 timeline query, full frontend unit, typecheck | Query state and saved-view persistence helpers are centralized without changing `view_schema_id`, `query_json`, `layout_json`, or startup semantics. |
| WB-REM-06 | Pending/autosave/payload seam | done | WB-REM-03 | Timeline pending saves, pending queue, timeline mutation services | Timeline payload model test, pending queue test, phase4 action/save-state, full frontend unit, typecheck | Replay/order/save-state and request payload construction are centralized without route-shape drift. |
| WB-REM-07 | Inspector/history/focus seam | done | WB-REM-03, WB-REM-04 | Timeline inspector/history/focus hooks/models | Inspector model, phase7, phase9 inspector, continuity, full frontend unit, typecheck | Row-subject invalidation, history retargeting, and continuity behavior are model/hook owned. |
| WB-REM-08 | Generic surface controller seam | done | WB-REM-05, WB-REM-04 | `GenericWorkbookSurface.tsx`, generic hooks/models/components | Generic model test passed; diagnostic surfaces run passed tests but failed narrow phase artifact completeness; full `make frontend-unit`, `make frontend-typecheck`, and `make frontend-import-boundary-check` passed | Generic surface mutation state, patch submission, refresh completion, and validation errors are hook-owned while presentation structure remains stable. |
| WB-REM-09 | Test-support consolidation | done | WB-REM-03 through WB-REM-08 | App-owned test support and shell tests | Affected phase tests passed at test level; phase4 save-state narrow run passed; phase3 autosave/payload narrow runs passed tests but failed narrow phase artifact completeness; full `make frontend-unit` and `make frontend-typecheck` passed | Duplicate default Timeline render setup is reduced without weakening phase characterization or runtime boundaries. |
| WB-REM-10 | Validation and handoff completion | done | WB-REM-01 through WB-REM-09 | Full remediation handoff | Final frontend, policy, markdown, finalize, and broader `make test-fast` gates passed | Tracker contains final changed files, run roots, skipped checks, residual risks, and next recommendation. |

### Remediation handoff log

| Time | Workstream | State | Files changed | Validation | Run roots / evidence | Next action |
| --- | --- | --- | --- | --- | --- | --- |
| 2026-06-28 remediation start | WB-REM-01 | Started behavior-preserving remediation from the approved plan. No Core spec, generated artifact, backend route, authorization, persistence, or public wire-contract changes are planned. | `docs/handoffs/workbook_refactor_tracker.md` | Not yet run in this session. | Pending. | Complete tracker preflight, then run baseline characterization before source edits. |
| 2026-06-28 remediation preflight | WB-REM-01 | Completed tracker preflight and activated baseline characterization. | `docs/handoffs/workbook_refactor_tracker.md` | Not yet run in this session; markdown validation deferred to WB-REM-10. | Pending. | Run Make-owned baseline discovery and targeted shell/surface characterization. |
| 2026-06-28 baseline characterization | WB-REM-02 | Completed baseline characterization. Narrow `WorkbookShell.surfaces.test.tsx` selection passed tests but failed harness phase artifact completeness, so full frontend unit is canonical baseline. | `docs/handoffs/workbook_refactor_tracker.md` | `make explain-target TARGET=frontend-unit DETAIL=summary` passed; `make explain-target TARGET=frontend-import-boundary-check DETAIL=summary` passed; `make frontend-unit VITEST_FLAGS='apps/web/src/workbook/WorkbookShell.surfaces.test.tsx'` failed with harness artifact completeness after tests passed; `make frontend-unit` passed; `make frontend-import-boundary-check` passed. | `.cartulary/test-results/20260628T054557Z-p2155973` partial harness failure; `.cartulary/test-results/20260628T054609Z-p2157052` frontend-unit pass; `.cartulary/test-results/20260628T054630Z-p2158777` import-boundary pass. | Start shell responsibility seam. |
| 2026-06-28 shell responsibility seam | WB-REM-03 | Completed initial shell seam. `WorkbookShell.tsx` now delegates selected grid-row materialization to `workbookGridRows`, generic edit-target selection to `genericWorkbookModel`, and pending grid-focus retry behavior to `useWorkbookPendingGridFocus`. | `apps/web/src/workbook/WorkbookShell.tsx`; `apps/web/src/workbook/hooks/useWorkbookPendingGridFocus.ts`; `apps/web/src/workbook/models/workbookContractRows.ts`; `apps/web/src/workbook/models/genericWorkbookModel.ts`; `apps/web/src/workbook/models/genericWorkbookModel.test.ts`; this tracker | `make frontend-unit VITEST_FLAGS='apps/web/src/workbook/models/genericWorkbookModel.test.ts'` passed; `make frontend-unit` passed; `make frontend-typecheck` passed. | `.cartulary/test-results/20260628T054857Z-p2161100` targeted model pass; `.cartulary/test-results/20260628T054949Z-p2162371` frontend-unit pass; typecheck passed with no emitted run root in command output. | Start grid facade seam. |
| 2026-06-28 grid facade seam | WB-REM-04 | Completed grid row metadata consolidation. Timeline grid-row metadata/test IDs now route through `buildTimelineGridRows`; selected grid-row metadata is shared through `workbookGridRows`; no new grid vendor imports were introduced. | `apps/web/src/workbook/timeline/components/TimelineWorkbook.tsx`; `apps/web/src/workbook/timeline/models/timelineRowsModel.ts`; `apps/web/src/workbook/timeline/models/timelineRowsModel.test.ts`; `apps/web/src/workbook/models/workbookContractRows.ts`; this tracker | `make frontend-unit VITEST_FLAGS='apps/web/src/workbook/timeline/models/timelineRowsModel.test.ts'` passed; diagnostic `make frontend-unit VITEST_FLAGS='apps/web/src/workbook/WorkbookShell.phase3.grid.test.tsx'` passed tests but failed narrow phase artifact completeness; `make frontend-unit` passed; `make frontend-import-boundary-check` passed; `make frontend-typecheck` passed. | `.cartulary/test-results/20260628T055422Z-p2167040` targeted timeline model pass before type fixes; `.cartulary/test-results/20260628T055557Z-p2172261` targeted timeline model pass after type fixes; `.cartulary/test-results/20260628T055557Z-p2172284` frontend-unit pass; `.cartulary/test-results/20260628T055448Z-p2169203` import-boundary pass; `.cartulary/test-results/20260628T055434Z-p2168098` diagnostic phase3 narrow harness artifact failure after tests passed. | Start saved-view/query/multi-surface seam. |
| 2026-06-28 saved-view/query seam | WB-REM-05 | Completed query surface-slot seam. `workbookSurfaceQueryRuntime.ts` owns view-schema-to-query-slot mapping and contract lookup; `useWorkbookShellRuntime.ts` uses it for current query state, saved-view query application, and clear-active-query behavior. | `apps/web/src/workbook/hooks/useWorkbookShellRuntime.ts`; `apps/web/src/workbook/models/workbookSurfaceQueryRuntime.ts`; `apps/web/src/workbook/models/workbookQuery.test.ts`; this tracker | `make frontend-unit VITEST_FLAGS='apps/web/src/workbook/models/workbookQuery.test.ts'` passed; `make frontend-unit VITEST_FLAGS='apps/web/src/workbook/WorkbookShell.phase8.query.test.tsx'` passed; `make frontend-unit VITEST_FLAGS='apps/web/src/workbook/WorkbookShell.phase4.timelineQuery.test.tsx'` passed; diagnostic `make frontend-unit VITEST_FLAGS='apps/web/src/workbook/WorkbookShell.surfaces.test.tsx'` passed tests but failed narrow phase artifact completeness; `make frontend-unit` passed; `make frontend-typecheck` passed. | `.cartulary/test-results/20260628T055811Z-p2175613` query model pass; `.cartulary/test-results/20260628T055825Z-p2176655` phase8 query pass; `.cartulary/test-results/20260628T055837Z-p2177708` phase4 timeline query pass; `.cartulary/test-results/20260628T055838Z-p2177776` diagnostic surfaces narrow harness artifact failure after tests passed; `.cartulary/test-results/20260628T055852Z-p2179804` frontend-unit pass; typecheck passed with no emitted run root in command output. | Start pending/autosave/payload seam. |
| 2026-06-28 pending/autosave/payload seam | WB-REM-06 | Completed payload-builder extraction for compact timeline action/history/conflict routes. Request body construction now lives in `timelineMutationRequests.ts`; component fetch paths, methods, transaction IDs, and response handling are unchanged. | `apps/web/src/workbook/timeline/components/TimelineWorkbook.tsx`; `apps/web/src/workbook/timeline/services/timelineMutationRequests.ts`; `apps/web/src/workbook/timeline/models/workbookTimelineModel.test.ts`; this tracker | `make frontend-unit VITEST_FLAGS='apps/web/src/workbook/timeline/models/workbookTimelineModel.test.ts'` passed; `make frontend-unit VITEST_FLAGS='apps/web/src/workbook/utils/workbookPendingQueue.test.ts'` passed; `make frontend-unit VITEST_FLAGS='apps/web/src/workbook/WorkbookShell.phase4.actionSequencing.test.tsx'` passed; `make frontend-unit VITEST_FLAGS='apps/web/src/workbook/WorkbookShell.phase4.saveState.test.tsx'` passed; diagnostic phase3 autosave/payload narrow runs passed tests but failed phase artifact completeness; `make frontend-unit` passed; `make frontend-typecheck` passed. | `.cartulary/test-results/20260628T060305Z-p2183073` timeline payload model pass; `.cartulary/test-results/20260628T060305Z-p2183147` pending queue pass; `.cartulary/test-results/20260628T060322Z-p2185170` phase4 action sequencing pass; `.cartulary/test-results/20260628T060322Z-p2185325` phase4 save-state pass; `.cartulary/test-results/20260628T060322Z-p2185137` phase3 autosave diagnostic harness artifact failure after tests passed; `.cartulary/test-results/20260628T060322Z-p2185139` phase3 payload diagnostic harness artifact failure after tests passed; `.cartulary/test-results/20260628T060337Z-p2189252` frontend-unit pass; typecheck passed with no emitted run root in command output. | Start inspector/history/focus seam. |
| 2026-06-28 inspector/history/focus seam | WB-REM-07 | Completed history-subject model extraction. `useTimelineHistoryState` delegates live/deleted/draft/no-row subject selection to `timelineHistoryModel.ts`; inspector model tests cover deleted/live/draft retargeting. | `apps/web/src/workbook/timeline/hooks/useTimelineHistoryState.ts`; `apps/web/src/workbook/timeline/models/timelineHistoryModel.ts`; `apps/web/src/workbook/models/workbookInspectorModel.test.ts`; this tracker | `make frontend-unit VITEST_FLAGS='apps/web/src/workbook/models/workbookInspectorModel.test.ts'` passed; `make frontend-unit VITEST_FLAGS='apps/web/src/workbook/WorkbookShell.phase7.test.tsx'` passed; `make frontend-unit VITEST_FLAGS='apps/web/src/workbook/WorkbookShell.phase9.inspector.test.tsx'` passed; `make frontend-unit VITEST_FLAGS='apps/web/src/workbook/utils/workbookContinuity.test.ts'` passed; diagnostic phase9 sentinel narrow run passed tests but failed phase artifact completeness; `make frontend-unit` passed; `make frontend-typecheck` passed. | `.cartulary/test-results/20260628T060552Z-p2191883` inspector model pass; `.cartulary/test-results/20260628T060607Z-p2192933` phase7 pass; `.cartulary/test-results/20260628T060607Z-p2192966` phase9 inspector pass; `.cartulary/test-results/20260628T060607Z-p2193137` continuity pass; `.cartulary/test-results/20260628T060607Z-p2192967` diagnostic phase9 sentinel harness artifact failure after tests passed; `.cartulary/test-results/20260628T060622Z-p2197026` frontend-unit pass; typecheck passed with no emitted run root in command output. | Start generic surface controller seam. |
| 2026-06-28 generic surface controller seam | WB-REM-08 | Completed generic mutation controller extraction. `GenericWorkbookSurface.tsx` now delegates mutation status, patch submission, refresh completion, and validation errors to `useGenericSurfaceMutationController`; generic text normalization and edit-target selection are model-owned. | `apps/web/src/workbook/components/GenericWorkbookSurface.tsx`; `apps/web/src/workbook/hooks/useGenericSurfaceMutationController.ts`; `apps/web/src/workbook/models/genericWorkbookModel.ts`; `apps/web/src/workbook/models/genericWorkbookModel.test.ts`; this tracker | `make frontend-unit VITEST_FLAGS='apps/web/src/workbook/models/genericWorkbookModel.test.ts'` passed; `make frontend-typecheck` passed; diagnostic `make frontend-unit VITEST_FLAGS='apps/web/src/workbook/WorkbookShell.surfaces.test.tsx'` passed tests but failed narrow phase artifact completeness; `make frontend-unit` passed; `make frontend-import-boundary-check` passed. | `.cartulary/test-results/20260628T061412Z-p2201355` generic model pass; typecheck passed with no emitted run root in command output; `.cartulary/test-results/20260628T061425Z-p2202759` diagnostic surfaces narrow harness artifact failure after tests passed; `.cartulary/test-results/20260628T061425Z-p2202785` frontend-unit pass; `.cartulary/test-results/20260628T061445Z-p2205518` import-boundary pass. | Start test-support consolidation. |
| 2026-06-28 test-support consolidation | WB-REM-09 | Completed scoped test-support consolidation. Added a narrow Timeline render helper and migrated selected phase3 autosave, phase3 payload, and phase4 save-state tests to reduce repeated default Timeline rendering without changing phase filenames or assertions. | `apps/web/src/testing/timelineWorkbookRenderTestSupport.tsx`; `apps/web/src/workbook/WorkbookShell.phase3.autosave.test.tsx`; `apps/web/src/workbook/WorkbookShell.phase3.payload.test.tsx`; `apps/web/src/workbook/WorkbookShell.phase4.saveState.test.tsx`; this tracker | `make frontend-unit VITEST_FLAGS='apps/web/src/workbook/WorkbookShell.phase3.autosave.test.tsx'` passed tests but failed narrow phase artifact completeness; `make frontend-unit VITEST_FLAGS='apps/web/src/workbook/WorkbookShell.phase3.payload.test.tsx'` passed tests but failed narrow phase artifact completeness; `make frontend-unit VITEST_FLAGS='apps/web/src/workbook/WorkbookShell.phase4.saveState.test.tsx'` passed; `make frontend-unit` passed; `make frontend-typecheck` passed. | `.cartulary/test-results/20260628T061716Z-p2206901` phase3 autosave diagnostic harness artifact failure after tests passed; `.cartulary/test-results/20260628T061716Z-p2206870` phase3 payload diagnostic harness artifact failure after tests passed; `.cartulary/test-results/20260628T061716Z-p2206929` phase4 save-state pass; `.cartulary/test-results/20260628T061730Z-p2209973` frontend-unit pass; typecheck passed with no emitted run root in command output. | Start final validation and handoff completion. |
| 2026-06-28 final validation and handoff | WB-REM-10 | Completed end-to-end validation and handoff. `make format` was run after the first `lint-biome` pass found formatting/import-order diagnostics in touched authored frontend files; final validation was rerun after formatting. | All files listed in the final remediation summary below; this tracker | Final `make frontend-typecheck`, `make frontend-import-boundary-check`, `make lint-biome`, `make frontend-unit`, `make json-shape-check`, `make generated-artifact-policy-check`, `make lint-markdown`, `make agent-finalize`, and broader `make test-fast` passed. | Final pass roots: `.cartulary/test-results/20260628T061954Z-p2217772` frontend-typecheck; `.cartulary/test-results/20260628T061954Z-p2217771` import-boundary; `.cartulary/test-results/20260628T061954Z-p2217832` lint-biome; `.cartulary/test-results/20260628T061954Z-p2217831` frontend-unit; `.cartulary/test-results/20260628T061954Z-p2217849` json-shape; `.cartulary/test-results/20260628T061954Z-p2217853` generated-artifact-policy; `.cartulary/test-results/20260628T062325Z-p2253055` lint-markdown after final tracker edit; `.cartulary/test-results/20260628T062019Z-p2221449` agent-finalize; `.cartulary/test-results/20260628T062033Z-p2222530` test-fast. First `lint-biome` failure root before format: `.cartulary/test-results/20260628T061831Z-p2212278`; format root: `.cartulary/test-results/20260628T061928Z-p2216203`. | No active remediation workstream remains. |

### Final remediation summary

Changed runtime seam files:
...
- `apps/web/src/workbook/hooks/useWorkbookPendingGridFocus.ts`
- `apps/web/src/workbook/hooks/useWorkbookShellRuntime.ts`
- `apps/web/src/workbook/models/genericWorkbookModel.ts`
- `apps/web/src/workbook/models/workbookContractRows.ts`
- `apps/web/src/workbook/models/workbookSurfaceQueryRuntime.ts`
- `apps/web/src/workbook/timeline/components/TimelineWorkbook.tsx`
- `apps/web/src/workbook/timeline/hooks/useTimelineHistoryState.ts`
- `apps/web/src/workbook/timeline/models/timelineHistoryModel.ts`
- `apps/web/src/workbook/timeline/models/timelineRowsModel.ts`
- `apps/web/src/workbook/timeline/services/timelineMutationRequests.ts`

Changed tests and test-support files:
...
- `apps/web/src/workbook/models/genericWorkbookModel.test.ts`
- `apps/web/src/workbook/models/workbookInspectorModel.test.ts`
- `apps/web/src/workbook/models/workbookQuery.test.ts`
...
- `docs/handoffs/workbook_refactor_tracker.md`

Skipped checks:

- `make check` was not run. The remediation remained frontend/workbook-scoped with no Core spec, generated contract, backend route, authorization, persistence, or public wire-contract changes; the broader gate selected for final coverage was `make test-fast`, which passed.

Residual risks:

- Several narrow `make frontend-unit VITEST_FLAGS=...` phase selections passed their tests but failed harness phase artifact completeness. Full `make frontend-unit` and final `make test-fast` passed, so those partial-run failures remain diagnostic harness-selection behavior rather than unresolved test failures.
- Future workbook work should continue to avoid broad component rewrites and keep each seam behind model/hook/service boundaries with targeted phase evidence.

Next recommendation:

- No active remediation workstream remains. Future expansion should start from a new tracker row or successor handoff and treat this tracker as the completed baseline for workbook shell, grid, query, pending/payload, inspector/history/focus, generic-surface, and test-support seams.
...
Readiness: this tracker records completed behavior-preserving workbook remediation with 2026-06-28 validation evidence above. It does not by itself authorize later behavior changes or broad runtime seam extraction without a new scoped workstream and slice-specific validation.
