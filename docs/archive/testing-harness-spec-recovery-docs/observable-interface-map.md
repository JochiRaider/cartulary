...
- S7 maintainer decisions require provider-neutral CI, explicit retained
  artifact identity for durable claims, and demotion of stale
  `run-harness-smoke-extended` from blocking phase advancement. Provider
  annotations, uploaded artifacts, and final visual snapshot refresh behavior
