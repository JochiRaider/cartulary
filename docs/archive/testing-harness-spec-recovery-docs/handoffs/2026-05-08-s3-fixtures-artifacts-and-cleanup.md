...
| `scripts/lib/artifact-discovery.mjs`, `scripts/lib/result-artifacts.mjs`, `tools/harness/test-support/harness-artifact-assert.mjs` | Result artifact discovery and log assertion behavior | `observed` | Retained artifacts were treated as source-limited. |
| `scripts/start-web-e2e.sh`, `scripts/reset-web-e2e-stack.sh`, `tools/harness/readiness/process-lifecycle.sh`, `scripts/lib/playwright-owned-stack.sh` | Browser stack runtime roots, ports, process groups, reset behavior, cleanup | `observed` | Browser/service commands were not executed. |
