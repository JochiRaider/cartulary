...
| Acceptance command results | Phase C retained exact harness artifact evidence for `SWFS-AC-014` only. Release-wide rows, SDK-only dependency rows, docs-wide rows, backup/restore rows, migration rows, and release-gate rows remain unclaimed. | See Sections 5.4, 6, and 8. |
...
- Phase B retained `cartulary.object_store_capability_probe.v1` with `result='pass'` for local `services-up` at `.cartulary/test-results/20260603T195218Z-p648759/services-up/object-store-capability-probe.json`. TODO: broader compatibility and adapter-hardening probe evidence.
...
### 5.5 Acceptance command backlog

All plan-owned acceptance commands below remain unrun and all rows remain unclaimed unless a retained artifact supplies the required evidence. Phase B makes `SWFS-AC-005` and `SWFS-AC-007` claimable with the retained local evidence named in Section 5.3. Phase C makes `SWFS-AC-014` claimable for harness artifacts with the retained evidence named in Section 5.4. No release-wide row is claimed.
...
Every `SWFS-AC-*` row remains non-claimable in this handoff unless the current status explicitly names retained evidence. Phase C names retained pass evidence only for `SWFS-AC-014`; Phase B local rows remain separately limited to their Phase B local evidence.
...
Phase C classified remaining `MinIO`/`minio` references after the harness replacement and Make-owned regeneration.
...
| Allowed `minio-go` SDK-only reference | `allowed` | `go.mod`, `go.sum`, `internal/platform/objectstore`, `internal/testutil/s3test`, `tools/objectstoreprobe`, and dependency-list docs retain `github.com/minio/minio-go/v7` or `minio.*` SDK symbols as generic S3-compatible client usage. This does not claim `SWFS-AC-002` or `SWFS-AC-022`; dependency-boundary and SBOM/license evidence remain later-phase work. |
...
| Later-phase release/SBOM/docs blocker | `blocked_outside_phase_c` | `scripts/generate-sbom-license-evidence.mjs` still scans for `minio/minio` images for release evidence; `tools/phase10browserrestore` still has later-phase backup/restore credential defaults; release/docs/source-archive cleanup, SBOM/license evidence, and full occurrence inventory remain Phase G or broader migration blockers. |
...
| Backend-neutral config/service-ref test label | `later_phase_or_owner_cleanup` | `internal/platform/config/config_phase0_test.go` still uses the literal `minio-primary` as a managed object-storage service-ref fixture. It is not a Phase C harness startup/lease/reporting surface; classifying or replacing service-ref label examples belongs with broader config/owner or occurrence-inventory cleanup. |
| Adapter implementation error wording | `later_phase_adapter_hardening` | `internal/platform/objectstore/objectstore.go` still says `create minio client` while using the SDK. This is adapter-facing implementation wording and remains outside Phase C except as a later Phase D cleanup candidate. |
...
4. Harden the object-store adapter beyond the Phase B local probe while preserving public blob-slot, attach, preview, download, authorization, and evidence lifecycle semantics.
5. Implement backup/restore and migration tooling only after the owner dependencies and adapter/probe evidence are in place.
6. Finish with SBOM/license, occurrence inventory, release manifest exposure scan, `make agent-finalize`, and the full release gate after all release blockers are closed.
