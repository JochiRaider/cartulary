...
Planning inspection was performed against commit `92fa79edbedc95c9c6ad51e666b107b09dc0755c`. Phase A contract cleanup and Phase B local service replacement were implemented in the working tree against that same commit. Phase C harness replacement was implemented in the working tree at live repository commit `92e679dcc8031e476e5586f0b11adfcaa93647d1`. Phase D adapter hardening and Phase E backup/restore were implemented in the current working tree without changing generated contracts, lockfiles, database schema, or public route families.
...
Every `SWFS-AC-*` row remains non-claimable in this handoff unless the current status explicitly names retained evidence. Phase D names retained pass evidence for `SWFS-AC-007`; Phase C names retained pass evidence for `SWFS-AC-014`; Phase B local evidence remains limited to `SWFS-AC-005`; Phase E names retained pass evidence for `SWFS-AC-016` and `SWFS-AC-017`.
...
Phase E reclassified remaining `MinIO`/`minio` references touched by backup/restore work and preserved the Phase D adapter boundary.
...
| Allowed `minio-go` SDK-only reference | `allowed_sdk_only` | `go.mod`, `go.sum`, `internal/platform/objectstore`, `internal/testutil/s3test`, `tools/objectstoreprobe`, and dependency-list docs retain `github.com/minio/minio-go/v7` or `minio.*` SDK symbols as generic S3-compatible client usage behind the object-store adapter, probe, and fixture boundary. Phase E recovery code does not import MinIO or SeaweedFS SDKs directly. This does not claim `SWFS-AC-002` or `SWFS-AC-022`; dependency-boundary and SBOM/license evidence remain later-phase work. |
| Invalid user-facing/default-service wording | `none_found_in_phase_e_scope` | The active Phase E runtime/default surfaces touched by this work no longer use MinIO user-facing defaults. `tools/phase10browserrestore` now uses SeaweedFS local credential defaults instead of the legacy `minioadmin` default. |
...
| Phase F/G blocker | `blocked_outside_phase_e` | `scripts/generate-sbom-license-evidence.mjs` still scans for `minio/minio` images for release evidence. Storage-ref migration-owner wording, release/docs/source-archive cleanup, SBOM/license evidence, and full occurrence inventory remain Phase F/G or broader migration blockers. |
...
- Runtime implementation evidence remains partial: Phase D retained adapter and backend route dependency-error evidence, and Phase E retained backup/restore evidence. Full runtime-outage E2E, browser evidence flows, readiness route shape, migration, and threat-model release checks remain missing. Storage-ref grammar and key-generation behavior remain owner-blocked by `SWFS-OWNER-STORAGEREF-001`.
...
6. Implement migration tooling only after the storage-ref owner dependency and adapter/probe evidence are in place.
