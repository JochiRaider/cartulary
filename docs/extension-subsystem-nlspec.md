...
coordinates Network Flow Activity 6.0.0/major 6/state 4, and selects only the
...
| `resource_ref_kind` | Exact Core 01 §3.3.9.1 registered kind, or qualified `<profile_id>.<local_key>` contribution admitted by that owner; maximum 129 ASCII bytes. This admits owner-registered identities, not compatibility aliases. |
...
The typed `admission_validation` fact carries `preflight_algorithmid` and
`post_migration_algorithmid` as exact nullable algorithm identities, alongside
`schema_id` and `dependency_probes`. Its descriptor projection retains those
identities. The implementation binding projects them to
`preflight_algorithm_id` and `post_migration_algorithm_id`; a non-null identity
MUST resolve to a statically packaged implementation. References in the logical
admission procedure identify these same algorithms; generators MUST NOT look
for prohibited owner locators in typed fragments or silently turn an algorithm
identity into `null`.
...
Flow Activity contract major `6` retains semantic-query v2, effective-limit,
...
verification routing. A client that does not support major `6` omits the
...
| `EXT-AC-156` | The standard client registry covers every claimable workspace profile including Network Flow major `6`/`network_analysis`, and linearized reservation, stale/current generation, concurrency, rollover, authorization loss, and Base identity preservation are exact. |
...
## Saved-graph initiating receipt boundary

Core 01 owns the narrow Common Job reference and permits adopted initiating
owners to return their committed resource alongside that reference. This does
not expose job/worker kinds or execution metadata. Network Flow owns its exact
saved-graph receipt and preserves it immutably. Finalization reconciles that
receipt through a transaction-bound owner port that validates request hash,
target and job identity without replacing bytes. Other Core job owners retain
their declared terminal-receipt behavior. Proof validation and job execution
