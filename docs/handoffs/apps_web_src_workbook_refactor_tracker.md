...
| `apps/web/src/workbook/models/workbookQuery.ts` | Query, saved-view query, layout serialization model | Query and layout types and functions | view-contracts | `/apps/web` query model | Production authored private model | View query request, saved-view `query_json`, `layout_json` | High | query model, phase8 unit and browser tests | Recommended first seam dependency. |
...
| `apps/web/src/workbook/models/workbookSavedViewRuntime.test.ts` | Saved-view runtime tests | none | view-contracts, Vitest, runtime model, saved-view/startup types | `/apps/web` tests | Test authored | Selection identity, modified state | Medium | Self test | Recommended first slice guard. |
| `apps/web/src/workbook/models/workbookSavedViewRuntime.ts` | Saved-view selection, deletion, modified-state helpers | Types and functions | view-contracts types, query model, saved-view/startup types, surface registry | `/apps/web` shell runtime model | Production authored private model | Saved-view identity, base-surface fallback, query/layout comparison | High | runtime and surface tests | Recommended first slice core. |
...
| S-07 phase-shaped production helper rename | S-04 | Rename `workbookShellPhase4.ts` to a behavior-named collaboration or mention service | `timeline/services/workbookShellPhase4.ts`, imports, tests | Payload shapes and WebSocket reducer behavior | `workbookSocketLifecycle.test.ts`, phase4 support tests, phase6 tests | `make frontend-unit`, `make frontend-typecheck`, `make frontend-import-boundary-check` | Pure rename plus import changes; no logic edits | TODO |
| S-08 browser characterization gap fill | Any high-risk UI slice | Add minimal characterization only when existing tests miss a moved behavior | Tests only, no production code | Product behavior unchanged | Decide after slice diff | `make frontend-unit` or browser target matching the gap | Revert added test if it encodes implementation detail instead of behavior | PLANNED |

Recommended first implementation slice: S-01 saved-view/query runtime seam.
...
| 2026-06-27 | Saved views and query controls | Strong model tests and browser phase8 coverage | Medium | Recommended first slice. |
...
| R-005 | Phase-shaped runtime helper name could invite production phase coupling. | `/apps/web` | WF-06, WF-09 | Rename in isolated no-logic slice after tests pass. |
...
| Field | Value |
| --- | --- |
| Date/time | `2026-06-27T17:49:41-04:00` after remediation evidence closure and validation |
| Branch/commit | `main` at `6b36f1362917c069362a0dd7d3fc63c484a36eff` |
| Target seam | `apps/web/src/workbook` workbook shell/controller/query/mutation/conflict/inspector/presence |
| Current workflow | WF-13 handoff and next-slice bootstrap |
| Completed workflows | WF-00 through WF-06 and WF-08 through WF-13 planning; remediation evidence closure for backend, generated, e2e, phase-map, retained-artifact, contract, and migration TODOs |
| Changed files | `docs/handoffs/apps_web_src_workbook_refactor_tracker.md` only |
| Commands run | `git status --short --branch`; `git rev-parse HEAD`; `date -Iseconds`; `rg --files apps/web/src/workbook`; targeted production boundary scan; backend route scan; generated no-edit header scan; full `apps/web/e2e` spec/test inventory; FE-P0 through FE-P11 phase-map summaries; contract/migration/query inventory; marker scan; `make generated-artifact-policy-check`; `make json-shape-check` |
| Passing validation | `make generated-artifact-policy-check` at `.cartulary/test-results/20260627T214933Z-p1300061`; `make json-shape-check` at `.cartulary/test-results/20260627T214933Z-p1300076` |
| Failing validation | None |
| Decisions made | Omit WF-07; first implementation slice should be saved-view/query runtime seam; no generated hand edits; backend/generated/e2e/phase-map/contract evidence is now inventoried but remains implementation-out-of-scope unless future behavior changes require it |
| Open questions | Future slice-specific retained artifacts and backend implementation details must be refreshed when a slice crosses those boundaries; no open blocker for this remediation artifact |
| Blockers | None identified |
| Next recommended workflow | Implement S-01 with WF-00 refresh and WF-09 checkpoints |
| Safe restart command | `git status --short --branch && rg --files apps/web/src/workbook | sort` |
