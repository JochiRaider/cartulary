...
| Completion checkpoint | Enterprise Authentication now has one owner-local `ProfileID` and constants for its four canonical contribution IDs. Application assembly derives one immutable admission decision with `PublicationCatalog.ExactProfileContributionSet`: an unadmitted profile has no executable projection, and an admitted profile must have exactly the four HTTP route contributions plus its exact implementation binding or startup fails. That single decision gates provider-manifest reads, secret-purpose registration, provider reconciliation, production OIDC/SAML verifier construction, the three Enterprise route families, and the shared user-auth-binding branch. Base authentication registration no longer installs `/api/v1/auth/providers`, `/api/v1/auth/oidc/*`, or `/api/v1/auth/saml/*`; those routes have a dedicated exact-catalog registrar, while `/api/v1/users/{user_id}/auth-bindings` is enabled through a narrow Base registrar option only when its row is admitted. Auth no longer consumes the broad HTTP epoch or independently tests claim state. The unused platform helper that could reconcile providers directly from raw `cfg.EnterpriseAuthentication.Claimed` was deleted. Unclaimed reserved-route behavior remains owned by the shared HTTP admission projection, and no inactive provider secret, manifest, verifier, or egress path is prepared. The adopted N/A posture is unchanged: this profile contributes no worker, job, participant, workspace, extension-state, portability, or backup binding. Public routes, provider/session/binding contracts, manifest format, configuration keys, authorization, discovery, and frontend behavior are unchanged. Authored implementation surfaces are `internal/app/extensionassembly/{publication_catalog.go,publication_catalog_test.go}`, `internal/app/server/runtime.go`, `internal/modules/auth/{routes.go,routes_test.go,enterprise_routes.go,user_admin_handlers.go}`, `internal/platform/enterpriseauth/manifest.go`, and this tracker; no contract, generated artifact, migration, or frontend source changed. `make format` passed at `.cartulary/test-results/20260724T205246Z-p6966`. `make test-slice OWNER=module.auth` passed 9/9 work units and 48 tests, including the production webserver-backed Enterprise browser cases, at `.cartulary/test-results/20260724T205306Z-p9844`; `make service-backed-test-slice OWNER=module.auth` passed 9/9 work units and 29 tests at `.cartulary/test-results/20260724T205755Z-p46628`. App-server focused and service-backed slices passed at `.cartulary/test-results/20260724T210326Z-p79594` and `.cartulary/test-results/20260724T210514Z-p6283`; the config inactive-policy slice passed at `.cartulary/test-results/20260724T210604Z-p31219`. The final full backend unit sweep passed 302 tests at `.cartulary/test-results/20260724T210755Z-p35891`. `make generate-drift` and `make generated-artifact-policy-check` passed at `.cartulary/test-results/20260724T210632Z-p31724` and `.cartulary/test-results/20260724T210645Z-p34855`; source scans find no auth broad-epoch claim read, no runtime Enterprise `resolvedClaims.Contains` branch, no `enterpriseClaimed` field, and no raw-config reconciliation helper; `git diff --check` passes. There were no failed validations or implementation retries. The remaining production HTTP global-default/testing APIs are not consumed by auth and remain explicitly scheduled for deletion in EPA-11. This intermediate is not independently deployable; rollback is the EPA-05 commit with no data or schema action. No blocker remains. Domain vocabulary unchanged. |
...
| Completion checkpoint | Import now registers only when the exact `import.sessions_route` contribution is admitted and binds `import.discovery_v1`/`import.apply_v1` to `import.discovery_worker_v1`/`import.apply_worker_v1` through the shared catalog and serve gate. An Import-owned finalization port delegates atomically to the shared owner finalizer; discovery and apply successes emit immutable proofs without exposing internal ownership metadata in public jobs. Discovery materializes every visible nonempty XLSX sheet in workbook order, with deterministic persisted `discovery_sequence`, bounded aggregate rows/cells, closed structured locators, and stable CSV behavior. Apply revalidates selected units, dispatches each target through its source-owner facade, persists exact per-unit terminal state, reports partial outcomes without presenting them as full success, and observes cancellation between units. Network Flow retains its Import-owned scheduling and exact contribution admission; it gains no worker. The production workbook adds one lazily loaded Import Assistant under incident controls, visible only when the installed Import route is admitted; it supports CSV/XLSX upload, all-unit previews, target and field mapping, explicit select/skip, readiness/overlap checks, apply progress, cancellation, partial-outcome presentation, result navigation, stale-response rejection, and claim-loss cleanup while preserving Base workbook and clipboard behavior. Authored implementation surfaces include `internal/modules/imports/{job_finalization.go,routes.go,store.go,xlsx.go,imports_integration_test.go}`, `internal/app/extensionassembly/import_jobs.go`, `internal/app/server/runtime.go`, Network Flow constants/tests, `db/migrations/00035_import_unit_discovery_order.sql`, migration history, `apps/web/src/{shared/importCoordinator.ts,extensions/extensionAvailability.ts,workbook/WorkbookShell.tsx,workbook/features/ImportAssistantFeature.tsx,workbook/hooks/useIncidentControlsDrawer.ts}`, UI contracts/tests, Import browser/unit selectors, verification/catalog inputs, affected cross-owner fixtures, generated SQL/contracts/topology, and this tracker. Import focused and service-backed Go slices passed at `.cartulary/test-results/20260724T214002Z-p99543` and `.cartulary/test-results/20260724T213948Z-p98867`; the final claimed and unclaimed production-UI browser selectors passed 2/2 at `.cartulary/test-results/20260724T215916Z-p629`. Network Flow's complete production browser selector run reached all browser passes before one raw SQL fixture failed at `.cartulary/test-results/20260724T214016Z-p450`; the corrected focused fixture passed at `.cartulary/test-results/20260724T214537Z-p37806`, and the authoritative Network Flow service-backed slice passed 4/4 work units and 20 tests at `.cartulary/test-results/20260724T214553Z-p38275`. Backend unit passed at `.cartulary/test-results/20260724T213153Z-p74811`; final frontend typecheck and unit passed at `.cartulary/test-results/20260724T220055Z-p20610` and `.cartulary/test-results/20260724T220055Z-p20612`; formatting and generation passed at `.cartulary/test-results/20260724T215859Z-p97691` and `.cartulary/test-results/20260724T215451Z-p74569`. Final generation drift, migration drift, generated-artifact policy, and frontend import-boundary checks passed at `.cartulary/test-results/20260724T220151Z-p22983`, `.cartulary/test-results/20260724T220151Z-p22991`, `.cartulary/test-results/20260724T220151Z-p22985`, and `.cartulary/test-results/20260724T220211Z-p28131`; `git diff --check` passes. Related failures corrected during the workstream were a stale service-test binary after migration, two cross-owner raw fixtures missing discovery order, unsupported Playwright evidence in the Import verification contract, direct-write fields incorrectly excluded from assistant mappings, and an unclaimed UI assertion initially run against the intentionally Import-claimed development profile. The final Base fallback selector therefore overrides only the canonical discovery response at the browser transport boundary while exercising the production server/UI stack; no runtime claim mutation API was added. `backend-module-boundary-check` exposed the pre-existing EPA-04 test-only platform-to-Extensions import at `.cartulary/test-results/20260724T220211Z-p28133`; it is unrelated to EPA-06 surfaces and is retained for the scheduled EPA-11 DAG-guard cleanup. Public routes, job resources, profile/config identities, and Network Flow ownership remain compatible. Migration `00035` is additive inside the clean pre-release cutover and requires development reset/reseed. This intermediate is not deployable; rollback is the EPA-06 commit plus development database reset, with shared proof/cancellation evidence left immutable. No EPA-06 blocker remains. Domain vocabulary unchanged. |
...
| Completion checkpoint | Incident Portability now owns one canonical constant set for `incident_portability.bundles_route`, `incident_portability.export_v1`, `incident_portability.import_v1`, and `incident_portability.bundle_worker_v1`; application route composition uses the owner constant and therefore registers no route or worker when the exact contribution is absent. A narrow owner finalization port and application adapter bind both success paths to the shared proof contract. Export persists the archive first, then atomically inserts its descriptor/files/payload update, terminal job result, immutable proof, and final idempotency outcome; a proven rollback removes the file, while an indeterminate commit triggers the lifecycle fatal sink and deliberately preserves the possibly committed file. Import retains the existing sorted participant preparation and one physical cross-owner transaction, but no participant directly completes the job: after every owner write and the final authorization/state recheck, the transaction finalizer commits the terminal job, incident reference, proof, and idempotency outcome in that same transaction. A failed finalizer rolls back imported incident state, attribution, membership/preferences/audit, payload completion, staged transfers, terminal job state, and proof together. Indeterminate cross-owner commit suppresses destructive abandon/cleanup and enters fatal shutdown. Payload completion now requires exactly one affected row. Accepted cancellation retains exactly one observation and no proof; terminal replay returns the proof-backed final job without duplicating owner effects. Network Flow remains a declarative `blocked_when_present` portability policy and receives no worker. Authored implementation surfaces are `internal/modules/incidentbundles/{api.go,api_test.go,attribution_resolver.go,cross_owner_import.go,job_finalization.go,routes.go,routes_integration_test.go,store.go,worker_service.go}`, `internal/app/extensionassembly/incident_bundle_jobs.go`, `internal/app/server/runtime.go`, and this tracker; no schema, migration, public route/resource, bundle format, generated contract, or frontend source changed. Focused proof-backed export/import passed at `.cartulary/test-results/20260724T220810Z-p37490`; cancellation, final-publication recheck, extension portability atomicity, and Network Flow blocking passed at `.cartulary/test-results/20260724T220939Z-p38465`. The final complete owner slice passed 20 tests at `.cartulary/test-results/20260724T221305Z-p54739`, and the final service-backed slice passed 10 tests at `.cartulary/test-results/20260724T221325Z-p55558`. Cross-owner transaction tests passed at `.cartulary/test-results/20260724T221046Z-p40466`; app-server publication characterization and process-lease service evidence passed at `.cartulary/test-results/20260724T221106Z-p41461` and `.cartulary/test-results/20260724T221120Z-p42178`; the full backend unit sweep passed 302 tests at `.cartulary/test-results/20260724T221129Z-p42562`. Formatting, generation drift, and generated-artifact policy passed at `.cartulary/test-results/20260724T221236Z-p48868`, `.cartulary/test-results/20260724T221240Z-p51364`, and `.cartulary/test-results/20260724T221240Z-p51366`; `git diff --check` and static scans for direct Incident Bundles success completion pass. Related failures corrected during the workstream were one undeclared local `err` after extracting the export transaction mutation (`.cartulary/test-results/20260724T220714Z-p33154`) and two mistyped app-server row IDs that failed with bounded usage diagnostics (`.cartulary/test-results/20260724T221053Z-p40980` and `.cartulary/test-results/20260724T221110Z-p41898`); the exact replacement selectors pass above. No blocker or migration remains. This intermediate is not deployable; rollback is the EPA-07 commit, with committed proof and cancellation observations left immutable. Domain vocabulary unchanged. |
...
| Completion checkpoint | Reference Pack now owns canonical constants for `reference_pack.packs_route`, the three versioned job kinds and operation kinds, and `reference_pack.lifecycle_worker_v1`; application composition validates the exact route contribution before seeding or registering any owner code. The disconnected minimum bundle is seeded only behind that exact admitted projection, while an unclaimed production runtime seeds zero pack rows and exposes only Core reserved-family rejection. The registrar requires the shared gated job runner and an owner-local success-finalization port; the ungated nil-runner recovery/goroutine fallback and direct success completion are removed. Import now commits its verified pack, attestation, payload identity, terminal job result, immutable proof, and final idempotency outcome in one transaction. Reverify does the same for its updated pack and attestation. Refresh verifies its complete selected record set before one atomic mutation/finalization transaction, preventing partially successful refresh state; zero-result refreshes still publish the required proof. Precommit cancellation is checked after verification and immediately before finalization, retains one cancellation observation, publishes no proof, and leaves no imported pack. A determinate finalization failure rolls back every owner and terminal mutation and retains the import staging file for gated recovery; an indeterminate commit is handled by the shared lifecycle fatal sink. Verification remains local/disconnected and introduces no egress. Failed verification retains the prior-active semantics and owner attestation behavior; activation, deployment-admin authorization, replay, frontend controls, public routes, public job resources, schemas, migrations, generated artifacts, and Core-managed state posture are unchanged. Changed authored surfaces are `internal/modules/reference_data/{api.go,job_finalization.go,minimum_disconnected_bundle.go,reference_pack_integration_test.go,routes.go,store.go}`, `internal/app/extensionassembly/reference_pack_jobs.go`, `internal/app/server/runtime.go`, and this tracker. The complete Reference Pack slice, including its production browser row, passed 16 tests at `.cartulary/test-results/20260724T223509Z-p78531`; its final service-backed slice passed 10 tests at `.cartulary/test-results/20260724T223608Z-p95021`. App-server focused and service-backed composition passed 29 and 22 tests at `.cartulary/test-results/20260724T223209Z-p17089` and `.cartulary/test-results/20260724T223303Z-p43011`; the backend unit sweep passed 302 tests at `.cartulary/test-results/20260724T223416Z-p72032`. Final generation drift, artifact policy, and JSON shape checks passed at `.cartulary/test-results/20260724T223722Z-p11692`, `.cartulary/test-results/20260724T223732Z-p14775`, and `.cartulary/test-results/20260724T223734Z-p15117`; formatting and `git diff --check` pass, and static scans find no direct Reference Pack success completion, old handler symbol, ungated recovery fallback, or broad profile claim read. Corrected related failures were an undeclared `err` introduced while extracting transaction mutations (`.cartulary/test-results/20260724T222004Z-p61252`) and a package-scoped S3 fixture lock caused by two parent-test server starts (`.cartulary/test-results/20260724T222256Z-p72270`, diagnosed by bounded stack capture at `.cartulary/test-results/20260724T222629Z-p76266`); sequential subtests pass at `.cartulary/test-results/20260724T222926Z-p80054`. No blocker, schema migration, extension-state metadata, codec, backup binding, network worker, or residual owner recovery path remains. This intermediate is not deployable; rollback is the EPA-08 commit, with committed proof and cancellation observations left immutable. Domain vocabulary unchanged. |
...
| Completion checkpoint | Snapshot/Reporting now owns one canonical constant set for its three route contributions, admitted `snapshot_reporting.render_export_v1` participant, shared participant context/result schemas, exact algorithm/output schema, three versioned job/operation pairs, and `snapshot_reporting.job_worker_v1`. Application assembly validates exact route and participant catalog parity, supplies a copied admitted participant projection with a bounded deadline, and registers the owner worker only behind the shared serve/dequeue gate; the prior asynchronous Reporting goroutine fallback is removed. The built-in participant receives only the immutable Reporting export model, returns the closed canonical participant result, and enforces cancellation plus byte/item ceilings. Reporting validates and admits that result before redaction or rendering. Snapshot and release workers use deterministic resource identities and one Reporting-owned finalization transaction that joins owner state, terminal common-job result, immutable proof, resource references, and the route-scoped idempotency outcome. Report Composition now persists a typed immutable preview source and delegates through a narrow Reporting-owned transactional port; the Reporting worker validates its incident, snapshot, profile-digest, template, graph, derivation, and output-option tuple, invokes only the admitted participant, and atomically publishes an `internal_draft` preview output plus terminal proof with no release or approval reference. Migration `00036_reporting_composition_preview_outputs.sql` structurally restricts this preview store to `internal_draft`; migration history and schema-object ownership are exact. Public routes, schemas, common-job resources, release semantics, and the optional report-builder UI posture are unchanged, and no report-builder UI was added. Core-managed state remains outside Extensions state metadata, migrations, codecs, and backup bindings. Authored implementation surfaces are `db/migrations/00036_reporting_composition_preview_outputs.sql`; `db/queries/reporting.sql`; `internal/modules/reporting/{api.go,application_service.go,composition_preview_port.go,job_finalization.go,render_export_participant.go,reporting_job_worker.go,routes.go,store.go}` and their tests; `internal/modules/reportcomposition/{preview_jobs.go,routes.go,store.go,types.go}`; `internal/app/extensionassembly/{reporting_jobs.go,reporting_participant.go,publication_catalog_test.go}`; `internal/app/server/runtime.go`; `apps/web/e2e/extensions.stateful.spec.ts`; Harness manifests/accounting/baselines; generated SQL/contracts/topology; migration/ownership manifests; and this tracker. Reporting and Report Composition focused slices passed at `.cartulary/test-results/20260724T230315Z-p88487` and `.cartulary/test-results/20260724T230326Z-p89291`; their service-backed slices passed at `.cartulary/test-results/20260724T231157Z-p11610` and `.cartulary/test-results/20260724T231206Z-p12264`. App-server focused/service-backed composition passed at `.cartulary/test-results/20260724T230355Z-p90516` and `.cartulary/test-results/20260724T230456Z-p17585`; exact Extensions BC-015 focused/service evidence passed at `.cartulary/test-results/20260724T231219Z-p12669` and `.cartulary/test-results/20260724T231226Z-p13086`, and its corrected production browser row passed at `.cartulary/test-results/20260724T232048Z-p24945`. The 737-test fast suite passed at `.cartulary/test-results/20260724T231236Z-p13581`, and the complete 158-work-unit service-backed schedule passed at `.cartulary/test-results/20260724T232613Z-p11500`. The retained run refreshed the Go duration baseline at `.cartulary/test-results/20260724T233046Z-p97784`; final format, generate, generation drift, migration drift, artifact policy, JSON shape, and Harness contract passed at `.cartulary/test-results/20260724T233253Z-p2409`, `.cartulary/test-results/20260724T233318Z-p11115`, `.cartulary/test-results/20260724T233329Z-p12985`, `.cartulary/test-results/20260724T233303Z-p5211`, `.cartulary/test-results/20260724T233329Z-p12998`, `.cartulary/test-results/20260724T233329Z-p12973`, and `.cartulary/test-results/20260724T233407Z-p17170`; `git diff --check` and obsolete-path/report-builder scans pass. Corrected related failures were missing transaction methods in the Reporting fake store (`.cartulary/test-results/20260724T224719Z-p23035`, `.cartulary/test-results/20260724T224811Z-p24976`), stale service schema hashes before rebuilding fixtures (`.cartulary/test-results/20260724T225246Z-p32156`, `.cartulary/test-results/20260724T225317Z-p34469`), invalid empty preview output options (`.cartulary/test-results/20260724T225757Z-p46127`), over-specific absent-resource and replay assertions (`.cartulary/test-results/20260724T225838Z-p49412`, `.cartulary/test-results/20260724T225857Z-p50125`), missing migration history/down/ownership accounting (`.cartulary/test-results/20260724T230550Z-p42631`, `.cartulary/test-results/20260724T230618Z-p44284`, `.cartulary/test-results/20260724T230706Z-p53237`), stale Go and browser client-support expectations (`.cartulary/test-results/20260724T230859Z-p55503`, `.cartulary/test-results/20260724T231722Z-p58973`), incomplete Harness count/baseline accounting (`.cartulary/test-results/20260724T231438Z-p56585`, `.cartulary/test-results/20260724T231515Z-p57749`), and a full-run tmpfs exhaustion after all work units completed (`.cartulary/test-results/20260724T232141Z-p41428`); the disposable Harness Go build cache was cleared and the exact successors pass above. `make backend-module-boundary-check` still reports only the pre-existing EPA-04 test-only `platform/extensionstore` import of `modules/extensions` at `.cartulary/test-results/20260724T230815Z-p54935`; it is explicitly owned by EPA-11 and no EPA-09 production import violates the boundary. No EPA-09 blocker remains. This intermediate is not deployable; rollback is the EPA-09 commit plus development database reset/reseed for migration `00036`, while committed proof and cancellation observations remain immutable. Domain vocabulary unchanged. |
...
| Completion checkpoint | The owner catalog now exposes one exact, command-addressable production-path row for every added remediation behavior. The refactor added nine rows and retired none: one app-server exact policy catalog row; four Extensions clean-cutover, finalization/cancellation/reconciliation, admission, and inactive-reconciliation rows; two Import claimed/Base production browser rows; one Report Composition preview-delegation row; and one Reporting admitted-participant row. EPA-10 corrected the last two authored row IDs to include their required stable 10-hex suffixes, regenerated the embedded contracts and topology render index, and proved exact `ROWS` selection rather than relying on whole-owner selection. All affected owners resolve against catalog digest `sha256:1c39157d9944c6bd30ba72ed5ee18dd7ae8856cb1d0b7285c36c8293d5b5aa33`: `app.server` 29 rows/22 service-backed; `module.extensions` 24/11; `module.auth` 48/29; `module.imports` 5/5; `module.incidentbundles` 20/10; `module.reference_data` 16/10; `module.reporting` 11/4; `module.reportcomposition` 2/1; `module.networkflow` 118/20; `platform.config` 9/0; and `web.workbook` 98/0. `make explain-test-owner OWNER=<owner> JSON=1` passed for all eleven owners. Exact focused selection passed for app-server at `.cartulary/test-results/20260724T233753Z-p26002`, the four Extensions rows at `.cartulary/test-results/20260724T233753Z-p26011`, both Import browser rows at `.cartulary/test-results/20260724T233753Z-p26031`, Reporting at `.cartulary/test-results/20260724T233949Z-p49516`, and Report Composition at `.cartulary/test-results/20260724T233949Z-p49524`. Exact service-backed selection passed for the two Extensions rows at `.cartulary/test-results/20260724T234002Z-p50361`, Report Composition preview at `.cartulary/test-results/20260724T234024Z-p50948`, and both Import production browser rows at `.cartulary/test-results/20260724T234036Z-p51342`. `make generate`, `make generate-drift`, `make generated-artifact-policy-check`, `make json-shape-check`, `make harness-contract`, and `make lint-markdown` passed at `.cartulary/test-results/20260724T233937Z-p47728`, `.cartulary/test-results/20260724T234221Z-p70628`, `.cartulary/test-results/20260724T234221Z-p70699`, `.cartulary/test-results/20260724T234221Z-p70657`, `.cartulary/test-results/20260724T234221Z-p71035`, and `.cartulary/test-results/20260724T234221Z-p71049`; `git diff --check` passes. Developer guidance now describes exact application catalogs and copied projections, generic generated claim materialization, `Prepare -> Commit -> Acknowledge -> Serve`, shared public/dequeue admission, atomic proof/cancellation finalization, typed Reporting participant invocation and output admission, Report Composition preview delegation, lazy Import Assistant/claim-loss behavior, semantic-owner evidence, and the mandatory tracker workflow. Changed authored surfaces are `docs/guides/cartulary-dev-guide.md`, `docs/guides/cartulary_implementation_testing_guide.md`, `docs/guides/cartulary_frontend_implementation_testing_guide.md`, `tools/test_families/module.reporting.json`, `tools/test_families/module.reportcomposition.json`, generated `internal/gen/contracts/contracts_gen.go`, generated `tools/execution_topology_render_index.json`, and this tracker. The only corrected EPA-10 failure was the public `ROWS` preflight rejection of the two initially un-suffixed row IDs; the normalized identities pass exactly above. No duplicate selector, retired row, helper-only production claim, blocker, compatibility change, or generated drift remains. Rollback is the EPA-10 evidence/documentation commit; it has no data posture. Domain vocabulary unchanged. |
...
| Completion checkpoint | Production HTTP composition now requires explicit discovery, claim, reserved-route, and workspace projections; `ExtensionEpochProvider`, static/broad epoch adapters, generated-registry defaults, nil fallback, process-global profile state, compatibility claim mutation, broad matching helpers, and the runtime HTTP-dependency claim override are deleted. Canonical claim state is materialized only from generated `claim_config_key` paths, and the installed publication supplies four copied narrow projections. Tests that construct HTTP handlers use `internal/testutil/httpapiextensions`, the only remaining generated-registry fixture reader; application-runtime tests activate profiles through production-shaped configuration and prerequisites. Workbook startup consumes only admitted workspace publications, and Import profile/route identity moved to the generic extension identity module so app/workbook composition reaches execution only through `ImportAssistantFeature`. The test-only platform-to-Extensions dependency was removed by relocating inactive reconciliation integration evidence to application assembly and splitting its exact semantic-owner selector; the catalog now has 879 rows. Backend guards forbid retired HTTP epoch/global APIs and pre-adoption handler identities outside their cutover rejection/audit-source locations, plus production platform/Extensions imports of concrete profile implementations. Frontend guards prevent extension projection code from importing concrete features and require workbook/app Import execution through the feature facade. Caller scans find no obsolete epoch, global setter, claim override, nil fallback, production generated-registry reader, legacy job dispatch identity, or platform-to-Extensions import; no unused proof wrapper existed to retain or remove. Authored surfaces include `internal/platform/httpapi/{extensions.go,httpapi.go}` and tests; `internal/app/server/{runtime.go,extensions_publication_characterization_test.go}`; `internal/testutil/httpapiextensions`; affected auth/import/incident-bundle/incidents/Network-Flow/workbook/harness-runtime tests and support; `internal/app/extensionassembly/job_reconciliation_integration_test.go`; the removed platform reconciliation test location; four frontend files; backend/frontend boundary manifests; Extensions test-family ownership; shared fixture inventory and Network Flow key-ring fixture; Harness exact counts and Go duration baselines; generated contracts/topology/scheduler outputs; and this tracker. Focused Extensions, Import, Workbook startup, and app-server publication evidence passed at `.cartulary/test-results/20260725T000739Z-p28004`, `.cartulary/test-results/20260725T001129Z-p92694`, `.cartulary/test-results/20260725T000500Z-p78114`, and `.cartulary/test-results/20260725T002935Z-p88318`. Service-backed app-server, Extensions, Auth, Import, Incident Portability, and Network Flow passed at `.cartulary/test-results/20260725T001634Z-p59837`, `.cartulary/test-results/20260725T001733Z-p85730`, `.cartulary/test-results/20260725T001825Z-p2872`, `.cartulary/test-results/20260725T002309Z-p35727`, `.cartulary/test-results/20260725T002444Z-p54688`, and `.cartulary/test-results/20260725T002503Z-p55574`. The 737-test fast suite passed at `.cartulary/test-results/20260725T001312Z-p12947`; frontend typecheck/unit passed at `.cartulary/test-results/20260725T001552Z-p57391` and `.cartulary/test-results/20260725T001603Z-p57945`. The complete 158-work-unit service-backed schedule passed at `.cartulary/test-results/20260725T003201Z-p6196`, and the baseline refresh/coverage passed at `.cartulary/test-results/20260725T003528Z-p75340` and `.cartulary/test-results/20260725T003541Z-p77228`. Final generation, Harness contract, generation drift, artifact policy, JSON shape, backend boundary, and frontend boundary passed at `.cartulary/test-results/20260725T003535Z-p75554`, `.cartulary/test-results/20260725T003543Z-p77466`, `.cartulary/test-results/20260725T003609Z-p78539`, `.cartulary/test-results/20260725T003619Z-p81615`, `.cartulary/test-results/20260725T003621Z-p81959`, `.cartulary/test-results/20260725T003624Z-p82512`, and `.cartulary/test-results/20260725T003626Z-p82845`; `git diff --check` passes. Corrected related failures were stale unused imports after global-helper deletion, a Network Flow dependency claim missing its Import claim and key-ring configuration, stale exact selector accounting after moving reconciliation evidence, unclassified shared fixture data, and Harness row-count/duration-baseline drift. The baseline updater correctly rejected partial owner roots before the successful full retained root was used. No blocker or residual dual path remains. Rollback is the EPA-11 commit only and has no schema/data mutation; obsolete globals must not be restored alongside the single publication path. Domain vocabulary unchanged. |
...
| EPA-05 | `DONE` | EPA-03 | wait for final rollout | exact catalog admission gates provider preparation and four owner-local route contributions; Auth/UI regression passes |
| EPA-06 | `DONE` | EPA-04 | wait for final rollout | exact Import jobs/workers/proofs, exhaustive ordered discovery, production assistant, Base fallback, and NF regression pass |
| EPA-07 | `DONE` | EPA-04 | wait for final rollout | export/import owner state, terminal job, proof, idempotency, staged cleanup, attribution, and NF blocking share exact final boundaries |
| EPA-08 | `DONE` | EPA-04 | wait for final rollout | exact import/reverify/refresh bindings, atomic owner proofs, gated recovery, inactive quiescence, disconnected/prior-active/UI regression pass |
...
| 2026-07-24 | Remediation execution | EPA-05 | complete | tracker; exact application contribution projection; app-server Enterprise preparation and route composition; auth Base/Enterprise registrar split and owner constants; raw-config platform helper removal; unit tests | Auth and app-server focused/service-backed slices, config inactive policy, production browser behavior, full backend unit, generation drift, generated-artifact policy, static caller scans, and diff check passed; exact roots recorded in EPA-05 | none | commit EPA-05, then activate EPA-06 |
| 2026-07-24 | Remediation execution | EPA-06 | complete | tracker; Import jobs/proofs, exhaustive unit discovery/apply state, migration 00035; exact application/NF admission; production Import Assistant/coordinator/availability; UI contracts; verification and browser selectors; generated contracts/SQL/topology; cross-owner fixtures | Import focused/service-backed, production claimed/Base browser paths, NF service-backed regression, backend/frontend units, typecheck, generation/migration/artifact drift, frontend boundary, and diff checks passed; exact roots and corrected failures recorded in EPA-06 | none; pre-existing EPA-04 test-only backend boundary violation scheduled for EPA-11 | commit EPA-06, then execute EPA-07 |
| 2026-07-24 | Remediation execution | EPA-07 | complete | tracker; Incident Bundles owner finalization port; app proof adapter; export descriptor and import cross-owner final boundary; canonical owner identities; cancellation/proof and portability regression evidence | complete Incident Bundles focused/service-backed suites, cross-owner coordinator, app-server publication/process lease, backend unit, generation drift, artifact policy, static scans, and diff checks passed; exact roots and corrected command/build failures recorded in EPA-07 | none | commit EPA-07, then execute EPA-08 |
| 2026-07-24 | Remediation execution | EPA-08 | complete | tracker; Reference Pack owner identities/finalization port; app proof adapter and exact contribution projection; gated lifecycle worker; atomic import/reverify/refresh state and proof publication; inactive/disconnected/proof/cancel evidence | complete Reference Pack focused/service-backed suites, production browser row, app-server composition suites, backend unit, generation drift, artifact policy, JSON shape, static scans, formatting, and diff checks passed; exact roots and corrected build/fixture failures recorded in EPA-08 | none | commit EPA-08, then execute EPA-09 |
