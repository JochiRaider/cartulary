...
Checkpoint commits completed before SL-01 entry are `23f756f1` (tracker bootstrap), `4aee70a2` (SL-00), `b32ee3c6` (IND-015), `f894d9ca` (IND-016), and `c3e54385` (SL-02). A slice records its own SHA in the next ledger-only gate update because a commit cannot contain its final self-referential SHA.
...
All 30 files currently under `internal/modules/indicators` are in scope and inventoried below. None is excluded.
...
| `internal/modules/indicators/incident_bundle_source_port.go` | Constructs the typed portability descriptor and adapts prepare/apply/validate operations. | `NewIncidentBundleSourcePort`. | Incident portability assembly catalog. | Incident Bundles source-port contract, portability functions, canonical Indicator identity, PGX. | Incident Bundle source-catalog, route, and identity preparation tests. | `contracts/incident-bundles/source_catalog.json`. | Indicators portability adapter. | critical | Preparation now delegates representation, canonical normalization, and identity uniqueness to the shared identity owner; the remaining seven invariant implementations belong to SL-07. |
...
| `IND-PORT-003` | Core 01 REQ-01-639/640 source-port allocation | SL-05, SL-07 | Port boundary tests and `AC-PORT-003` through `AC-PORT-005` | SL-05 | READY |
| `IND-PORT-004` | Core 01 error owner and Core 04 AC-531 | SL-07 | `AC-PORT-001`, `AC-PORT-002`, `AC-PORT-007` | Exact invariant selection adopted | READY |
| `IND-PORT-005` | Core 01 REQ-01-641 transaction/publication owner | SL-07 | `AC-PORT-006` | Final-transaction contract adopted | READY |
| `IND-PORT-006` | Testing Harness evidence routing only | SL-08 | Focused rows, retained run roots, generated drift gates | Product requirements implemented | READY |
...
| Portability declares ten invariants and still lacks seven complete implementations plus deterministic multi-defect selection. | `incident_bundle_source_port.go`; Core 01 REQ-01-640; source catalog; `IND-PORT-001` through `IND-PORT-006`. | critical | `must_fix` | Indicators source port | SL-02 established shared canonical identity for the first three invariants; SL-07 must prove all `AC-PORT-001` through `AC-PORT-007` without repair behavior. |
...
| WF-01 | Target inventory | chain | WF-00 | WF-02, WF-03, WF-04 | Account for every target file, caller, dependency, and test surface. | `internal/modules/indicators/**` plus live callers. | Re-run file count/search before implementation. | All 30 current files have one Section 2 row. |
...
WF-00 through WF-08 planning is complete and implementation is active. Tracker bootstrap, SL-00, IND-015, IND-016, and SL-02 are complete. SL-01 is the next admitted workstream; SL-03 through SL-08 retain their recorded dependency gates.
...
| SL-07 | SL-00, SL-02, SL-05, IND-016 | **Authorized and owner-unblocked:** enforce `IND-PORT-003` through `IND-PORT-006` before Incident Bundle publication. | Indicator source port/identity component, Incident Bundle tests, and authored harness inputs. | Previously accepted malformed bundles may fail; invariant selection, error safety, and final-transaction atomicity may drift. | `AC-PORT-001` through `AC-PORT-007`, including ten exact negative fixtures, precedence, v1/v2 round trips, repeated/tombstone preservation, failure injection, and safe errors. | `make service-backed-test-slice OWNER=module.indicators`; `make backend-module-boundary-check`; `make test-fast`; `make generate-drift`; `make generated-artifact-policy-check`; `make json-shape-check` | Revert the behavior-correction commit as one unit; never weaken the descriptor or add repair, skip, merge, compatibility, or warning-only behavior. | Every acceptance case passes; exact safe failures are deterministic; final-transaction failure publishes no state; focused evidence and drift checks pass. |
...
| IND-002 | Inventory all target files and live callers | WF-01 | DONE | IND-001 | Section 2 with 30 rows | Every current target file is accounted for. |
...
| IND-010 | Close all declared portability invariants | WF-06 | TODO | IND-008 and SL-05; IND-016 is complete | SL-07; `IND-PORT-*`; `AC-PORT-*` | Authorized validation proves all ten invariants, deterministic safe failure, and final-transaction atomicity. |
...
- [x] Every file in `internal/modules/indicators` is inventoried; all 30 current files have an individual Section 2 row and none is silently excluded.
