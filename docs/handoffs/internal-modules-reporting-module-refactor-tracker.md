...
| Composition tuple validation and binding | `api.go`, `store.go` | Reporting consumer boundary plus `reportcomposition` authoring owner | Split | `composition_id/version/sha256` decode, `validateReleaseCompositionTupleTx`, `report_composition_release_bindings`. | Do not move authoring lifecycle into reporting; use owner facade if introduced. |
| Graph projection ref validation | `api.go`, `store.go` | Reporting consumer boundary plus projections owner | Split | `validateSourceProjectionRefs`, `validateReleaseGraphProjectionRefsTx` reads `graph_projection_runs`. | Reporting consumes completed projection refs; graph lifecycle remains projection-owned. |
...
| Composition tuple consumption | Core 01 release tuple; Report Composition NLSpec authoring; Reporting consumption | `api.go`, `store.go`, `reportcomposition` package | `redaction_test.go`, integration coverage via release tuple paths is partial. | TODO: characterize digest mismatch, incident/template mismatch, binding insert before facade move. | High | Reporting must not own composition authoring lifecycle. |
| Graph projection refs | Core 01 release tuple; Graph Projection owner; Reporting consumer | `api.go`, `store.go`, `graph_projection_runs` migration | `redaction_test.go`; TODO integration coverage for DB mismatch/incomplete cases. | TODO: add DB-backed graph ref characterization before moving to projections facade. | High | Reporting consumes completed digest-bound projection refs only. |
...
| Graph projection validation reads projection tables directly. | `validateReleaseGraphProjectionRefsTx` queries `graph_projection_runs`. | High | `should_fix` | Projections owner facade consumed by reporting. | Define a read-only validation facade or defer until projection owner supports it. |
| Composition tuple validation and binding reach into composition tables. | `validateReleaseCompositionTupleTx` joins `report_compositions` and `report_composition_versions`; `insertCompositionReleaseBindingTx` writes `report_composition_release_bindings`. | High | `should_fix` | Report composition authoring owner plus reporting release consumer boundary. | Plan an owner facade for tuple validation/binding without moving authoring into reporting. |
...
| Import-boundary guard is narrow and file-specific. | `boundary_guard_test.go` only constrains sibling import of `incidents` to `routes.go`. | Medium | `should_fix` | Reporting architecture tests. | Extend guard only after desired facades are planned; avoid blocking existing behavior prematurely. |
...
| --- | --- | --- | --- | --- |
| `RB-001` | TODO: identify or create a reporting-only public Make validation target, if one exists or is warranted. | Narrow validation reduces cost and risk for reporting-only refactors. | Make target inventory or harness owner decision. | Open; searched `make help`, `make help-all`, `make explain-target`, `make task-guide`, `make explain-phase`; no reporting-only public target found. |
| `RB-002` | TODO: characterize graph projection DB validation outcomes before replacing raw table reads. | Facade movement could alter `graph_projection_not_bound`, `graph_projection_incomplete`, `graph_projection_digest_mismatch`, or ambiguity behavior. | Integration tests or projection owner facade evidence. | Open; current unit tests cover structural decode/duplicate graph view refs. |
| `RB-003` | TODO: characterize composition tuple DB mismatch and binding cases before replacing raw table reads/writes. | Facade movement could alter not-found, incident mismatch, template mismatch, digest mismatch, version parsing, or release-bound behavior. | Integration tests or reportcomposition owner facade evidence. | Open; structural tuple decode is covered, DB behavior needs focused evidence. |
