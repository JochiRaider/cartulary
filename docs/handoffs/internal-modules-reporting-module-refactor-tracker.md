...
| Template contract and render bundle assembly | `redaction.go`, `routes.go`, `render_bundle.go` | Reporting render internals | Fix in RB-005 remediation | `ResolveTemplateContract`, `renderReportBundle`, `renderReleaseCandidate`. | New successful releases bind to a bundle manifest hash and persist render bytes only through bundle files; full Slidev/Mermaid renderers still need to replace the temporary source serializers. |
...
| Current renderer is simplified relative to adopted Reporting NLSpec. | `renderReportBundle` emits deterministic source files and bundle manifests, but does not yet run the full Slidev/Mermaid supervisor, composition-aware diagram selection, or sandbox/toolchain artifact validation. | High | `should_fix` | Reporting render stack. | RB-005 owner direction is full Reporting NLSpec stack replacement; do not claim the temporary source serializers are conformant full renderers. |
...
| `RB-005` | Replace the temporary source serializers with the full Reporting NLSpec stack. | Output bytes, render manifests, and toolchain semantics are behavior, not just structural movement. | Owner decision in this remediation task: full NLSpec stack is required; temporary serializers are not conformant full renderers. | Direction closed; implementation in progress through P2-P6 remediation workstreams. |
