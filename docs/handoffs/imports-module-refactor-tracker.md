...
- **Current control point:** `RS-03` checkpoint complete; only `RS-04` is eligible.
- **Allowed change in the next control point:** A Timeline-owned import-create facade, its
  registration through the common owner registry and coordinator, application composition, exact
  Timeline effect/rollback/replay tests, and the mandatory tracker checkpoint. Later changes MUST
  remain inside the separately recorded `RS-05` through `RS-11` workstream boundaries.
...
The current workstream baseline is tracker checkpoint `c379b104`; the application-composed RS-03
owner registry is substantive commit `c372c1ba`.
...
| `internal/modules/imports/owner_registry.go` | Builds the exact expected enabled generic view-owner binding set from the generated target registry | Internal registry composition contract | Application Imports assembly | Generated target registry and ownerfacade registry | Imports assembly and boundary tests | Generated backend registry projection | imports | high | Timeline is deliberately excluded until RS-04 registers its owner facade through the same path. |
...
| `internal/modules/imports/ownerfacade/registry.go` | Closed binding, adapter, and exact owner-create registry contract | Binding, facade, adapter constructor, registry constructor, ordered bindings, and exact resolver | Source-owner constructors, Imports assembly, apply coordinator | Owner-create request/result contract | Dedicated registry and assembly tests | Core owner-create facade and generated target binding | imports shared facade contract | high | Construction fails closed on missing, duplicate, unexpected, mismatched, or nil facades. |
...
The 22 tracked files above include the RS-00 characterization additions and the RS-03 registry
contract. The filesystem also contains an empty, untracked
...
| WF-01 | Live target inventory | chain | WF-00 | WF-02 through WF-04 | Account for every file, caller, dependency, and test | Repository inspection | All 22 tracked files have target-specific rows. |
...
| IMP-002 | Inventory every tracked target file and direct boundary | WF-01 | DONE | IMP-001 | §2 | All 22 tracked files have live target-specific rows. |
...
| IMP-010 | Inject owner registry and remove ownership leakage | WF-05 | IN_PROGRESS | IMP-007 through IMP-009; GATE-05 | RS-03 `c372c1ba`; RS-04 pending | The 13 generic enabled targets resolve exactly once with no concrete peer stores or cross-owner SQL in Imports; Timeline convergence remains. |
...
| --- | --- | --- | --- | --- |
| RB-001 | Core owns generic orchestration and semantic obligations; the target NLSpec owns exact target payloads; the binding and two-level commit model join them. | PARTIALLY_IMPLEMENTED | RS-03 runtime registry injection is complete; Timeline convergence and unit-commit implementation are not yet changed. | RS-04 and RS-06 |
...
- [x] RS-03 injects 13 exact generic owner facades with no Imports-owned peer stores or peer-table
  SQL; only RS-04 Timeline convergence is eligible.
