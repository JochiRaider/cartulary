...
- **Current control point:** `RS-04` checkpoint complete; only `RS-05` is eligible.
- **Allowed change in the next control point:** Behavior-preserving file-level decomposition of
  Imports HTTP handlers, services, jobs, discovery/mapping, apply coordination, persistence, and
  XLSX responsibilities, its characterization/boundary evidence, and the mandatory tracker
  checkpoint. Later changes MUST remain inside the separately recorded `RS-06` through `RS-11`
...
The current workstream baseline is tracker checkpoint `cfb01613`; the Timeline convergence in
RS-04 is substantive commit `7e891c66`.
...
- [x] RS-04 converges Timeline behind the exact 14-owner registry and common transaction coordinator
  with effect, replay, and rollback evidence; only RS-05 decomposition is eligible.
