...
### 2026-07-23 — T-010 independent Fallow report admission complete

- Source: mandatory T-010 reopen checkpoint `d6c036cb`. The current
  `frontend-fallow-static` profile now starts its four authored, read-only
  report invocations as concurrent children after retaining one validated
  effective configuration. Each child owns distinct output and log paths.
- Determinism and failure behavior: the parent uses settle-all admission,
  drains all children, retains successful sibling artifacts, and restores
  authored order before report normalization, log discovery, warning emission,
  and primary-failure selection. Child-start failure is normalized as a failed
  authored result rather than allowing an early rejection to abandon siblings.
- Contract proof: TH-HARNESS-REQ-070 now owns the concurrency, full-drain,
  partial-artifact, and stable-order behavior. Its focused fixture proves four
  overlapping children, out-of-order completion, simultaneous ordinary and
  start failures, settle-all behavior, and authored result order. Full
  `harness-contract` passed at
  `.cartulary/test-results/20260723T041356Z-p3242020`; ShellCheck passed at
  `.cartulary/test-results/20260723T041226Z-p3239079`.
- Timing proof: three direct diagnostic invocation envelopes completed in
  `6.086 s`, `6.034 s`, and `6.071 s` at
  `.cartulary/test-results/20260723T041015Z-p3234110`,
  `.cartulary/test-results/20260723T041320Z-p3241253`, and
  `.cartulary/test-results/20260723T041424Z-p3243126`. All are below the fixed
  `7.1255 s` frontend-fallow-static limit and retain the same JSON, SARIF,
  Markdown, dupes, health, resolved-config, and normalized summary roles.
- Generated closure: `make generate` passed at
  `.cartulary/test-results/20260723T041159Z-p3237007`; generation drift,
  generated-artifact policy, and JSON shape passed at
  `.cartulary/test-results/20260723T041439Z-p3243864`,
  `.cartulary/test-results/20260723T041439Z-p3243881`, and
  `.cartulary/test-results/20260723T041439Z-p3243913`. Biome and Markdown lint
  passed at `.cartulary/test-results/20260723T041439Z-p3244243` and
  `.cartulary/test-results/20260723T041439Z-p3244280`.
- Active: T-010 is closed and T-012 is the sole `IN_PROGRESS` item. Freeze this
  implementation and tracker as the new candidate source, recollect every
  provider window, and add one isolated direct backend-process window. All
  `7d10a46a` candidate roots remain diagnostic-only after this code change.

### 2026-07-23 — T-012 isolated backend window reopens T-008 process packing

- Source: clean frozen candidate checkpoint
  `10ccbf5ee83ea01197422322a91a3230543d8819`. Agent-finalize, test-fast, lint,
  frontend-fallow-static, and benchmark-claim-check completed qualified window
  shapes before the isolated backend-process result made acceptance impossible.
  The partial go-gosec-targeted window was interrupted deliberately after that
  diagnosis and is rejected as `interrupted_execution`.
- Fallow confirmation: the strict frontend-fallow-static window passed with
  tool bodies of `5.827 s`, `5.795 s`, and `5.812 s` at
  `.cartulary/test-results/20260723T042459Z-p3397175`,
  `.cartulary/test-results/20260723T042506Z-p3397853`, and
  `.cartulary/test-results/20260723T042514Z-p3398520`. T-010 remains closed.
- Backend rejection: isolated backend-process roots
  `.cartulary/test-results/20260723T042521Z-p3399147`,
  `.cartulary/test-results/20260723T042604Z-p3417015`, and
  `.cartulary/test-results/20260723T042652Z-p3435231` retained all 36 tests with
  no retry or contamination. The measured target bodies were `30.124 s` and
  `21.453 s`; their midpoint cannot clear the fixed `23.885 s` limit. The
  window is valid evidence of a regression and will not be rerun unchanged.
- Diagnosis: the global service exact-symbol limits of eight symbols and
  12,000 estimated test milliseconds split one compatible backend-process
  selection into four app-server, two recovery, and one extension capture.
  Six capture workers then admit up to 24 process-lifecycle tests through the
  child `-parallel 4` settings while duplicating Go package startup across seven
  processes. These tests share Postgres/object-store fixtures and runtime
  binaries, so the generic fine-grained packing creates contention instead of
  useful host utilization.
- Disposition: T-008 is reopened as the sole `IN_PROGRESS` item and T-012
  returns to `TODO`. Add an explicit backend-process exact-symbol profile of at
  most 16 symbols and 24,000 estimated test milliseconds per compatible shard,
  yielding two app-server, one recovery, and one extension capture for the
  current 36-test inventory. Preserve every compatibility dimension, symbol,
  row, fixture, artifact, and failure rule. Project the profile into retained
  execution policy and validate an explicit
  `backend_process_shard_consolidation` baseline transition; do not hide the
  change behind a digest-only match. All `10ccbf5e` roots become diagnostic-only
  after implementation changes.

### 2026-07-23 — T-008 backend-process packing complete

- Source: mandatory T-008 reopen checkpoint `e52176ba`. Service exact-symbol
  packing now resolves through one closed target profile owner. The default
  remains eight symbols and 12,000 estimated test milliseconds;
  `backend-process` explicitly owns 16 symbols and 24,000 milliseconds.
- Current plan: the unchanged 36-test process inventory resolves to four
  physical captures: two `app.server.process` shards of 16 and 12 symbols, one
  six-symbol `module.recovery.process` shard, and one two-symbol
  `module.extensions.process` shard. All package, runtime-binary, fixture
  policy/budget, isolation, resource-profile, evidence-class, selector,
  missing/duplicate/unexpected output, and stable ordering rules remain intact.
  Backend-unit independently retains its 30-process, 255-test plan.
- Policy proof: retained measurement policy projects the exact 16/24,000
  process profile plus its four-worker host-derived pool semantics. The
  reference baseline now declares
  `backend_process_shard_consolidation`; strict comparison validates the exact
  candidate projection and rejects stale symbol limits. The baseline writer
  passed at `.cartulary/test-results/20260723T043956Z-p3556578` without
  changing any retained reference root or timing statistic.
- Functional validation: final `harness-contract` passed at
  `.cartulary/test-results/20260723T044034Z-p3558946`; `backend-unit` retained
  all 255 tests at `.cartulary/test-results/20260723T044013Z-p3556832`; and
  `test-fast` retained all 651 tests at
  `.cartulary/test-results/20260723T044105Z-p3560078` in `124.303 s`.
- Timing proof: direct backend-process diagnostics retained all 36 tests in
  `20.408 s` and `20.634 s` at
  `.cartulary/test-results/20260723T043646Z-p3503496` and
  `.cartulary/test-results/20260723T043736Z-p3521673`. Root
  `.cartulary/test-results/20260723T043819Z-p3539063` is rejected with
  `retry_observed` after a Postgres startup retry and is not timing evidence.
- Generated closure: final generation passed at
  `.cartulary/test-results/20260723T043606Z-p3500671`; generation drift,
  generated-artifact policy, and JSON shape passed at
  `.cartulary/test-results/20260723T044352Z-p3600530`,
  `.cartulary/test-results/20260723T044352Z-p3600571`, and
  `.cartulary/test-results/20260723T044352Z-p3600527`. Biome and Markdown lint
  passed at `.cartulary/test-results/20260723T044352Z-p3600900` and
  `.cartulary/test-results/20260723T044352Z-p3600937`.
- Development rejection: `harness-contract` root
  `.cartulary/test-results/20260723T043459Z-p3499367` correctly exposed a
  private backend import before the policy was exported through the existing
  backend shard-plan facade. The final state has no forbidden planning edge.
- Active: T-008 is closed and T-012 is the sole `IN_PROGRESS` item. Freeze the
  implementation and tracker as a new candidate source and restart every
  provider window; no root from `10ccbf5e` may enter accepted candidate evidence.

### 2026-07-23 — T-012 clean backend window reopens T-008 admission

- Source: clean frozen candidate checkpoint
  `dfeb166968b98aaf8b4f4be0a81692aa2dd05dd4`. Agent-finalize, test-fast,
  lint, frontend-fallow-static, and backend-process completed qualified window
  shapes before candidate collection stopped. No unchanged failing window will
  be rerun and no acceptance threshold is relaxed.
- Confirmed Fallow window: warm root
  `.cartulary/test-results/20260723T045307Z-p3751114` and measured roots
  `.cartulary/test-results/20260723T045315Z-p3751795` and
  `.cartulary/test-results/20260723T045322Z-p3752458` retained tool bodies of
  `5.990 s`, `6.234 s`, and `5.948 s`; T-010 remains closed.
- Backend rejection: isolated backend-process warm root
  `.cartulary/test-results/20260723T045330Z-p3753109` and measured roots
  `.cartulary/test-results/20260723T045414Z-p3770683` and
  `.cartulary/test-results/20260723T045503Z-p3788060` are clean, complete,
  retry-free observations of all 36 tests. Their measured target bodies were
  `31.545 s` and `23.414 s`, whose midpoint cannot clear the fixed `23.885 s`
  no-regression limit.
- Diagnosis: consolidation removed three redundant capture processes, but all
  four remaining service-backed Go processes are still admitted together.
  The measured roots show variable Postgres/object-store setup and concurrent
  app-server/recovery lifecycle pressure. Collapsing each family to one process
  is not a durable correction: only nine app-server tests call `t.Parallel`, so
  most of that family would become serial within one package process.
- Disposition: T-008 is reopened as the sole `IN_PROGRESS` item and T-012
  returns to `TODO`. Make the target-specific process plan own an explicit,
  bounded lifecycle admission policy in addition to symbol packing. Preserve
  the four-shard identity, all compatibility dimensions, exact symbol parity,
  stable output/failure order, and backend-unit's independent six-worker plan.
  Validate the normalized admission projection as part of the existing
