...
| Operator command grammar and result schemas differ from Core 01. | Current commands and schema IDs in `operator.go`; Core 01 requires logical commands and `cartulary.operator_recovery_result.v1`. | Behavior change affects operator automation and evidence. | must_fix | Core 01 recovery CLI owner with app compatibility facade | Closed for planning by `TH-HARNESS-REQ-655`; legacy aliases may remain only when exact, cheap, and owner-approved. |
...
| SL-06 | SL-03 | Implement canonical recovery CLI behavior under Core 01/Core 04. | `internal/app/operator.go`; `cmd/operator`; recovery modules; tests and owner docs if changed. | Behavior change: command grammar, result envelope, exit codes, local-operator authorization, journaling, progress, compatibility aliases. | New owner-aligned characterization and conformance tests required; legacy recovery tests must be deleted, rewritten, or classified as non-authoritative. | `make phase-slice PHASE=phase10`; `make service-backed-slice PHASE=phase10`; `make browser-e2e-webserver-backed`; `make test-fast`; possibly `make check`. | Requires later implementation authorization; rollback all behavior changes together. | Recovery CLI evidence satisfies Core 01/Core 04 and harness `TH-HARNESS-REQ-654` through `REQ-656`. |
...
| IA-007 | Design canonical operator/recovery facade extraction. | WF-05 | TODO | IA-005, IA-006 | SL-03, SL-06 | Owner-approved path separates canonical recovery implementation from optional exact aliases. |
