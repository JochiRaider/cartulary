...
| S15-01 | Semantic owner reporting | DONE | S15-00 | Owner-facade buckets match semantic owner boundaries. | Split import-boundary owner-facade reporting so phase-slice planning reports under `phase_accounting`, service-backed planning under `service_backed_execution`, and scheduler retained diagnostics under `scheduler_diagnostics`; updated harness contract assertions and confirmed the old scheduler bucket no longer carries those semantic owners. Files changed: `tools/harness/static-analysis/harness-import-boundary.mjs` and `tools/harness/tests/test-harness-contracts.mjs`. Validation passed: `make frontend-import-boundary-check` (`.cartulary/test-results/20260704T181950Z-p562656`), `make harness-contract`, and `make lint-scripts`. No generated files changed. Retained-run drift checks were skipped because this slice changed only diagnostic owner reporting and did not touch capacity policy, duration values, timing or retained-run interpretation, generated owner inputs, or finalizer behavior. Next: S15-02. |
| S15-02 | Phase-slice plan contract | DONE | S15-01 | `cartulary.phase_slice_plan.v1` is schema-owned or explicitly diagnostic-only. | Promoted `cartulary.phase_slice_plan.v1` to a present Section 8 schema attachment, added a closed top-level schema for emitted phase-slice JSON plan output, added positive/negative JSON-shape fixtures, and validated real base/frontend `--json` phase-slice output in the phase-slice smoke test. Files changed: `docs/testing-harness-nlspec.md`, `tools/schemas/cartulary.phase_slice_plan.v1.schema.json`, `tools/harness/generated-artifacts/tests/test-json-shapes.sh`, and `tools/harness/phase-accounting/tests/test-run-phase-slice.sh`. Validation passed: `make lint-markdown`, `make json-shape-check` (`.cartulary/test-results/20260704T182629Z-p586937`), `make harness-contract`, and `make run-harness-smoke-extended` (`.cartulary/test-results/20260704T182648Z-p588016`). No generated files changed. Retained-run drift checks were skipped because this slice changed schema/spec/test coverage only and did not touch capacity policy, duration values, timing or retained-run interpretation, generated owner inputs, or finalizer behavior. Next: S15-03. |
| S15-03 | Scheduler family registry closure | DONE | S15-02 | Scheduler family metadata has one owner path. | Added `tools/harness/scheduler/scheduler-family-contract.mjs` as the implementation facade for the current closed scheduler families, routed scheduler manifest/resource validation through it, classified it as a scheduler owner facade, and added schema/registry/generated-manifest parity coverage. Files changed: `docs/testing-harness-nlspec.md`, `tools/harness/scheduler/scheduler-family-contract.mjs`, `tools/harness/scheduler/scheduler-manifest.mjs`, `tools/harness/scheduler/scheduler-resource-policy.mjs`, `tools/harness/scheduler/scheduler-resources.mjs`, `tools/harness/static-analysis/harness-import-boundary.mjs`, and `tools/harness/tests/test-harness-contracts.mjs`. Validation passed: `make lint-markdown`, `make lint-scripts`, `make frontend-import-boundary-check` (`.cartulary/test-results/20260704T184305Z-p652333`), `make json-shape-check` (`.cartulary/test-results/20260704T184313Z-p652711`), `make harness-contract`, and `make run-harness-smoke-fast`. No generated files changed. Retained-run drift checks were skipped because this slice changed scheduler family validation/metadata only and did not touch capacity policy, duration values, timing or retained-run interpretation, generated owner inputs, or finalizer behavior. Next: S15-04. |
| S15-04 | Registry-backed capacity policy | DONE | S15-03 | Phase and service-backed capacity policy is shared and validated. | Moved check/service-backed auto-limit resolver selection into `scheduler-resource-policy.mjs`, moved service-backed Go CPU/IO estimators out of the service-backed CLI, and routed base/frontend phase-slice plans through `phase_slice_default` plus Section 10 auto policies before pruning unclaimed resources from emitted plans. Accepted capacity deltas: phase-slice `process` now follows registry default `6`, Go CPU/IO and Postgres clone/reset resolve by Section 10, and frontend browser stack resolves through the shared auto policy. Files changed: `tools/harness/scheduler/scheduler-resource-policy.mjs`, `tools/harness/scheduler/check-schedule-cli.mjs`, `tools/harness/scheduler/service-backed-schedule-cli.mjs`, `tools/harness/phase-accounting/phase-slice-plan.mjs`, `tools/harness/phase-accounting/frontend-phase-slice-plan.mjs`, and `tools/harness/scheduler/tests/test-check-scheduler.sh`; retained-run finalizer refreshed `tools/browser_e2e_duration_baselines.json`, `tools/go_test_duration_baselines.json`, `tools/harness_smoke_duration_baselines.json`, `tools/service_backed_make_target_duration_baselines.json`, and `tools/execution_topology_render_index.json`. Validation passed: `make lint-scripts`, `make json-shape-check` (`.cartulary/test-results/20260704T193544Z-p764570`), `make harness-contract`, `make run-harness-smoke-fast`, `make run-harness-smoke-extended`, `make scheduler-summary-timing-drift RESULTS_DIR=.cartulary/test-results/20260704T153646Z-p4080542` (`.cartulary/test-results/20260704T193255Z-p760059`), `make scheduler-event-order-drift RESULTS_DIR=.cartulary/test-results/20260704T153646Z-p4080542` (`.cartulary/test-results/20260704T193430Z-p760421`), `make duration-baseline-drift-suite RESULTS_DIR=.cartulary/test-results/20260704T153646Z-p4080542`, `make agent-finalize RESULTS_DIR=.cartulary/test-results/20260704T153646Z-p4080542 ALLOW_OLDER_RESULTS_DIR=1` (`.cartulary/test-results/20260704T193452Z-p761073`, `retained_run_selection.status=older_with_override`), `make generated-artifact-policy-check` (`.cartulary/test-results/20260704T193524Z-p763239`), `make phase-schedule-drift` (`.cartulary/test-results/20260704T193529Z-p763424`), and `make generate-drift` (`.cartulary/test-results/20260704T193538Z-p763621`). The first `agent-finalize RESULTS_DIR=.cartulary/test-results/20260704T153646Z-p4080542` attempt failed as expected with `older_rejected`; rerun with the explicit older-root override passed. Next: S15-05. |
| S15-05 | Final validation and handoff | DONE | S15-04 | Validation, retained-run status, skipped checks, and residual risks are recorded. | Completed final validation and handoff closure for the scheduler evolution pass. Validation passed: `make lint-markdown`, `make lint-scripts`, `make run-harness-smoke-fast`, `make run-harness-smoke-extended`, `make agent-finalize RESULTS_DIR=.cartulary/test-results/20260704T153646Z-p4080542 ALLOW_OLDER_RESULTS_DIR=1` (`.cartulary/test-results/20260704T194122Z-p827791`, generated files unchanged on this rerun, `retained_run_selection.status=older_with_override`), `make generated-artifact-policy-check` (`.cartulary/test-results/20260704T194335Z-p830198`), `make phase-schedule-drift` (`.cartulary/test-results/20260704T194441Z-p830560`), `make generate-drift` (`.cartulary/test-results/20260704T194444Z-p830732`), `make json-shape-check` (`.cartulary/test-results/20260704T194450Z-p831667`), `make harness-contract`, and `git diff --check`. Retained root used for finalizer and retained-run checks: `.cartulary/test-results/20260704T153646Z-p4080542`. Residual risk: public target identities and scheduler family tokens are unchanged, but phase-slice capacity values intentionally changed to registry/Section 10 policy and may affect local lane pressure; retained drift/finalizer checks passed with explicit older-root override. No further scheduler evolution slice remains. |
...
Skipped checks for S15-03:

- Generated drift targets were skipped because S15-03 did not change generated owner inputs or generated artifacts.
- Retained-run drift targets and `make agent-finalize RESULTS_DIR=<warm-root>` were skipped because S15-03 did not change capacity policy, duration values, scheduler timing/drift interpretation, retained-run interpretation, or finalizer behavior.

Skipped checks and notes for S15-04:

- No scheduler-family schema/spec algorithm update was made because NLSpec Section 10 already owns the registry defaults and auto algorithms used by this slice.
- The retained warm root `.cartulary/test-results/20260704T153646Z-p4080542` was not the latest sibling run, so finalizer validation required `ALLOW_OLDER_RESULTS_DIR=1`; the passing finalizer summary records `older_with_override`.
- An exploratory `make phase-slice PHASE=FE-P3 PHASE_NAMESPACE=frontend EXTRA_ARGS=--json` command was invalid for plan-only inspection because the public wrapper uses `JSON=1`; it executed the frontend slice and failed in `build-web` row-accounting. Corrected `JSON=1` plan inspections passed for base `phase4` and frontend `FE-P3`.
