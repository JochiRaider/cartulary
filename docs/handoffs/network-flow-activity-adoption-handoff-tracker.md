...
| `NFA-IMPL-013` | Implement Network Flow storage and table lifecycle | `WS-13` | `TODO` | `NFA-IMPL-011`, `NFA-IMPL-012` | `internal/modules/networkflow` | lifecycle ACs | module, migrations, authored queries | Immutable row, table-limit, rename, soft-delete, and metric evidence | `make backend-unit`; `make backend-store`; `make migration-drift` | Only active tables are queryable and retained counts are observable |
...
| Commands run | `git status --short --branch`; `git rev-parse HEAD`; `date -Iseconds`; tracker/source `rg`; `sed`; `make format-go`; `make generate`; `make backend-unit`; `make backend-integration`; `make generate-drift`; `make generated-artifact-policy-check`; `make json-shape-check`; `make lint`; `make go-gosec-targeted`; `git diff --check` |
| Passing validation | `NFA-IMPL-012` artifact validation is recorded in §15.30; completion checkpoint validation is pending this tracker edit |
| Failing validation | Prior resolved `NFA-IMPL-011` failures are recorded in §15.29. The only `NFA-IMPL-012` failure was the resolved first backend-integration run `.cartulary/test-results/20260710T103853Z-p87610` before reserved-family gating moved ahead of the incident catch-all. |
| Decisions recorded | `network_flow_activity` is discoverable and reserved but default-unclaimed; deployment config fails closed for premature claims; unclaimed reserved-route dispatch is pre-mux so module catch-all handlers cannot expose reserved extension roots |
| Open questions | none for `NFA-IMPL-012`; `NFA-IMPL-013` must define durable table storage/lifecycle without exposing query/import apply behavior prematurely |
| Blockers | Network Flow storage/import/query/graph/UI, immutable fixtures, transcripts, executable Phase 12 evidence, and final coordinated adoption remain blocked behind later rows |
| Next recommended task/workflow | Validate and commit this completion checkpoint, then begin `NFA-IMPL-013` storage and table lifecycle as the only active row |
| Safe restart command | `rg -n -e 'NFA-IMPL-013' -e 'networkflow' -e 'network_flow_table' docs/handoffs/network-flow-activity-adoption-handoff-tracker.md docs/network-flow-activity-nlspec.md contracts/network-flow internal/modules db/migrations db/queries tools/migration_history_manifest.json` |
