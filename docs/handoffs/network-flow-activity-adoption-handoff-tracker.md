...
| `NFA-IMPL-010` | Replace closure-only jobs with durable named handlers | `WS-10` | `IN_PROGRESS` | `NFA-GEN-004`, `NFA-C01-005`, `NFA-PHASE12-001` | platform/jobs and async module owners | `NF-BLOCK-003`, `NF-BLOCK-011`, `NF-BLOCK-016` | `internal/platform/jobs`; async modules | Leases, attempts, recovery, cancellation, drain, and migration tests | `make backend-unit`; `make backend-integration` | Every async family recovers after restart and duplicate delivery |
...
| Commands run | `git status --short --branch`; `git rev-parse HEAD`; `date -Iseconds`; tracker/source `rg`; `sed`; source review of `internal/platform/jobs`, imports, reporting, incident bundles, and reference-data async call sites; start checkpoint validation commands in §15.28 |
| Passing validation | `NFA-PHASE12-001` start, artifact, retained no-op slice, and completion checkpoint validation are recorded in §15.27; `NFA-IMPL-010` start checkpoint validation is recorded in §15.28 |
| Failing validation | Prior failing validation remains recorded in the relevant §15 rows. No new `NFA-IMPL-010` validation failure is recorded yet. |
| Decisions recorded | `NFA-IMPL-010` will add a shared durable named-handler substrate instead of patching each closure-only caller independently; module-specific payload semantics remain with their current owners |
| Open questions | none for the start checkpoint; implementation review must choose the narrowest migration that preserves completed job readability and fails closed for unsafe nonterminal jobs |
| Blockers | Import v2, Network Flow storage/import/query/graph/UI, immutable fixtures, transcripts, executable Phase 12 evidence, and final coordinated adoption remain blocked behind later rows |
| Next recommended task/workflow | Validate and commit this start checkpoint, then implement durable named job handlers in `internal/platform/jobs` and existing async callers |
