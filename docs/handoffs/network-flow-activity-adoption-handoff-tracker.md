...
| `NFA-GEN-004` | Enforce generated and schema drift | generated contracts | `IN_PROGRESS` | `NFA-GEN-003`, `NFA-TH-007` | harness | `NF-BLOCK-007` | Drift scripts and manifests | Passing retained drift artifacts | `make generate-drift` | Clean regeneration is byte-identical |
...
| Field | Value |
| --- | --- |
| Date/time | `2026-07-10T05:15:47-04:00` |
| Branch/commit | `main`; start snapshot `aa28972dd007136a571463010262ec8cf3d5e8cb` |
| Dirty-tree state | Tracker-only start checkpoint edit for `NFA-GEN-004`; drift-evidence artifact not started |
| Current workflow/task | Network Flow generated-drift start checkpoint; `NFA-GEN-004` is active and downstream implementation rows remain blocked |
...
| Passing validation | `NFA-GEN-003` start, artifact, and completion checkpoint validation are recorded in §15.25; `NFA-GEN-004` start validation is recorded in §15.26 |
...
| Decisions recorded | `NFA-GEN-004` is validation/evidence-only unless drift scripts or manifests fail closed and require owner-aligned repair |
| Open questions | none for starting `NFA-GEN-004`; implementation rows remain blocked until this checkpoint completes |
...
| Next recommended task/workflow | Validate and commit this start checkpoint, then run the generated/schema drift evidence bundle for `NFA-GEN-004` |
| Safe restart command | `rg -n -e 'NFA-GEN-004' -e 'generate-drift' -e 'NetworkFlowArtifacts' -e 'networkFlowArtifacts' docs/handoffs/network-flow-activity-adoption-handoff-tracker.md contracts tools/harness internal/gen packages/protocol-ts` |
