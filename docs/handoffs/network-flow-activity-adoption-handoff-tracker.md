...
| `NF-GATE-011` | Adopt ephemeral Graph Projection adapter boundary | Graph Projection | `NFA-GP-001`, `NFA-GP-002`, `NFA-GP-003` | Subsystem amendment and aligned evidence | Exact input/success/failure fixtures | `BLOCKED` | Ephemeral operation owner exists in `4e446354`; adapter mapping/outcome owner text exists in `f177fb6b`; evidence drift remains | Ephemeral invocation and mappings are adopted |
...
| `NF-BLOCK-015` | Graph Projection accepts the exact adapter boundary | Graph Projection | `NFA-GP-001..003`, `NFA-FIX-015`, `NFA-FIX-028` | Subsystem amendment and evidence alignment | Input/outcome/aggregate fixtures | `BLOCKED` | Ephemeral operation and adapter mapping/outcome contracts exist; matrix/corpus/checker drift and adapter fixtures remain | Adapter inputs and outcomes pass |
...
| `NFA-RISK-008` | Draft Graph Projection metadata/type wording is incompatible. | Graph Projection/Network Flow | `NFA-GP-003` | Adapter rejection, leakage, or private type fork. | Owner/downstream text now rejects private direct-copy members and defines exact property, metadata, result, and outcome mapping; executable evidence repair remains required. | `MITIGATED` |
