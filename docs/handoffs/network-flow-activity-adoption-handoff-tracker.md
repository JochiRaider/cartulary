...
| `NFA-GEN-003` | Regenerate Go and TypeScript outputs | generated contracts | `IN_PROGRESS` | `NFA-GEN-002` | generator | `NF-BLOCK-007` | Generated roots in §2.3 | Generator-produced diff | `make generate` | Outputs contain generated markers and no hand edit |
...
| Field | Value |
| --- | --- |
| Date/time | `2026-07-10T05:04:12-04:00` |
| Branch/commit | `main`; start snapshot `fb458479f452f1a43f0ee72daa13859c4e468631` |
| Dirty-tree state | Tracker-only start checkpoint edit for `NFA-GEN-003`; generator activation artifact not started |
| Current workflow/task | Network Flow generated-output start checkpoint; `NFA-GEN-003` is active and `NFA-GEN-004` remains blocked |
...
| Other changed files | none expected before this start checkpoint |
| Commands run | `git status --short --branch`; `git rev-parse HEAD`; `date -Iseconds`; tracker/source `rg`; `sed`; `make lint-markdown`; `make generated-artifact-policy-check`; `make json-shape-check`; tracker table-column check; `git diff --check` |
| Passing validation | `NFA-GEN-002` start, artifact, and completion checkpoint validation are recorded in §15.24; `NFA-GEN-003` start validation is recorded in §15.25 |
...
| Decisions recorded | `NFA-GEN-003` will activate the Network Flow contract family only after the start checkpoint; generated roots must be changed only by `make generate` |
| Open questions | none for starting `NFA-GEN-003`; `NFA-GEN-004` remains the later drift-enforcement checkpoint |
...
| Next recommended task/workflow | Validate and commit this start checkpoint, then activate the `network-flow` family and run `make generate` |
| Safe restart command | `rg -n -e 'NFA-GEN-003' -e 'contracts/index.json' -e 'NetworkFlowArtifacts' -e 'networkFlowArtifacts' docs/handoffs/network-flow-activity-adoption-handoff-tracker.md contracts tools/contractgen internal/gen packages/protocol-ts packages/ui-contracts` |
