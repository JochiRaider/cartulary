...
| `NFA-GEN-003` | Regenerate Go and TypeScript outputs | generated contracts | `TODO` | `NFA-GEN-002` | generator | `NF-BLOCK-007` | Generated roots in §2.3 | Generator-produced diff | `make generate` | Outputs contain generated markers and no hand edit |
...
| Network Flow authored contract ownership | `contracts/index.json`; `contracts/network-flow/index.json`; `contracts/network-flow/routes.v1.json`; `contracts/network-flow/errors.v1.json`; `contracts/network-flow/schemas.v1.json`; `contracts/network-flow/timezone/tzdb-2026c.provenance.json` | Future generated outputs remain under `internal/gen/contracts/`, `packages/protocol-ts/src/generated/`, and `packages/ui-contracts/src/generated/` once activated | Future module, routes, web UI, and harness | Planned family declared and authored contracts exist; family is not emitted until `NFA-GEN-003` activates generated outputs | `make generate` | `make json-shape-check`; `make generate-drift` | `NFA-GEN-001..004` | `DONE for ownership and authored inputs; generated outputs blocked` |
...
| Commands run | `git status --short --branch`; `date -Iseconds`; tracker/source `rg`; `sed`; direct JSON parse and Node syntax checks; `node tools/harness/generated-artifacts/check-json-shapes.mjs --kind network-flow-contract-index --file contracts/network-flow/index.json`; `make json-shape-check`; `make harness-contract`; `make lint-scripts`; `make generated-artifact-policy-check`; `make generate-drift`; `git diff --check` |
| Passing validation | `NFA-LOC-001` start, artifact, and completion checkpoint validation are recorded in §15.23; `NFA-GEN-002` start and artifact validation are recorded in §15.24 |
| Failing validation | `make json-shape-check` `.cartulary/test-results/20260710T082738Z-p15417` failed on the new locator-fragment check because the manifest expected lowercase `adopted` while the table used `Adopted`; fixed and reran pass. One broad ad hoc multi-file table-column check failed on escaped-pipe examples outside changed tables; targeted tracker/Table 1-B/Table 8-D checks passed. Prior unrelated `make harness-contract` failure `.cartulary/test-results/20260710T081325Z-p2192` remains documented in §15.22. |
| Decisions recorded | `NFA-GEN-002` authored closed owner-derived route/error/public-schema inputs while the contract family stays planned; generated outputs stay blocked until `NFA-GEN-003..004` |
| Open questions | none for starting `NFA-GEN-003`; exact generated-output activation remains the next slice |
| Blockers | Generated roots, Phase 12 maps, implementation, immutable fixtures, executable evidence, and final coordinated adoption remain blocked |
| Next recommended task/workflow | Validate and commit this completion checkpoint, then start `NFA-GEN-003` from a clean worktree |
| Safe restart command | `rg -n -e 'NFA-GEN-003' -e 'contracts/network-flow' -e 'network_flow_table' -e 'network_flow_activity' docs/handoffs/network-flow-activity-adoption-handoff-tracker.md contracts tools/contractgen tools/harness` |
