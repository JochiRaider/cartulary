...
| `NFA-TH-006` | Audit-count and no-audit replay assertions | Testing Harness | `IN_PROGRESS` | `NFA-TH-001`, `NFA-C04-004` | Testing Harness | `NF-GATE-012`, `NF-BLOCK-016` | Harness §12 test controls; Core 04 audit occurrence owner | Start checkpoint in §15.19; audit-count control edits not started | `make lint-markdown`; `make generated-artifact-policy-check`; `make json-shape-check`; `git diff --check` | Exact occurrences and zero replay increments are proven |
...
| Field | Value |
| --- | --- |
| Date/time | `2026-07-10T03:25:05-04:00` |
| Branch/commit | `main`; Testing Harness auth-transition checkpoint `cc11c4f30ee51ad8ba3fa00ca558c0bf0d5fbf97` |
| Dirty-tree state | Tracker-only start checkpoint edit for `NFA-TH-006`; audit-count control edits not started |
| Current workflow/task | Testing Harness audit-count/no-audit replay start checkpoint; `NFA-TH-006` is the single active row |
...
| Passing validation | `NFA-TH-005` completion validation is recorded in §15.18; `NFA-TH-006` start validation passed with `make lint-markdown` `.cartulary/test-results/20260710T072608Z-p47124`, `make generated-artifact-policy-check` `.cartulary/test-results/20260710T072608Z-p47134`, `make json-shape-check` `.cartulary/test-results/20260710T072608Z-p47136`, and `git diff --check` |
| Failing validation | none |
| Decisions recorded | `NFA-TH-006` is limited to fixture-only exact-count and no-audit replay assertions; generated-contract and drift accounting remain separate rows |
| Open questions | Exact audit assertion response schema, event-code vocabulary, expected-count fields, replay-silence semantics, and reset behavior must be resolved in the owner artifact |
...
| Next recommended task/workflow | Commit this start checkpoint, then add fixture-only audit-count and no-audit replay assertion controls |
...
| Harness audit-count artifact | Pending |
| Targeted audit-count review | Pending |
