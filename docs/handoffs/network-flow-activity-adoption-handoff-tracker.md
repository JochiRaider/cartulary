...
| `NFA-IMPL-013` | Implement Network Flow storage and table lifecycle | `WS-13` | `IN_PROGRESS` | `NFA-IMPL-011`, `NFA-IMPL-012` | `internal/modules/networkflow` | lifecycle ACs | module, migrations, authored queries | Immutable row, table-limit, rename, soft-delete, and metric evidence | `make backend-unit`; `make backend-store`; `make migration-drift` | Only active tables are queryable and retained counts are observable |
...
| Next recommended task/workflow | Validate and commit this start checkpoint, then implement Network Flow storage and table lifecycle |
