...
| `NFA-IMPL-011` | Add import source streams, mapping v2, and transaction coordination | `WS-11` | `TODO` | `NFA-IMPL-010`, `NFA-C01-002..005` | imports/platform | `NF-BLOCK-003`, `NF-BLOCK-011`, `NF-BLOCK-012` | imports, object store, migrations | Opaque source lifecycle and atomic extension-target stub | `make backend-unit`; `make backend-integration`; `make migration-drift` | v1 remains valid and v2 commits or fails without partial publication |
...
| Commands run | `git status --short --branch`; `git rev-parse`; `date -Iseconds`; tracker/source `rg`; `sed`; implementation source review; `make format-go`; validation commands recorded in §15.28 |
| Passing validation | `NFA-PHASE12-001` start, artifact, retained no-op slice, and completion checkpoint validation are recorded in §15.27; `NFA-IMPL-010` start, artifact, unit, integration, drift, lint, targeted security, and diff validation are recorded in §15.28 |
| Failing validation | Resolved `NFA-IMPL-010` backend-unit failures are recorded in §15.28; no unresolved validation failure remains for this workstream |
| Decisions recorded | Shared durable named-handler substrate was implemented in `internal/platform/jobs`; module-specific payload semantics stayed with imports, reporting, reference-data, and incident-bundle owners; migration 27 fails unsafe legacy nonterminal jobs closed instead of trying to infer missing handlers |
| Open questions | none for `NFA-IMPL-010`; `NFA-IMPL-011` must define the opaque import source stream and mapping v2 boundary before adding Network Flow extension-target dispatch |
...
| Next recommended task/workflow | Commit this tracker completion checkpoint, then begin `NFA-IMPL-011` with a fresh start checkpoint |
| Safe restart command | `rg -n -e 'NFA-IMPL-010' -e 'NFA-IMPL-011' -e 'durable named handlers' docs/handoffs/network-flow-activity-adoption-handoff-tracker.md internal tools docs` |
