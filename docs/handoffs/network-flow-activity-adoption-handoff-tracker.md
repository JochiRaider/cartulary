...
| `NFA-IMPL-017` | Implement invalidation and Network Analysis workspace | `WS-17` | `TODO` | `NFA-C03-002`, `NFA-IMPL-012..016`, `NFA-DESIGN-001` | WebSocket/frontend | UI ACs | WebSocket contracts and `apps/web` | Backend invalidation and frontend workspace remain unimplemented; start with the backend/WS invalidation sub-slice now that prerequisites are `DONE` | `make frontend-typecheck`; `make frontend-unit`; `make browser-e2e-stateful`; `make browser-e2e-a11y`; `make browser-e2e-visual` | Claimed workspace remains current-authorization safe and Base surfaces do not change |
