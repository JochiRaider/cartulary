...
| `NFA-LOC-001` | Adopted versions and immutable locators for Table 1-B | dependency registry | `IN_PROGRESS` | non-final owner amendment tasks; final adoption flip remains `NFA-ADOPT-001` | all dependency owners | `NF-BLOCK-010`, `NF-AC-106` | Network Flow Table 1-B | Exact adopted version plus section/schema per row and `NF-FIX-027` soft-delete/no-purge reconciliation | `make lint-markdown`; `make json-shape-check`; `git diff --check` | No `TODO:` dependency cell remains |
...
| Commands run | `git status --short --branch`; `git rev-parse HEAD`; `date -Iseconds`; tracker/source `rg`; `sed`; `make lint-markdown`; `make generated-artifact-policy-check`; `make json-shape-check`; `git diff --check`; tracker table-column check |
| Passing validation | `NFA-LOC-001` start checkpoint validation is recorded in §15.23; `NFA-TH-007` artifact and completion checkpoint validation are recorded in §15.22 |
| Failing validation | Initial `make harness-contract` run `.cartulary/test-results/20260710T081325Z-p2192` failed in unrelated dev-service lifecycle smoke due SeaweedFS readiness timeout; immediate rerun passed `.cartulary/test-results/20260710T081354Z-p5062` |
| Decisions recorded | Structural accounting belongs to the generated-artifact harness facade; the generic contract-family registry now allows future Network Flow activation while the Network Flow accounting checker blocks premature activation until generated outputs contain the family |
| Open questions | none for `NFA-LOC-001`; authored Network Flow contracts, generated outputs, fixtures, Phase 12 maps, and product implementation remain later |
...
| Next recommended task/workflow | Validate and commit this start checkpoint, then fill Table 1-B locators and reconcile `NF-FIX-027` |
| Safe restart command | `rg -n -e 'NFA-LOC-001' -e 'Table 1-B' -e 'NF-FIX-027' -e 'retention-purge' -e 'retention-soft-delete' docs/handoffs/network-flow-activity-adoption-handoff-tracker.md docs/network-flow-activity-nlspec.md tools/harness/generated-artifacts/check-json-shapes.mjs tools/network_flow_activity_accounting.json` |
