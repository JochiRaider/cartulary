...
`docs/network-flow-activity-nlspec.md` through the adopted/current Network
Flow Activity profile and its post-adoption handoff state. It coordinates owner
decisions, derived contracts, generated artifacts, implementation work, fixture
bytes, executable conformance evidence, validation, and the final status
transition. It is not normative authority. Every behavior decision remains with
the cited owner document.
...
3. The adopted/current Network Flow Activity NLSpec within its extension boundary.
...
If an adopted owner contradicts the Network Flow Activity NLSpec, record
`BLOCKED: owner contradiction`, cite both seams, and obtain an owner decision.
If code differs from an owner, record drift without treating the code as
authority. Behavior-affecting closure must proceed in this order: owner
amendment; derived contract; generator and generated outputs; implementation;
fixtures and executable evidence; drift validation; adoption/status transition.

This tracker does not authorize:

- silently changing Network Flow behavior or treating this tracker as normative;
...
| Network Flow owner proposal | Initial snapshot was `status: draft`; final adoption artifact `f4409d98` updates `docs/network-flow-activity-nlspec.md` to `status: adopted/current`, `document_version: 1.0.0`, `contract_major: 1` |
...
The commit and checksums above freeze the initial inspection. They are not
adopted document versions or immutable dependency locators for Table 1-B.
Those initial locator blockers are closed by the later owner artifacts and
final adoption evidence recorded in §15.
...
| Contract generator | `contracts/index.json`; `tools/contractgen/main.go`; `tools/contractgen/families.go`; `tools/harness/generated-artifacts/generate-artifacts.sh` | Generator families are registry-owned; Network Flow authored inputs are active and emitted through owner-derived contracts |
...
| `NFA-C00-002` | Record future-only incident purge and coordinate the final registry flip | Core 00 | `DONE` | `NFA-C00-001`, `NFA-ADOPT-001` | Core 00 | `NF-GATE-001`, `NF-BLOCK-001` | Core 00 §§4.3, 5.1 | Explicit v1 non-purge decision plus coordinated final Core/NLSpec amendment in artifact `f4409d98`; §15.42 evidence | `make json-shape-check`; `make generate-drift`; `make phase-slice PHASE=phase12` | Current v1 makes no purge claim and final adoption is atomic |
...
| `NFA-ADOPT-001` | Final owner review, version, locator, and status transition | adoption | `DONE` | every gate, blocker, fixture, generated task, AC row, validation task | all owners | `NF-AC-106` and all adoption IDs | Core 00 and Network Flow status headers/registries; `tools/phase12_test_map.json` | Artifacts `f4409d98` and `e83b3719`; §15.42 evidence; NLSpec is `adopted/current` and profile claim accounting is active | `make backend-unit`; `make json-shape-check`; `make generate-drift`; `make phase-map-check`; `make phase-ledger-drift`; `make phase-schedule-drift`; `make phase-slice PHASE=phase12` | Nothing required remains open for adoption |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `NF-GATE-001` | Add Network Flow to extension ownership and adopted map | Core 00 | `NFA-C00-001`, `NFA-C00-002`, `NFA-LOC-001` | Owner amendment; final registry/status coordination | Core 00/NLSpec adoption artifacts `f4409d98` and Phase 12 claim accounting `e83b3719` | `DONE` | Final claimability/status flip is complete | Core lists the adopted bounded profile and keeps v1 no-private-purge posture |
| `NF-GATE-002` | Discover the claimed route family | Core 01/extensions | `NFA-C01-001`, `NFA-GEN-002`, `NFA-IMPL-012`, `NFA-VAL-002` | Discovery owner contract, derived schemas, implementation | Claimed/unclaimed discovery evidence in retained Phase 12 roots `.cartulary/test-results/20260710T151138Z-p53460`, `.cartulary/test-results/20260710T154140Z-p73390` | `DONE` | None | Exact adopted discovery behavior passes |
| `NF-GATE-003` | Admit extension analytical import targets | Core 01/imports | `NFA-C01-002`, `NFA-C01-003`, `NFA-IMPL-014`, `NFA-VAL-002` | Import target/result and facade amendments plus implementation | Import mapping/apply fixtures and Phase 12 retained evidence | `DONE` | None | Exact `network_flow_table` target is adopted and implemented |
| `NF-GATE-004` | Publish terminal Network Flow resource results | Core 01/jobs/imports | `NFA-C01-002`, `NFA-C01-005`, `NFA-IMPL-010`, `NFA-IMPL-014`, `NFA-VAL-002` | Result union, named jobs, publication implementation | Replay/cancel/recovery selectors in retained Phase 12 and final check roots | `DONE` | None | Terminal result references an extension resource safely |
| `NF-GATE-005` | Admit an extension top-level incident tab | Core 03 | `NFA-C03-001`, `NFA-DESIGN-001`, `NFA-IMPL-017`, `NFA-TEST-006` | Extension surface contract and UI implementation | Claimed/unclaimed browser evidence in Phase 12 and browser workspace artifacts | `DONE` | None | Base built-in list stays unchanged and claimed-only workspace passes |
| `NF-GATE-006` | Add Network Flow route authorization | Core 04 | `NFA-C04-001`, `NFA-IMPL-012`, `NFA-TEST-002` | Authorization matrix and route hooks | Membership/role/admin selectors in retained Phase 12 roots | `DONE` | None | Route-time reauthorization passes |
| `NF-GATE-007` | Provide one cross-owner unit of work | Core 01 with Core 02/Core 04 participants | `NFA-C01-004`, `NFA-C02-002`, `NFA-C04-004`, `NFA-TH-002`, `NFA-IMPL-014`, `NFA-IMPL-016` | Transaction capability and participants | Failure-at-final-commit, import, binding, audit, and replay selectors pass in retained evidence | `DONE` | None | All-or-nothing state is proven |
| `NF-GATE-008` | Designate IP identity and binding transaction interface | Core 02 | `NFA-C02-001`, `NFA-C02-002`, `NFA-C01-004`, `NFA-IMPL-016`, `NFA-VAL-002` | Indicator registry/canonicalization and transaction-participant amendments | Canonical IP, concurrent binding, dedupe, and fault fixtures pass | `DONE` | None | Adopted family-specific tokens and atomic binding are exact |
| `NF-GATE-009` | Provide extension-resource invalidation | Core 03 with Core 01 wire owner | `NFA-C03-002`, `NFA-TH-005`, `NFA-IMPL-017`, `NFA-TEST-006` | Event/wire and UI consequence amendments | Rename/delete/auth-loss invalidation evidence in backend, frontend, and Phase 12 roots | `DONE` | None | Current authorization invalidates resources deterministically |
| `NF-GATE-010` | Provide cursor, audit, secret, and soft-delete/source-retention hooks | Core 04 with Core 01 cursor wire owner | `NFA-C04-002..005`, `NFA-TH-003`, `NFA-TH-006`, `NFA-IMPL-015`, `NFA-VAL-003` | Security lifecycle and conformance amendments | Cursor/rotation/audit/source-expiry/soft-delete fixtures and security/finalizer bundle pass | `DONE` | None | Every named security fixture passes without a current incident-purge claim |
| `NF-GATE-011` | Adopt ephemeral Graph Projection adapter boundary | Graph Projection | `NFA-GP-001`, `NFA-GP-002`, `NFA-GP-003`, `NFA-IMPL-016`, `NFA-VAL-002` | Subsystem amendment and aligned evidence | Graph adapter input/success/failure/aggregate selectors pass in retained Phase 12 roots | `DONE` | None | Ephemeral invocation and mappings are adopted and implemented |
| `NF-GATE-012` | Execute Network Flow contracts, fixtures, lint, and drift | Testing Harness | `NFA-TH-001..007`, `NFA-VAL-002`, `NFA-VAL-003` | Harness amendment, manifests, targets, accounting | Retained full conformance and finalization bundle | `DONE` | Phase 12 retained conformance roots `.cartulary/test-results/20260710T151138Z-p53460` and `.cartulary/test-results/20260710T151210Z-p71906`; full warm check root `.cartulary/test-results/20260710T152852Z-p53813`; finalizer root `.cartulary/test-results/20260710T153119Z-p53856` | All §22 and §23 rows execute |
| `NF-BLOCK-001` | Core 00 recognizes the profile | Core 00 | `NFA-C00-001`, `NFA-C00-002`, `NFA-ADOPT-001` | Same owner amendment as gate 001 | Core registry and NLSpec adoption evidence in `f4409d98`; phase claim accounting in `e83b3719` | `DONE` | None | Core registry and NLSpec status agree |
| `NF-BLOCK-002` | Discovery lists the route only as claimed behavior | Core 01 | `NFA-C01-001`, `NFA-IMPL-012`, `NFA-VAL-002` | Discovery contract/implementation | Claimed/unclaimed fixture evidence passes in Phase 12 roots | `DONE` | None | Discovery is exact and generated |
| `NF-BLOCK-003` | Import terminal refs admit Network Flow tables | Core 01 | `NFA-C01-002`, `NFA-C01-005`, `NFA-IMPL-010`, `NFA-IMPL-014` | Result union amendment and implementation | Terminal/replay fixture evidence passes | `DONE` | None | Extension ref is owner-adopted and implemented |
| `NF-BLOCK-004` | Extension tab without base-tab expansion | Core 03 | `NFA-C03-001`, `NFA-DESIGN-001`, `NFA-IMPL-017`, `NFA-TEST-006` | Extension-surface contract | Browser surface fixture evidence passes | `DONE` | None | Claimed-only tab behavior passes |
| `NF-BLOCK-005` | Authorization/conformance hooks exist | Core 04 | `NFA-C04-001`, `NFA-IMPL-012`, `NFA-TEST-002` | Route-family matrix and criteria | Route authorization results pass | `DONE` | None | No membership/admin bypass drift |
...
| `NF-BLOCK-007` | Generated contracts exist and do not drift | Contract owners/harness | `NFA-GEN-001..004`, `NFA-TH-007`, `NFA-VAL-003` | Authored inputs, generator, outputs, drift | Passing retained generation artifacts | `DONE` | Generated drift passed at `.cartulary/test-results/20260710T151448Z-p92224`; full warm check passed at `.cartulary/test-results/20260710T152852Z-p53813`; finalizer shape/drift checks passed at `.cartulary/test-results/20260710T153119Z-p53856` | Clean regeneration is byte-identical |
...
| `NF-BLOCK-011` | Import facade and atomic publication are adopted | Core 01 | `NFA-C01-003..005`, `NFA-TH-002`, `NFA-IMPL-010`, `NFA-IMPL-014` | Owner facade/stream/source-change/publication amendments | Facade, fault, cancel, and replay fixtures pass in retained evidence | `DONE` | None | Two-operation boundary passes |
| `NF-BLOCK-012` | IP identity and binding unit-of-work behavior is adopted | Core 02 | `NFA-C02-001..002`, `NFA-C01-004`, `NFA-IMPL-016` | Indicator and transaction-participant amendments plus binding implementation | IP/link/fault fixtures and binding selectors pass | `DONE` | None | Exact owner behavior passes |
| `NF-BLOCK-013` | Invalidation topics and consequences are adopted | Core 03 | `NFA-C03-002`, `NFA-TH-005`, `NFA-IMPL-017` | Interaction/wire amendments | Rename/delete/auth-loss fixtures and UI invalidation evidence pass | `DONE` | None | Every consequence is current-authorization safe |
| `NF-BLOCK-014` | Cursor, digest, audit, and retention behavior is adopted | Core 04/Core 01 | `NFA-C04-002..005`, `NFA-TH-003`, `NFA-TH-006`, `NFA-VAL-003` | Security owner amendments | Rotation/count/source-expiry/soft-delete/cursor fixtures | `DONE` | Cursor, safe-digest, audit, and retention owners resolved in `90401fb2`, `cd645750`, `71258589`, and `663c8684`; implementation evidence exists in `a8827188`, `b98ced41`, `2e586310`, `8a7c95f2`; retained validation/finalization passed in §15.40-15.41 | Exact security lifecycle passes without a current purge claim |
| `NF-BLOCK-015` | Graph Projection accepts the exact adapter boundary | Graph Projection | `NFA-GP-001..003`, `NFA-FIX-015`, `NFA-FIX-028`, `NFA-IMPL-016`, `NFA-VAL-002` | Subsystem amendment and evidence alignment | Input/outcome/aggregate fixtures pass in retained Phase 12 roots | `DONE` | None | Adapter inputs and outcomes pass |
...
The authoritative Phase 12 selector map now exists in
`tools/phase12_test_map.json`, and `docs/testing/phase12_coverage_ledger.md` is
the generated companion ledger. This embedded tracker matrix is retained as a
workstream handoff view: every row cites the executable selector, retained run
evidence, and closure state. “Structural” means document or contract structure,
not a waiver from executable validation.

| AC ID | Behavior | Test level | Test path/node | Fixture IDs | Dependency tasks | Expected artifact | Last command/result | Status | Gap |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `NF-AC-001` | Unclaimed tab and routes unavailable. | browser + route integration | apps/web/e2e/phase12.network-flow.spec.ts::E-12-NFAC001-01 Network Flow unclaimed workspace remains unavailable | n/a: claim-state scenario | `NFA-C00-001`; `NFA-C01-001`; `NFA-C03-001`; `NFA-C04-001` | E-12-NFAC001-01; executable Phase 12 Network Flow selector for NF-AC-001 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-002` | Claimed empty state and authorized import action. | browser | apps/web/e2e/phase12.network-flow.spec.ts::E-12-NFAC002-02 Network Analysis claimed empty state exposes import entry | n/a: claim/role scenario | `NFA-C00-001`; `NFA-C03-001`; `NFA-C04-001` | E-12-NFAC002-02; executable Phase 12 Network Flow selector for NF-AC-002 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-003` | Core envelopes and unknown-member rejection. | contract + route integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC003_03_CoreEnvelopeAdmissionEvidence | `NF-FIX-010-json-admission` | `NFA-C01-004`; `NFA-C04-001`; `NFA-GEN-001` | I-12-NFAC003-03; executable Phase 12 Network Flow selector for NF-AC-003 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-004` | Duplicate JSON members rejected before mutation/idempotency. | parser + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC004_04_DuplicateJSONAdmissionEvidence | `NF-FIX-010-json-admission` | `NFA-C01-005`; `NFA-TH-002`; `NFA-GEN-001` | I-12-NFAC004-04; executable Phase 12 Network Flow selector for NF-AC-004 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-005` | UTF-8, JSON, object, null, and member admission is deterministic. | parser + route integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC005_05_InvalidJSONAdmissionEvidence | `NF-FIX-010-json-admission` | `NFA-GEN-001`; `NFA-TH-001` | I-12-NFAC005-05; executable Phase 12 Network Flow selector for NF-AC-005 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-006` | Minimal import creates one table and inner tab. | integration + browser | apps/web/e2e/phase12.network-flow.spec.ts::E-12-NFAC006-06 Network Analysis import creates one inner table tab | `NF-FIX-001-cisco-sna-minimal` | `NFA-C01-003..005`; `NFA-C03-001`; `NFA-TH-001` | E-12-NFAC006-06; executable Phase 12 Network Flow selector for NF-AC-006 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-007` | Filename-derived display naming and suffixing. | unit + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC007_07_TableNameDerivationEvidence | `NF-FIX-013-filename-display` | `NFA-C01-003`; `NFA-TH-001` | I-12-NFAC007-07; executable Phase 12 Network Flow selector for NF-AC-007 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-008` | Apply replay returns same table without duplicate. | transactional integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC008_08_ImportReplayEvidence | `NF-FIX-020-atomic-import-commit` | `NFA-C01-004..005`; `NFA-TH-002` | I-12-NFAC008-08; executable Phase 12 Network Flow selector for NF-AC-008 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-009` | Duplicate headers use source ordinals. | parser + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC009_09_DuplicateHeaderOrdinalEvidence | `NF-FIX-003-duplicate-headers` | `NFA-C01-003`; `NFA-TH-001` | I-12-NFAC009-09; executable Phase 12 Network Flow selector for NF-AC-009 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-010` | Cisco SNA required fields enforced. | parser conformance | internal/modules/networkflow/phase12_network_flow_unit_test.go::TestPhase12NetworkFlow_U_12_NFAC010_10_CiscoSNARequiredFields | `NF-FIX-001-cisco-sna-minimal`; `NF-FIX-004-rejected-rows` | `NFA-TH-001` | U-12-NFAC010-10; executable Phase 12 Network Flow selector for NF-AC-010 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-011` | Reserved profiles cannot be claimed. | contract + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC011_11_ReservedSourceProfileEvidence | n/a: source-profile vectors | `NFA-C00-001`; `NFA-GEN-001` | I-12-NFAC011-11; executable Phase 12 Network Flow selector for NF-AC-011 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-012` | CSV edge cases follow exact parser outcomes. | parser conformance | internal/modules/networkflow/phase12_network_flow_unit_test.go::TestPhase12NetworkFlow_U_12_NFAC012_12_CSVParserEdgeOutcomes | `NF-FIX-005-csv-parser-edges`; `NF-FIX-021-preview-boundaries` | `NFA-TH-001` | U-12-NFAC012-12; executable Phase 12 Network Flow selector for NF-AC-012 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-013` | Formula-looking values remain inert. | parser + security | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC013_13_FormulaValuesRemainInertEvidence | `NF-FIX-005-csv-parser-edges` | `NFA-TH-001` | I-12-NFAC013-13; executable Phase 12 Network Flow selector for NF-AC-013 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-014` | Invalid rows yield deterministic diagnostics and no query/graph rows. | integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC014_14_RejectedRowsExcludedEvidence | `NF-FIX-004-rejected-rows` | `NFA-GP-001`; `NFA-TH-004` | I-12-NFAC014-14; executable Phase 12 Network Flow selector for NF-AC-014 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-015` | Partly valid import retains accepted rows and rejected count. | integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC015_15_PartialImportAcceptedRowsEvidence | `NF-FIX-004-rejected-rows` | `NFA-C01-004`; `NFA-TH-001` | I-12-NFAC015-15; executable Phase 12 Network Flow selector for NF-AC-015 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-016` | All-invalid import creates no table. | transactional integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC016_16_AllRejectedImportEvidence | `NF-FIX-004-rejected-rows` | `NFA-C01-004`; `NFA-TH-002` | I-12-NFAC016-16; executable Phase 12 Network Flow selector for NF-AC-016 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-017` | Five digest/ID algorithms match exact vectors. | unit + fixture conformance | internal/modules/networkflow/phase12_network_flow_unit_test.go::TestPhase12NetworkFlow_U_12_NFAC017_17_DigestAndIDVectors | `NF-FIX-001-cisco-sna-minimal`; `NF-FIX-006-cross-table-graph`; `NF-FIX-019-canonical-json-unicode` | `NFA-LOC-001`; `NFA-TH-004` | U-12-NFAC017-17; executable Phase 12 Network Flow selector for NF-AC-017 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-018` | Timestamp folds, gaps, leap seconds, inference, and reversed intervals reject. | unit + fixture conformance | internal/modules/networkflow/phase12_network_flow_unit_test.go::TestPhase12NetworkFlow_U_12_NFAC018_18_TimestampRejectsInvalidInputs | `NF-FIX-004-rejected-rows`; `NF-FIX-012-sys-uptime-timestamps`; `NF-FIX-022-timestamp-rulesets` | `NFA-TZ-001`; `NFA-TH-003` | U-12-NFAC018-18; executable Phase 12 Network Flow selector for NF-AC-018 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-019` | Canonical IP output and invalid forms. | unit conformance | internal/modules/networkflow/phase12_network_flow_unit_test.go::TestPhase12NetworkFlow_U_12_NFAC019_19_IPCanonicalizationVectors | `NF-FIX-004-rejected-rows`; `NF-FIX-017-indicator-link-mismatch` | `NFA-C02-001` | U-12-NFAC019-19; executable Phase 12 Network Flow selector for NF-AC-019 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-020` | Unsigned-decimal grammar and max bound. | unit conformance | internal/modules/networkflow/phase12_network_flow_unit_test.go::TestPhase12NetworkFlow_U_12_NFAC020_20_Uint64DecimalGrammar | `NF-FIX-004-rejected-rows`; `NF-FIX-028-graph-aggregate-bounds` | `NFA-TH-001` | U-12-NFAC020-20; executable Phase 12 Network Flow selector for NF-AC-020 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-021` | Table lifecycle excludes renamed state. | structural + state-machine unit | internal/modules/networkflow/store_test.go::TestPhase12NetworkFlow_U_12_NFAC021_21_TableLifecycleVocabularyHasNoRenamedState | n/a: structural | `NFA-GEN-001`; `NFA-VAL-002` | U-12-NFAC021-21; executable Phase 12 Network Flow selector for NF-AC-021 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-022` | Rename changes metadata/version only and preserves identities/cursors/bindings. | transactional integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC022_22_RenamePreservesIdentityEvidence | `NF-FIX-013-filename-display`; `NF-FIX-014-cursor-pagination` | `NFA-C01-004`; `NFA-C04-002`; `NFA-TH-002` | I-12-NFAC022-22; executable Phase 12 Network Flow selector for NF-AC-022 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-023` | Soft delete hides table and invalidates graph/cursors terminally. | integration + browser | apps/web/e2e/phase12.network-flow.spec.ts::E-12-NFAC023-23 Network Analysis soft delete removes active table tab | `NF-FIX-009-soft-delete-stale-graph`; `NF-FIX-027-retention-soft-delete` | `NFA-C03-002`; `NFA-C04-002`; `NFA-C04-005` | E-12-NFAC023-23; executable Phase 12 Network Flow selector for NF-AC-023 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-024` | Filters use field keys and reject visible labels. | contract + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC024_24_FieldKeyFilterEvidence | `NF-FIX-024-query-normalization-cursors` | `NFA-GEN-001` | I-12-NFAC024-24; executable Phase 12 Network Flow selector for NF-AC-024 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-025` | Filter `in` rejects duplicate/empty values. | unit + route integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC025_25_FilterInDuplicateEvidence | `NF-FIX-024-query-normalization-cursors` | `NFA-GEN-001` | I-12-NFAC025-25; executable Phase 12 Network Flow selector for NF-AC-025 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-026` | CIDR rejects family mismatch and mapped-family coercion. | unit conformance | internal/modules/networkflow/phase12_network_flow_unit_test.go::TestPhase12NetworkFlow_U_12_NFAC026_26_CIDRFamilyVectors | `NF-FIX-024-query-normalization-cursors` | `NFA-C02-001` | U-12-NFAC026-26; executable Phase 12 Network Flow selector for NF-AC-026 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-027` | Sort semantics and default tail are exact. | integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC027_27_SortAndDefaultTailEvidence | `NF-FIX-014-cursor-pagination`; `NF-FIX-024-query-normalization-cursors` | `NFA-C04-002`; `NFA-TH-001` | I-12-NFAC027-27; executable Phase 12 Network Flow selector for NF-AC-027 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-028` | Cursor invalidation covers TTL, actor, route, query, delete, and auth loss. | security + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC028_28_CursorInvalidationEvidence | `NF-FIX-009-soft-delete-stale-graph`; `NF-FIX-014-cursor-pagination`; `NF-FIX-024-query-normalization-cursors` | `NFA-C04-002`; `NFA-C03-002`; `NFA-TH-003`; `NFA-TH-005` | I-12-NFAC028-28; executable Phase 12 Network Flow selector for NF-AC-028 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-029` | Graph filter selector exposes table-selection controls. | browser | apps/web/e2e/phase12.network-flow.spec.ts::E-12-NFAC029-29 Network Analysis graph mode exposes table selection controls | `NF-FIX-006-cross-table-graph` | `NFA-C03-001`; `NFA-GP-001` | E-12-NFAC029-29; executable Phase 12 Network Flow selector for NF-AC-029 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-030` | Table-tab graph defaults to active table scope. | browser + integration | apps/web/e2e/phase12.network-flow.spec.ts::E-12-NFAC030-30 Network Analysis table graph defaults to active-table scope | `NF-FIX-006-cross-table-graph` | `NFA-C03-001`; `NFA-GP-001` | E-12-NFAC030-30; executable Phase 12 Network Flow selector for NF-AC-030 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-031` | Selected tables reject duplicate IDs. | route integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC031_31_DuplicateSelectedTablesEvidence | `NF-FIX-024-query-normalization-cursors` | `NFA-GEN-001`; `NFA-GP-001` | I-12-NFAC031-31; executable Phase 12 Network Flow selector for NF-AC-031 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-032` | Multiple selected tables compose one filtered graph. | integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC032_32_CrossTableGraphEvidence | `NF-FIX-006-cross-table-graph` | `NFA-GP-001..003` | I-12-NFAC032-32; executable Phase 12 Network Flow selector for NF-AC-032 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-033` | Canonical endpoint merges across selected tables with provenance. | integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC033_33_EndpointMergeEvidence | `NF-FIX-006-cross-table-graph` | `NFA-C02-001`; `NFA-GP-001..003` | I-12-NFAC033-33; executable Phase 12 Network Flow selector for NF-AC-033 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-034` | Default edge aggregation and table provenance are exact. | integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC034_34_DefaultEdgeAggregationEvidence | `NF-FIX-006-cross-table-graph`; `NF-FIX-028-graph-aggregate-bounds` | `NFA-GP-001..003` | I-12-NFAC034-34; executable Phase 12 Network Flow selector for NF-AC-034 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-035` | Time overlap and zero-duration handling are exact. | unit + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC035_35_GraphTimeOverlapEvidence | `NF-FIX-006-cross-table-graph`; `NF-FIX-022-timestamp-rulesets` | `NFA-TZ-001`; `NFA-GP-001` | I-12-NFAC035-35; executable Phase 12 Network Flow selector for NF-AC-035 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-036` | Vertex selection pivots by stable identity. | browser + integration | apps/web/e2e/phase12.network-flow.spec.ts::E-12-NFAC036-36 Network Analysis vertex selection uses stable graph identity | `NF-FIX-025-graph-contributors` | `NFA-GP-001..003`; `NFA-C03-001` | E-12-NFAC036-36; executable Phase 12 Network Flow selector for NF-AC-036 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-037` | Edge selection opens deterministically grouped contributors. | browser + integration | apps/web/e2e/phase12.network-flow.spec.ts::E-12-NFAC037-37 Network Analysis edge selection opens ordered contributor drawer | `NF-FIX-025-graph-contributors` | `NFA-GP-001..003`; `NFA-C03-001` | E-12-NFAC037-37; executable Phase 12 Network Flow selector for NF-AC-037 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-038` | Graph over-limit errors are deterministic with no partial graph. | integration + fault | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC038_38_GraphLimitFailureEvidence | `NF-FIX-008-large-limits`; `NF-FIX-018-resource-limits`; `NF-FIX-028-graph-aggregate-bounds` | `NFA-GP-003`; `NFA-TH-002` | I-12-NFAC038-38; executable Phase 12 Network Flow selector for NF-AC-038 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-039` | Example refs and truncation fields are exact. | integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC039_39_ExampleRefsEvidence | `NF-FIX-006-cross-table-graph`; `NF-FIX-025-graph-contributors` | `NFA-GP-002`; `NFA-TH-001` | I-12-NFAC039-39; executable Phase 12 Network Flow selector for NF-AC-039 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-040` | Adapter input fields and safe metadata are exact. | contract + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC040_40_GraphProjectionAdapterEvidence | `NF-FIX-015-graph-adapter-input` | `NFA-GP-001..003` | I-12-NFAC040-40; executable Phase 12 Network Flow selector for NF-AC-040 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-041` | Existing-indicator link binds source without rewriting row. | transactional integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC041_41_ExistingIndicatorBindingEvidence | `NF-FIX-007-indicator-linking` | `NFA-C02-001..002`; `NFA-C01-005` | I-12-NFAC041-41; executable Phase 12 Network Flow selector for NF-AC-041 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-042` | Create-indicator uses Core owner and enforces role. | authorization + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC042_42_CreateIndicatorRoleEvidence | `NF-FIX-007-indicator-linking`; `NF-FIX-017-indicator-link-mismatch` | `NFA-C02-001..002`; `NFA-C04-001` | I-12-NFAC042-42; executable Phase 12 Network Flow selector for NF-AC-042 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-043` | Duplicate link returns existing binding. | transactional integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC043_43_DuplicateIndicatorLinkEvidence | `NF-FIX-007-indicator-linking` | `NFA-C02-002`; `NFA-C01-005` | I-12-NFAC043-43; executable Phase 12 Network Flow selector for NF-AC-043 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-044` | Rejected/deleted/stale/unmapped selectors fail. | integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC044_44_ClosedSelectorFailureEvidence | `NF-FIX-009-soft-delete-stale-graph`; `NF-FIX-017-indicator-link-mismatch`; `NF-FIX-025-graph-contributors` | `NFA-C02-002`; `NFA-GP-003`; `NFA-C04-001` | I-12-NFAC044-44; executable Phase 12 Network Flow selector for NF-AC-044 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-045` | Deployment admin without membership has no incident access. | authorization integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC045_45_DeploymentAdminNoBypassEvidence | `NF-FIX-025-graph-contributors` | `NFA-C04-001`; `NFA-TH-005` | I-12-NFAC045-45; executable Phase 12 Network Flow selector for NF-AC-045 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-046` | Network Flow actions perform no third-party egress. | security integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC046_46_NoThirdPartyEgressEvidence | n/a: egress-observation scenario | `NFA-C04-001`; `NFA-TH-001` | I-12-NFAC046-46; executable Phase 12 Network Flow selector for NF-AC-046 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-047` | Logs, telemetry, audit, and diagnostics obey raw-value policy. | security + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC047_47_RawValueRedactionEvidence | `NF-FIX-016-redaction` | `NFA-C04-003..004`; `NFA-TH-004` | I-12-NFAC047-47; executable Phase 12 Network Flow selector for NF-AC-047 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-048` | Limits are discoverable, lowerable within bounds, and phase-enforced. | contract + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC048_48_LimitDiscoveryAndEnforcementEvidence | `NF-FIX-008-large-limits`; `NF-FIX-018-resource-limits` | `NFA-GEN-001`; `NFA-TH-001` | I-12-NFAC048-48; executable Phase 12 Network Flow selector for NF-AC-048 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-049` | Route errors include required details. | contract conformance | internal/modules/networkflow/phase12_network_flow_unit_test.go::TestPhase12NetworkFlow_U_12_NFAC049_49_ErrorDetailShape | `NF-FIX-010-json-admission`; `NF-FIX-018-resource-limits` | `NFA-GEN-001` | U-12-NFAC049-49; executable Phase 12 Network Flow selector for NF-AC-049 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-050` | Large timing remains engineering-only absent Core 05. | structural + harness accounting | internal/modules/networkflow/phase12_network_flow_unit_test.go::TestPhase12NetworkFlow_U_12_NFAC050_50_LargeTimingEvidenceClassification | `NF-FIX-008-large-limits` | `NFA-C05-001`; `NFA-TH-007` | U-12-NFAC050-50; executable Phase 12 Network Flow selector for NF-AC-050 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-051` | Unmapped raw remains inert provenance. | integration + security | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC051_51_UnmappedRawInertEvidence | `NF-FIX-011-alias-collision`; `NF-FIX-016-redaction` | `NFA-GEN-001`; `NFA-GP-002` | I-12-NFAC051-51; executable Phase 12 Network Flow selector for NF-AC-051 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-052` | Every fixture has immutable bytes and expected artifacts before adoption. | structural + fixture drift | internal/modules/networkflow/phase12_network_flow_unit_test.go::TestPhase12NetworkFlow_U_12_NFAC052_52_FixtureCorpusIsFrozen | all `NF-FIX-001` through `NF-FIX-028` full IDs in §8 | `NFA-FIX-001..028`; `NFA-TH-001`; `NFA-VAL-003` | U-12-NFAC052-52; executable Phase 12 Network Flow selector for NF-AC-052 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-053` | Every MAY has omission behavior. | structural lint | internal/modules/networkflow/phase12_network_flow_unit_test.go::TestPhase12NetworkFlow_U_12_NFAC053_53_MayStatementsHaveOmissionBehavior | n/a: owner document | `NFA-VAL-002`; `NFA-ADOPT-001` | U-12-NFAC053-53; executable Phase 12 Network Flow selector for NF-AC-053 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-054` | Normative behavior stays in adopted owner documents. | authority review + structural lint | internal/modules/networkflow/phase12_network_flow_unit_test.go::TestPhase12NetworkFlow_U_12_NFAC054_54_NormativeAuthorityStaysInOwners | n/a: document set | `NFA-AUTH-001`; `NFA-VAL-002` | U-12-NFAC054-54; executable Phase 12 Network Flow selector for NF-AC-054 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-055` | Internal references resolve. | structural lint | internal/modules/networkflow/phase12_network_flow_unit_test.go::TestPhase12NetworkFlow_U_12_NFAC055_55_DocumentReferencesAndFixtureRowsResolve | n/a: owner document | `NFA-VAL-002` | U-12-NFAC055-55; executable Phase 12 Network Flow selector for NF-AC-055 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-056` | Edge IDs and null-port aggregation match exact vectors. | unit + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC056_56_EdgeIDNullPortEvidence | `NF-FIX-006-cross-table-graph`; `NF-FIX-028-graph-aggregate-bounds` | `NFA-GP-001..003`; `NFA-TH-001` | I-12-NFAC056-56; executable Phase 12 Network Flow selector for NF-AC-056 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-057` | Time-bucket request members remain unknown. | contract + route integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC057_57_TimeBucketUnknownMembersEvidence | `NF-FIX-010-json-admission` | `NFA-GEN-001` | I-12-NFAC057-57; executable Phase 12 Network Flow selector for NF-AC-057 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-058` | Only binding observation mode; created observation refs empty. | contract + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC058_58_BindingOnlyObservationEvidence | `NF-FIX-007-indicator-linking` | `NFA-C02-002`; `NFA-GEN-001` | I-12-NFAC058-58; executable Phase 12 Network Flow selector for NF-AC-058 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-059` | Graph digests ignore deployment/caller limit changes. | integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC059_59_GraphDigestSemanticEvidence | `NF-FIX-006-cross-table-graph`; `NF-FIX-018-resource-limits` | `NFA-GP-001..003`; `NFA-TH-001` | I-12-NFAC059-59; executable Phase 12 Network Flow selector for NF-AC-059 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-060` | Timestamp precision, epochs, and uptime derivation are exact. | unit + fixture conformance | internal/modules/networkflow/phase12_network_flow_unit_test.go::TestPhase12NetworkFlow_U_12_NFAC060_60_TimestampPrecisionAndUptimeVectors | `NF-FIX-012-sys-uptime-timestamps`; `NF-FIX-022-timestamp-rulesets` | `NFA-TZ-001`; `NFA-TH-003` | U-12-NFAC060-60; executable Phase 12 Network Flow selector for NF-AC-060 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-061` | Duplicate rename error and cursor survival are exact. | transactional integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC061_61_DuplicateRenameAndCursorEvidence | `NF-FIX-013-filename-display`; `NF-FIX-014-cursor-pagination` | `NFA-C04-002`; `NFA-C01-004`; `NFA-TH-002` | I-12-NFAC061-61; executable Phase 12 Network Flow selector for NF-AC-061 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-062` | Only active/soft-deleted tables and exact limit accounting. | structural + integration | internal/modules/networkflow/store_test.go::TestPhase12NetworkFlow_I_12_NFAC062_62_StoreLifecycleAndLimitAccounting | `NF-FIX-027-retention-soft-delete` | `NFA-C04-005`; `NFA-GEN-001` | I-12-NFAC062-62; executable Phase 12 Network Flow selector for NF-AC-062 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-063` | All-rejected error has ordered safe diagnostics and no table. | transactional integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC063_63_AllRejectedErrorDetailsEvidence | `NF-FIX-004-rejected-rows` | `NFA-C01-004`; `NFA-TH-002`; `NFA-TH-004` | I-12-NFAC063-63; executable Phase 12 Network Flow selector for NF-AC-063 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-064` | Query defaults and graph override bounds are exact. | contract + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC064_64_QueryAndGraphLimitEvidence | `NF-FIX-018-resource-limits`; `NF-FIX-024-query-normalization-cursors` | `NFA-GEN-001`; `NFA-GP-001` | I-12-NFAC064-64; executable Phase 12 Network Flow selector for NF-AC-064 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-065` | Preview slice counts and apply outcomes are independent. | integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC065_65_PreviewApplyBoundaryEvidence | `NF-FIX-021-preview-boundaries` | `NFA-C01-002..005`; `NFA-TH-001` | I-12-NFAC065-65; executable Phase 12 Network Flow selector for NF-AC-065 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-066` | Binding source refs/truncation and duplicate selector handling are exact. | integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC066_66_BindingSourceRefsEvidence | `NF-FIX-007-indicator-linking` | `NFA-C02-002`; `NFA-GEN-001` | I-12-NFAC066-66; executable Phase 12 Network Flow selector for NF-AC-066 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-067` | Dedupe uses resolved binding identity tuple. | transactional integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC067_67_BindingDedupeIdentityEvidence | `NF-FIX-007-indicator-linking` | `NFA-C02-002`; `NFA-C01-005` | I-12-NFAC067-67; executable Phase 12 Network Flow selector for NF-AC-067 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-068` | Exact-value mismatch fails before mutation with safe details. | transactional integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC068_68_ConfirmExactValueMismatchEvidence | `NF-FIX-017-indicator-link-mismatch` | `NFA-C02-001..002`; `NFA-TH-002` | I-12-NFAC068-68; executable Phase 12 Network Flow selector for NF-AC-068 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-069` | Adapter uses endpoint/edge IDs and no invalid retention token. | contract + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC069_69_GraphProjectionIdentityEvidence | `NF-FIX-015-graph-adapter-input` | `NFA-GP-001..003` | I-12-NFAC069-69; executable Phase 12 Network Flow selector for NF-AC-069 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-070` | Interface fields are bounded text/null and code-point sorted. | unit + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC070_70_InterfaceTextEvidence | `NF-FIX-002-cisco-sna-interface-fields` | `NFA-GEN-001`; `NFA-TH-001` | I-12-NFAC070-70; executable Phase 12 Network Flow selector for NF-AC-070 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-071` | Aggregation and mapping combinability are closed to v1 values. | contract + route integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC071_71_ClosedAggregationEvidence | `NF-FIX-010-json-admission`; `NF-FIX-011-alias-collision` | `NFA-GEN-001` | I-12-NFAC071-71; executable Phase 12 Network Flow selector for NF-AC-071 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-072` | Rename/delete/link/apply idempotency follows exact comparison/replay points. | transactional integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC072_72_IdempotencyReplayEvidence | `NF-FIX-020-atomic-import-commit`; `NF-FIX-026-audit-and-replay` | `NFA-C01-004..005`; `NFA-C02-002`; `NFA-TH-002` | I-12-NFAC072-72; executable Phase 12 Network Flow selector for NF-AC-072 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-073` | Mapping accepts only three variants and rejects extras/sentinels. | contract + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC073_73_MappingVariantBoundaryEvidence | `NF-FIX-011-alias-collision` | `NFA-C01-003`; `NFA-GEN-001` | I-12-NFAC073-73; executable Phase 12 Network Flow selector for NF-AC-073 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-074` | Alias keys/warnings require explicit complete approval. | parser + browser + integration | apps/web/e2e/phase12.network-flow.spec.ts::E-12-NFAC074-74 Network Analysis alias collision requires explicit approval | `NF-FIX-011-alias-collision` | `NFA-C01-003`; `NFA-C03-001` | E-12-NFAC074-74; executable Phase 12 Network Flow selector for NF-AC-074 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-075` | Every success data object matches exact closed schema. | contract conformance | internal/modules/networkflow/phase12_network_flow_unit_test.go::TestPhase12NetworkFlow_U_12_NFAC075_75_SuccessResourceShape | `NF-FIX-001-cisco-sna-minimal`; `NF-FIX-023-import-facade-source-change`; `NF-FIX-025-graph-contributors` | `NFA-C01-004`; `NFA-GEN-001` | U-12-NFAC075-75; executable Phase 12 Network Flow selector for NF-AC-075 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-076` | All resource limits match defaults, phases, failures, and invalid config. | integration + configuration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC076_76_ResourceLimitTableEvidence | `NF-FIX-008-large-limits`; `NF-FIX-018-resource-limits` | `NFA-GEN-001`; `NFA-TH-001`; `NFA-TH-007` | I-12-NFAC076-76; executable Phase 12 Network Flow selector for NF-AC-076 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-077` | Safe/source samples follow exact raw/numeric/null rules. | unit + security | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC077_77_SafeSamplesEvidence | `NF-FIX-016-redaction` | `NFA-C04-003`; `NFA-TH-004` | I-12-NFAC077-77; executable Phase 12 Network Flow selector for NF-AC-077 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-078` | Route errors have exact details and deterministic precedence. | contract + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC078_78_ErrorOrderingEvidence | `NF-FIX-004-rejected-rows`; `NF-FIX-018-resource-limits` | `NFA-GEN-001`; `NFA-TH-004` | I-12-NFAC078-78; executable Phase 12 Network Flow selector for NF-AC-078 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-079` | Linking accepts only IP endpoints and validates Core identity. | integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC079_79_IndicatorCandidateBoundaryEvidence | `NF-FIX-007-indicator-linking`; `NF-FIX-017-indicator-link-mismatch` | `NFA-C02-001..002` | I-12-NFAC079-79; executable Phase 12 Network Flow selector for NF-AC-079 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-080` | Filename/display naming cases match exact rules. | unit + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC080_80_FilenameDisplayEvidence | `NF-FIX-013-filename-display` | `NFA-C01-003`; `NFA-TH-001` | I-12-NFAC080-80; executable Phase 12 Network Flow selector for NF-AC-080 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-081` | Final commit faults expose no partial state/audit. | fault + transactional integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC081_81_AtomicImportCommitEvidence | `NF-FIX-020-atomic-import-commit` | `NFA-C01-004`; `NFA-TH-002` | I-12-NFAC081-81; executable Phase 12 Network Flow selector for NF-AC-081 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-082` | Explicit duplicate names fail; omitted names suffix under commit lock. | concurrency + transactional integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC082_82_DisplayNameCollisionEvidence | `NF-FIX-013-filename-display`; `NF-FIX-020-atomic-import-commit` | `NFA-C01-004`; `NFA-TH-002`; `NFA-TH-004` | I-12-NFAC082-82; executable Phase 12 Network Flow selector for NF-AC-082 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-083` | Preview stops at 50 complete data records with exact counting. | parser conformance | internal/modules/networkflow/phase12_network_flow_unit_test.go::TestPhase12NetworkFlow_U_12_NFAC083_83_CSVPreviewBoundary | `NF-FIX-021-preview-boundaries` | `NFA-C01-003`; `NFA-TH-001` | U-12-NFAC083-83; executable Phase 12 Network Flow selector for NF-AC-083 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-084` | Row limit counts logical records and stops at `limit+1`. | parser + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC084_84_RowLimitCountingEvidence | `NF-FIX-021-preview-boundaries` | `NFA-TH-001` | I-12-NFAC084-84; executable Phase 12 Network Flow selector for NF-AC-084 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-085` | Cisco profile allows only required plus two interface targets. | parser + contract | internal/modules/networkflow/phase12_network_flow_unit_test.go::TestPhase12NetworkFlow_U_12_NFAC085_85_CiscoSNATargetBoundary | `NF-FIX-002-cisco-sna-interface-fields`; `NF-FIX-011-alias-collision` | `NFA-C01-003`; `NFA-GEN-001` | U-12-NFAC085-85; executable Phase 12 Network Flow selector for NF-AC-085 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-086` | ASCII-space trim and empty policy are exact. | unit conformance | internal/modules/networkflow/phase12_network_flow_unit_test.go::TestPhase12NetworkFlow_U_12_NFAC086_86_TrimASCIISpaceOnly | `NF-FIX-019-canonical-json-unicode` | `NFA-TH-001` | U-12-NFAC086-86; executable Phase 12 Network Flow selector for NF-AC-086 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-087` | Timestamp variants, grammar, ruleset, uptime, and ordinals are closed. | contract + unit conformance | internal/modules/networkflow/phase12_network_flow_unit_test.go::TestPhase12NetworkFlow_U_12_NFAC087_87_TimestampObjectBoundary | `NF-FIX-012-sys-uptime-timestamps`; `NF-FIX-022-timestamp-rulesets` | `NFA-TZ-001`; `NFA-GEN-001`; `NFA-TH-003` | U-12-NFAC087-87; executable Phase 12 Network Flow selector for NF-AC-087 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-088` | Preview is side-effect-free; apply uses opaque source and fails closed on change. | integration + fault | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC088_88_ImportFacadeSourceChangeEvidence | `NF-FIX-023-import-facade-source-change` | `NFA-C01-003..005`; `NFA-TH-002` | I-12-NFAC088-88; executable Phase 12 Network Flow selector for NF-AC-088 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-089` | Descriptors/dispositions/mappings and provenance are exact. | contract + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC089_89_SourceColumnDispositionEvidence | `NF-FIX-003-duplicate-headers`; `NF-FIX-011-alias-collision`; `NF-FIX-023-import-facade-source-change` | `NFA-C01-002..005`; `NFA-GEN-001` | I-12-NFAC089-89; executable Phase 12 Network Flow selector for NF-AC-089 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-090` | Public rows include nullable fields, unmapped values, and observation ref. | contract + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC090_90_PublicRowNullableFieldsEvidence | `NF-FIX-001-cisco-sna-minimal`; `NF-FIX-011-alias-collision` | `NFA-GEN-001` | I-12-NFAC090-90; executable Phase 12 Network Flow selector for NF-AC-090 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-091` | Parallel diagnostic discovery is byte-identical and ordered. | concurrency + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC091_91_DiagnosticDeterminismEvidence | `NF-FIX-004-rejected-rows` | `NFA-TH-004` | I-12-NFAC091-91; executable Phase 12 Network Flow selector for NF-AC-091 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-092` | Table scopes reject invalid variants without resource disclosure. | authorization + contract integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC092_92_TableScopeVariantEvidence | `NF-FIX-024-query-normalization-cursors` | `NFA-C04-001`; `NFA-GEN-001`; `NFA-TH-005` | I-12-NFAC092-92; executable Phase 12 Network Flow selector for NF-AC-092 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-093` | Filter normalization canonicalizes and rejects canonical duplicates. | unit + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC093_93_FilterNormalizationEvidence | `NF-FIX-019-canonical-json-unicode`; `NF-FIX-024-query-normalization-cursors` | `NFA-C02-001`; `NFA-GEN-001` | I-12-NFAC093-93; executable Phase 12 Network Flow selector for NF-AC-093 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-094` | Initial/continuation variants and token byte/expiry bounds are exact. | security + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC094_94_CursorTokenBoundaryEvidence | `NF-FIX-014-cursor-pagination`; `NF-FIX-024-query-normalization-cursors` | `NFA-C04-002`; `NFA-TH-003`; `NFA-GEN-001` | I-12-NFAC094-94; executable Phase 12 Network Flow selector for NF-AC-094 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-095` | Three cursor families use independent full keysets without gaps/duplicates. | integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC095_95_KeysetCursorEvidence | `NF-FIX-014-cursor-pagination`; `NF-FIX-024-query-normalization-cursors`; `NF-FIX-025-graph-contributors` | `NFA-C04-002`; `NFA-GP-003`; `NFA-TH-001` | I-12-NFAC095-95; executable Phase 12 Network Flow selector for NF-AC-095 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-096` | Arbitrary-precision aggregation and fixed failure order have no partial output. | unit + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC096_96_AggregateCounterEvidence | `NF-FIX-028-graph-aggregate-bounds` | `NFA-GP-003`; `NFA-TH-001` | I-12-NFAC096-96; executable Phase 12 Network Flow selector for NF-AC-096 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-097` | Projection metadata and outcome mapping are exact and safe. | contract + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC097_97_GraphProjectionMetadataEvidence | `NF-FIX-015-graph-adapter-input` | `NFA-GP-001..003` | I-12-NFAC097-97; executable Phase 12 Network Flow selector for NF-AC-097 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-098` | Graph success schema is exact and closed. | contract + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC098_98_GraphSuccessSchemaEvidence | `NF-FIX-025-graph-contributors` | `NFA-GP-001..003`; `NFA-GEN-001` | I-12-NFAC098-98; executable Phase 12 Network Flow selector for NF-AC-098 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-099` | Contributors recompute composition/auth and paginate without fallback. | authorization + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC099_99_ContributorRecomputeEvidence | `NF-FIX-025-graph-contributors` | `NFA-GP-001..003`; `NFA-C03-002`; `NFA-TH-005` | I-12-NFAC099-99; executable Phase 12 Network Flow selector for NF-AC-099 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-100` | Indicator variants and exact confirmation are closed; create input is constrained. | contract + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC100_00_IndicatorClosedVariantsEvidence | `NF-FIX-017-indicator-link-mismatch` | `NFA-C02-001..002`; `NFA-GEN-001` | I-12-NFAC100-00; executable Phase 12 Network Flow selector for NF-AC-100 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-101` | Indicator create/dedupe plus binding is atomic with exact statuses. | transactional integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC101_01_IndicatorAtomicCommitEvidence | `NF-FIX-007-indicator-linking`; `NF-FIX-026-audit-and-replay` | `NFA-C02-002`; `NFA-C01-004`; `NFA-TH-002` | I-12-NFAC101-01; executable Phase 12 Network Flow selector for NF-AC-101 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-102` | Safe digests carry key IDs and compare only within a key ID. | security + unit/integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC102_02_SafeDigestKeyEvidence | `NF-FIX-016-redaction`; `NF-FIX-026-audit-and-replay` | `NFA-C04-003`; `NFA-TH-004` | I-12-NFAC102-02; executable Phase 12 Network Flow selector for NF-AC-102 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-103` | Audit occurrences/replay/truncation counts are exact. | transactional + audit integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC103_03_AuditOccurrenceEvidence | `NF-FIX-026-audit-and-replay` | `NFA-C04-004`; `NFA-C01-005`; `NFA-TH-006` | I-12-NFAC103-03; executable Phase 12 Network Flow selector for NF-AC-103 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-104` | Soft delete and incident closure retain but make Network Flow data non-queryable, and v1 exposes no incident-purge claim. | retention + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC104_04_RetentionSoftDeleteEvidence | `NF-FIX-009-soft-delete-stale-graph`; `NF-FIX-027-retention-soft-delete` | `NFA-C00-002`; `NFA-C04-005`; `NFA-TH-005`; `NFA-TH-006` | I-12-NFAC104-04; executable Phase 12 Network Flow selector for NF-AC-104 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-105` | Every route status/schema/error/reason/detail/retry is exact. | generated-contract + integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC105_05_RouteStatusCatalogEvidence | all applicable route fixtures in §8 | `NFA-GEN-001`; `NFA-C01-004`; `NFA-C04-001` | I-12-NFAC105-05; executable Phase 12 Network Flow selector for NF-AC-105 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-106` | All locators, blockers, and immutable fixtures close before adoption. | structural + adoption review | internal/modules/networkflow/phase12_network_flow_unit_test.go::TestPhase12NetworkFlow_U_12_NFAC106_06_AdoptionPrerequisitesAreConcrete | all `NF-FIX-001` through `NF-FIX-028` full IDs in §8 | `NFA-LOC-001`; `NFA-FIX-001..028`; `NFA-ADOPT-001` | U-12-NFAC106-06; executable Phase 12 Network Flow selector for NF-AC-106 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |
| `NF-AC-107` | Pre-commit cancellation leaves nothing; post-commit recovery publishes once. | worker fault + transactional integration | internal/modules/networkflow/phase12_network_flow_integration_test.go::TestPhase12NetworkFlow_I_12_NFAC107_07_CancellationRecoveryEvidence | `NF-FIX-020-atomic-import-commit`; `NF-FIX-023-import-facade-source-change` | `NFA-C01-004..005`; `NFA-TH-004`; `NFA-TH-007` | I-12-NFAC107-07; executable Phase 12 Network Flow selector for NF-AC-107 | `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390`; retained service-backed `.cartulary/test-results/20260710T151210Z-p71906` where applicable | `DONE` | Closed by executable selector promotion, retained validation, final adoption artifacts `f4409d98`/`e83b3719`, and §15.42 evidence. |

The adoption task consumed each row individually; retained broad targets are
valid only because every row has an executable selector, fixture/expected artifact
where applicable, and closed dependency task.

## 10. Generated artifacts and anti-drift inventory

`contracts/index.json` declares generated contract families. Network Flow
authored inputs are active and emitted through the owner-derived contract
families; generated Go/TypeScript roots remain downstream artifacts and must
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `tools/contractgen` OpenAPI family | `contracts/openapi/cartulary.openapi.yaml`; `contracts/network-flow/routes.v1.json`; `contracts/network-flow/schemas.v1.json` | `internal/gen/contracts/contracts_gen.go`; `packages/protocol-ts/src/generated/contracts.ts`; generated TS index | HTTP server, Go callers, web clients | Network Flow routes and schemas are emitted and drift-clean | `make generate` | `make generate-drift` | `NFA-GEN-001..004`; `NFA-ADOPT-001` | `DONE` |
| `tools/contractgen` WebSocket family | `contracts/ws/index.schema.json`; Network Flow invalidation event owner inputs | Same generated Go/TS contract roots | collaboration transport and web client | `extension_resource_changed` is emitted and drift-clean | `make generate` | `make generate-drift` | `NFA-C03-002`; `NFA-GEN-001..004`; `NFA-IMPL-017` | `DONE` |
| `tools/contractgen` view-schema family | `contracts/view-schemas/index.json`; observed `contracts/view-schemas/*.json` | Same generated Go/TS contract roots | workbook/view-schema consumers | No Network Flow saved-view/view-schema modeling is introduced | `make generate` | `make generate-drift` | `NFA-GEN-001`; `NFA-AUTH-001`; `NFA-ADOPT-001` | `DONE for no-extension-view-schema drift` |
| `tools/contractgen` error family | `contracts/errors/index.json`; `contracts/network-flow/errors.v1.json` | Same generated Go/TS contract roots | server error registry and clients | Network Flow route-local errors are emitted and drift-clean | `make generate` | `make generate-drift`; `make json-shape-check` | `NFA-GEN-001..004`; `NFA-ADOPT-001` | `DONE` |
| `tools/contractgen` extension family | `contracts/extensions/index.json`; `contracts/network-flow/index.json` | Same generated Go/TS contract roots | extension discovery and clients | Network Flow profile discovery is active and drift-clean | `make generate` | `make generate-drift`; `make json-shape-check` | `NFA-C00-001`; `NFA-C01-001`; `NFA-GEN-001..004`; `NFA-ADOPT-001` | `DONE` |
...
| Network Flow fixture manifests and transcripts | `fixtures/network-flow/*/manifest.json`; `fixtures/network-flow/*/source/*`; `fixtures/network-flow/*/expected/*`; `fixtures/network-flow/*/transcripts/*` | Frozen authored corpus in artifact `49ff3e6a`; Phase 12 selector map in `tools/phase12_test_map.json` | Network Flow conformance target | All 28 fixture families are frozen and all 107 AC selector/result rows are promoted | n/a; authored fixture corpus | `make json-shape-check` `.cartulary/test-results/20260710T143237Z-p96990`; `make phase-slice PHASE=phase12` `.cartulary/test-results/20260710T154140Z-p73390` | `NFA-TH-001`; `NFA-TRANSCRIPT-001`; `NFA-VAL-002`; `NFA-ADOPT-001` | `DONE` |
...
| `CP-38` | `NFA-VAL-002`; `NFA-VAL-003` | Full validation/security/fault/drift bundle | `CP-20`; `CP-29`; `CP-36`; `CP-37` | `make phase-slice PHASE=phase12`; `make service-backed-slice PHASE=phase12`; `make generate-drift`; `make go-gosec-targeted`; `make go-gosec-audit`; `make check`; `make agent-finalize RESULTS_DIR=.cartulary/test-results/20260710T152852Z-p53813` | Retained conformance roots `.cartulary/test-results/20260710T151138Z-p53460` and `.cartulary/test-results/20260710T151210Z-p71906`; full check root `.cartulary/test-results/20260710T152852Z-p53813`; finalizer root `.cartulary/test-results/20260710T153119Z-p53856` | Stop on any product, harness, infra, fixture, or drift failure | Complete adoption evidence bundle |
| `CP-39` | `NFA-ADOPT-001`; `NFA-C00-002` | Coordinated Core 00 registry and NLSpec version/status | Every gate, blocker, locator, fixture, generated check, AC, and validation closed | Artifacts `f4409d98` and `e83b3719`; §15.42 validation | Core 00/NLSpec status transition plus Phase 12 profile-claim accounting | Stop or revert both owner/status and claim-accounting sides together | Network Flow adopted/current transition is complete |
...
validated. Phase 12 now owns the Network Flow executable selector map and
claim-accounting evidence.
...
| `make generate-drift` | Prove generated outputs reproduce | `CP-29`, `CP-38`, `CP-39` | retained drift summary | Pass at `.cartulary/test-results/20260710T154024Z-p62262` after adoption artifact | generated-artifact drift | named run root |
...
| `make phase-slice PHASE=phase12` | Execute Network Flow fixtures and 107 AC rows | `CP-30..39` | row-accounted retained run | Pass at `.cartulary/test-results/20260710T154140Z-p73390`; retained prior roots in §15.40 | product failure, fixture mismatch, harness defect, or infrastructure failure | named run root |
...
| --- | --- | --- | --- | --- | --- | --- |
| `NFA-RISK-001` | Draft behavior contradicts closed adopted Core shapes. | Core 00–04 | `NFA-C00-*` through `NFA-C04-*` | Private extension choices could break Core compatibility. | Each contradiction received an adopted owner decision and the NLSpec is now adopted/current. | `MITIGATED` |
| `NFA-RISK-002` | Core 00 omits Network Flow and keeps whole-incident purge future-only. | Core 00 | `NFA-C00-001..002`; `NFA-ADOPT-001` | Premature discovery/adoption or an unsupported deletion promise. | Profile boundary is adopted while preserving the future-only generic incident-removal decision. | `MITIGATED` |
| `NFA-RISK-003` | Flow resources could be coerced into Core record/view semantics. | Core 01; Network Flow | `NFA-C01-002..005` | Identity, storage, envelope, and compatibility corruption. | Analytical extension resource/result boundary is adopted and validated. | `MITIGATED` |
| `NFA-RISK-004` | Cross-owner import/indicator/binding/audit commit lacks one unit of work. | Core 01/Core 02/Core 04 | `NFA-C01-004`; `NFA-C02-002`; `NFA-C04-004`; `NFA-TH-002` | Ghost or partial state and mismatched audit. | Owner participants, implementation, and retained fault/finalization evidence exist through §15.41. | `MITIGATED` |
...
| `NFA-RISK-006` | Secrets or raw flow values could enter fixtures/logs/manifests/transcripts. | Core 04/Harness | `NFA-C04-003`; `NFA-FIX-016`; `NFA-FIX-026` | Credential or incident-data disclosure. | Core 04 owner boundary exists in `cd645750`; deterministic harness material, fixture bytes, retained Phase 12 rows, and security scans pass through §15.41. | `MITIGATED` |
| `NFA-RISK-007` | Soft-delete retention, idempotency, terminal publication, raw-source expiry, and audit retention may disagree. | Core 01/Core 04 | `NFA-C01-003..005`; `NFA-C04-004..005` | Replay resurrection, duplicate audit, source over-retention, or inaccessible retained-table saturation. | Audit and retention owners, implementation, retained-count instrumentation, source cleanup, exact fixture evidence, and final validation are recorded through §15.41. | `MITIGATED` |
...
| `NFA-RISK-011` | Host timezone, locale, collation, Unicode, or line endings may affect bytes. | Network Flow/Harness | `NFA-TZ-001`; `NFA-FIX-005`; `NFA-FIX-019`; `NFA-FIX-022` | Cross-platform digest and behavior drift. | Environmental inputs and immutable `tzdb-2026c` are frozen, and retained Phase 12/finalization evidence passes through §15.41. | `MITIGATED` |
| `NFA-RISK-012` | IDs, time, CSPRNG, scheduling, or ordering may destabilize transcripts. | Testing Harness | `NFA-TH-002..006`; all fixtures | Non-repeatable acceptance evidence. | Fake clock/randomness/fault/scheduler/auth/audit controls are resettable and retained validation passes through §15.41. | `MITIGATED` |
| `NFA-RISK-013` | Generated outputs may be hand-edited or a family may bypass generator ownership. | Contract owners/Harness | `NFA-GEN-001..004` | Go/TS/client drift and hidden compatibility changes. | Contract-family registry `d8dbaf3f` makes family ownership explicit; authored/generated/drift/finalizer checks pass through §15.41. | `MITIGATED` |
...
| `NFA-RISK-015` | Status/version may flip before all dependencies and bytes close. | All owners | `NFA-ADOPT-001`; `NF-AC-106` | False adoption claim and unresolved behavior. | Coordinated Core 00/NLSpec review, Phase 12 claim accounting, and retained validation prove zero open prerequisites. | `MITIGATED` |
| `NFA-RISK-016` | Large-limit timing may be presented as claim evidence. | Core 05/Network Flow | `NFA-FIX-008`; `NF-AC-050` | Unsupported performance publication. | Kept engineering-only; Core 05 publication scope is not activated by v1 adoption. | `DEFERRED` |
| `NFA-RISK-017` | Assumption: current clean `main` snapshot is the intended planning base. | Tracker maintainer | `NFA-INV-001` | Rebase could invalidate checksums and inventories. | Every resumed workstream refreshed status and committed artifact/tracker checkpoints before proceeding. | `MITIGATED` |
...
- `2026-07-10T11:32:38-04:00` — completed `NFA-VAL-003` validation support
  and finalization. Generated drift and security scans passed; the first
  `agent-finalize` preflight correctly rejected the Phase 12 slice root because
  finalization requires a successful full warm `make check` root. Full check
  then exposed stale duration/scheduler accounting and one supplemental Phase 0
  test name; artifact `31b2df2a` refreshes the Make-owned baselines/schedules
  and renames the supplemental config test. `make check` passed at
  `.cartulary/test-results/20260710T152852Z-p53813`; `make agent-finalize
...
- `2026-07-10T11:35:21-04:00` — began `NFA-ADOPT-001` and `NFA-C00-002` from
  ...
  checkpoint.
- `2026-07-10T15:46:04-04:00` — completed the owner/status side of
  `NFA-ADOPT-001` in artifact `f4409d98` and the Phase 12 aggregate
  claim-accounting side in artifact `e83b3719`. Backend, JSON-shape, generated
  drift, Phase 12, phase map, phase ledger drift, phase schedule drift, and
  whitespace checks passed. The next safe slice is final handoff closeout after
  this tracker checkpoint is committed.

### 14.9 Current session handoff

| Field | Value |
| --- | --- |
| Date/time | `2026-07-10T15:46:04-04:00` |
| Branch/commit | `main`; latest adoption artifact `e83b3719` after owner/status artifact `f4409d98` |
| Dirty-tree state | Tracker-only adoption checkpoint edit in progress |
| Current workflow/task | Commit `NFA-ADOPT-001`/`NFA-C00-002` tracker checkpoint, then execute final `NFA-HANDOFF-001` closeout |
| Completed tasks | All owner, generated, implementation, fixture, workspace, Phase 12 selector, retained conformance, security/drift, full check, finalization, Core 00/NLSpec adoption, and Phase 12 aggregate claim-accounting workstreams through `NFA-ADOPT-001` |
| Tracker file changed | `docs/handoffs/network-flow-activity-adoption-handoff-tracker.md` |
| Other changed files | none expected for this tracker checkpoint; adoption artifacts already committed `f4409d98` and `e83b3719` |
| Commands run | `make backend-unit`; `make json-shape-check`; `make generate-drift`; `make phase-slice PHASE=phase12`; `make phase-ledgers`; `make phase-schedules`; `make phase-map-check`; `make phase-ledger-drift`; `make phase-schedule-drift`; `git diff --check` |
| Passing validation | Adoption validation is recorded in §15.42; retained conformance and final validation remain recorded in §§15.40-15.41 |
| Failing validation | none for the adoption checkpoint |
| Decisions recorded | Network Flow Activity is adopted/current v1; `network_flow_activity` is claimable by extension configuration; v1 still excludes private incident purge, Core 05 publication claims, raw IPFIX, live collection, export, saved graph views, binding read/list routes, time buckets, third-party enrichment, and automatic maliciousness inference |
| Open questions | none for this checkpoint |
| Blockers | Final tracker handoff closeout and end-run validation remain |
| Next recommended task/workflow | Commit this tracker checkpoint, then run final handoff validation and mark `NFA-HANDOFF-001` done |
| Safe restart command | `git status --short && rg -n "NFA-HANDOFF-001\|15\.42\|adopted/current\|network_flow_activity" docs/handoffs/network-flow-activity-adoption-handoff-tracker.md docs/network-flow-activity-nlspec.md docs/spec/00_document_set_status_and_precedence.md tools/phase12_test_map.json` |
...
### 15.41 `NFA-VAL-003` security, fault, drift, and finalization results

| Check | Result |
| --- | --- |
| Start checkpoint | Pass; `NFA-VAL-003` started from clean checkpoint `605440fe` with adoption/status changes explicitly out of scope |
| Generated drift | Pass; `make generate-drift` passed at `.cartulary/test-results/20260710T151448Z-p92224` |
| Targeted security scan | Pass; `make go-gosec-targeted` passed at `.cartulary/test-results/20260710T151454Z-p93189` |
| Audit security scan | Pass; `make go-gosec-audit` passed at `.cartulary/test-results/20260710T151503Z-p8873` |
| Finalizer preflight correction | Pass; initial `make agent-finalize RESULTS_DIR=.cartulary/test-results/20260710T151138Z-p53460` failed at `.cartulary/test-results/20260710T151512Z-p17667` because finalization requires a successful full warm `make check` root containing `check/tool-run-summary.json`, not a retained Phase 12 slice root |
| Full-check repair round 1 | Pass after fix; `make check` at `.cartulary/test-results/20260710T151532Z-p18019` exposed Biome formatting, frontend selector-policy, and Go staticcheck failures introduced by the Phase 12 selector artifact; artifact `abe227a5` fixed those issues and `make phase-slice PHASE=phase12` passed again at `.cartulary/test-results/20260710T151929Z-p70827` |
| Full-check repair round 2 | Pass after fix; `make check` at `.cartulary/test-results/20260710T152034Z-p92977` exposed missing Network Flow Go duration baselines and an unregistered Phase 0-shaped supplemental config test name; artifact `31b2df2a` renamed the supplemental test, refreshed Make-owned duration baselines from `.cartulary/test-results/20260710T151929Z-p70827`, regenerated schedules, and preserved generated-root ownership |
| Accounting gates | Pass; `make go-test-duration-baseline-coverage` passed at `.cartulary/test-results/20260710T152747Z-p16942`; `make phase-test-name-check` passed at `.cartulary/test-results/20260710T152328Z-p30632`; `make json-shape-check` passed at `.cartulary/test-results/20260710T152847Z-p53282`; `make phase-schedule-drift` passed at `.cartulary/test-results/20260710T152847Z-p53302` |
| Retained full warm check | Pass; `make check` passed at `.cartulary/test-results/20260710T152852Z-p53813` with 335/335 work units, 1058 tests, zero failures, and zero missing rows |
| Finalization | Pass; `make agent-finalize RESULTS_DIR=.cartulary/test-results/20260710T152852Z-p53813` passed at `.cartulary/test-results/20260710T153119Z-p53856`; scheduler drift validation, phase ledger/schedule refresh and drift, JSON shape validation, generated-artifact policy, duration refresh/checks, final report validation, and worktree diff checks passed |
| Residual unrelated evidence | Pass; a full `make test-service-backed` timing run at `.cartulary/test-results/20260710T152417Z-p38665` failed on an unrelated Phase 5 visual snapshot and was not used for retained adoption evidence; the retained `make check` and finalizer roots above supersede it for this slice |
| Tracker transition | Pass; `NFA-VAL-003`, `NF-GATE-012`, `NF-BLOCK-007`, `NF-BLOCK-014`, and `CP-38` are complete; `NFA-ADOPT-001` and `NFA-C00-002` are ready to start in the next committed workstream |
| Explicit deferrals | Pass; no Core 00 registry flip, Network Flow NLSpec status/version flip, private incident purge, Core 05 timing publication claim, binding read/list route, export, raw IPFIX, live collector, or automatic maliciousness inference was added |
| Next safe step | Pass; commit this tracker checkpoint, then begin the coordinated final adoption/status transition as a separate workstream checkpoint |

### 15.42 `NFA-ADOPT-001` and `NFA-C00-002` adoption/status results

| Check | Result |
| --- | --- |
| Start checkpoint | Pass; `NFA-ADOPT-001` and `NFA-C00-002` started from clean checkpoint `3ffd1e91` after retained validation bundle checkpoint `23dde6e4` |
| Core 00 and NLSpec adoption artifact | Pass; artifact `f4409d98` updates Core 00 extension-profile claimability, flips `docs/network-flow-activity-nlspec.md` to `status: adopted/current` with `document_version: 1.0.0`, aligns Core 04 discovery criteria, and accepts `network_flow_activity.claimed=true` in config validation |
| Phase 12 claim-accounting artifact | Pass; artifact `e83b3719` sets `tools/phase12_test_map.json` `network_flow_activity` profile claim to `claimed=true`, attaches all 107 direct evidence IDs, regenerates `docs/testing/phase12_coverage_ledger.md`, and refreshes the execution topology render index from owner inputs |
| Backend unit validation | Pass; `make backend-unit` passed at `.cartulary/test-results/20260710T154024Z-p62180` |
| JSON shape validation | Pass; `make json-shape-check` passed at `.cartulary/test-results/20260710T154024Z-p62223` |
| Generated drift validation | Pass; `make generate-drift` passed at `.cartulary/test-results/20260710T154024Z-p62262` |
| Phase 12 validation | Pass; `make phase-slice PHASE=phase12` passed at `.cartulary/test-results/20260710T154140Z-p73390` with 107 tests, zero failures, and zero missing rows |
| Phase ledger refresh | Pass; `make phase-ledgers` passed at `.cartulary/test-results/20260710T154542Z-p95816` |
| Phase schedule refresh | Pass; `make phase-schedules` passed at `.cartulary/test-results/20260710T154542Z-p95843` |
| Phase map and drift validation | Pass; `make phase-map-check`, `make phase-ledger-drift` `.cartulary/test-results/20260710T154604Z-p96565`, and `make phase-schedule-drift` `.cartulary/test-results/20260710T154604Z-p96571` passed |
| Whitespace validation | Pass; `git diff --check` passed after the adoption-accounting artifact |
| Tracker transition | Pass; `NFA-C00-002`, `NFA-ADOPT-001`, `NF-GATE-001..011`, `NF-BLOCK-001..005`, `NF-BLOCK-011..013`, `NF-BLOCK-015`, `CP-39`, and all `NF-AC-*` rows are complete; `NFA-HANDOFF-001` remains active for final closeout only |
| Explicit deferrals | Pass; Core 05 timing publication, private incident purge, raw IPFIX, live collector, export, saved graph views, binding read/list routes, time buckets, third-party enrichment, and automatic maliciousness inference remain out of v1 scope |
| Next safe step | Pass; commit this tracker checkpoint, then run the final handoff validation bundle and close `NFA-HANDOFF-001` |
...
- [x] Core 00 extension-profile and explicit v1 non-purge decision adopted.
...
- [x] Full security/fault/retention/drift evidence bundle passes.
- [x] Core 00 registry and Network Flow status/version transition pass one coordinated final review.
