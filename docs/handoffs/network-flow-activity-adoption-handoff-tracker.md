...
| `NFA-IMPL-010` | Replace closure-only jobs with durable named handlers | `WS-10` | `TODO` | `NFA-GEN-004`, `NFA-C01-005`, `NFA-PHASE12-001` | platform/jobs and async module owners | `NF-BLOCK-003`, `NF-BLOCK-011`, `NF-BLOCK-016` | `internal/platform/jobs`; async modules | Leases, attempts, recovery, cancellation, drain, and migration tests | `make backend-unit`; `make backend-integration` | Every async family recovers after restart and duplicate delivery |
...
| Field | Value |
| --- | --- |
| Date/time | `2026-07-10T05:37:05-04:00` |
| Branch/commit | `main`; evidence snapshot `721b8c2661b98314ad6ca3e2166e1210a3f156eb` |
| Dirty-tree state | Tracker-only completion checkpoint edit for `NFA-PHASE12-001`; no phase, generated, fixture, transcript, or implementation artifact remains uncommitted |
| Current workflow/task | Phase 12 topology completion checkpoint; `NFA-PHASE12-001` is complete and `NFA-IMPL-010` is the next safe implementation row |
| Completed tasks | `WS-00` artifact/checkpoint `1bb6fdbd`/`46731b5b`; `WS-01` artifact/checkpoint `155b5f64`/`58e57ea`; `WS-02` owner/checkpoint `89580f0c`/`537b7068`; `WS-03` artifact/checkpoint `344486e7`/`2869c850`; `WS-04` owner/checkpoint `08fa716e`/`2d997ae9`; `WS-05` route authorization owner/checkpoint `3b942fe0`/`864ba5ca`; cursor-security owner/checkpoint `90401fb2`/`9b1fcbab`; safe-digest owner/checkpoint `cd645750`/`785f4a98`; audit occurrence owner/checkpoint `71258589`/`7ba84b02`; retention owner/checkpoint `663c8684`/`ea35923d`; Graph Projection ephemeral owner/checkpoint `4e446354`/`3ca750b`; Graph Projection adapter start/owner/checkpoint `252f1235`/`f177fb6b`/`31dd003a`; Graph Projection evidence start/artifact/checkpoint `e09828da`/`81941bba`/`53c156e6`; Testing Harness manifest start/artifact/checkpoint `32b2bec5`/`b3f46bfd`/`a7ce696e`; Testing Harness fault-control start/artifact/checkpoint `0b8dc777`/`ecd4edd5`/`d9080de9`; Testing Harness fake-clock start/artifact/checkpoint `d65308ff`/`30880992`/`6a19aa6c`; Testing Harness deterministic randomness start/artifact/checkpoint `9b2aac15`/`84eb7fb5`/`ce510906`; Testing Harness auth-transition start/artifact/checkpoint `fca79909`/`43b0fe36`/`cc11c4f3`; Testing Harness audit-count start/artifact/checkpoint `a2f60652`/`d96f9ad2`/`d8856caa`; timezone provenance start/artifact/checkpoint `2a466ab1`/`42338a1d`/`0504f532`; contract-generator ownership start/artifact/checkpoint `e7b7a349`/`d8dbaf3f`/`93c3fc47`; Testing Harness structural-accounting start/artifact/checkpoint `2cc44891`/`0eaa8db3`/`74555f60`; dependency-locator start/artifact/checkpoint `2bcc06bc`/`a5dc9847`/`54138e41`; authored-contract start/artifact/checkpoint `8b65d79e`/`9bb6711f`/`fb458479`; generated-contract start/artifact/checkpoint `411421c4`/`540921da`/`aa28972d`; generated-drift start/checkpoint `90f6f672`/`13e89b4f`; Phase 12 topology start/artifact `92eec077`/`721b8c26` |
| Tracker file changed | `docs/handoffs/network-flow-activity-adoption-handoff-tracker.md` |
| Other changed files | none expected after artifact commit; only this tracker completion checkpoint should be dirty |
| Commands run | `git status --short --branch`; `git rev-parse HEAD`; `date -Iseconds`; tracker/source `rg`; `find`; `sed`; `make lint-markdown`; `make generated-artifact-policy-check`; `make json-shape-check`; `make phase-ledgers`; `make phase-schedules`; `make phase-map-check`; `make phase-ledger-drift`; `make phase-schedule-drift`; `make phase-slice PHASE=phase12`; `make service-backed-slice PHASE=phase12`; `make explain-phase PHASE=phase12`; `make harness-contract`; `make generate-drift`; `make lint-scripts`; tracker table-column check; tracker count/contiguity check; `git diff --check` |
| Passing validation | `NFA-GEN-003` start, artifact, and completion checkpoint validation are recorded in §15.25; `NFA-GEN-004` start, evidence, and completion checkpoint validation is recorded in §15.26; `NFA-PHASE12-001` start, artifact, and retained no-op slice validation are recorded in §15.27 |
| Failing validation | `make json-shape-check` `.cartulary/test-results/20260710T082738Z-p15417` failed on the new locator-fragment check because the manifest expected lowercase `adopted` while the table used `Adopted`; fixed and reran pass. One broad ad hoc multi-file table-column check failed on escaped-pipe examples outside changed tables; targeted tracker/Table 1-B/Table 8-D checks passed. Prior unrelated `make harness-contract` failure `.cartulary/test-results/20260710T081325Z-p2192` remains documented in §15.22. During `NFA-GEN-003`, `make harness-contract` `.cartulary/test-results/20260710T090918Z-p50578` failed because the accounting test still expected active-without-generated-marker failure; the test was updated for the active family state and reran pass. During `NFA-PHASE12-001`, initial `make phase-map-check` failed before the generated Phase 12 ledger existed, and `make json-shape-check` `.cartulary/test-results/20260710T093134Z-p81537` failed with `active phase12 ledger missing: docs/testing/phase12_coverage_ledger.md`; both were fixed by generating the ledger and schedules from the Phase 12 owner inputs. |
| Decisions recorded | Phase 12 is active with 107 blocked rows and incomplete aggregate claim status; blocked rows are traceability placeholders only, no fake selectors were added, no Network Flow conformance is claimed, and durable-job implementation may start next |
| Open questions | none for the Phase 12 topology setup slice; exact implementation and fixture selectors remain later |
| Blockers | Implementation, immutable fixtures, transcripts, executable Phase 12 evidence, and final coordinated adoption remain blocked |
| Next recommended task/workflow | Validate and commit this completion checkpoint, then start `NFA-IMPL-010` durable named job handlers |
| Safe restart command | `rg -n -e 'NFA-IMPL-010' -e 'internal/platform/jobs' -e 'durable named handlers' docs/handoffs/network-flow-activity-adoption-handoff-tracker.md internal tools docs` |
