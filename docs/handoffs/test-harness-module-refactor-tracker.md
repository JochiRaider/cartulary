...
| Planning status | Implementation authorized for the remediation plan in this tracker, gated by S-01 characterization before harness implementation edits. |
| Allowed changes in this session | Update this tracker, run Make-owned characterization/validation targets, and apply behavior-preserving implementation slices S-02 through S-08 only after S-01 is recorded. |
| Non-goals | No product behavior refactor, no public harness semantic change without owner-spec update, no generated-artifact hand edits, no package configuration edits unless required by an implemented slice, no migration edits, and no multi-slice behavior change without recording the dependency in this tracker first. |
| Later authorization | Separate authorization remains required for product behavior changes, public Make command semantic changes, output/schema/failure-contract changes, route/auth changes, generated contract changes, or any change outside the behavior-preserving slice boundaries. |
...
| S-00R | S-00 | Adopt the remediation plan as executable tracker control before implementation. | `docs/handoffs/test-harness-module-refactor-tracker.md` | Documentation/process drift only. | Preserve S-01 as a hard implementation gate and add final validation/handoff completion coverage. | Tracker review plus later S-01 baseline commands. | Revert only S-00R tracker edits. | Tracker records implementation authorization, S-09 final completion, and updated handoff status. |
...
| S-09 | S-02 through S-08 as applicable | Complete validation and handoff after implemented slices. | Tracker, retained run artifacts, changed harness files. | Incomplete evidence, stale tracker state, overbroad or skipped validation. | Preserve final command/result history, skipped-check reasons, RB-003 finalizer disposition, and next safe slice. | Narrow changed-slice targets; broaden to `make check` when multiple seams or public behavior are affected; `make agent-finalize RESULTS_DIR=<dir>` only when RB-003 applies. | Revert final tracker-only updates if they misstate evidence; do not revert implementation through S-09. | Tracker has final status, files changed, commands/results, result roots/artifacts, skipped checks with reasons, remaining risks, and handoff conclusion. |
...
| --- | --- | --- | --- | --- |
| public surface baseline | `make task-surface-report TASK_SURFACE_REPORT_ARGS='--check --all'` | Public Make/task-surface parity and command metadata. | yes | Required before any harness movement or public command-surface edit. S-01 and final task-surface checks passed and are recorded in Section 10. |
| unit | `make check-harness-smoke`; `make harness-contract` | Harness self-test smoke plus extended harness contract checks. | yes | Required before broad harness movement. S-01 baseline and post-implementation runs passed and are recorded in Section 10. |
...
Commands are discovered from `AGENTS.md`, `make help`, `make help-all`, and `docs/testing-harness-nlspec.md`. Required baseline, targeted, full, finalizer, and post-finalizer validation results are recorded in Section 10.
...
| TH-014 | Adopt executable remediation plan | WF-00 | DONE | TH-013 | S-00R tracker update | Tracker records implementation authorization, S-01 gate, and S-09 completion slice. |
| TH-006 | Establish characterization baseline | WF-03 | DONE | TH-004 | `make task-surface-report TASK_SURFACE_REPORT_ARGS='--check --all'` pass; `make check-harness-smoke` pass at `.cartulary/test-results/20260703T200345Z-p868623`; `make harness-contract` pass at `.cartulary/test-results/20260703T200345Z-p868615` | Required baseline commands passed and no S-01 blocker remains. |
| TH-007 | Implement core contract facade split | WF-05 | DONE | TH-006 | Added `tools/harness/core/public-contract.mjs` and moved cross-seam imports to it. | `make check-harness-smoke` and `make harness-contract` passed after the facade split. |
| TH-008 | Implement scheduler core/adapter split | WF-05 | DONE | TH-006 | Added `tools/harness/scheduler/adapters/*`; scheduler direct backend/browser/planning imports now live only in adapters. | Scheduler smoke coverage in `make check-harness-smoke` passed after adapter split. |
| TH-009 | Implement browser lifecycle/Playwright split | WF-05 | DONE | TH-006 | Added `tools/harness/browser/runtime/paths.mjs`; browser scheduler dependency path construction uses runtime path helpers. | `make check-harness-smoke` passed after browser runtime path split. |
| TH-010 | Implement backend/frontend/generated/static/readiness slices | WF-06 | DONE | TH-006 | Added backend runner/duration/drift facades, frontend evidence/design/readiness facades, generated-artifact contract shim, and static-analysis design shim. | `make json-shape-check` passed after Make-generated `phase-schedules`; remaining static/generated checks are tracked in TH-011. |
| TH-011 | Run generated/static/harness validation for implementation | WF-07 | DONE | TH-010 | Generated/static/browser/backend/frontend validation artifacts recorded in Section 10; `lint-scripts` wrapper fixed and passed at `.cartulary/test-results/20260703T201812Z-p910602`. | Required targeted commands passed after the `lint-scripts` zero-scope wrapper issue was corrected. |
| TH-012 | Finalize later implementation handoff | WF-08 | DONE | TH-011 | Section 10 final S-09 handoff rows | Next agent can continue without rediscovery. |
| TH-015 | Complete S-09 validation and handoff | WF-08 | DONE | TH-011 | `make check` pass at `.cartulary/test-results/20260703T201831Z-p911292`; `make agent-finalize RESULTS_DIR=.cartulary/test-results/20260703T201831Z-p911292` pass at `.cartulary/test-results/20260703T202110Z-p990766`; post-finalizer drift checks pass; final `make lint-markdown` pass at `.cartulary/test-results/20260703Tfinal-tracker-lint`. | Final status, validation results, skipped checks, RB-003 disposition, and residual risks are recorded. |
...
| 2026-07-03T20:02:56Z | Codex / S-00R adoption | User authorized implementation of the remediation plan; tracker remains the controlling artifact and S-01 remains the hard gate before harness implementation edits. | Touched: `docs/handoffs/test-harness-module-refactor-tracker.md`. Inspected: tracker status, slice plan, validation plan, work tracker, and clean git status. | `git status --short --branch`; targeted `sed`; `date -u +%Y-%m-%dT%H:%M:%SZ`. | Updated tracker status from planning-only to implementation-authorized, added S-00R and S-09, and recorded final handoff completion tracking. | RB-002 still blocks implementation until characterization commands are recorded. | Run S-01 baseline commands and update this tracker with results before any harness source edit. |
| 2026-07-03T20:04:04Z | Codex / S-01 baseline | Baseline characterization completed before harness implementation edits. | Touched: `docs/handoffs/test-harness-module-refactor-tracker.md`. Inspected: retained test-results summaries and git status. | `make task-surface-report TASK_SURFACE_REPORT_ARGS='--check --all'`; `make check-harness-smoke`; `make harness-contract`; `find .cartulary/test-results ...`; `git status --short`. | All three S-01 commands exited 0. Smoke retained root: `.cartulary/test-results/20260703T200345Z-p868623`. Harness-contract retained root: `.cartulary/test-results/20260703T200345Z-p868615`. Task-surface report emitted passing stdout diagnostics and no central run root was required. | None for S-01. | Start S-02 core contract facade and update tracker before S-03. |
| 2026-07-03T20:12:31Z | Codex / S-02 through S-08 seam implementation | Behavior-preserving seam facades/adapters implemented after S-01. | Touched tracker plus harness facade/import files under `tools/harness/core`, `scheduler`, `browser`, `backend`, `frontend`, `generated-artifacts`, and `static-analysis`; Make-generated `tools/execution_topology_render_index.json` updated by `make phase-schedules`. | `node --check ...`; targeted ESM import checks; `make check-harness-smoke`; `make harness-contract`; `make json-shape-check`; `make phase-schedules`; `make phase-schedule-drift`. | Syntax/import checks passed. `make check-harness-smoke` passed at `.cartulary/test-results/20260703T201148Z-p873943`. `make harness-contract` passed at `.cartulary/test-results/20260703T201148Z-p873924`. First `make json-shape-check` failed at `.cartulary/test-results/20260703T201148Z-p873923` because schedule inputs were stale after a renderer import change; `make phase-schedules` passed at `.cartulary/test-results/20260703T201200Z-p875794`; rerun `make json-shape-check` passed at `.cartulary/test-results/20260703T201212Z-p876005`; `make phase-schedule-drift` passed at `.cartulary/test-results/20260703T201212Z-p876021`. | Remaining validation: generated-artifact policy/drift, static import/lint checks, and final S-09 handoff. | Run TH-011 validation set, then decide whether broader `make check` and RB-003 finalizer are required. |
| 2026-07-03T20:16:33Z | Codex / TH-011 targeted validation | Targeted generated, static, backend, frontend, and browser validation completed for the implemented seam batch. | Touched: `docs/handoffs/test-harness-module-refactor-tracker.md`. Inspected retained logs for the failed `lint-scripts` run. | `make generated-artifact-policy-check`; `make generate-drift`; `make phase-ledger-drift`; `make lint-scripts`; `make explain-run RESULTS_DIR=.cartulary/test-results/20260703T201307Z-p876935 TARGET=lint-scripts DETAIL=logs`; retained stdout/stderr inspection; `make lint-shell`; `make frontend-import-boundary-check`; `make backend-module-boundary-check`; `make frontend-unit`; `make migration-drift`; `make task-surface-report TASK_SURFACE_REPORT_ARGS='--check --all'`; `make browser-e2e-webserver-backed`. | Pass roots: generated policy `.cartulary/test-results/20260703T201307Z-p876903`; generate drift `.cartulary/test-results/20260703T201307Z-p876904`; phase ledger drift `.cartulary/test-results/20260703T201307Z-p876912`; lint shell `.cartulary/test-results/20260703T201334Z-p878912`; frontend boundary `.cartulary/test-results/20260703T201334Z-p878928`; backend boundary `.cartulary/test-results/20260703T201334Z-p878960`; frontend unit `.cartulary/test-results/20260703T201407Z-p879842`; migration drift `.cartulary/test-results/20260703T201407Z-p879882`; browser webserver-backed `.cartulary/test-results/20260703T201429Z-p883466` with 88 tests. Task-surface report passed as stdout diagnostics. `make lint-scripts` failed at `.cartulary/test-results/20260703T201307Z-p876935` because Biome processed zero files under `scripts` due current ignored-path configuration; no changed harness files were implicated. | Remaining validation: S-09 broad check/final handoff. RB-003 finalizer applies only if a successful full warm `make check` retained root is produced and selected for maintenance. | Run final S-09 verification, then update final handoff status and skipped-check rationale. |
| 2026-07-03T20:23:49Z | Codex / S-09 validation and handoff | Final remediation validation completed. RB-003 applied because scheduler/default-check wiring and duration/finalizer-adjacent surfaces changed. | Touched tracker, `tools/harness/static-analysis/scripts-biome.sh`, Make/finalizer-managed duration/scheduler artifacts, and implemented harness seam facade/import files. Inspected final retained summaries, scheduler adapter import scan, task-surface report, and git diff/status. | `make lint-scripts`; `make lint-shell`; first `make check`; fixed `scripts-biome.sh`; `make lint-scripts`; `make lint-shell`; `make check`; `make agent-finalize RESULTS_DIR=.cartulary/test-results/20260703T201831Z-p911292`; post-finalizer `make generate-drift`; `make json-shape-check`; `make phase-schedule-drift`; `make duration-baseline-drift-suite RESULTS_DIR=.cartulary/test-results/20260703T201831Z-p911292`; `make generated-artifact-policy-check`; `make phase-ledger-drift`; final `make task-surface-report TASK_SURFACE_REPORT_ARGS='--check --all'`; scheduler direct-import scan; final `make lint-markdown`. | Initial `make check` failed at `.cartulary/test-results/20260703T201705Z-p899359` because `lint-scripts` inherited the zero-scope issue. `scripts-biome.sh` now discovers tracked `scripts/**/*.mjs` files through Git; direct `make lint-scripts` passed at `.cartulary/test-results/20260703T201812Z-p910602`. Final `make check` passed at `.cartulary/test-results/20260703T201831Z-p911292` with 981 tests and 0 failures. `make agent-finalize` passed at `.cartulary/test-results/20260703T202110Z-p990766`, refreshed 5 duration/scheduler artifacts, and validated scheduler event/timing drift. Post-finalizer drift checks passed: generate drift `.cartulary/test-results/20260703T202147Z-p992998`, JSON shape `.cartulary/test-results/20260703T202147Z-p993019`, phase schedule drift `.cartulary/test-results/20260703T202147Z-p993060`, duration baseline drift `.cartulary/test-results/20260703T202147Z-p993079`, generated policy `.cartulary/test-results/20260703T202158Z-p994692`, phase ledger drift `.cartulary/test-results/20260703T202158Z-p994717`. Final task-surface check passed; scheduler cross-seam imports remain only in `tools/harness/scheduler/adapters/*`; final Markdown lint passed at `.cartulary/test-results/20260703Tfinal-tracker-lint`. | None. No skipped required checks. | Remediation is complete; next work should start from a fresh tracker/update cycle if public semantics or additional slices are requested. |
...
| 2026-07-03T20:23:49Z | Codex / final validation | Implemented slices S-02 through S-08 are covered by targeted validation, final `make check`, RB-003 finalizer, and post-finalizer drift checks. | Touched harness implementation files and tracker; no product tests or specs were edited. | See final S-09 row above. | All required Make-owned validation passed after the `lint-scripts` wrapper fix. | None. | No immediate next action. |
...
| 2026-07-03T20:23:49Z | Codex / final handoff | Remediation effort is complete for the authorized behavior-preserving slices. | Tracker and changed harness/generated-maintenance files are listed by git status. | Final status/diff inspection, task-surface check, and scheduler adapter import scan. | No unresolved blocker remains. No required check was skipped. Generated-maintenance artifacts were updated only through Make/finalizer. | None. | Handoff complete. |

## 11. Open Questions and Blockers

| ID | Question or blocker | Why it matters | Needed authority or evidence | Current status |
| --- | --- | --- | --- | --- |
| RB-001 | Code implementation authority is gated. | Tracker/supporting-plan updates are authorized, but code work must not proceed under ambiguous scope. | User authorized the remediation plan on 2026-07-03; implementation remains limited to behavior-preserving slices after RB-002 baseline. | RESOLVED for this remediation effort; code implementation requires S-01/RB-002 baseline and tracker update before each slice. |
| RB-002 | Characterization baseline has not been run in this session. | Refactoring harness internals without baseline risks public Make/output/scheduler drift. | `make task-surface-report TASK_SURFACE_REPORT_ARGS='--check --all'`, `make check-harness-smoke`, `make harness-contract`, and seam-specific commands from Section 8. | RESOLVED for this remediation effort: S-01 baseline passed on 2026-07-03 with smoke root `.cartulary/test-results/20260703T200345Z-p868623` and harness-contract root `.cartulary/test-results/20260703T200345Z-p868615`. |
| RB-003 | Finalizer evidence is conditional by slice. | Retained-run finalizer evidence validates timing/maintenance artifacts only when the selected slice affects those surfaces. | A successful full warm `make check` retained run root and `make agent-finalize RESULTS_DIR=<dir>` only when the slice affects `agent-finalize`, cache/readiness reuse, scheduler timing, duration baselines, default `make check` topology, `check-service-backed` warm-run behavior, public target summary emission, or final handoff maintenance evidence. | RESOLVED/APPLIES for this remediation effort: `make check` passed at `.cartulary/test-results/20260703T201831Z-p911292`; `make agent-finalize RESULTS_DIR=.cartulary/test-results/20260703T201831Z-p911292` passed at `.cartulary/test-results/20260703T202110Z-p990766` and refreshed duration/scheduler maintenance artifacts. |
| RB-004 | Scheduler engine/adapter split boundary needed focused design before edits. | Scheduler imports backend/browser/planning helpers and has public event/resource contracts. | Section 7 scheduler split design contract plus scheduler characterization tests and import graph for selected S-03 work. | RESOLVED for this remediation effort: scheduler adapter split implemented and validated; final import scan leaves backend/browser/planning imports only under `tools/harness/scheduler/adapters/*`. |
...
| Validation commands are discovered or marked `TODO` with a reason. | DONE. Section 8 lists Make-owned commands; Section 10 records the S-01 baseline, targeted seam validation, final `make check`, RB-003 finalizer, and post-finalizer drift checks. |
...
| RB-002 requires S-01 baseline evidence before code movement. | DONE. S-01 baseline commands passed and retained roots are recorded in Section 10. |
...
| No generated artifact is hand-edited. | DONE. Generated and maintenance outputs were refreshed only through `make phase-schedules`, `make agent-finalize`, and drift/validation targets. |
| Public Make target names, command IDs, output/artifact schemas, failure mapping, scheduler schemas, route/auth behavior, and product contracts are unchanged. | DONE. Final task-surface, harness contract, smoke, full check, finalizer, and drift validation passed without owner-spec changes. |

No product behavior refactor was performed by this remediation effort.
