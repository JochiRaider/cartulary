...
| `IDB-007` | Harness Section 4 public registry and TH-HARNESS-REQ-060 | `build-operator` is already a public helper build target and a permitted `cartulary.cache.build_artifact.v1` build-artifact cache profile. | Default local `check` does not build the operator unless owner inputs add scheduler-visible work. No current owner row inspected by this handoff defines how operator Go tests consume a scheduler-built binary. | `HRF-002`, `HRG-002`. |
...
| `EVID-HIDDEN-OPERATOR-BUILD` | `cmd/operator/operator_phase10_test.go` | Source fact applies to both runs. | `buildOperatorBinary` shells out to `make --no-print-directory build-operator OPERATOR_BIN=<temp>` per test helper use. | `HRF-002` |
...
| `HRF-002` | High | `cmd/operator` tests; hidden `buildOperatorBinary`; `make build-operator` | `EVID-HIDDEN-OPERATOR-BUILD`; live owner inspection confirms `build-operator` is public helper build and build-artifact cache family member. | Harness Sections 1, 4, 5, 8; `IDB-002`; `IDB-007`. | Do not invent a provisional operator-binary injection interface. Owner must define exact operator binary injection interface, omission behavior, invalid-path behavior, summary emission, and child forwarding before implementation can replace hidden per-test rebuilds. | Missing owner rule; likely owner surfaces include harness input matrix, task-surface metadata, scheduler child environment contract, and `cmd/operator/operator_phase10_test.go`. | `TODO(owner-closure): define exact operator binary injection interface, omission behavior, invalid-path behavior, summary emission, and child forwarding.` Row can leave `owner_closure_required` only after the owner rule exists and this handoff imports it. | `owner_closure_required` | No owner-declared operator test-consumption input or environment contract is present in the inspected owner surface. |
...
| `HRG-002` | Operator build-artifact cache and test-consumption interface | `HRF-002`, `HRF-006` | Harness Sections 4, 5, 8; `IDB-002`; `IDB-007` | Owner must define exact operator binary injection interface, omission behavior, invalid-path behavior, summary emission, and child forwarding before hidden rebuild removal proceeds. | `TODO(owner-closure): define exact operator binary injection interface, omission behavior, invalid-path behavior, summary emission, and child forwarding.` | `owner_closure_required` |
...
| `HR-1`: close operator binary injection owner gap before hidden rebuild removal | Harness Sections 4, 5, 8; `IDB-007` | No implementation proceeds until owner defines operator binary injection. After owner closure, schedule/cache `build-operator` and remove hidden per-test rebuilds. | `make build-operator` | Before owner closure: `HRF-002` remains `owner_closure_required`. After owner closure: build-operator cache record exists and Go test logs contain no child `make build-operator`. | Missing injection owner rule. |
...
9. `HRF-002` or `HRG-002` remains `owner_closure_required` and implementation would require choosing an operator binary injection interface.
...
- Do not modify `docs/testing-harness-nlspec.md` as part of this handoff revision. Missing owner behavior is surfaced as `owner_closure_required`.
