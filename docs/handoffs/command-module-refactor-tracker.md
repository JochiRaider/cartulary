...
| 2026-07-12 | Codex command forward-design remediation session | Section 13 is complete: production/harness profiles, exact operator registry and migrate grammar, deterministic runtime binaries, semantic process-test ownership, and architectural guardrails are in place. | Command/refactor implementation, owner tests, authored phase/topology inputs, generated outputs, duration baselines, docs, and this tracker. | Full warm `make check`; `make agent-finalize RESULTS_DIR=.cartulary/test-results/20260712T014318Z-p94370`; `make test-fast`; harness, generated, shape, boundary, phase, scheduler, and duration drift gates. | Final current-code `check` passed 338/338 work units at `.cartulary/test-results/20260712T014318Z-p94370`; retained finalization passed at `.cartulary/test-results/20260712T014602Z-p3255`; final fast and integrity validations passed at the roots recorded in §13.7A. SeaweedFS remains release-only and its release gate passed at `.cartulary/test-results/20260712T010633Z-p76507`. | None. Cold-run readiness and nested service-backed timing accounting findings were corrected structurally; no command-refactor blocker remains. | Handoff complete; retire SeaweedFS preservation only when its documented release-support condition is met. |
...
| --- | --- | --- | --- | --- |
| NXT-001 | Adopt legacy disposition and production/harness profile rules | DONE | none | `docs/testing-harness-nlspec.md` §4.1B and active guides define the three identities, two server profiles, exact migration grammar, retirement triggers, and no nested-build fallback. `make lint-markdown`, `make generated-artifact-policy-check`, and `make json-shape-check` passed. |
| NXT-002 | Add deterministic production and harness server builds | DONE | NXT-001 | `build-server` is production-only; `build-server-harness` uses the private `cartulary_harness` profile. Production rejects harness keys before config load; both artifacts and backend unit tests pass. |
| NXT-003 | Replace two-stage operator parsing with exact owner registry | DONE | NXT-001 | `internal/app` owns exact descriptor registration/help; recovery retains canonical namespace rejection and its result contracts. App aliases and retired top-level recovery matching are removed. |
| NXT-004 | Contract migrate grammar and move `up-to` support | DONE | NXT-001 | Production accepts exactly `migrate up`; invalid vectors stop before config/DB. Penultimate scratch application is database-contract-owned and no longer invokes the deployable CLI. |
| NXT-005 | Remove contextless wrappers, app aliases, global CLI factories, and nested builds | DONE | NXT-003, NXT-004 | Four runtime artifacts are registered and injected by producer; browser/process consumers use `server-harness`; raw fallback builds are removed. |
| NXT-006 | Move process evidence out of `cmd` and update owner maps | DONE | NXT-002, NXT-003, NXT-004 | Process evidence now has application/recovery owner packages; `cmd` contains exactly three `main.go` roots; phase selectors and generated ledgers/schedules are updated. |
| NXT-007 | Add architectural guardrails and regenerate downstream artifacts | DONE | NXT-005, NXT-006 | Command-root, retired API/token, production harness-import, and nested test-build policy fixtures pass; downstream artifacts were regenerated through Make. |
| NXT-008 | Complete warm validation, retained-run refresh, and handoff | DONE | NXT-007 | Final current-code `make check` passed 338/338 at `.cartulary/test-results/20260712T014318Z-p94370`; retained finalization passed at `.cartulary/test-results/20260712T014602Z-p3255`; post-finalization drift, shape, boundary, duration, and fast-suite gates passed. |
...
### 13.6A Iteration execution log

| Workstream | State | Files changed | Validation and handoff |
| --- | --- | --- | --- |
| NXT-001 | DONE | `docs/testing-harness-nlspec.md`; `docs/guides/cartulary-dev-guide.md`; `docs/guides/cartulary_repository_bootstrap_guide.md`; this tracker. | `make lint-markdown`; `make generated-artifact-policy-check` at `.cartulary/test-results/20260712T003142Z-p97169`; `make json-shape-check` at `.cartulary/test-results/20260712T003143Z-p97312`; next: NXT-002. |
| NXT-002 | DONE | `internal/app/server.go`; build-tagged server profile files/tests; `Makefile`; build-artifact helper; Phase 0 owner input; topology source and generated task-surface outputs; this tracker. | `make format-go`; `make phase-schedules` at `.cartulary/test-results/20260712T003557Z-p3365`; `make backend-unit`; `make build-server`; `make build-server-harness`; `make json-shape-check`; `make phase-schedule-drift`; next: NXT-003. |
| NXT-003 | DONE | `internal/app/operator*.go`; `internal/modules/recovery/operatorcli/cli.go`; direct owner-contract test decoding; this tracker. | `make format-go`; `make backend-unit`; `make build-operator` at `.cartulary/test-results/20260712T004023Z-p53991`; `git diff --check`; next: NXT-004. |
| NXT-004 | DONE | `internal/app/migrate.go` and tests; Postgres migration test support; migration scratch driver/test; topology owner input and generated task surface; support inventory; this tracker. | `make format`; `make backend-unit`; `make migration-scratch-apply`; `make json-shape-check`; `make phase-schedules`; `make phase-schedule-drift`; `git diff --check`; next: NXT-005. |
| NXT-005 | DONE | Runtime-binary registry and phase owner inputs; browser stack, scheduler, process test support, topology source/generated task surface; operator process test; this tracker. | `make phase-schedules` at `.cartulary/test-results/20260712T005017Z-p8201`; `make json-shape-check` at `.cartulary/test-results/20260712T005024Z-p8365`; `make harness-contract` at `.cartulary/test-results/20260712T005045Z-p9492`; `make lint-shell` at `.cartulary/test-results/20260712T005057Z-p10147`; `make build-server-harness` at `.cartulary/test-results/20260712T005112Z-p11168`; `make backend-process`; no nested build fallback remains; next: NXT-006. |
| NXT-006 | DONE | Process evidence moved from `cmd/server` to `internal/app/serverprocess` and from `cmd/operator` to `internal/modules/recovery/operatortest`; phase 0/1/2/5/10/12 selector inputs and generated ledgers/schedules; this tracker. | `make backend-process` at `.cartulary/test-results/20260712T005658Z-p69187`; `make backend-module-boundary-check` at `.cartulary/test-results/20260712T005752Z-p84605`; `make phase-ledgers` at `.cartulary/test-results/20260712T005753Z-p84763`; `make phase-schedule-drift`; `make generated-artifact-policy-check`; `rg --files cmd` reports only the three `main.go` roots; next: NXT-007. |
| NXT-007 | DONE | `tools/backend_module_boundaries.json`; backend-boundary implementation/fixtures; generated scheduler/task-surface outputs; this tracker. | `make harness-contract` at `.cartulary/test-results/20260712T010111Z-p94232`; `make backend-module-boundary-check` at `.cartulary/test-results/20260712T010124Z-p94858`; `make generate-drift` at `.cartulary/test-results/20260712T010125Z-p95016`; `make phase-ledger-drift` at `.cartulary/test-results/20260712T010157Z-p2789`; `make phase-schedule-drift` at `.cartulary/test-results/20260712T010200Z-p3112`; `make generated-artifact-policy-check` and `make json-shape-check`; next: NXT-008. |
| NXT-008 | DONE | Duration baseline owner artifacts; scheduler projection/accounting guardrails; generated topology index/scheduler outputs; this tracker. | The initial full run exposed relative runtime-binary propagation, missing/duplicate check-schedule runtime producers, and scheduler-family resource-claim leakage; those were corrected in the shared scheduler. A cold retained root was rejected as designed, then qualifying warm evidence passed. The nested `check-service-backed` event projection was corrected so its lifecycle envelope matches its child timing. Final current-code `make check` passed 338/338 at `.cartulary/test-results/20260712T014318Z-p94370`; `make scheduler-event-order-drift`, `make scheduler-summary-timing-drift`, and `make duration-baseline-drift-suite` passed; `make agent-finalize RESULTS_DIR=...` passed at `.cartulary/test-results/20260712T014602Z-p3255`; final integrity roots are in §13.7A. SeaweedFS remains release-only pending its recorded retirement trigger. |
...
### 13.7A Execution completion validation

| Command or gate | Result | Evidence or disposition |
| --- | --- | --- |
| Final full warm `make check` | PASS | `.cartulary/test-results/20260712T014318Z-p94370`; 338/338 scheduled work units, current-code production/harness profiles and semantic process owners exercised. |
| Retained-run maintenance | PASS | `make agent-finalize RESULTS_DIR=.cartulary/test-results/20260712T014318Z-p94370` at `.cartulary/test-results/20260712T014602Z-p3255`; generated artifacts unchanged and advisory duration refresh completed. |
| Scheduler and duration drift | PASS | `scheduler-event-order-drift`, `scheduler-summary-timing-drift`, and `duration-baseline-drift-suite` passed against the final retained root. The prior cold run was rejected by readiness thresholds, and a child-event lifecycle mismatch was fixed rather than waived. |
| Fast, boundary, and harness gates | PASS | `make test-fast` at `.cartulary/test-results/20260712T014629Z-p5346`; `make harness-contract` at `.cartulary/test-results/20260712T014838Z-p66766`; `make backend-module-boundary-check` at `.cartulary/test-results/20260712T014902Z-p70704`. |
| Generated, phase, shape, and documentation gates | PASS | `lint-markdown` `.cartulary/test-results/20260712T014851Z-p67411`; `json-shape-check` `.cartulary/test-results/20260712T014853Z-p68811`; generated policy/drift and phase drift passed at `.cartulary/test-results/20260712T014855Z-p69115` through `...T014901Z-p70568`; duration-baseline coverage passed at `...T014902Z-p70849`. |
| Compatibility disposition | COMPLETE | Only seven exact operator commands remain; production migrate accepts only `up`; `cmd` has exactly the three `main.go` roots; production server rejects harness-only keys and no longer links harness capabilities. SeaweedFS legacy-source preservation remains explicit release-only support until its documented retirement condition is met. |
