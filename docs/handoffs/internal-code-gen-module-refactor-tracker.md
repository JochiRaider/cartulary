...
- Current status: this tracker now records the authorized remediation for RB-001 through RB-004, superseding the original planning-only posture.
- Authorized implementation scope: delete unused authored SQL query inputs, refresh generated SQLC outputs through repository generators, tighten SQLC boundary guardrails, and clarify the timeline workbook projection source/storage boundary.
- Non-goals: no DB schema migration, no public HTTP/WebSocket/workbook/auth/recovery contract change, no graph projection behavior change, and no generated-artifact hand edit.
- Implementation posture: `internal/gen/**` remains generated output only; generated changes must come from owner SQL/contract inputs and repository generators.
...
- The generated auth/session/bootstrap SQLC path had no continuing implementation value; platform auth persistence remains owned by `internal/platform/authn/store.go`, and the unused authored auth SQL input is removed in this remediation.
- The uncalled `GetSessionMemberships` and `GetLatestSuccessfulRetainedBackupSet` SQLC inputs are removed; retained recovery backup-set listing is renamed to metadata-specific language so SQLC names do not imply artifact/proof-safe backup selection.
...
| `internal/gen/sql/auth_phase1.sql.go` | Removed generated auth/session/bootstrap SQLC output after deleting unused authored auth query input. | Former `GetLocalUserByEmail`, `GetSessionByFingerprint`, `ListActiveSessionsForUser`, and `GetBootstrapTokenByFingerprint` methods disappear after generation. | No direct production import use found; `internal/platform/authn/store.go` implements live auth persistence. | None after generated cleanup. | Auth handlers use platform auth store. | Former SQLC output from deleted `db/queries/auth_phase1.sql`. | Platform auth persistence. | Medium | RB-001 resolved by deleting the unused authored SQLC input rather than preserving a parallel security-sensitive persistence path. |
| `internal/gen/sql/incidents_phase2.sql.go` | Generated incident, membership, visibility, lifecycle, and audit queries. | Incident query methods and row/param types including visibility, membership, lifecycle, audit, admin count; `GetSessionMemberships` is removed. | `internal/modules/incidents/store.go`, `open_guard.go`, `import_finalization.go`. | `context`, `pgtype`, `DBTX`. | Phase 2 incident request tests; downstream route behavior tests. | SQLC output from `db/queries/incidents_phase2.sql`. | `incidents`; session membership summaries remain owned by platform auth store APIs when needed. | High | Owns no behavior itself; surrounding incidents module maps SQL rows to domain records and API results. |
...
| `internal/gen/sql/recovery_phase10.sql.go` | Generated backup-set metadata and restore-verification persistence queries. | Backup-set metadata and restore-verification query methods and row/param types; `GetLatestSuccessfulRetainedBackupSet` is removed and retained listing is named `ListRetainedBackupSetMetadata`. | `internal/modules/recovery/store.go`. | `context`, `pgtype`, `DBTX`. | Recovery store/operator tests indirectly. | SQLC output from `db/queries/recovery_phase10.sql`. | `recovery`. | Medium | Metadata-only SQL must not be reintroduced as latest-backup admission; artifact/proof-safe selection belongs behind `BackupCatalog`. |
...
| `internal/gen/sql/timeline_phase3.sql.go` | Generated timeline projection row and projection-source queries. | `GetTimelineProjectionRow`, `ListTimelineProjectionRows`, `ListTimelineProjectionSourceRows` and row types. | `internal/modules/timeline/store.go`, `query_projection_store.go`, `workbookprojection/store.go`. | `context`, `pgtype`, `DBTX`. | Timeline, revisions, evidence, workbook query, and projection-related tests indirectly. | SQLC output from `db/queries/timeline_phase3.sql`. | `timeline` source semantics and workbook row shape; `projections` storage lifecycle. | High | Graph Projection NLSpec does not own workbook projections; source extraction is behind `internal/modules/timeline/workbookprojection`. |
...
Every retained file under `internal/gen` has been inventoried above. The former `internal/gen/sql/auth_phase1.sql.go` row is retained as the cleanup record for the generated output removed by this remediation.
...
| Auth/session generated SQL | Former `internal/gen/sql/auth_phase1.sql.go` | Platform auth persistence | Remove | Search found no direct production caller; `internal/platform/authn/store.go` uses raw SQL and remains the live owner. | Delete `db/queries/auth_phase1.sql` and regenerate so stale auth SQLC methods disappear. |
| Incident and membership SQL | `internal/gen/sql/incidents_phase2.sql.go` | `incidents`; platform auth for session-membership summary APIs | Keep generated incident output, remove stale query | `internal/modules/incidents` imports generated SQL; `GetSessionMemberships` was uncalled and duplicates platform auth membership-summary ownership. | Remove only the stale authored query and preserve incident store behavior. |
| Timeline projection SQL | `internal/gen/sql/timeline_phase3.sql.go` | `timeline` source facade plus `projections` storage lifecycle | Split boundary now | Timeline store and `workbookprojection` source facade consume generated rows; projections owns delete/upsert. | Graph Projection NLSpec explicitly does not own workbook-grid projections. |
...
| Recovery SQL | `internal/gen/sql/recovery_phase10.sql.go` | `recovery` | Keep generated output, remove unsafe/stale names | Recovery store consumes backup metadata and restore verification queries. | Remove metadata-only latest-backup query and rename retained listing to metadata-specific terminology. |
...
| Unused generated SQL queries | Former auth, incident membership, and recovery latest-backup SQLC methods | Owning module plus authored SQL owner | Remove | Search found no production caller and owner semantics live elsewhere. | Query removal is via authored SQL inputs plus generation, not generated-file hand edits. |
...
| Auth generated queries were unused while platform auth store uses raw SQL. | Search found generated functions and raw SQL in `internal/platform/authn/store.go`. | Drift between authored SQLC query inputs and live auth persistence behavior. | resolved | Platform auth persistence. | Delete unused authored auth SQL input and regenerate; retain raw SQL store as the complete tested implementation. |
| Some generated queries were uncalled. | `GetSessionMemberships` and `GetLatestSuccessfulRetainedBackupSet` were found only in generated files and authored SQL. | Dead generated query inputs can confuse ownership and tests. | resolved | Incidents, recovery, platform auth. | Remove stale authored queries; recovery metadata listing uses explicit metadata naming. |
...
| Workbook projection SQL is adjacent to timeline logic. | Timeline `workbookprojection` facade reads source rows through generated SQL and derives projection DTOs; projections owns projection-table delete/upsert. | Projection/source confusion could cause behavior drift. | resolved | Timeline plus workbook projections. | Preserve this boundary in provider registry, manifest, tests, and future projection-provider additions. |
...
| S-03 | S-01 | Remove stale generated SQL query inputs and regenerate. | `db/queries/auth_phase1.sql`, `db/queries/incidents_phase2.sql`, `db/queries/recovery_phase10.sql`, owners in auth/incidents/recovery. | Auth/session, incident membership summary, backup selection behavior. | Preserve owner tests proving live behavior remains in platform auth, incidents, and recovery catalog/store paths. | `make generate`; `make generate-drift`; `make backend-unit`; `make backend-store` if SQL behavior is affected. | Revert authored SQL cleanup and regenerated outputs together. | Removed methods are absent from SQLC output and live behavior is unchanged. |
| S-04 | S-01 | Enforce module-owned persistence facades and keep SQLC types adapter-local. | Module stores in incidents, timeline, savedviews, workbook startup, recovery, reporting; `internal/modules/codegenboundary`. | Public route envelopes, authorization, projection rows, release state. | Add import-boundary guard and behavior-specific characterization before moving additional call sites. | `make backend-unit`; `make backend-integration`; phase-specific slice when applicable. | Revert facade-only changes before changing generated or authored SQL. | Exported APIs and cross-package facades do not expose generated SQLC types. |
...
The original planning session ran no validation. Remediation validation is recorded in the implementation session handoff and final report.
...
| T-002 | Inventory every file under `internal/gen`. | WF-01 | DONE | T-001 | Section 2 inventory table. | Retained generated files are listed; removed auth SQLC output is recorded as a cleanup row. |
| T-003 | Map generated SQL files to authored SQL and consuming owner modules. | WF-02 | DONE | T-002 | Sections 2 and 3. | Each retained generated SQL file has an owner; removed queries have accepted cleanup decisions. |
...
| T-011 | Resolve owner decision for unused generated auth/session/recovery queries. | WF-05 | DONE | T-003 | RB-001, RB-002. | Accepted decision removes stale authored SQL inputs and regenerates outputs. |
| T-012 | Add module facade guardrail for direct SQLC type coupling. | WF-05 | DONE | T-011 | S-04; `internal/modules/codegenboundary/sqlc_boundary_test.go`. | SQLC imports are allowed only in owner persistence adapter files, and exported declarations cannot expose SQLC types. |
...
| 2026-07-08T21:07:02-04:00 | Codex remediation implementation | RB-001 through RB-004 are accepted for cleanup: stale SQLC inputs removed or renamed, timeline workbook projection source facade introduced, SQLC boundary guard added, and projection provider metadata aligned. | `db/queries/*`, `internal/modules/recovery/*`, `internal/modules/projections/*`, `internal/modules/timeline/workbookprojection`, `internal/modules/codegenboundary`, `contracts/projection-providers/index.json`, Appendix I. | `rg`, `sed`, `git status`; generation and validation recorded in final task handoff. | Owner decisions no longer deferred. | None for RB-001/RB-002/RB-004; RB-003 guardrail is active and future module splits must preserve it. | Run generated drift, phase slices, and backend validation before handoff. |

## 11. Open Questions and Blockers

| ID | Question or blocker | Why it matters | Needed authority or evidence | Current status |
| --- | --- | --- | --- | --- |
| RB-001 | Should generated auth SQL in `auth_phase1.sql.go` be adopted by platform auth, retained unused, or removed by deleting authored SQL queries? | Current auth persistence uses raw SQL, creating drift risk between generated query inputs and live behavior. | Auth/platform owner decision; characterization of session/bootstrap behavior; authored SQL review. | RESOLVED: remove the unused authored auth SQL input and keep platform auth store as owner. |
| RB-002 | Are uncalled generated queries such as `GetSessionMemberships` and `GetLatestSuccessfulRetainedBackupSet` intentionally retained? | Removing them requires authored SQL changes and regenerated outputs; retaining them should be intentional. | Incidents/recovery owner decision; search confirmation in implementation session. | RESOLVED: remove stale queries and rename recovery retained-list query to metadata-specific naming. |
| RB-003 | Should modules hide SQLC row/param types behind narrower persistence facades? | SQLC types currently appear in module conversion helpers and can couple code to generated schema shape. | Module owner review and characterization tests for affected stores. | RESOLVED: SQLC imports stay adapter-local, exported SQLC type exposure is guarded, and new timeline workbook projection DTOs hide generated row shape. |
| RB-004 | What is the exact owner boundary for workbook projection SQL versus timeline module logic? | Timeline generated SQL reads projection/source rows, but projection behavior is Core-owned and graph projection is explicitly out of scope. | Core 01/Core 03 owner reading and timeline/projections design review. | RESOLVED: timeline owns source semantics and workbook row DTOs; projections owns projection-table delete/upsert, incident rebuild orchestration, and restore rebuild orchestration. |
...
- Behavior changes proposed: no public product behavior change; internal generated-query cleanup and boundary refactor accepted.
...
- Handoff current: yes, as of 2026-07-08T21:07:02-04:00.
