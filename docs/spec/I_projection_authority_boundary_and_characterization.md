...
`docs/graph_projection_nlspec.md` declares `status: adopted/current` and version 2.1.0. It governs pure graph-oriented projection behavior, immutable result identity, narrow result capabilities, bounded source-owner-scoped cleanup ports, and the distinct current Graph-owned `graphprojection.restore_rebuild.v3` participant. Its exact v2 dispatcher remains only for supported retained pre-GP3 backup catalogs. Network Flow owns saved graph declarations, temporal meaning, and production materialization; Reporting owns exact leased consumption. Graph Projection does not govern workbook-grid projection tables, `view_row_v1`, workbook query routes, Core saved views, import owner facades, workbook provider descriptors, or the workbook `RestoreProjectionRebuilder`. Research reports R01 through R09 remain informative unless a finding is explicitly promoted into Core, ADR, SPEC, or adopted NLSpec material.
