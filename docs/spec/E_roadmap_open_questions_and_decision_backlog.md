...
| `DQ-010` | **Public interface surface contract** | The normative core now closes the browser-facing wire contract as a versioned HTTP+JSON API rooted at `/api/v1/` plus a bounded WebSocket stream rooted at `/ws/v1/`, with session-based auth, stable route families, field-key-based mutation, generic success and error envelopes, cursor pagination, blob-slot and job routes, and a bounded event vocabulary. | Resolved in the current normative core. Future work MAY add route families or optional response members within the same major version, but breaking changes to required fields or event semantics require a new major version root. |
