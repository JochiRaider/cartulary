...
| `DQ-045` | `deployment_admin` application-level scope | Core 04 §2 and §9.10; Core 01 §3.3.3, §17, and §20 | Resolved in core. | Future work MAY add granular deployment capabilities only through a later profile or explicit versioned capability registry; current v1 implementations MUST NOT locally reinterpret `deployment_admin`. |
...
| `AC-427` | Core 04 §2 and §9.10; Core 01 §3.3.3, §17, and §20 | Replaced the account-only `deployment_admin` wording with an exhaustive authorization matrix and a route-inventory audit that classifies every current public route family as `deployment_admin` required, insufficient, or irrelevant. |
