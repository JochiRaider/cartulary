...
| `DQ-046` | Recovery operator interface closure | Core 01 §3.3.3 and §12.2.1; Core 04 §2 and §9.14 | Resolved in core. Base recovery is a deployment-local CLI, not a browser, public route, WebSocket, incident, common-job, session, or `deployment_admin` runtime surface. | Future work MAY add packaging aliases, deployment scripts, or broader restore profiles only if they preserve the logical command grammar, local-operator authorization boundary, and no-public-route rule. |
