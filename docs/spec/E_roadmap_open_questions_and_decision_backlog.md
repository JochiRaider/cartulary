...
| `DQ-044` | Incident metadata mutability and vocabulary closure | Core 01 §3.3.5.3-§3.3.5.3.1 and §18; Core 02 §4.5 and §18; Core 04 §9.8 and §9.10 | Resolved in core. | Future work MAY add incident close/reopen lifecycle routes, suggested severity or phase values, or presentation labels only if they preserve PATCH mutability for `description` and `severity`, exact machine-readable TLP tokens, and open bounded severity and phase storage. |
