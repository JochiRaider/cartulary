...
- The base-profile built-in and system view registry is no longer implicit. Core 01 now fixes exactly nine pack-independent `view_schema_id` entries for Timeline, Hosts, Identities, Evidence, Notes, Indicators, Assessments, Task Requests, and Decisions, each with an ordered field set, deterministic default sort tuple, filter whitelist, and explicit write-back contract. For Hosts, Identities, and Evidence, Core 01 §7.4.2 through §7.4.4 now act as exhaustive per-field registries with stable `field_key` values, explicit write targets or actions, `conflict_resolution_class`, `entity_binding_mode` where applicable, and the explicit evidence-action boundary for blob attach or replacement; Core 02 keeps exact-match reuse precedence authoritative for host and identity create-or-upsert; and Core 04 AC-116 through AC-120 make these registry contracts inspectable and fail closed on undeclared or read-only writes.
