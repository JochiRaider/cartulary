...
| `DQ-042` | Operational backup and coherent restore contract | Core 01 §12.1-§12.2; Core 04 §12 and §9.14 | Resolved in core. Manual backup creation is `operator backup create`; failed candidates are never successful retained backups; scheduling remains deployment-owned external orchestration. | Future work MAY add offsite replication, managed scheduler integrations, or broader restore profiles without reinterpreting the base recovery contract. |
| `DQ-046` | Recovery operator interface closure | Core 01 §3.3.3 and §12.2.1; Core 04 §2 and §9.14 | Resolved in core. Base recovery is a deployment-local CLI, not a browser, public route, WebSocket, incident, common-job, session, `deployment_admin` runtime surface, or browser-visible "Backup now" action. | Future work MAY add packaging aliases, deployment scripts, or broader restore profiles only if they preserve the logical command grammar, local-operator authorization boundary, no-public-route rule, and deployment-owned scheduling boundary. |
| `DQ-043` | Shared authenticated post-login landing | Core 01 §3.3.2.1A; Core 03 §2.4; Core 04 §9.10 | Resolved in core. | Future authentication flows MAY add validated same-origin return targets or presentation refinements only if they reuse the shared Base Profile landing contract and do not widen incident visibility. |
| `DQ-044` | Incident metadata mutability, vocabulary closure, and lifecycle closure | Core 01 §3.3.5.3-§3.3.5.3.2, §3.3.10, and §18; Core 02 §4.5 and §18; Core 03 §4.3.2 and §4.4; Core 04 §9.8 and §9.10 | Resolved in core. Close and reopen are current-profile lifecycle actions. | Future work MAY add suggested severity or phase values, presentation labels, incident archive, incident hard deletion, incident soft deletion, or purge only if a later owner spec defines them without reinterpreting current close/reopen behavior, PATCH mutability for `description` and `severity`, exact machine-readable TLP tokens, or open bounded severity and phase storage. |
| `DQ-045` | `deployment_admin` application-level scope | Core 04 §2 and §9.10; Core 01 §3.3.3, §17, and §20 | Resolved in core, including claimed reference-pack route-family authorization. | Future work MAY add granular deployment capabilities only through a later profile or explicit versioned capability registry; current v1 implementations MUST NOT locally reinterpret `deployment_admin` or add non-admin raw reference-pack metadata views. |
| `DQ-048` | Deployment administration browser entry | Core 01 §3.3.2.1B; Core 03 §2.4; Core 04 §2 and §9.10 | Resolved in core. Deployment administration is a distinct `/deployment-administration` browser context and does not alter `/`, workbook startup, built-in tabs, system views, saved views, or `sheet_ref` semantics. | Future work MAY add granular deployment capabilities or additional owner-backed panels only through later owner specs. |
| `DQ-049` | Imported-incident initial access | Core 01 §12.3.6 and §17.5; Core 03 §2.4; Core 04 §9.11 | Resolved in core. Successful incident-bundle import bootstraps exactly one target-local importer `admin` membership during final publication. | Future work MAY add migration helper tooling only if it preserves target-local membership creation, historical actor inertness, and no portable membership. |
| `DQ-050` | Reference Pack exhaustive search and filters | Core 01 §17.4; Core 04 §9.4 | Resolved in core. `GET /api/v1/reference-packs` owns server-side exhaustive search and exact filters before pagination. | Future work MAY add additional filters only through the route owner without weakening authorization-first validation or cursor binding. |
| `DQ-051` | No generic deployment settings or all-incident catalog | Core 01 §3.3.2.1B; Core 04 §2 and §9.10 | Resolved in core. The current profile has no deployment-wide incident catalog, generic General settings panel, browser provider-definition editor, browser recovery control, or cross-incident policy-default object. | Future work MAY add those concepts only through a later owner spec defining resource shape, authorization, disclosure, defaults, persistence, and conformance. |
| `DQ-052` | Inspector feature-group registry and row-context workflow routing | Core 01 §7.4, Core 03 §2.3A, Core 04 §9, Core 00 §5.1 | Resolved in core. Every current-profile standardized surface now has an exhaustive emitted feature-group registry, deterministic inspector interaction semantics, and conformance checks. | Future work may add external enrichment or new workflow families only through an explicit extension profile defining trust boundary, disclosure, authorization, audit, egress, route shape, and conformance. |
...
| `DQ-047` | Current-account profile and density preferences | Core 01 §3.3.2.3; Core 02 §14.1; Core 03 §2.4; Core 04 §1.1.1, §2, §3, and §9.10 | Resolved in core. Ordinary authenticated users can edit only their own display name and one density override; email remains the authoritative deployment-admin-managed local login identifier; `density_mode=null` means no override with Timeline `compact` and every other workbook surface `default`. | Future work MAY add self-service email change, login aliases, locale, time zone, notification settings, theme selection, global home or default incident, custom density, or custom row heights only through a later owner spec. |
...
| `DQ-036` | Filter operand canonicalization, full-text token semantics, and list-search behavior | Core 01 §3.3.4.1 and §3.3.7.1; Core 04 §9.10 | Resolved in core. | Future work MAY add phrase, fuzzy, language-aware search, or route-specific search operators only through new operators or later profiles. |
...
| `AC-212`, `AC-214` | Core 01 §3.3.5.3.1; Core 01 §18; Core 02 §18 | Closed the incident metadata PATCH mutable set to `description`, `severity`, `tlp`, `current_phase`, and `primary_external_case_ref`; removed `description` and `severity` from the invalid PATCH set; and made TLP clear semantics, exact-token validation, and string-contract failures explicit. |
...
| `AC-429..AC-432` | Core 01 §3.3.2.3; Core 02 §14.1; Core 03 §2.4; Core 04 §1.1.1, §2, §3, and §9.10 | Closed the current-account surface to display-name and density override only, with exact resources, singleton pagination rejection, required mutation members, `null` density semantics, idempotency ordering, stale-version errors, no-op version behavior, no session revocation, forbidden adjacent features, authorization boundaries, and portability exclusion. |
| `AC-433..AC-436` | Core 00 §5.1; Core 01 §20; Core 02 §14.1; Core 04 §1.2, §9.5, and §12.3.4 | Closed enterprise-provider configuration to a startup-only deployment-local manifest with exact path rules, JSON schema, defaults, bounds, omitted-versus-`null` behavior, `secret_ref_v1` handling, referenced-file validation, deterministic reconciliation, disabled-provider discovery behavior, and binary no-runtime-provider-mutation evidence. |
...
| `AC-418..AC-426` | Core 01 §3.3.5.3.2 and §3.3.10, Core 02 §18, Core 03 §4.3.2 and §4.4, Core 04 §9.10 | Closed incident lifecycle behavior to exact close/reopen routes, exact request members, required reason validation, two-state invariants, idempotency replay order, closed-operation matrix, close-versus-source-mutation binary criterion, migration remediation, closed workbook state, rejected-draft non-replay, and closed WebSocket behavior. |
| `AC-427` | Core 04 §2 and §9.10; Core 01 §3.3.3, §17, and §20 | Replaced the account-only `deployment_admin` wording with an exhaustive authorization matrix and a route-inventory audit that classifies every current public route family as `deployment_admin` required, insufficient, or irrelevant; closed claimed reference-pack routes to `403 authorization_denied` for authenticated non-admins while preserving unclaimed-family `extension_profile_not_claimed` precedence. |
| `AC-428` | Core 01 §12.2.1; Core 04 §2 and §9.14 | Closed the recovery operator interface to exactly five logical CLI commands, exact output and progress schemas, timeout defaults and bounds, exit-code mapping, required operator error codes and reason codes, target-preflight failures, no timestamp restore, no interactive confirmation, encrypted recovery journal records, safe administrative-audit summaries, and the forbidden-output value set. |
...
- The remaining current-profile equivalence leak in performance and cursor conformance is no longer open. Core 04 now binds `perf.typing_ack.v1` to one exact Timeline summary typing acknowledgment surface and removes `equivalent deterministic test hook` language from AC-375, while Core 01 clarifies that live operational cursors are signed, authorization-rederived continuation tokens and reserves `cursor_snapshot_unavailable` for future explicit snapshot-backed route families.
...
- The workbook inspector configuration gap is no longer open for the current profile. Core 01 now owns `inspector_config_v1` as required view-schema metadata, including closed panel, route-binding, route-owner, disabled-state, result-behavior, feature-key, seed-binding, and exhaustive per-surface feature-group registries. Core 02 states that inspector configuration is not incident source state and introduces no inspector-state or workflow record family. Core 03 owns default-closed, saved-view inheritance, no-row, deterministic workflow execution, stale-retargeting, and focus/scroll continuity behavior. Core 04 owns registry completeness, feature-object validity, hot-path non-regression, stale-state invalidation, server-side authorization re-derivation, saved-view containment, optional-surface omission, same-shell execution, and the base-profile non-egress rule.
...
- Administrative audit read access is no longer open for the current profile. Core 01 now defines separate Base Profile read projections for deployment administration and incident membership administration, including exact resource shape, action-code and target-kind registries, filters, ordering, pagination, and cursor binding. Core 02 defines the immutable deployment-local persistence invariant, and Core 04 defines the authorization split, denial behavior, mandatory redaction, retention, backup, portability, and fail-closed legacy behavior. AC-437 through AC-440 close the route, query, authorization, and retention/redaction criteria.
