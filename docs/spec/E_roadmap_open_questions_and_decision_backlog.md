...
- The workbook inspector configuration gap is no longer open for the current profile. Core 01 now owns `inspector_config_v1` as required view-schema metadata, including closed panel and route-binding vocabularies plus the per-surface matrix. Core 02 states that inspector configuration is not incident source state and introduces no inspector-state or workflow record family. Core 03 owns default-closed, saved-view inheritance, no-row, stale-retargeting, and focus/scroll continuity behavior. Core 04 owns server-side authorization re-derivation and the base-profile non-egress rule.
