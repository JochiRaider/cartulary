...
When the inspector is open and no saved row is selected, it MUST enter the exact semantic no-row state `no_row_selected` and MUST render the exact ordinary-user sentence `Select a saved row to inspect its details.` The token `no_row_selected` remains the required `inspector_config_v1.no_row_state`, reducer and status identity, disabled-condition identity, semantic selector identity, and conformance-test anchor; it MUST NOT be rendered as ordinary visible heading or message copy. The no-row state MUST NOT display details, confirmations, previews, merge plans, rollback plans, local forms, or workflow state from a prior row.
...
- `parser_profile_id`, fixed to `cartulary.import.workbook.v1` for the current profile,
- `parser_version`, fixed to `workbook_import_adapter_v1` for the current profile,
- `assistant_profile`, fixed to `workbook_import_v1` for the current profile,
- `session_status`,
- `selected_unit_ids[]`,
