...
- **AC-422**: For a `closed` incident, incident list/get, workbook queries, record history, evidence preview/download, saved views, workbook preferences, incident membership administration, extension-authorized snapshot/report/release/export, and `reopen` remain allowed under their ordinary authorization rules, while incident metadata patch, row creation, record mutation, delete, restore, rollback, merge, supersede, conflict resolution, mention resolution, blob-slot creation, evidence attachment, import apply, and any job commit of authoritative incident source state fail or terminally fail with `409 error.code='incident_closed'`.
