...
- `TODO: assign next AC id`: Patching `timeline.host_refs` with `dismiss_item` marks the targeted `entity_mention` `dismissed` and removes or tombstones any derived resolved host link in the same `change_set`.
