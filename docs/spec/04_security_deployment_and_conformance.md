...
- **AC-537**: Database migration conformance proves that PostgreSQL connectivity, root and service binding resolution, generic query and transaction ports, and PostgreSQL telemetry remain in the PostgreSQL adapter while migration source inspection, typed apply-to-head execution, test-only targeted execution, lineage readiness, remediation, evidence, and migration recovery metadata exist only under the database-migrations owner; that no compatibility alias, forwarding package, duplicate implementation, generic Goose command surface, process-global migration filesystem, or process-global Goose logger remains; that migration code accepts only already-opened database handles and exposes no DSN, credential, secret reference, database root, server identity, SQL text, bind value, or upstream secret through diagnostics, logs, telemetry, remediation, evidence, or operator output; that all authored migration filenames, versions, order, bytes, hashes, and lineage identifiers remain unchanged; and that cancellation, empty-source behavior, per-invocation source/logger isolation, readiness reason codes, remediation/evidence schemas, recovery contribution identity, operator stream framing, and PostgreSQL telemetry privacy pass their exact owner-routed evidence.
