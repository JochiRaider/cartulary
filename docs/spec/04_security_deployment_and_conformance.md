...
| `perf.timeline_summary_focus_edit.v1` | `AC-043` | The same exact Timeline summary selection state holds; a single primary pointer click is accepted. | The editor for the same `record_id` and `field_key` is visibly active and focused with a collapsed caret at the rendered text end. | `cartulary.perf.large_grid.v1`; p95 |
...
| `perf.timeline_blank_row_create.v1` | `AC-043` | Timeline is loaded in its default state; a visible blank row receives its first qualifying non-empty summary input; automatic create admission is accepted. | The committed row is visibly painted with stable `record_id`, integer `row_version`, and the entered value in the target field. | `cartulary.perf.large_grid.v1`; p95 |
...
- **AC-561**: A clean install and valid upgrade create and deterministically
  backfill exactly the Parties active claims required by REQ-02-271; invalid
  or competing existing values block migration with bounded safe diagnostics
  and no selected winner or partial schema change. Concurrent same-key creates
  converge on one active Party. Every create, explicit create-from-text,
  patch, clear, delete, restore, rollback, Imports write, Incident Bundle
  import, and recovery path locks the complete old/current/proposed tuple set
...
- **AC-562**: The authored Party field registry and generated runtime/view
  ...
  clearability posture, contracts, scalar bounds, equality, claim, hash, and
  conflict metadata. Exact-boundary and max-plus-one fixtures cover each
  string; omission, JSON `null`, normalized-empty, wrong-type, control,
  malformed email, invalid Party-kind, and optional-clear matrices produce the
  one adopted result with no side effects. Every Party consumer obtains the
  same stored, equality, claim, and canonical-hash values, with no field-name
  or line-versus-note inference. `timezone_name_v1` uses only the packaged
  `iana.tzdb.2026c` registry, admits exact reviewed Zone and Link spellings,
  preserves admitted Links, rejects nonmembers and path-like values, and has
  no host-tzdb, locale, network, latest-version, or Network Flow dependency.
  RFC 8785 vectors and create, patch, and conflict-resolution goldens prove the
  named Party hash, including NFC, email-case, notes-line-ending, and
  external-reference-case consequences. The replay cutover removes exactly
  deterministically identified Party rows, preserves every unrelated row
  byte-for-byte, and leaves no caller-supplied hash, old hasher, fallback, or
  dual comparison.
...
- **AC-563**: Versions `3` and `4` are the admitted Incident Bundle versions;
  ...
  equality. Duplicate-aware preparation, fixed-column apply, exact
  transaction-state and claim validation, and deterministic invariant
  precedence each have direct causal positive, negative, permutation, and
  failure-injection evidence. Valid Party bytes import and semantically
  re-export without drift; cancellation or failure at every phase leaves no
...
- **AC-568**: Graph Projection boundary and current-contract evidence proves
  ...
  Network Flow major 7/state 4 admits v2 only; fresh state 4, both valid state-3
...
- **AC-480**: Saved-view selection, startup, create, update, duplicate, dirty comparison, and reset round-trip the exact `cartulary.layout.v2` semantic column permutation, hidden set, sparse widths, frozen-through field identity, and ordered sort list; the live grid applies them; all-columns-hidden recovery is keyboard accessible; widths remain within `40..4096`; and no structural grid column or vendor coordinate enters persistence. Ordinary sort activation replaces while Ctrl/Cmd activation adds, cycles, or removes and the eight-entry maximum fails closed before query dispatch.
  - Verifies: REQ-01-627, REQ-03-295..REQ-03-296
- **AC-480B**: Production shared Workbook grids freeze the complete visible leading
  semantic prefix, preserve hidden boundary identity and logical order, and
  recompute after reorder. Columns remains keyboard-operable with all fields
  hidden. At the design-owned 240px work-area boundary, freezing suspends and
  resumes deterministically without changing layout bytes, dirty state, widths,
...
- **AC-484**: Each closed grid data state and interaction mode has exact message posture, action availability, ARIA behavior, authorized-row and draft preservation rules, and mutation permissions. Permission loss clears protected rows; refreshing and stale errors retain prior authorized rows; closed incidents display `Closed, read-only`, permit read and copy, and dispatch no editor, paste, fill, create, or bulk mutation. State presentation uses no fake records. A confirmed `incident_access_revoked` terminal clears protected incident content and immediately returns to the directory while the account session and access to other incidents remain usable without login. Failed independent session refresh neither reopens the incident nor implies logout; actual session revocation still requires reauthentication. Duplicate terminal/close events, invalid reasons, foreign incidents and obsolete socket/account callbacks cannot repeat or misdirect these effects. Uncertain authorization keeps protected work suspended until authoritative recovery.
  - Verifies: REQ-03-299
...
- **AC-413**: A default runtime exposes no `/api/v1/test/*` or `/ws/v1/test/*` route. When test routes are enabled without the harness-owned marker or without a valid test-route token, startup fails closed before serving those routes. In a harness-owned runtime, missing or wrong `X-Cartulary-Test-Route-Token` values return `403 error.code='test_route_forbidden'` for every test-only harness route adopted by Testing Harness NLSpec Section 12, including clock controls, runtime identity, public-error fault control, Network Flow runtime controls, saved-view system fixtures, auth touch, timeline snapshot, and test WebSocket routes; session cookies, bearer sessions, bootstrap tokens, incident roles, and `deployment_admin` do not bypass that token requirement. The retired runtime-reset path is not found because browser reset is an out-of-process backend-replacement lifecycle. If a harness API or browser origin is configured, wrong host, missing origin, malformed origin, or unapproved origin requests fail before any runtime-control mutation, and the response does not enable permissive CORS.
  - Verifies: REQ-04-109
...
- **AC-569**: All 12 contextual coordination actions declare the create-only source seed and retain raw authoring through source/version/selection/presentation changes. Retained or explicitly replaced sources create exactly one atomic `references_artifact` association; explicit clear creates none. No semantic target reference, minimum signal, source autosave, lifecycle state or history is manufactured. Invalid source/target pairs and foreign/deleted sources fail atomically without disclosure. Complete linked receipts survive detachment and exact replay; response loss and malformed success remain uncertain; refresh recovery performs reads only. Existing ordinary-create hashes and linked-Note replay remain compatible. Incident closure/reopen, role loss, incident/session revocation, uncertain authorization and account replacement preserve their current concealment and lifetime boundaries.
  - Verifies: REQ-01-311, REQ-01-503..REQ-01-506, REQ-01-674, REQ-03-256, REQ-03-258, REQ-03-299..REQ-03-302, REQ-04-022..REQ-04-023
...
- **AC-487**: Two exports from identical authoritative incident state and
  identical normalized export inputs produce byte-identical version `4`
