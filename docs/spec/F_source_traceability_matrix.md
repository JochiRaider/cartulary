...
| `8. Record lifecycle and IR workflow model` | Core 03, Appendix D, Appendix G | Lifecycle, timeline create flow, evidence attach flow, pending-blob timeout and cleanup defaults, resolution flow, rollback flow, paste/import behavior, and auto-resolution policy promoted. |
...
| Cross-cutting client/server interface surface for workbook, evidence, and jobs | Core 01 §3.3 and §3.3.10.1, Core 03 §3.3.4, §4.3, §4.3.1, §8, and §15, Core 04 §1, §2, §4.5, and §9.10, Appendix E | Closed as one versioned HTTP+JSON API plus a bounded incident-scoped WebSocket stream using stable identifiers, session-based auth, generic envelopes, cursor pagination, explicit blob-slot and job routes, concrete blob-slot expiry metadata and lease semantics, and a concrete v1 collaboration protocol with a common envelope, typed handshake and presence messages, replayable-versus-ephemeral delivery classes, bounded resume and reset behavior, application-level heartbeats, origin validation for cookie-authenticated browser sockets, and per-incident ordering. |
...
| `9. How denormalized entity/evidence views are composed` | Core 01 §7.4, Appendix D, Appendix G | Promoted as the authoritative Hosts, Identities, Evidence, Notes, Indicators, Assessments, Task Requests, and Decisions view-schema instances in the base-profile registry. |
| `11. Reference-pack and export trust boundaries` | Core 04 §4, Appendix G | Promoted as trust-boundary requirements. |
| `4. Backup, restore, portability, failure modes` | Core 00 §4.2 and §6, Core 01 §12.3 and §13, Core 04 §4.2 and §9.9, Appendix E, and Appendix G | Closed as a separate Incident Portability Extension Profile with a deterministic logical bundle layout, authoritative-source-only export, checksum-verified import, portable actor descriptors, optional embedded snapshot/reference-pack sections, and no-partial-visibility failure semantics. |
