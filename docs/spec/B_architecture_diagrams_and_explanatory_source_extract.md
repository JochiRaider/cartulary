...
The exploratory source drew one boundary especially sharply: clipboard interaction stays on the core workbook hot path, while file-based import sits behind a dedicated `imports` module. Clipboard, CSV, and XLSX adapters can still normalize into the same canonical `TabularSource` and shared mapping engine, but parser drift, workbook-shape heuristics, preview/header mapping, and other spreadsheet-compatibility maintenance sit with the imports module in that realization. The resolved implementation direction keeps spreadsheet compatibility at discovery, preview, mapping, warning, provenance, and apply-dispatch boundaries; source-domain creation remains behind owner create facades rather than workbook compatibility stores or parser-shaped DTOs.
