...
**REQ-01-183**
The request MUST decode as exactly one duplicate-free JSON object. A malformed
body, scalar, top-level `null`, duplicate member at any nesting level, or
trailing JSON value or token MUST fail with `400 Bad Request`,
`error.code=invalid_mutation_payload`, and
`error.details.reason_code=request_not_object`, with no mutation, projection,
history, idempotency-success, or collaboration effect. The object MUST include:
...
**REQ-01-592**
When a fresh incident `close` state transition commits, and only after that commit
succeeds, every currently connected `/ws/v1/incidents/{incident_id}` collaboration
socket for that incident MUST receive one terminal `error` message with
`payload.code = incident_closed`, `payload.retryable = false`, and then close. Exact
idempotency replay, rollback, commit failure, pre-commit failure, and any rejected or
no-op close MUST produce no terminal effect. The committed administrative-audit event
identifier MUST be the internal effect identity so a replay cannot be mistaken for a
fresh effect. Delivery is process-local under Core 04 REQ-04-145's contract for
exactly one active application process and creates no durable delivery or retry
guarantee.

A new connection or resume attempt for an already closed incident MAY authenticate
and authorize the member for ordinary HTTP reads, but it MUST NOT establish a writable
collaboration subscription, MUST NOT emit `hello_ack` or `resume_ack` that advertises
writable collaboration state, and MUST terminate with the same terminal `error` family
using `code = incident_closed`.
Profiles: base
