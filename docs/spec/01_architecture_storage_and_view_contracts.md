...
A successful create response MUST include `incident_id`, `object_blob_id`, `upload_state`, `target_expires_at`, `pending_expires_at`, the short-lived `upload_target`, and `accepted_contract`. In the base profile, `upload_target` MUST be an opaque same-origin `PUT /api/v1/object-uploads/{upload_token}` capability and MUST NOT expose raw bucket names, storage keys, presigned object-store query parameters, or object-store hostnames. `accepted_contract` MUST echo the server-accepted normalized values for `incident_id`, `byte_size`, `filename_hint`, `content_type_hint`, and `sha256_hex`. Omitted optional members MUST be serialized as explicit `null` inside `accepted_contract` rather than by field omission.
Profiles: base
...
- writable fields:
  ...
  - `evidence.storage_ref`: read `storage_ref`; write target `evidence_records.storage_ref`; `string_contract_id=locator_text_v1`; `conflict_resolution_class=atomic_replace`
    ...
    - User-authored external locators remain governed by `locator_text_v1` and MAY be stored when they do not match the reserved server-managed form. Object-store migrations MUST NOT rewrite, normalize, or reinterpret existing non-server external locators.
...
Provider descriptors MUST identify `provider_id`, descriptor `schema_version`, `source_owner_module`, `projection_storage_owner_module`, `view_schema_ids`, `projection_table_ids`, `source_authorities`, explicit `capabilities`, `restore_rebuild` participation, provider `status`, and approved `facade_packages` before boundary guard enforcement. `source_owner_module` identifies the module that owns source semantics and query descriptor intent. `projection_storage_owner_module` identifies the module that owns the physical derived projection storage lifecycle. Descriptors MUST NOT define public query semantics independently of the route/viewquery contract in §3.3.4.

The current descriptor schema is `projection_provider_descriptor.v2`. Current descriptor `status` values are exactly `active`, `deprecated`, and `experimental`. Current `restore_rebuild` values are exactly `required`, `nonparticipating`, and `unsupported`. Current descriptor capabilities are exactly `query`, `refresh_row`, `incident_rebuild`, and `restore_rebuild`; omitted capabilities are invalid in code-backed descriptors and invalid in canonical validation manifests.
