...
| Evidence-access handle issuance and redemption | Core 01 §16, with route-family declaration in Core 01 §3.3.8 | Core 02 §13 and §18; Core 03 §8.4; Core 04 §2, §4.3, §4.5, and §9.10; Appendix F | Core 01 owns issuance request and response shape, redemption semantics, handle lifetime, revocation, filename and disposition rules, and route-specific error use. Core 02 owns lifecycle state, object metadata, and exact token sets only. Core 03 owns same-surface preview and blocked-state UI only. Core 04 owns authorization re-derivation, active-content blocking, and fail-closed behavior only. |  |  |  |
