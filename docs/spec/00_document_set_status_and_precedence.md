...
This revision records a behavior-affecting closure for deployment administration entry, imported-incident initial access, Reference Pack list search and filters, prohibited aggregate administration concepts, the Timeline operational-field refactor, and view-schema-owned workbook inspector configuration. With those contracts adopted in Core 00 through Core 04, current-profile behavior, profile boundaries, and implementation-conformance scope are closed again here.
...
Adopted subsystem NLSpecs derived from this core MAY define bounded implementation-conformance requirements for their named subsystem only. Each adopted subsystem NLSpec MUST state its scope, non-goals, owner interactions, and whether it adds deployment-configuration keys. An adopted telemetry NLSpec owns telemetry generation, telemetry configuration, telemetry export, resource identity, privacy, and telemetry verification only. It does not redefine Core 00 through Core 04 product behavior or Core 05 claim-publication behavior unless a later Core revision explicitly says so.
...
| Operational backup, coherent restore, retention floor, equivalence, restore verification, and operator recovery CLI | Core 01 §12.1 and §12.2 | Core 04 §2, §4.4, §4.5, §6, §9.0.1, §9.14, and §12; Appendix B; Appendix E; Appendix F | Core 01 owns `backup_set`, `backup_attestation`, `consistency_point_at`, backup creation admission and publication, failed-candidate non-publication, latest-success selection stability, minimum artifact-set requirements, coherent restore semantics, restore order, retention floor, restore verification, equivalence criteria, separation from incident portability, logical operator recovery command grammar, backup selection rules, operator recovery output schemas, timeout defaults and bounds, exit codes, operator error vocabulary, and the boundary that backup scheduling is deployment-owned external orchestration. Core 04 owns only deployment-local operator authorization, no-listener and no-browser trust boundaries, recovery-operation exclusion, restore-target preflight, backup-storage binding, encrypted-root consequences, operator output redaction, encrypted recovery journal requirements, safe administrative-audit summaries, and pass/fail conformance criteria. Appendices MAY describe realization examples, operator checklists, roadmap notes, or traceability only. | REQ-00-023 | base | AC-231 |
...
| Reference Pack list-query search/filter contract | Core 01 §17.4 | Core 04 §2 and §9.4; Appendix D; Appendix E; Appendix F; `docs/design.md`; UI/UX guide | Core 01 owns the accepted query members, source fields, exact filters, ordering, complete-collection evaluation, cursor binding, and route-specific list-query failures for `GET /api/v1/reference-packs`. Core 04 owns only authorization and conformance. Appendices, design material, and guides MAY describe UI request sequencing and presentation only. | REQ-00-059 | reference_pack | AC-443 |
...
| Inspector configuration and row-context workflow entry | Core 01 §6 and §7.4 for discovery shape, `inspector_config_v1`, route-binding vocabulary, and per-surface matrix; Core 03 §2.3 and §10.1 for interaction behavior | Core 02 §2 for source-state boundary; Core 04 §2 for authorization and egress security; appendices and guides for non-normative usage and design only; Core 05 only for claim-bearing timing if later asserted | Core 01 owns view-schema metadata shape and emitted config validity. Core 02 owns the statement that inspector configuration is not incident source state and introduces no record family. Core 03 owns default-closed behavior, saved-view inheritance, no-row state, stale row-bound invalidation, scroll separation, and continuity. Core 04 owns authorization re-derivation and base-profile non-egress. Appendices and guides MAY illustrate usage but MUST NOT create required workflow cadence, per-edit rituals, authorization behavior, or route semantics. | REQ-00-061 | base | AC-453 |
...
| Enterprise-auth provider manifest, startup validation, and provider-definition reconciliation | Core 04 §12.3.4 and §12.6 | Core 01 §20; Core 02 §14.1; Core 04 §1.2 and §9.5; Appendix B; Appendix C; Appendix D; Appendix E; Appendix F; implementation guides | Core 04 owns the `enterprise_authentication.provider_manifest_path` deployment-configuration key, manifest schema, defaults, omitted-versus-`null` behavior, bounds, referenced-file and `secret_ref_v1` validation, fail-closed startup diagnostics, and deterministic provider-definition reconciliation. Core 01 owns only the public route consequences, including safe provider discovery and the absence of runtime provider-definition mutation routes. Core 02 owns only deployment-local provider-definition persistence invariants. Appendices and guides MAY illustrate, trace, or support implementation but MUST NOT widen the runtime provider configuration surface. | REQ-00-055 | enterprise_authentication | AC-433..AC-436 |
