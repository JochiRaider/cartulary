...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `base` | Selected by algorithm above. | Incident identity, built-in primary tabs, `System views`, current surface title, presence summary. | Full active-surface controls. | Primary and visible while inspector is open. | Adjacent right panel. | Visible primary save label and secondary message. | Claimed. |
| `narrow_desktop` | Selected by algorithm above. | Incident identity, `Surfaces`, `System views`, current surface title, presence summary. | Full active-surface controls plus one active-chip overflow row. | Primary; visible when overlays are closed. | Full-height right overlay; grid inert behind overlay. | Visible primary save label and secondary message. | Claimed. |
| `compact_desktop` | Selected by algorithm above. | Incident identity, `Surfaces`, `System views`, current surface title. | Active-surface controls remain reachable; chips move to `Filters` popover. | Primary when overlays are closed. | Full-screen or full-height overlay; grid inert behind overlay. | Visible primary save label; presence summary assigned here. | Claimed. |
...
| Sort control | View bar | View bar | View bar | Not required |
| Group control | View bar | View bar | View bar | Not required |
| Filter control | View bar | View bar | View bar as `Filters` popover | Not required |
...
| Filter control | No filters. | Filter chips shown or `Filters` overflow. | Invalid filter chip marked and excluded from query submission. | Clears all filters or one selected chip. | 5 |
...
| `D-AC-035` | §8.3 | View-bar test | View bar control order, chip order, and saved-view dirty state are deterministic. | Control ordering or dirty state differs across implementations. |
