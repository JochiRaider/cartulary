...
Design contract. Each density mode MUST define its row rhythm through `rowHeight`, `cellPadding`, `fontSize`, and `lineHeight`. `cellPadding` is the effective logical inset of rendered grid content. Adapter-private outer cells MAY reserve or receive structural padding for virtualization, row-span positioning, or vendor layout mechanics, but that structural padding MUST NOT consume, replace, or redefine the visible density inset. Workbook grid containers, owned cell-content wrappers, row-gutter content, read-only cell anchors, and grid-mode cell editors MUST inherit those density metrics from the active density mode. Density changes MUST scale text and controls together rather than only changing row height.
...
## 8. Grid, view-bar query controls, and workbook interaction
...
#### Workbook Find presentation

Core 03 §13.5 owns matching and lifecycle. Each active Timeline, Hosts or Identities
surface exposes one Find entry that opens
...
that only committed values and displayed collection items in loaded rows are
...
On Hosts and Identities, ordinary Inspector scalar and alias drafts keep their
explicit submission controls and original subject while Find borrows focus or
moves to a match. Grid scalar rejection keeps the original editor and local
feedback; Find does not introduce another save or recovery presentation.
...
| `Ctrl/Cmd+F` | Timeline, Hosts or Identities `grid_navigation`; Timeline `grid_range_selection` | Grid navigation owns the event. | Open or refocus Find under Core 03 §13.5. | Borrow focus without submitting or discarding authoring. | Yes, only in the owned context. |
...
§13.5. Authority retirement clears protected Find state without announcing the
retired search term or cell contents.
...
| `D-AC-035` | §8.3 | Shell-control test | View-bar query control order, chip order, and saved-view dirty state are deterministic. | Control ordering or dirty state differs across implementations. |
...
| `D-AC-076` | §4.6 | Authenticated-root cardinality test | Zero visible incidents renders a successful empty directory; one and multiple visible incidents remain in the directory until explicit selection. | A count case is missing, produces an error, or opens an incident without explicit selection. |
