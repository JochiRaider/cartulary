...
- `make test`: run the authoritative full corpus, including manifest-verified browser E2E and explicit supplemental support suites. Backend service-backed work and webserver-backed browser evidence should run through one service-backed stage scheduled from `tools/service_backed_schedule_manifest.json` by declared resource capacity; authoritative browser functional rows are selected from phase manifests but executed as duration-balanced Playwright spec shards using `tools/browser_e2e_duration_baselines.json`. Aggregate target summaries are finalized after the `cartulary-test-services` wrapper has completed teardown so leak-check, janitor, and service-termination spans remain visible; the final `browser-e2e` owned stack should then run only the isolated stateful, measurement, and visual browser batches with explicit reset boundaries. Summary artifacts report wall, critical-path wall, executed, logical, reused, derived, and teardown durations as separate fields, with backend service-backed and aggregate browser duration groups reported separately.
- `make lint`: run Go vet or lint plus frontend lint and type-check.
- `make check`: run the full developer gate and fail if any authoritative phase-manifest row is absent from execution. Keep toolchain drift and frontend install as early setup blockers, then let `tools/check_schedule_manifest.json` drive the remaining developer-gate DAG by declared resources rather than by fixed serial phases. The readiness gate prepares build artifacts and service images before the independent check work starts. `check-service-backed` schedules service-backed backend phases and webserver-backed browser evidence. Static validation, harness smoke, local product checks, frontend unit work, and Go duration-baseline drift remain separate check-scheduler work units. The top-level `browser-e2e` work unit claims the service stack and runs after those heavy check units have completed, so isolated stateful, measurement, and visual browser suites run after the gate quiesces. Finalize the service-backed aggregate summary after wrapper teardown and fail on hidden lifecycle teardown failures as non-test failures. Browser phase expansion is split by execution dependency: webserver-backed functional rows are planned as duration-balanced spec shards, while isolated ordering and reset policy stay in `tools/browser_e2e_batch_manifest.json`. Service-backed schedule v6 expands manifest-declared work-unit sources into Go shards and explicit Make targets, derives Go shard `go_cpu` and `go_io` resource profiles from committed timing and fixture metadata, and keeps schedule-level `resource_limits` as the concurrency authority.
  The check and service-backed schedulers print concise start, progress, and summary lines by default; raw resource maps and per-transition details are reserved for scheduler JSON artifacts and `VERBOSE=1` or `CI_VERBOSE=1`.
