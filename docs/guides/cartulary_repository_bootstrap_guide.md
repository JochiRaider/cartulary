...
- `make check`: run the full developer gate and fail if any authoritative phase-manifest row is absent from execution. Keep toolchain drift and frontend install as early setup blockers, run parallel-safe static validation, harness smoke, and product checks together after setup passes, run service-backed backend phases through the weighted capacity-aware service-backed scheduler, finalize the service-backed aggregate summary after wrapper teardown, fail on hidden lifecycle teardown failures as non-test failures, and run all browser suites afterward through one `browser-e2e` owned-stack batch. Browser phase expansion belongs in `tools/browser_e2e_batch_manifest.json`, not in backend service-backed schedules. For v2 service-backed schedules, `--jobs` is the active global concurrency cap, and `SERVICE_BACKED_JOBS` supplies that cap for the standard service-backed developer-gate schedules.
