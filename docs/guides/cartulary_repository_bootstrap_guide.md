...
- `make check`: run the full developer gate and fail if any authoritative phase-manifest row is absent from execution. Keep toolchain drift and frontend install as early setup blockers, then let `tools/execution_topology_manifest.json` expand reusable check schedule profiles plus per-target `check_schedule` metadata into `tools/check_schedule_manifest.json`, which drives the remaining developer-gate DAG by resources declared in `tools/scheduler_resource_registry.json` rather than by fixed serial phases. The readiness gate prepares build artifacts and service images before service-backed work and build-dependent checks start, while bounded host CPU claims leave room for independent validation work. `check-service-backed` is a nested scheduler work unit: the parent check scheduler claims the exclusive `service_stack` lane plus a bounded `host_cpu` and `host_io` budget for that unit, forwards those bounded limits through `check_host_to_service_backed_go` to service-backed `go_cpu` and `go_io`, and lets `tools/execution_topology_manifest.json` own backend and browser shard placement through the generated `tools/service_backed_schedule_manifest.json` while cheap independent check work can run beside it. Migration verification remains a separate check-scheduler work unit, but it applies migrations against scratch Postgres databases and therefore declares Postgres service requirements and claims the same exclusive `service_stack` lane. Static validation leaves, backend lint, script lint, pure backend unit evidence, frontend type-checking, frontend unit evidence, harness smoke, and generated-artifact drift are separate check-scheduler work units; duration-baseline drift checks remain downstream of `check-service-backed` because they consume its timing artifacts. Finalize the service-backed aggregate summary after wrapper teardown and fail on hidden lifecycle teardown failures as non-test failures. Browser phase expansion is split by execution dependency: webserver-backed functional rows are planned as duration-balanced Playwright spec shards, while isolated ordering and reset policy stay in the topology-derived `tools/browser_e2e_batch_manifest.json`. `tools/execution_topology_manifest.json` renders service-backed schedule v8, which expands manifest-declared work-unit sources into Go shards and explicit Make targets, derives Go shard `go_cpu` and `go_io` resource profiles from committed timing and fixture metadata, derives explicit Make-target weights from `tools/service_backed_make_target_duration_baselines.json`, and keeps schedule-level `resource_limits` as the concurrency authority.
  The check and service-backed schedulers print concise work-unit start and summary lines plus operator-oriented `[PROGRESS]` lines by default, while run summaries distinguish scheduler work units from evidence summary targets. Quiet progress states the completed work count, nested scheduler count when present, currently running work, blocker, next unblocker, slowest running work, and scheduler artifact path in human-readable text. Raw key/value progress, per-shard `[STEP]` lines, raw resource maps, and per-transition details are reserved for scheduler JSON artifacts, `CARTULARY_OUTPUT_MODE=machine`, `VERBOSE=1`, or `CI_VERBOSE=1`.
