...
- `make test`: run the authoritative full corpus, including manifest-verified browser E2E and explicit supplemental support suites. Backend service-backed work and web-server-backed browser E2E should share one service-backed stage scheduled from `tools/service_backed_schedule_manifest.json` by declared resource capacity, with separately reported backend and browser duration groups; service-backed aggregate target summaries are finalized after the `cartulary-test-services` wrapper has completed teardown so leak-check, owned-stack fixture-reclaim, janitor, and service-termination spans remain visible; summary artifacts report wall, critical-path wall, executed, logical, reused, derived, and teardown durations as separate fields; stateful, measurement, and visual browser suites should run afterward in one manifest-driven isolated browser batch with explicit reset boundaries.
- `make lint`: run Go vet or lint plus frontend lint and type-check.
- `make check`: run the full developer gate and fail if any authoritative phase-manifest row is absent from execution. Keep toolchain drift and frontend install as early setup blockers, run parallel-safe static validation, harness smoke, and product checks together after setup passes, run service-backed backend phases and the shared-stack browser suite through the weighted capacity-aware service-backed scheduler, finalize the service-backed aggregate summary after wrapper teardown, report backend service-backed, browser webserver-backed, and isolated browser duration groups separately, fail on hidden lifecycle teardown failures as non-test failures, and run stateful or timing-sensitive browser suites afterward through one isolated owned-stack batch. Owned service-backed browser fixtures are reclaimed by terminating the ephemeral Postgres and MinIO stack after a bounded leak check; explicit DB and bucket cleanup remains the fallback for stale, pass-through, and prepare-failure fixtures. `SERVICE_BACKED_JOBS` remains a legacy scheduler compatibility knob and is not the active concurrency cap for v2 resource-capacity schedules.
