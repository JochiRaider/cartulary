...
- `make check`: run the full developer gate and fail if any authoritative phase-manifest row is absent from execution. Keep toolchain drift and frontend install as early setup blockers, then let `tools/check_schedule_manifest.json` drive the remaining developer-gate DAG by resources declared in `tools/scheduler_resource_registry.json` rather than by fixed serial phases. The readiness gate prepares build artifacts and service images before the independent check work starts. `check-service-backed` is a nested scheduler work unit: the parent check scheduler claims the exclusive `service_stack` lane plus a bounded `host_cpu` and `host_io` budget for that unit, forwards those bounded limits through `check_host_to_service_backed_go` to service-backed `go_cpu` and `go_io`, and lets `tools/service_backed_schedule_manifest.json` own backend and browser shard placement while cheap independent check work can run beside it. Static validation, harness smoke, local product checks, frontend unit work, and Go duration-baseline drift remain separate check-scheduler work units; duration-baseline drift checks remain downstream of `check-service-backed` because they consume its timing artifacts. Finalize the service-backed aggregate summary after wrapper teardown and fail on hidden lifecycle teardown failures as non-test failures. Browser phase expansion is split by execution dependency: webserver-backed functional rows are planned as duration-balanced spec shards, while isolated ordering and reset policy stay in `tools/browser_e2e_batch_manifest.json`. Service-backed schedule v8 expands manifest-declared work-unit sources into Go shards and explicit Make targets, derives Go shard `go_cpu` and `go_io` resource profiles from committed timing and fixture metadata, and keeps schedule-level `resource_limits` as the concurrency authority.
