...
- Duration-baseline refresh and drift commands belong to maintenance with generated-artifact, toolchain, migration, phase-ledger, and benchmark-claim checks. Task-surface, scheduler, browser, and topology generation and drift are owned by `make generate` and `make generate-drift`.
