...
- `make check`: run the full developer gate and fail if any authoritative phase-manifest row is absent from execution. Keep toolchain drift and frontend install as early setup blockers, prepare build artifacts and service images as the readiness gate, then start service-backed backend phases and webserver-backed browser evidence through the weighted capacity-aware scheduler while parallel-safe static validation, harness smoke, and product checks continue concurrently. Finalize the service-backed aggregate summary after wrapper teardown, fail on hidden lifecycle teardown failures as non-test failures, and run only isolated browser suites afterward through one `browser-e2e` owned-stack batch. Browser phase expansion is split by execution dependency: webserver-backed rows belong in scheduler-safe browser sources, while final isolated rows stay in `tools/browser_e2e_batch_manifest.json`. Service-backed schedule v4 expands manifest-declared work-unit sources into Go shards and explicit Make targets, and schedule-level `resource_limits` are the concurrency authority.
