...
| Topic | Owner | Owner locator | Applicability | Current summary | Omission/unavailable behavior | Guide consequence |
| --- | --- | --- | --- | --- | --- | --- |
| Authenticated root | Core 01 and `design.md` | Core 01 §3.3.2.1A; `design.md` §4.6 | `base` | Zero visible incidents produces a successful empty directory; one opens the sole workbook without an explicit launch `sheet_ref`; two or more remain in the directory until selection. | Permission loss clears protected materialization and returns to `/`; no inaccessible incident is retained as visible content. | Treat the root as incident selection, never as a dashboard with inferred priority. |
...
| `UXG-RP-AC-013` | Root-composition review | §5 restates zero, one, and multiple incident outcomes without adding selection logic. | A cardinality is absent or extra logic appears. |
