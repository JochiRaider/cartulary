...
Failure summaries must name the most specific available owner: `work_unit` for scheduler or sequence work, `child_target` for aggregate evidence, `failure_class`, `failure_origin`, duration, artifact root, summary JSON, log artifact, rerun command, and investigation command. Children skipped because a scheduler stopped after an earlier failure are skipped evidence with preserved skip provenance, not missing artifact failures. Use `-` only when the identifier or artifact genuinely does not exist.
