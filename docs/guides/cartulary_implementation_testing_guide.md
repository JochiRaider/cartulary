...
`CARTULARY_OUTPUT_MODE=machine` is a strict machine contract: each top-level command emits exactly one parseable `cartulary.tool_run_summary.v1` JSON object or one parseable JSON pointer, and it must not stream raw child output, scheduler progress prose, or duplicate run/target JSON objects. The canonical summary requires non-null RFC3339 `started_at` and `completed_at` timestamps, `summary_artifacts[]`, `log_artifacts[]`, work-unit or evidence-target accounting, `failure_class`, `failure_origin`, rerun commands, and investigation commands.

Failure summaries must name the most specific available owner: `work_unit` for scheduler or sequence work, `child_target` for aggregate evidence, `failure_class`, `failure_origin`, duration, artifact root, summary JSON, log artifact, rerun command, and investigation command. Use `-` only when the identifier or artifact genuinely does not exist.
