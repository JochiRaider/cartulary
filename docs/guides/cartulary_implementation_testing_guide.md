...
Runner summaries use the following accounting buckets. `authoritative` is normative owner evidence with semantic row IDs. `support` is explicitly cataloged regression evidence. `raw` is a declared aggregate suite owned by the catalog and execution topology. `tooling_support` is cataloged helper, harness, configuration, or runner-support coverage. `unowned_regression` is intentional product regression coverage that is not normative conformance evidence. `unmapped` is reserved for unexpected executed tests that have no exact catalog selector. Public Make-owned verification targets must finish with `unmapped=0`; an otherwise passing target with unmapped executed tests fails as `failure_class="harness"` and `failure_reason="test_accounting_unmapped"`. Add or revise the owning family manifest under `tools/test_families/`; do not add filename heuristics or a second classification registry. Use `make explain-run RESULTS_DIR=<run-root> TARGET=<target> DETAIL=accounting` to group retained inventory during triage.
...
| U-6-09 | Save-state labels use only `Syncing`, `Saved`, and `Conflict`; secure client transaction IDs remain stable for uncertain replay; the local pending queue is FIFO and bounded; a transaction blocker can be atomically re-keyed or locally discarded only through its typed recovery transition; later work is retained; and retry still uses current row-version and same-field checks. | REQ-03-072, REQ-03-089, REQ-03-095, REQ-03-099..REQ-03-100, REQ-03-301..REQ-03-302 | AC-376..AC-382, AC-486                         |
...
| E-6-05 | Within one browser runtime, queued unsent writes survive transient disconnect or session revocation, replay in FIFO order after re-authentication, and halt on the first blocker. Browser mutations use collision-resistant opaque transaction IDs across workbook remounts; a `client_txn_conflict` exposes actionable retry and discard; retry uses a fresh ID and current committed row version; a genuine same-field conflict enters the normal resolver; discard restores committed display without a network mutation; and queued work is never silently restored after a full reload. | REQ-01-070, REQ-01-250..REQ-01-277, REQ-03-099..REQ-03-100, REQ-03-301..REQ-03-302 | AC-377..AC-382, AC-486 |
...
| E-12-NFAC050-50 | Real production-grid Network Flow measurement for NF-AC-050: the supported load envelope is exercised without a timing threshold and remains engineering evidence unless Core 05 claim-publication requirements are separately satisfied. | NF-REQ-180; Network Flow Table 23-A | NF-AC-050 |
...
| `U-GRID-03` | Unit              | Group headers, loading rows, spacer rows, and presentation-only rows never emit mutation-capable events; the private RDG bottom-summary channel realizes only the recordless create draft and does not establish a summary API. |
...
| `E-GRID-03` | E2E               | Keyboard entry, exit, active-cell restoration, header navigation, and bottom-draft transitions preserve or intentionally clear Cartulary anchors; RTL remains deferred and disabled.            |
...
The delivery tables above remain implementation-sequencing history and requirement-orientation aids. They are not executable test ownership, selection, or evidence-accounting inputs.

`tools/test_catalog_owner.json` and `tools/test_families/*.json` own the executable cross-runner row inventory. Every active row has one semantic owner and family, stable verification references, exact runner selector, evidence class, claim posture, runtime profile, resource profile, fixture profile, and default-check declaration. Catalog validation, owner diagnostics, and retained evidence replace checked-in Markdown coverage ledgers. A delivery table may help a reader find product context, but filename, delivery milestone, package, and runner never determine ownership.

When a change adds or promotes an authoritative service-backed Go owner row, refresh `tools/go_test_duration_baselines.json` from uncontaminated successful service-backed evidence in the same change. Do not rely on fallback weights for newly declared rows; missing explicit baselines remain a maintenance failure.

Execution topology is owned separately by `tools/execution_topology_manifest.json`, which supplies scheduler work-unit structure and the closed runtime, resource, and fixture profiles referenced by catalog rows. Public target identity, output class, artifact policy, schema policy, command identity, lifecycle state, and side-effect declarations remain owned by the testing harness public registry and `tools/task_surface_owner.json`. Future test growth starts with the verification owner and one catalog row; execution topology changes only when a genuinely new profile, scheduler resource, gate policy, or browser reset boundary is required.
