...
- Frontend accessibility row evidence MUST use `make browser-e2e-a11y` and the retained `cartulary.frontend_accessibility_summary.v2` artifact. Accessibility rows remain blocked from frontend phase completion until the target exists, emits the summary artifact, and maps each implemented `FE-A11Y-*` row to executed scenarios. Blocked future-row smoke MAY run through `make browser-e2e-a11y-preflight` and `cartulary.frontend_accessibility_preflight_summary.v1`, but that preflight evidence does not complete blocked rows.
...
`make browser-e2e-a11y-preflight` is an explicit helper target for blocked future-row smoke. Its `cartulary.frontend_accessibility_preflight_summary.v1` artifact may list blocked rows and smoke scenarios, but it is not row-completion evidence.
...
1. Every phase row executes in its intended layer: unit rows in frontend unit/typecheck targets, integration rows in frontend integration-style or browser-backed targets, browser rows in Playwright/browser targets, visual rows in `make browser-e2e-visual`, implemented accessibility rows in `make browser-e2e-a11y`, blocked accessibility smoke in `make browser-e2e-a11y-preflight`, and support rows in their named support targets.
...
12. Accessibility row evidence exists for every implemented phase row that introduces user-visible UI and is represented in `cartulary.frontend_accessibility_summary.v2`; blocked rows MAY appear only in preflight evidence.
...
| FE-GUIDE-AC-028 | `make browser-e2e-a11y` is the implemented accessibility closure target; `make browser-e2e-a11y-preflight` is limited to blocked future-row smoke. |
