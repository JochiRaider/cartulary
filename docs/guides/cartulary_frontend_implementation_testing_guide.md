...
| Field | Value |
| --- | --- |
| Status | Frontend implementation and verification contract. This guide is authoritative only for frontend planning and verification artifacts; it is not product-behavior authority. |
| Document class | `frontend_verification_contract` |
| Schema family | `cartulary.frontend_verification_contract.v1` |
...
| Owned scope | Frontend phase registry, frontend phase maps, frontend row accounting, frontend coverage ledgers, frontend visual fixture registry, accessibility-readiness mapping, claim-publication-intent routing, and guide-local support criteria. |
| Non-owned scope | Core product behavior, route semantics, authorization, storage, runtime workflow, and Core 05 publication criteria. |
...
Frontend verification contract. When sources differ, follow this hierarchy. When two owner sections appear to conflict, record the contradiction as a corpus defect and do not pick a side. When an owner reference cannot be located, set the row to `claim_status="blocked"`, add an explicit `blockers[]` object, and exclude the affected row from authoritative completion until resolved.
...
### 1.3A Normative language and statement classes

Frontend verification contract. `MUST`, `MUST NOT`, and `MAY` are normative only inside this guide's owned frontend verification boundary. `default` defines the value or behavior used when an omitted member is allowed. Guide-owned contract text does not create product behavior unless the same behavior is owned by Core 00 through Core 04 or by an adopted NLSpec.

Frontend verification contract. Guide-owned contract text uses these statement classes:

| Statement class | Required meaning |
| --- | --- |
| `Frontend verification contract.` | Binding guide-owned requirement for frontend planning, evidence, schemas, target mapping, drift checks, and readiness accounting. |
| `Core restatement.` | Non-authoritative summary of Core 00 through Core 04 behavior. Must name the owner section and requirement IDs when applicable. |
| `Harness restatement.` | Non-authoritative summary of adopted Testing Harness NLSpec mechanics. Must name the harness section or requirement when applicable. |
| `Design-direction import.` | Non-product-conformance design or UI evidence imported from `docs/design.md`, the UI/UX guide, or the visual golden guide. |
| `Implementation-support contract.` | Binding guide-owned support rule that does not define product behavior. |
| `Source limit.` | Unavailable, stale, partial, blocked, or unverified evidence. |
| `Rationale.` | Explanation only; must not contain normative keywords. |

Frontend verification contract. Lowercase `must`, `may`, and `should` are non-normative unless they quote source text, appear in a title, or are part of a row summary. New guide-owned contract text MUST use one of the statement classes above.

### 1.3B Owner-reference validation

Frontend verification contract. `tools/frontend_phase_maps/fe_p*_test_map.json` v3 rows MUST represent `owner_refs[]` as closed objects with `source_key`, `path`, `section_ref`, `heading_text`, `req_ids[]`, `ac_ids[]`, `role`, and `resolution_status`. The closed `source_key` values are `core00`, `core01`, `core02`, `core03`, `core04`, `core05`, `testing_harness_nlspec`, `design_md`, `ui_ux_guide`, `visual_golden_guide`, `dev_guide`, `implementation_testing_guide`, `research_rationale`, and `guide_local`.

Frontend verification contract. Product-conformance owner references with `resolution_status="resolved"` MUST point to an existing owner path and to existing `REQ-*` IDs in that owner file. Any row with `resolution_status` other than `resolved` MUST use `claim_status="blocked"` and a non-empty `blockers[]` entry. Blocked owner-reference rows MUST NOT close from target success, generated ledgers, retained old artifacts, or broad `make check`.
...
Browser evidence under `/apps/web/e2e` uses phase/frontend spec files only as Playwright discovery and accounting facades. Shared setup, payload construction, transport, cleanup, and assertions must be imported from exact semantic support files. `@cartulary/test-utils` imports must use its `grid`, `accessibility`, or `visual` subpath and remain route-free. E2E workbook identity/status metadata must come through `support/contracts/workbookSurfaces.ts`, not the app registry or visible labels. Privileged test-control calls require the dedicated token-bound client; product sessions or deployment-admin state are never substitutes. Structural accessibility and visual refactors retain exact scenario titles, fixture IDs, viewport/mask/focus choreography, normalized accessibility output, and committed PNG bytes.
...
- Historical implementation plans and handoffs are non-closure evidence for frontend phase status. When narrative plan text conflicts with the live registry, phase maps, generated ledgers, or retained `frontend-row-accounting.json` artifacts, the live frontend artifacts own current closure.
- Frontend accessibility row evidence MUST use `make browser-e2e-a11y`, retained `cartulary.frontend_accessibility_summary.v3` artifacts, and v4 frontend row accounting. Accessibility rows remain incomplete until the mapped target exists, emits its normalized summary artifact, and maps each implemented `FE-A11Y-*` row to executed scenarios.
...
Release-readiness aggregation may carry frontend rows forward as release-gate inputs, but it does not change the row evidence class. Visual, accessibility, support, and design-direction rows remain non-product-conformance evidence in `cartulary.release_readiness_evidence.v2`, and no frontend release-readiness artifact is Core 05 publication evidence unless an owner document later activates a claim-bearing publication route.
...
| `FE-SUPPORT-AC-007` | Frontend design-token artifacts MUST be generated or drift-checked from `docs/design.md` through `/packages/ui-contracts`; `/apps/web` may consume those artifacts but MUST NOT become the token source of truth. |
...
Browser integration rows cover workbook shell behavior, grid adapter behavior, keyboard choreography, focus, scroll anchors, cell editors, clipboard, fill-down, group rows, and public route/UI transitions in a real browser environment. Resize remains design/support evidence unless an owner spec adopts it as product conformance.
...
Accessibility rows verify keyboard access, visible focus, accessible state communication, ARIA where applicable, icon-only control names, color contrast, and non-color-only state communication. The canonical row-evidence target is `make browser-e2e-a11y`.
...
The target MUST retain exactly one normalized accessibility evidence artifact named `frontend-accessibility-summary.json` with `schema_id="cartulary.frontend_accessibility_summary.v3"`. The artifact represents implemented accessibility rows only. Raw Playwright output, DOM snapshots, screenshots, traces, and rule-engine output are diagnostic inputs; they MUST NOT replace the normalized summary artifact.

The testing harness NLSpec owns `make browser-e2e-a11y` target mechanics and the normalized `cartulary.frontend_accessibility_summary.v3` schema. This guide maps implemented frontend accessibility rows to that target; it MUST NOT define an incompatible schema shape. As a derived restatement, the current harness schema includes these arrays:

| Field | Required content |
| --- | --- |
| `phase_rows[]` | Every implemented frontend accessibility row claimed by the target, including row ID, phase ID, evidence class, claim status, and target mapping. |
...
Performance and responsiveness rows may verify implementation support such as sparse patch reference preservation, virtualization behavior, post-scroll settle, and row/cell anchor stability. The supported-load fixture uses 500 committed rows in grouped and ungrouped modes, a horizontally virtualized wide-column set, bounded mounted DOM, and semantic deep-target translation. Live browser evidence additionally covers scroll, reorder, selection, editor, and focus continuity. Test-support renderers may disable virtualization only for isolated semantic fixtures and cannot close virtualization claims. These checks must be clearly separated from claim-bearing timed or fixture-sensitive publication. Any published timing claim must satisfy Core 05.
...
| `make browser-e2e-a11y` | Deterministic implemented-row browser accessibility scenarios and `cartulary.frontend_accessibility_summary.v3` evidence. |
...
| Item | Value |
| --- | --- |
| Scope | Generated protocol facade, view-contract adapters, selector/test-id builders, design-token artifacts, import-boundary checks, generated drift, and package ownership. |
...
| FE-S-P0-04 | Support | Validate `docs/design.md` token front matter and generated `@cartulary/ui-contracts` token artifacts without treating design tokens as product-conformance behavior. | `docs/design.md` Sections 1 and 3; development guide Sections 6.1 and 6.6 | `N/A: implementation_support; Core owner not claimed` | `FE-SUPPORT-AC-007`, `D-AC-010`..`D-AC-015`, `D-AC-073` | `make generated-artifact-policy-check`; `make generate-drift`; `make frontend-import-boundary-check` | `implementation_support` |
...
| Primary owner sections | Core 01 Sections 2, 3.3.1, and 3.3.2; Core 04 Sections 2, 3, 5; development guide Section 6.1. |
...
FE-P2 unit hardening MUST keep startup and registry behavior keyed by `sheet_ref`, `view_schema_id`, and `saved_view_id`, not visible labels. Incident-control summary coverage MUST include non-null structured workbook preference pointers and MUST verify that displayed bootstrap defaults preserve `sheet_ref` identity rather than treating preference values as strings. Deleted and missing saved-view startup pointers use `saved_view_not_found`; unsupported cleared pointer kinds use `unsupported_sheet_ref_kind`; pack-dependent startup clearing uses `required_reference_pack_unavailable` only when a view contract declares unavailable `required_reference_pack_keys`. Runtime evidence that `scope='system'` saved views do not replace canonical system views remains owned by `FE-E-P2-01` and depends on the active-surface saved-view selector.

| ID | Layer | Test | Exact owner sections | Exact REQs | Exact ACs | Repository target or TODO | Evidence class |
| --- | --- | --- | --- | --- | --- | --- | --- |
| FE-U-P2-01 | Unit | Verify startup-surface resolution order: explicit launch surface, user preference, incident default, Timeline fallback, with invalid pointers clearing and falling through. | Core 03 Section 2.4; Core 01 Section 3.3.5.2 | `REQ-03-027`..`REQ-03-032`, `REQ-01-138`..`REQ-01-151` | `AC-146`..`AC-153`, `AC-231`, `AC-233` | `make frontend-unit` | `product_conformance` |
...
| FE-B-P2-01 | Browser integration | Verify continuous workbook shell composition: top bar with tab and `System views` subregions, view bar, grid-first active surface slot, inspector sections, and status strip slots stay in one shell while incident controls remain reachable only through the explicit secondary control surface. | Core 03 Sections 1 and 2; UI/UX guide Sections 5, 6, 8, 9 | `REQ-03-001`..`REQ-03-011` | `AC-001`, `AC-002`, `AC-005`, `AC-043`, `AC-078`, `AC-085`..`AC-090`, `AC-112`, `AC-116`, `AC-121`, `AC-122`, `AC-231`, `AC-410`, `AC-411`; `R2-AC-017`..`R2-AC-026` | `make browser-e2e-webserver-backed` | `product_conformance` |
...
| FE-V-P2-01 | Visual regression | Capture the fixed `1440x900` first viewport for the Default Timeline workbook shell with top bar, compact view bar, compact Timeline grid, row gutter, header affordances, selected row, focused Summary cell, row-context inspector, status strip, and Core 01 default Timeline fields only. | UI/UX guide Sections 5, 6, 8, 9, 13; `docs/design.md` Sections 3, 7, 13, 15, and 17; visual golden guide Sections 2, 3, 5 | `N/A: design_direction; Core owner not claimed` | `R2-AC-017`..`R2-AC-026`, `R2-AC-073`..`R2-AC-079`, `D-AC-001`..`D-AC-006`, `D-AC-055`, `D-AC-060`, `D-AC-062`..`D-AC-064` | `make browser-e2e-visual` | `design_direction` |
...
| Item | Value |
| --- | --- |
| Scope | `/packages/grid-adapter` RDG containment, stylesheet ownership, `record_id` row identity, `field_key` cells, renderer/editor registry, sort/paste/fill/focus translation, group/presentation rows, sparse patches, lifecycle cleanup, and imperative API containment. |
| Primary owner sections | Core 01 Section 3.3.4; Core 03 Sections 4.1, 14, 18, and 19; development guide Sections 6.3, 6.7, 6.8, 6.9, 6.10; implementation guide Sections 14.7, 14.8, 14.9A; R09 research as rationale only. |
...
| Introduced user-observable behavior | Contract-derived grid rendering with stable row/cell identity, read/write affordances, keyboard navigation, sorting commands, clipboard/fill behavior, and presentation-only group rows. |
...
| Completion criteria | Every rendered data row is keyed by `record_id`; every editable or queryable cell is keyed by `field_key`; vendor coordinates translate to `record_id + field_key`; missing or duplicate `record_id` fails before mutation-capable rendering; group/loading/spacer/presentation rows never emit mutation-capable events; editability derives from explicit editor adapters and contract writeability; `react-data-grid` `editable=true` alone is not sufficient; one primary click on a writable committed scalar cell creates and focuses exactly one adapter-owned editor with a collapsed caret after the existing text and no selected text; navigation cells contain no editable control; read-only and embedded action paths do not create an editor; accepted and rejected inter-cell transitions preserve the required commit and draft behavior; sort changes compile to view-query `sort[]`; local row reordering is not authoritative sorting; copy/paste/fill-down/editor entry obey contracts; pointer fill and `Ctrl/Cmd+D` share exact semantic targets and vendor double-click fill-to-end is suppressed; active cell and edit mode restore or intentionally clear by `record_id + field_key`; sparse patches preserve unchanged row references where required; cleanup removes stale subscriptions, portals, observers, timers, and row references; imperative mutation-capable vendor APIs are inaccessible outside the adapter; RDG stylesheet is imported exactly once before workbook-grid rendering; conflict, invalid, pending, active, bulk-selected, inspector-active, read-only/derived, stale, and saved state resolve through semantic inputs and design precedence; styling uses adapter-private classes, CSS variables, stable semantic selectors, accessible state attributes, and non-color markers without asserting RDG class hashes or DOM nesting. |
...
| FE-U-P3-02 | Unit | Translate vendor row/column coordinates to `record_id + field_key` for single-click edit, copy, paste, fill-down, selection, focus, and anchor assertions. | Core 01 Section 3.3.4; Core 03 Sections 4.1, 13, 18, and 19; development guide Sections 6.3 and 6.9 | `REQ-01-034`, `REQ-01-035`, `REQ-01-058`, `REQ-03-217`..`REQ-03-222`, `REQ-03-263`..`REQ-03-265`, `REQ-03-300` | `AC-003`, `AC-005`, `AC-009`, `AC-040`, `AC-043`, `AC-124`..`AC-127`, `AC-184`, `AC-185`, `AC-231`, `AC-354`, `AC-394`..`AC-396`, `AC-485` | `make frontend-unit` | `product_conformance` |
...
| FE-B-P3-01 | Browser integration | Verify one-click focused editing from an unselected cell, rejected-transition retention, sort, filter, group, paste, exact-range pointer and keyboard fill-down, scroll-to-cell, group expand/collapse, and anchor assertions through browser command helpers. | Core 03 Sections 4.1, 13, 14, 18, and 19; implementation guide Section 14.9A | `REQ-03-217`..`REQ-03-235`, `REQ-03-263`..`REQ-03-265`, `REQ-03-300` | `AC-003`, `AC-005`, `AC-013`, `AC-014`, `AC-024`..`AC-026`, `AC-040`, `AC-043`, `AC-044`, `AC-047`, `AC-231`, `AC-354`, `AC-360`, `AC-363`, `AC-364`, `AC-394`..`AC-396`, `AC-485` | `make browser-e2e-support`; `make browser-e2e-webserver-backed` | `product_conformance` |
| FE-V-P3-01 | Visual regression | Capture test-only grid-adapter support specimens for frozen column, resize handle, fill-down handle, edit cell, group outline row, and empty successful query; this row does not satisfy the default Timeline shell fixture. | UI/UX guide Sections 10 and 13; visual golden guide Sections 2, 3, 5 | `N/A: design_direction; Core owner not claimed` | `R2-AC-033`..`R2-AC-039`, `R2-AC-051`..`R2-AC-054`, `R2-RDG-AC-001`..`R2-RDG-AC-010`, `R2-AC-073`..`R2-AC-079` | `make browser-e2e-visual` | `design_direction` |
| FE-A11Y-P3-01 | Accessibility | Verify grid cells, editors, group rows, active cell, edit mode, disabled/read-only state, and blocked actions are keyboard accessible and announced without color-only signals. | UI/UX guide Sections 10.5 and 14; `docs/design.md` Accessibility Direction | `N/A: design_direction; Core owner not claimed` | `R2-AC-080`..`R2-AC-086`, `D-AC-009`, `D-AC-012` | `make browser-e2e-a11y` | `design_direction` |
...
| Item | Value |
| --- | --- |
| Scope | Timeline query, rough row creation, inline edit, paste, save state, secure client transaction identity, pending queue replay and blocked-edit recovery, and public mutation route use. |
| Primary owner sections | Core 01 Sections 3.3.4, 3.3.6; Core 02 Sections 5 and 15; Core 03 Sections 4.1, 4.2, 4.4, 15, and 16.3; development guide Section 6.1. |
...
| Introduced user-observable behavior | Timeline query results, low-friction rough row creation, inline edit, paste, queued save state, replay after transient failure, actionable transaction-conflict retry or discard, and cell-level validation/errors. |
...
| Completion criteria | Timeline creates, patches, and workbook-dispatched row actions use public route contracts with secure opaque `client_txn_id`, `record_id`, `base_row_version`, and `field_key`; row actions and explicit transaction-conflict retry use the latest committed `base_row_version`; uncertain transport replay preserves request identity; explicit recovery alone may re-key a blocker; discard is local-only and preserves later FIFO work; a same-field conflict after retry enters the normal resolver; rough input is preserved; paste and inline edit obey write contracts; save-state strip shows one primary state label and one actionable same-surface recovery path; earlier phases remain green. |
| Out of scope | Reload-durable or cross-tab queues, entity mention resolution, evidence handles, broader WebSocket live updates, same-field resolver implementation beyond recovery handoff, and saved-view persistence. |

| ID | Layer | Test | Exact owner sections | Exact REQs | Exact ACs | Repository target or TODO | Evidence class |
| --- | --- | --- | --- | --- | --- | --- | --- |
| FE-U-P4-01 | Unit | Verify secure opaque client transaction generation, sync-engine FIFO ordering, uncertain replay identity retention, atomic transaction-conflict re-key, local-only blocker discard, current-row-version dispatch, invalid recovery refusal, and same-field resolver handoff. | Core 01 Section 3.3.6; Core 03 Sections 4.1, 4.2, 4.4, and 15 | `REQ-01-057`..`REQ-01-070`, `REQ-03-217`..`REQ-03-222`, `REQ-03-236`..`REQ-03-241`, `REQ-03-282`, `REQ-03-301`, `REQ-03-302` | `AC-003`, `AC-005`, `AC-040`, `AC-043`, `AC-119`, `AC-120`, `AC-124`..`AC-127`, `AC-181`..`AC-183`, `AC-188`..`AC-193`, `AC-200`..`AC-218`, `AC-221`..`AC-225`, `AC-231`, `AC-299`, `AC-354`, `AC-394`..`AC-396`, `AC-486` | `make frontend-unit` | `product_conformance` |
...
| FE-I-P4-01 | Integration | Verify Timeline query response rows render full `view_row_v1` cells and preserve row identity through create, patch, validation error, and refresh. | Core 01 Sections 3.3.4 and 3.3.6; Core 03 Section 15 | `REQ-01-034`..`REQ-01-036`, `REQ-01-057`..`REQ-01-070`, `REQ-03-236`..`REQ-03-241` | `AC-119`, `AC-120`, `AC-124`..`AC-127`, `AC-181`..`AC-183`, `AC-188`..`AC-193`, `AC-200`..`AC-218`, `AC-221`..`AC-225`, `AC-231`, `AC-238`..`AC-243`, `AC-299`, `AC-361`, `AC-366`, `AC-367`, `AC-372`..`AC-374` | `make frontend-unit`; `make browser-e2e-webserver-backed` | `product_conformance` |
| FE-E-P4-01 | E2E | Verify public Timeline mutations plus cross-remount transaction uniqueness and blocked-edit retry, discard, current-row-version replay, and same-field resolver handoff through `/api/v1/` route contracts. | Core 01 Sections 3.3.4 and 3.3.6; Core 02 Section 5; Core 03 Sections 4.1, 4.4, and 15 | `REQ-01-057`..`REQ-01-070`, `REQ-02-024`, `REQ-02-025`, `REQ-03-217`..`REQ-03-222`, `REQ-03-236`..`REQ-03-241`, `REQ-03-301`, `REQ-03-302` | `AC-003`, `AC-005`, `AC-040`, `AC-043`, `AC-119`, `AC-120`, `AC-124`..`AC-127`, `AC-181`..`AC-183`, `AC-188`..`AC-193`, `AC-200`..`AC-218`, `AC-221`..`AC-225`, `AC-231`, `AC-299`, `AC-354`, `AC-394`..`AC-396`, `AC-406`, `AC-486` | `make browser-e2e-webserver-backed`; `make browser-e2e-stateful` | `product_conformance` |
| FE-V-P4-01 | Visual regression | Capture save-state, pending replay, transaction recovery, inline edit, and empty Timeline fixtures. | UI/UX guide Sections 8, 10, 13; visual golden guide Sections 2, 3, 5 | `N/A: design_direction; Core owner not claimed` | `R2-AC-023`..`R2-AC-026`, `R2-AC-033`..`R2-AC-039`, `R2-AC-045`..`R2-AC-050`, `R2-AC-073`..`R2-AC-079` | `make browser-e2e-visual`; fixture IDs from `tools/frontend_visual_fixture_registry.json` | `design_direction` |
| FE-A11Y-P4-01 | Accessibility | Verify grid navigation, edit entry/exit, paste and validation feedback, save-state communication, actionable transaction recovery, retry-then-discard tab order, focus entry without focus theft, and `Esc` priority are keyboard and screen-reader safe. | UI/UX guide Sections 10.1, 10.2, 10.5, 14 | `N/A: design_direction; Core owner not claimed` | `R2-AC-033`..`R2-AC-039`, `R2-AC-080`..`R2-AC-086`, `D-AC-009`, `D-AC-012` | `make browser-e2e-a11y` | `design_direction` |
...
| Primary owner sections | Core 01 Sections 3.3.4, 8.5; Core 02 Sections 6, 7.1, 7.3, and 8.2; Core 03 Sections 4.3, 15, and 16.1; UI/UX guide Section 10.3. |
...
| Introduced user-observable behavior | Entity sheets and mention chips with unresolved, resolved, auto-resolved, dismissed, and resolved-state manual-resolution metadata. |
...
| ID | Layer | Test | Exact owner sections | Exact REQs | Exact ACs | Repository target or TODO | Evidence class |
| --- | --- | --- | --- | --- | --- | --- | --- |
| FE-U-P5-01 | Unit | Verify mention chip view models preserve unresolved, resolved, auto-resolved, dismissed, and manual-resolution metadata by stable identifiers and field keys. | Core 02 Sections 6 and 7.1; Core 03 Section 4.3 | `REQ-02-026`..`REQ-02-044`, `REQ-03-276`..`REQ-03-281` | `AC-006`, `AC-019`..`AC-023`, `AC-028`, `AC-029`, `AC-188`..`AC-190`, `AC-201`, `AC-205`, `AC-221`..`AC-225`, `AC-231`, `AC-388`..`AC-393` | `make frontend-unit` | `product_conformance` |
| FE-I-P5-01 | Integration | Verify Hosts, Identities, and Notes grids render contract-derived columns and preserve mention/entity provenance through edit and refresh. | Core 01 Sections 3.3.4 and 8.5; Core 02 Sections 7.3 and 8.2; Core 03 Sections 15 and 16.1 | `REQ-01-034`..`REQ-01-036`, `REQ-01-303`..`REQ-01-311`, `REQ-02-054`..`REQ-02-061`, `REQ-03-242`..`REQ-03-249`, `REQ-03-272` | `AC-006`, `AC-020`, `AC-023`, `AC-045`, `AC-068`..`AC-075`, `AC-097`..`AC-100`, `AC-112`, `AC-116`..`AC-118`, `AC-124`, `AC-127`, `AC-184`..`AC-187`, `AC-196`, `AC-209`, `AC-210`, `AC-231`, `AC-278`, `AC-279`, `AC-300`..`AC-303`, `AC-315`, `AC-318`, `AC-366`..`AC-368`, `AC-396`, `AC-410`, `AC-411` | `make frontend-unit`; `make browser-e2e-webserver-backed` | `product_conformance` |
| FE-E-P5-01 | E2E | Verify manual mention resolution, dismissal, auto-resolution disclosure, and undo through public mutation routes and refreshed rows. | Core 01 Section 3.3.6; Core 02 Sections 6, 7.1, and 7.3; Core 03 Section 4.3 | `REQ-01-057`..`REQ-01-070`, `REQ-02-026`..`REQ-02-044`, `REQ-02-054`..`REQ-02-061`, `REQ-03-276`..`REQ-03-281` | `AC-006`, `AC-019`..`AC-023`, `AC-028`, `AC-029`, `AC-124`..`AC-127`, `AC-181`..`AC-183`, `AC-186`..`AC-190`, `AC-200`..`AC-218`, `AC-221`..`AC-225`, `AC-231`, `AC-279`, `AC-299`, `AC-388`..`AC-393` | `make browser-e2e-webserver-backed`; `make browser-e2e-stateful` | `product_conformance` |
| FE-V-P5-01 | Visual regression | Capture unresolved token, resolved chip, auto-resolved chip, dismissed mention, and manual resolution metadata fixtures. | UI/UX guide Sections 10.3 and 13; visual golden guide Sections 2, 3, 5 | `N/A: design_direction; Core owner not claimed` | `R2-AC-040`..`R2-AC-044`, `R2-AC-073`..`R2-AC-079` | `make browser-e2e-visual`; fixture IDs from `tools/frontend_visual_fixture_registry.json` | `design_direction` |
| FE-A11Y-P5-01 | Accessibility | Verify mention chip states and manual-resolution controls have accessible names, visible focus, and non-color-only distinction. | UI/UX guide Sections 10.3, 10.5, 14; `docs/design.md` Accessibility Criteria | `N/A: design_direction; Core owner not claimed` | `R2-AC-040`..`R2-AC-044`, `R2-AC-080`..`R2-AC-086`, `D-AC-050`, `D-AC-051` | `make browser-e2e-a11y` | `design_direction` |
...
| Primary owner sections | Core 01 Sections 3.3.8 and 3.3.9; Core 02 Section 13; Core 03 Sections 16.1 and 16.2; Core 04 Sections 3, 5, 6; UI/UX guide Section 12. |
| Frontend package or app surfaces | `/apps/web`, `/packages/protocol-ts`, `/packages/ui-contracts`, `/packages/test-utils`. |
...
| FE-V-P6-01 | Visual regression | Capture Evidence grid count, Timeline inspector evidence count, affordance, available, requested, pending, blocked, failed, inconsistent, preview, and download-handle state fixtures. | UI/UX guide Sections 12 and 13; visual golden guide Sections 2, 3, 5 | `N/A: design_direction; Core owner not claimed` | `R2-AC-059`..`R2-AC-062`, `R2-AC-073`..`R2-AC-079` | `make browser-e2e-visual`; fixture IDs from `tools/frontend_visual_fixture_registry.json` | `design_direction` |
...
| Primary owner sections | Core 01 Section 3.3.10; Core 03 Sections 3.1, 3.3.5, 3.3.6, 4.2, and 4.4; Core 04 Sections 2, 5; UI/UX guide Sections 10.4 and 10.5. |
...
| FE-U-P7-02 | Unit | Verify same-field conflict anchors, conflict queue, and resolver state use `record_id + field_key + base_row_version` rather than visible indexes. | Core 03 Sections 3.1, 3.3.5, 3.3.6, 4.2, and 4.4 | `REQ-03-033`..`REQ-03-084` | `AC-009`, `AC-013`, `AC-037`..`AC-042`, `AC-047`, `AC-126`, `AC-163`, `AC-203`, `AC-204`, `AC-226`..`AC-231`, `AC-381` | `make frontend-unit` | `product_conformance` |
| FE-I-P7-01 | Integration | Verify conflict resolver actions submit public mutations and refresh rows without losing focus or pending queue ordering. | Core 01 Section 3.3.6; Core 03 Sections 3.1, 3.3.5, 3.3.6, 4.2, and 4.4 | `REQ-01-057`..`REQ-01-070`, `REQ-03-033`..`REQ-03-084` | `AC-009`, `AC-013`, `AC-037`..`AC-042`, `AC-047`, `AC-124`..`AC-127`, `AC-181`..`AC-183`, `AC-188`..`AC-190`, `AC-200`..`AC-218`, `AC-221`..`AC-231`, `AC-299`, `AC-381` | `make frontend-unit`; `make browser-e2e-webserver-backed` | `product_conformance` |
| FE-E-P7-01 | E2E | Verify multi-client live row update, presence anchoring, reset/invalidate handling, stale-row requery, and same-field conflict resolver through `/ws/v1/` and `/api/v1/`. | Core 01 Sections 3.3.6 and 3.3.10; Core 03 Sections 3.1, 3.3.5, 3.3.6, 4.2, and 4.4 | `REQ-01-057`..`REQ-01-070`, `REQ-01-250`..`REQ-01-253`, `REQ-03-033`..`REQ-03-084` | `AC-009`, `AC-013`, `AC-037`..`AC-042`, `AC-047`, `AC-124`..`AC-129`, `AC-131`..`AC-136`, `AC-156`..`AC-163`, `AC-181`..`AC-183`, `AC-188`..`AC-190`, `AC-200`..`AC-218`, `AC-221`..`AC-233`, `AC-299`, `AC-381` | `make browser-e2e-stateful`; `make browser-e2e-webserver-backed` | `product_conformance` |
| FE-V-P7-01 | Visual regression | Capture same-field conflict, row-gutter presence, presence hint, conflict resolver, reset/invalidate notice, and save-state conflict fixtures. | UI/UX guide Sections 8, 10.4, 10.5, 13; visual golden guide Sections 2, 3, 5 | `N/A: design_direction; Core owner not claimed` | `R2-AC-023`..`R2-AC-026`, `R2-AC-045`..`R2-AC-050`, `R2-AC-073`..`R2-AC-079` | `make browser-e2e-visual`; fixture IDs from `tools/frontend_visual_fixture_registry.json` | `design_direction` |
...
| Item | Value |
| --- | --- |
| Scope | Saved views, sorting, filtering, grouping, layout state, active chips, startup/default-surface UI, group rows, and query-control persistence. |
...
| Introduced user-observable behavior | Saved-view selector, sort/group/filter controls, active filter chips, layout persistence, default/startup surface UI, and non-writable group rows. |
...
Future-RDG remediation adds product-conformance coverage for Core 01 REQ-01-627..REQ-01-628, Core 03 REQ-03-295..REQ-03-299, and Core 04 AC-480..AC-484. Authored frontend phase maps must assign live production-adapter scenarios for semantic saved layout, ordered multi-sort, command-gated selection, typed editor lifecycle, range clipboard and fill, explicit data and interaction states, semantic styling, keyboard/focus behavior, and always-on virtualization. Test-support renderers remain limited to fast semantic mapping and cannot close live RDG claims.

Keyboard evidence must distinguish semantic helper coverage from the production event boundary. Fast tests may prove anchor movement, range expansion, editor outcomes, and rejection rules. Live production-RDG tests must prove grouped and ungrouped flattened-position mapping, Shift+Arrow range selection, Page/Home/End movement, printable and clear entry, Enter and Tab commit transitions, Escape priority, focus restoration after refresh or collapse, duplicate-command prevention, and polite/assertive announcements. A mocked test-support renderer may retain selector compatibility, but it must not be cited as focus, DOM, ARIA, or vendor-event conformance.
...
| FE-B-P8-01 | Browser integration | Verify browser command helpers for sort, filter, group, active chips, layout persistence, group expand-collapse, and startup/default surface UI. | Core 03 Sections 3 and 14; implementation guide Section 14.9A | `REQ-03-012`..`REQ-03-032`, `REQ-03-223`..`REQ-03-235` | `AC-013`, `AC-014`, `AC-024`..`AC-026`, `AC-044`, `AC-047`, `AC-146`..`AC-153`, `AC-231`, `AC-233`, `AC-360`, `AC-363`, `AC-364` | `make browser-e2e-webserver-backed`; `make browser-e2e-support` | `product_conformance` |
...
| FE-V-P8-01 | Visual regression | Capture saved-view selector, active chips, grouped result, group row, default/startup state indicator, and empty successful query fixtures. | UI/UX guide Sections 7, 10.5, 13; visual golden guide Sections 2, 3, 5 | `N/A: design_direction; Core owner not claimed` | `R2-AC-027`..`R2-AC-032`, `R2-AC-051`..`R2-AC-054`, `R2-AC-073`..`R2-AC-079`, `R2-RDG-AC-001`..`R2-RDG-AC-010` | `make browser-e2e-visual`; fixture IDs from `tools/frontend_visual_fixture_registry.json` | `design_direction` |
| FE-A11Y-P8-01 | Accessibility | Verify sort, filter, group, saved-view menu, active chips, group expand-collapse, and default/startup controls are keyboard reachable and announced. | UI/UX guide Sections 7, 10.5, 14; `docs/design.md` Accessibility Criteria | `N/A: design_direction; Core owner not claimed` | `R2-AC-027`..`R2-AC-032`, `R2-AC-051`..`R2-AC-054`, `R2-AC-080`..`R2-AC-086`, `D-AC-050`, `D-AC-051` | `make browser-e2e-a11y` | `design_direction` |

### 4.9 Phase FE-P9: Inspector And Row-Local Actions

| Item | Value |
| --- | --- |
| Scope | Config-derived row-local inspector panels and feature groups for Details, Relationships, Evidence, History, Workflow, rollback, destructive actions, evidence access, merge, same-shell create-related workflows, and stale-state invalidation. |
| Primary owner sections | Core 01 Sections 3.3.4, 3.3.6, 3.3.7, 3.3.8, and 7.4; Core 02 Sections 2, 13, 15, and 18; Core 03 Sections 2.3, 2.3A, 10.1, 16.1, and 16.2; Core 04 Sections 2 and 9; UI/UX guide Sections 8, 9, 10, 12. |
...
| Introduced user-observable behavior | Inspector panels and feature actions render from `inspector_config_v1`; row details, relationship links, evidence panel, history timeline, Workflow actions, rollback preview/action, and destructive-action confirmation/errors. |
| Shared harnesses triggered | Frontend route/API boundary conformance; same-origin evidence handle behavior; frontend error-state rendering; stable selector/test-id contracts; inspector feature registry rendering; keyboard and focus traversal; accessibility names and ARIA. |
| Completion criteria | Inspector is row-local and anchored by `record_id`; panels and actions render from active `view_schema_id` config; relationships and evidence use stable IDs; history and rollback use public route contracts; destructive actions re-check current authorization and render public envelopes; stale row-bound state invalidates deterministically; Timeline hot-path create/edit/paste works with the inspector closed; focus returns to the originating row/cell when appropriate; earlier phases remain green. |
| Out of scope | Coordination workbook surfaces beyond relationships, WebSocket live updates, visual claim publication. |

| ID | Layer | Test | Exact owner sections | Exact REQs | Exact ACs | Repository target or TODO | Evidence class |
| --- | --- | --- | --- | --- | --- | --- | --- |
| FE-U-P9-02 | Unit | Verify active `view_schema_id` selects `inspector_config_v1`, saved views inherit the same config from immutable `view_schema_id`, no-row state is `no_row_selected`, and stale confirmations/previews/merge plans/forms invalidate on row, row-version, authorization, incident-lifecycle, delete, merge, hard refresh, or active surface changes. | Core 01 Section 7.4; Core 03 Sections 2.3, 2.3A, and 10.1 | `REQ-01-615`..`REQ-01-617`, `REQ-03-013`, `REQ-03-285`, `REQ-03-291`..`REQ-03-292` | `AC-453`, `AC-457`, `AC-462` | `make frontend-unit` | `product_conformance` |
| FE-U-P9-03 | Unit | Verify inspector panels and feature groups render from active config keys and reject unknown panel IDs, route owners, result behaviors, disabled tokens, duplicate keys, missing required keys, and extra current-profile keys before rendering. | Core 01 Section 7.4; Core 03 Section 2.3A; Core 04 Section 9 | `REQ-01-615`..`REQ-01-617`, `REQ-03-292` | `AC-454`, `AC-455` | `make frontend-unit` | `product_conformance` |
| FE-U-P9-01 | Unit | Verify inspector selection, tab state, details, relationships, evidence, and history anchors are `record_id` based and survive row refresh. | Core 01 Sections 3.3.4, 3.3.7, 3.3.8; Core 03 Sections 16.1 and 16.2 | `REQ-01-034`..`REQ-01-036`, `REQ-01-048`..`REQ-01-056`, `REQ-01-243`..`REQ-01-247`, `REQ-03-242`..`REQ-03-249`, `REQ-03-272` | `AC-006`, `AC-015`, `AC-020`, `AC-023`, `AC-045`, `AC-072`..`AC-075`, `AC-097`..`AC-100`, `AC-102`, `AC-103`, `AC-107`..`AC-111`, `AC-124`, `AC-127`, `AC-154`, `AC-155`, `AC-184`, `AC-185`, `AC-209`, `AC-210`, `AC-231`, `AC-278`..`AC-280`, `AC-313`, `AC-315`, `AC-318`, `AC-321`, `AC-366`, `AC-367`, `AC-372`..`AC-374` | `make frontend-unit` | `product_conformance` |
| FE-I-P9-01 | Integration | Verify history and rollback preview/action use public route contracts, preserve retained history, and render public error envelopes. | Core 01 Section 3.3.7; Core 02 Section 15; Core 03 Section 16.2 | `REQ-01-048`..`REQ-01-056`, `REQ-01-089`..`REQ-01-108`, `REQ-02-205`..`REQ-02-218`, `REQ-02-238`..`REQ-02-242` | `AC-107`..`AC-111`, `AC-124`..`AC-128`, `AC-154`, `AC-155`, `AC-215`..`AC-218`, `AC-231`, `AC-383`..`AC-386`, `AC-412` | `make frontend-unit`; `make browser-e2e-webserver-backed` | `product_conformance` |
| FE-E-P9-01 | E2E | Verify inspector Details, Relationships, Evidence, History, rollback, and destructive-action authorization through public browser routes. | Core 01 Sections 3.3.6, 3.3.7, 3.3.8; Core 02 Sections 13 and 15; Core 04 Sections 3 and 5 | `REQ-01-057`..`REQ-01-070`, `REQ-01-089`..`REQ-01-108`, `REQ-01-243`..`REQ-01-247`, `REQ-02-186`..`REQ-02-201`, `REQ-02-205`..`REQ-02-218`, `REQ-04-021`..`REQ-04-030`, `REQ-04-052`, `REQ-04-053` | `AC-049`..`AC-055`, `AC-102`, `AC-103`, `AC-107`..`AC-111`, `AC-124`..`AC-128`, `AC-149`, `AC-154`, `AC-155`, `AC-178`..`AC-180`, `AC-181`..`AC-183`, `AC-188`..`AC-190`, `AC-200`..`AC-218`, `AC-221`..`AC-225`, `AC-231`, `AC-232`, `AC-234`, `AC-251`..`AC-257`, `AC-260`, `AC-261`, `AC-278`, `AC-280`, `AC-299`, `AC-313`, `AC-321`, `AC-340`..`AC-342`, `AC-352`, `AC-370`, `AC-371`, `AC-402` | `make browser-e2e-webserver-backed`; `make browser-e2e-stateful` | `product_conformance` |
| FE-E-P9-02 | E2E | Verify default-closed inspector state, no-row state, surface switch config changes, saved-view switch over the same `view_schema_id` keeps the same config, closed incident read-only behavior, server-denied action behavior, authorization demotion while open, and Timeline create/edit/paste without opening the inspector. | Core 01 Section 7.4; Core 03 Sections 2.3, 2.3A, and 10.1; Core 04 Sections 2 and 9 | `REQ-01-615`..`REQ-01-617`, `REQ-03-013`, `REQ-03-291`..`REQ-03-292`, `REQ-04-127` | `AC-453`, `AC-456`, `AC-457`, `AC-459`, `AC-462` | `make browser-e2e-webserver-backed`; `make browser-e2e-stateful` | `product_conformance` |
| FE-E-P9-03 | E2E | Verify Timeline inspector Workflow create-related actions create or link Task Request, Decision, Evidence, Communications Log, Handoff, Status Review, and Lesson rows without leaving the workbook shell. | Core 01 Section 7.4; Core 03 Section 2.3A | `REQ-01-617`, `REQ-03-292` | `AC-458` | `make browser-e2e-webserver-backed`; `make browser-e2e-stateful` | `product_conformance` |
| FE-V-P9-01 | Visual regression | Capture inspector Details, Relationships, Evidence, History, rollback preview, destructive confirmation, and public error fixtures. | UI/UX guide Sections 9, 12, 13; visual golden guide Sections 2, 3, 5 | `N/A: design_direction; Core owner not claimed` | `R2-AC-023`..`R2-AC-026`, `R2-AC-059`..`R2-AC-062`, `R2-AC-073`..`R2-AC-079` | `make browser-e2e-visual`; fixture IDs from `tools/frontend_visual_fixture_registry.json` | `design_direction` |
...
| FE-A11Y-P9-02 | Accessibility | Verify keyboard open/close, panel navigation, `Esc`, focus restoration, disabled/blocked states, no-row empty state, and destructive confirmation focus for config-driven inspector behavior. | UI/UX guide Sections 8, 14 | `N/A: design_direction; Core owner not claimed` | `R2-AC-080`..`R2-AC-086` | `make browser-e2e-a11y` | `design_direction` |

### 4.10 Phase FE-P10: Remaining Workbook Surfaces And Keyboard Completion

| Item | Value |
| --- | --- |
| Scope | Task Requests, Decisions, Parties, Communications Log, Handoff, Status Review, Lesson, full keyboard contract, clipboard behavior, fill-down, frozen columns, virtual scroll, group rows, focus restoration, and `Esc` behavior. |
| Primary owner sections | Core 01 Sections 8.5 and Appendix F; Core 02 Sections 18 and 19; Core 03 Sections 2.2, 4.1, 14, 16.3, 16.4, 18, and 19; UI/UX guide Sections 6, 10, 11, 13, 14. |
| Frontend package or app surfaces | `/apps/web`, `/packages/grid-adapter`, `/packages/view-contracts`, `/packages/protocol-ts`, `/packages/ui-contracts`, `/packages/test-utils`. |
...
| FE-V-P11-02 | Visual regression | Ensure the visual fixture matrix includes the default Timeline workbook shell, unresolved/resolved entity state, same-field conflict, row-gutter presence, evidence affordance, grouped result, Task Requests or Decisions, save-state strip, frozen column, resize handle, fill-down handle, edit cell, group outline row, exposed theme states, and empty successful query. | Visual golden guide Sections 2, 3, 5; UI/UX guide Sections 10 through 13 | `N/A: implementation_support; Core owner not claimed` | `R2-AC-040`..`R2-AC-062`, `R2-AC-073`..`R2-AC-079`, `R2-RDG-AC-001`..`R2-RDG-AC-010` | `make browser-e2e-visual`; fixture IDs from `tools/frontend_visual_fixture_registry.json` | `implementation_support` |
| FE-V-P11-03 | Visual regression | Capture test-only exposed `dark_graphite` token and theme-state specimens with deterministic density, color, component, focus, and semantic-state samples; this row does not satisfy the default Timeline shell fixture. | `docs/design.md` Sections 3, 14, and 15; visual golden guide Sections 2, 3, and 5 | `N/A: design_direction; Core owner not claimed` | `D-AC-015`, `D-AC-055`, `D-AC-060`, `D-AC-062`, `D-AC-063`, `VG-AC-002`, `VG-AC-004`, `VG-AC-005` | `make browser-e2e-visual` | `design_direction` |
...
| FE-H-04 | Sync-engine pending queue and replay behavior | P4 through P10 | Core 01 Section 3.3.6; Core 03 Sections 4.1, 4.2, 4.4, and 15 | `make frontend-unit`; `make browser-e2e-stateful` | Pending creates, patches, paste batches, row-versioned actions, retries, failures, and replay are ordered by stable mutation identifiers and public route results. |
...
| FE-H-06 | Same-field conflict anchoring | P4, P7 through P10 | Core 03 Sections 3.1, 3.3.5, 3.3.6, 4.2, and 4.4 | `make frontend-unit`; `make browser-e2e-stateful` | Conflict state is anchored by `record_id + field_key + base_row_version`; resolver focus returns or intentionally clears by that anchor. |
...
| FE-H-08 | Save-state presentation | P2, P4, P7 through P10 | Core 03 Sections 4.1, 4.4; UI/UX guide Sections 8 and 10.4 | `make frontend-unit`; `make browser-e2e-webserver-backed`; `make browser-e2e-visual` | Status strip shows one primary save-state label and one secondary same-surface message; same-field conflict summaries use user-facing count copy rather than raw record, field, token, route, or error text. |
| FE-H-09 | Browser command helpers | P3 through P10 | Core 03 Sections 4.1, 14, 18, and 19; implementation guide Section 14.9A | `make browser-e2e-support`; `make browser-e2e-webserver-backed` | Helpers cover sort, filter, group, paste, fill-down, scroll-to-cell, group expand/collapse, and anchor assertions; resize helpers remain support/design measurement only. |
...
| FE-H-11 | Keyboard and focus traversal | P1 through P11 | Core 03 Sections 4.1, 18, and 19; UI/UX guide Section 14 | `make browser-e2e-support`; `make browser-e2e-a11y` | Keyboard access, visible focus, focus restoration, edit entry/exit, menus, switchers, inspector controls, conflict resolver, and `Esc` priority are covered. |
| FE-H-12 | Accessibility names, ARIA, and state communication | P1 through P11 | UI/UX guide Section 14; `docs/design.md` Accessibility Direction | `make browser-e2e-a11y` | Active tab/surface/menu/group/conflict/save/presence/evidence states are named and communicated without color alone. |
| FE-H-13 | Stable selector and test-id contracts | P0 through P11 | Development guide Sections 6.4 and 7.4; implementation guide Section 14.9A | `make frontend-unit`; `make browser-e2e-support` | Test IDs are generated from stable IDs, row-history `history_item_ref`, and closed vocabularies; visible labels are not stable selectors; opaque selector inputs stay non-empty and losslessly encoded unless an owner spec defines stricter syntax. |
...
| FE-H-19 | Enterprise-provider discovery UI boundary | P1 through P10 when enterprise auth is rendered | Core 01 Section 20; Core 04 Section 12.3.4 | `make frontend-unit`; `make browser-e2e-webserver-backed` | Enterprise-auth UI treats `GET /api/v1/auth/providers` as read-only sign-in discovery and renders no provider create, edit, delete, metadata-upload, certificate-upload, secret-management, redirect-URI, ACS-URL, or provider-policy form. |
| FE-H-20 | Inspector feature registry rendering | P9, P10 | Core 01 Section 7.4; Core 03 Section 2.3A; Core 04 `AC-453`..`AC-462` | `make frontend-unit`; `make browser-e2e-webserver-backed`; `make browser-e2e-visual`; `make browser-e2e-a11y` | Generated config fixture coverage proves feature keys render from `view_schema_id`; stale feature state invalidates; hot-path grid operations pass with inspector closed; feature controls are keyboard reachable and named. |

### 5.1 Visual fixture matrix

Frontend verification contract. The visual harness MUST maintain one owned-stack Playwright screenshot suite unless a newer adopted harness source changes that boundary. The current suite location is `apps/web/e2e/workbook.visual.spec.ts`; the current target is `make browser-e2e-visual`.

Frontend verification contract. `tools/frontend_visual_fixture_registry.json` is the deterministic fixture registry for an append-only contiguous `FE-VFIX-*` range and uses schema `cartulary.frontend_visual_fixture_registry.v3`. The current registry declares FE-VFIX-01 through FE-VFIX-21; future fixtures MUST be appended as the next contiguous ID. Each registry entry MUST include fixture ID, status, row links, scenario title, primary golden filename, optional supporting golden artifacts when a fixture owns multiple snapshots, seed, viewport, capture scope, masks, scroll state, focus state, editor state, inspector state, density, and replacement metadata. Missing fixture implementation MUST be represented by a blocked row or blocked fixture entry with a concrete blocker; generic TODO text does not close visual rows. Test-only support specimens such as FE-P3 grid-adapter fixtures and FE-P11 token/theme fixtures MUST NOT be treated as default product-shell evidence for `FE-VFIX-01`; `FE-VFIX-01` specifically requires the compact Timeline first viewport with row gutter, header affordances, selected row, focused Summary cell, adjacent inspector, status strip, no admin/demo stack, and only Core 01 default Timeline columns.
...
Source limit. Visual-regression results are implementation-quality evidence and are not claim-bearing benchmark evidence unless Core 05 publication requirements are separately satisfied.

## 6. Phase Completion Checklist

Frontend verification contract. Frontend phase completion is binary. A phase can be represented as `active_green` only when all applicable rules below are satisfied:

1. Every active current phase row executes in its intended layer: unit rows in frontend unit/typecheck targets, integration rows in frontend integration-style or browser-backed targets, browser rows in Playwright/browser targets, visual rows in `make browser-e2e-visual`, implemented accessibility rows in `make browser-e2e-a11y`, and support rows in their named support targets.
2. Every shared harness triggered by introduced or changed surfaces has executed.
...
7. Rows with unresolved owner references, non-empty `blockers[]`, or any non-implemented claim status are invalid for active current completion.
8. Missing repository artifacts are not presented as existing targets or manifests.
...
12. Accessibility row evidence exists for every implemented phase row that introduces user-visible UI and is represented in `cartulary.frontend_accessibility_summary.v3` plus `cartulary.frontend_row_accounting.v5`.
...
Frontend verification contract. Every frontend row MUST map to exact owner sections. Product-conformance rows MUST map to exact `REQ-*` identifiers and exact Core `AC-*` identifiers. A row MAY cite UI/UX guide, `docs/design.md`, visual-golden, development-guide, or guide-local acceptance criteria for design direction or implementation support, but those evidence classes MUST NOT be `product_conformance`.

Frontend verification contract. Rows without exact owner evidence MUST use evidence class `TODO_owner_lookup`, `claim_status="blocked"`, and non-empty `blockers[]`. Rows that intentionally have non-Core ownership MUST declare that Core ownership is not claimed and MUST cite support/design acceptance criteria.

### 7.2 Row-to-repository target mapping

Frontend verification contract. Rows MUST map to exact existing repository targets through closed target-reference objects. Current exact target names available for frontend guide rows are listed in Section 3. Do not invent target names, manifest names, package names, or scripts.

Frontend verification contract. The existing repository has `tools/phase*_test_map.json`, `docs/testing/phase*_coverage_ledger.md`, `tools/browser_e2e_batch_manifest.json`, `tools/execution_topology_manifest.json`, and `tools/test_accounting_classification.json`. Frontend phase rows use a separate namespace and MUST NOT be appended to base `phaseN` maps.

Frontend verification contract. `tools/frontend_phase_registry.json` MUST declare `schema_id="cartulary.frontend_phase_registry.v5"`, `schema_version=5`, `phase_namespace="frontend"`, `guide_path`, `guide_digest`, and `phases[]`. Valid phase IDs are the current contiguous declared range beginning at `FE-P0`; the current registry declares `FE-P0` through `FE-P11`. Current frontend registry `status` MUST be `active`, and current `row_rollup_state` MUST be `active_green`. Each phase item MUST include `phase_id`, `status`, `row_rollup_state`, `base_phase_join`, `manifest_path`, `manifest_digest`, `ledger_path`, `ledger_digest`, `owner_refs[]`, `depends_on[]`, `activation_blockers[]`, and `evidence_freshness_digest`. `base_phase_join` is either a base `phase<N>` identifier or `null`; it is the only owner for frontend/base phase browser selection and duration joins, and it MUST NOT be inferred from the `FE-P<N>` numeric suffix.

Frontend verification contract. Current frontend phases are executable only when their frontend map, generated ledger, public targets, target schedule metadata, row evidence, evidence-class owner metadata, and evidence freshness agree in the same active-current change set. Active phases MUST have `row_rollup_state="active_green"` and empty `activation_blockers[]`. `phase-slice`, `service-backed-slice`, and direct standalone frontend-aware targets MUST select only active phases and implemented active rows; planned, stale, retired, blocked, or not-implemented rows are not current completion inputs.

Frontend verification contract. Each `tools/frontend_phase_maps/fe_p*_test_map.json` file MUST declare `schema_id="cartulary.frontend_phase_test_map.v4"`, `schema_version=4`, `phase_namespace="frontend"`, matching `phase_id`, `guide_digest`, and `rows[]`. Each row MUST include `phase_id`, closed `id`, closed `layer`, closed `evidence_class`, `default_check_required`, structured `owner_refs[]`, `core_req_ids[]`, `core_ac_ids[]`, `support_or_design_ac_ids[]`, structured `targets[]`, `scenario_titles[]`, `claim_status="implemented"`, structured `claim`, `blockers[]`, and `out_of_scope[]`. Maps MUST NOT contain `null`, unknown object members, duplicate row IDs, duplicate object members, or lifecycle contradictions. `FE-V-*` and `FE-A11Y-*` rows MUST remain `design_direction` or `implementation_support` unless they use an explicit Core 05 `claim_publication_boundary` route; they MUST NOT use `product_conformance`. `default_check_required` means the row belongs in the local default-check projection; explicit browser, visual, accessibility, support, and claim-publication evidence remains reachable through its direct target even when this field is `false`.

Frontend verification contract. Generated frontend ledgers under `docs/testing/frontend_phase_coverage_ledgers/` MUST render registry metadata, row metadata, target mapping, evidence class, owner refs, Core IDs, support/design AC IDs, claim status, scenario titles, and source-map claims. The maps are the source of truth; ledgers are rendered companions and historical plans are non-closure evidence.

Frontend verification contract. Frontend-aware targets that map one or more frontend rows MUST retain `<target>/frontend-row-accounting.json` with schema ID `cartulary.frontend_row_accounting.v5` whenever the target maps frontend rows or an explicit slice scope disables frontend row accounting. That artifact is the durable row-to-scenario closure record for target runs and MUST include accounting scope, command ID, map and registry digests, guide digest, run root, scenario results, row results, and required-target closure. Standalone broad frontend-aware targets MUST use `accounting_scope.mode="active_target"` and enforce implemented rows mapped to the target from active frontend phases only. Whole `PHASE_NAMESPACE=frontend PHASE=FE-P<N>` and explicit `ROWS=<FE-row-id,...>` slices MUST use `accounting_scope.mode="selected_rows"` and enforce only selected implemented rows from active frontend phases through `FE-P<N>`. Base `PHASE=phaseN` slices MUST use `accounting_scope.mode="disabled"` for frontend-aware child summaries and MUST NOT fail on non-selected `FE-*` rows. Target and tool-run summaries MUST reference `frontend-row-accounting.json` through artifact refs rather than duplicating row accounting under `extensions["cartulary.frontend_row_accounting"]`. Base browser `manifest-selected-tests.json` files remain base phase inventory and MUST NOT be used as frontend row closure evidence. Product-conformance and design-direction rows MUST close from exact `scenario_titles[]` in the intended target artifact. An implemented `implementation_support` row with intentionally empty `scenario_titles[]` MAY close from a passing mapped target when the support assertion is target-level rather than scenario-level. A target that exits successfully while an implemented frontend row inside its accounting scope is not closed MUST fail target-summary generation with `failure_class="harness"` and `failure_reason="frontend_row_accounting"`. `release-readiness-evidence` consumes only v5 row-accounting artifacts from the current run root and derives explicit release-gate, product-conformance, and claim-publication effects from the row class. Retained v1/v2/v3/v4 artifacts are unsupported and MUST NOT close v5 rows or satisfy release-readiness aggregation. Frontend row accounting remains harness/frontend verification evidence and MUST NOT move into `/apps/web`, `/packages/grid-adapter`, or product runtime architecture unless a later owner document creates that runtime contract.
...
Frontend verification contract. Frontend drift coverage MUST fail when a frontend map is missing, a generated frontend ledger is stale, a row has invalid evidence-class metadata, a row targets an unknown public command, a browser-backed row lacks required `scenario_titles[]`, a visual fixture is missing registry metadata, freshness digests are stale, or an active frontend phase is not `active_green`.

Frontend verification contract. Explicit guide row `make ...` target restatements MUST name targets present in the matching live frontend phase-map row; the phase maps remain the row-evidence routing source of truth, so guide omissions are permitted but stale extra targets are drift.

### 7.6 Conditional claim-publication review

Frontend verification contract. Frontend readiness evidence MUST NOT activate Core 05 by default. Release-support metadata MAY declare `claim_publication_intent` through `cartulary.frontend_claim_publication_review.v1` with exactly one of these values:
...
Frontend verification contract. When `claim_publication_intent="none"`, no Core 05 gate MUST run. When `claim_publication_intent="informative_engineering_measurement"`, measurement artifacts MAY be retained but MUST NOT be treated as claim-bearing evidence. When `claim_publication_intent="claim_bearing_publication"`, release validation MUST require:
...
| FE-GUIDE-AC-011 | Visual regression requirements cover deterministic fixture data, viewport, zoom, ordering, capture scope, masks, scroll anchors, focus/editor state, inspector state, settle behavior, fixture matrix, and golden refresh rules. |
...
| FE-GUIDE-AC-019 | `make browser-e2e-a11y` emits `cartulary.frontend_accessibility_summary.v3` with implemented row, scenario, keyboard, state-communication, contrast, violation, and artifact-ref arrays. |
...
| FE-GUIDE-AC-022 | `make check` includes frontend map validation and frontend ledger drift; `make release-check` additionally runs mapped frontend support, visual, and accessibility readiness targets before `release-readiness-evidence`. |
| FE-GUIDE-AC-023 | Guide-owned normative text is classified as frontend verification, implementation support, source limit, design-direction import, Core restatement, harness restatement, or rationale. |
| FE-GUIDE-AC-024 | Frontend phase registry validation uses `cartulary.frontend_phase_registry.v5`, permits only active current phases, requires `row_rollup_state=active_green`, and requires explicit `base_phase_join`. |
| FE-GUIDE-AC-025 | Frontend phase map validation uses `cartulary.frontend_phase_test_map.v4` with implemented active rows, closed owner-reference objects, closed target-reference objects, structured claims, no `null`, and no unknown members. |
| FE-GUIDE-AC-026 | Frontend row accounting validation uses `cartulary.frontend_row_accounting.v5` and includes accounting scope, command ID, registry/map/guide digests, run root, scenario results, row results, required-target closure, and stale or not-closed reasons. |
| FE-GUIDE-AC-027 | Old retained frontend row-accounting artifacts are readable only for diagnostics and cannot close revised rows. |
| FE-GUIDE-AC-028 | `make browser-e2e-a11y` is the implemented accessibility closure target; no public accessibility-preflight target participates in current frontend completion. |
| FE-GUIDE-AC-029 | Visual fixtures are governed by `tools/frontend_visual_fixture_registry.json` and `cartulary.frontend_visual_fixture_registry.v3`; missing fixtures block affected rows instead of using generic TODO text. |
| FE-GUIDE-AC-030 | Evidence freshness includes guide, registry, phase maps, owner refs, visual fixture registry, target command IDs, schemas, run root, artifact hash, and working-tree state. |
| FE-GUIDE-AC-031 | Dirty-worktree frontend evidence is non-release evidence and cannot activate a frontend phase. |
| FE-GUIDE-AC-032 | Claim-publication review uses `cartulary.frontend_claim_publication_review.v1`, defaults to `claim_publication_intent="none"`, and requires Core 05 predicate and artifact-bundle metadata only for claim-bearing publication. |
| FE-GUIDE-AC-033 | FE-P0 has seven current rows, and generated inventories, ledgers, and task guidance derive row counts from phase maps. |
...
| FE-GUIDE-AC-038 | Frontend inspector verification covers feature-group registry rendering, config-derived panels, no-row state, stale-state invalidation, same-shell workflow actions, closed-hot-path operations, authorization demotion, keyboard access, and visual/a11y fixture coverage. |
