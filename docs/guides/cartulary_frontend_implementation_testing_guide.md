...
Frontend verification contract. `tools/frontend_phase_registry.json` MUST declare `schema_id="cartulary.frontend_phase_registry.v2"`, `schema_version=2`, `phase_namespace="frontend"`, `guide_path`, `guide_digest`, and `phases[]`. Valid phase IDs are exactly `FE-P0` through `FE-P11`. Valid `status` values are `planned`, `active`, and `retired`, and `status` represents execution eligibility only. Each phase item MUST include `phase_id`, `status`, `row_rollup_state`, `manifest_path`, `manifest_digest`, `ledger_path`, `ledger_digest`, `owner_refs[]`, `depends_on[]`, `activation_blockers[]`, and `evidence_freshness_digest`.
...
| FE-GUIDE-AC-024 | Frontend phase registry validation uses `cartulary.frontend_phase_registry.v2` and separates execution `status` from `row_rollup_state`. |
...
| FE-GUIDE-AC-029 | Visual fixtures are governed by `tools/frontend_visual_fixture_registry.json` and `cartulary.frontend_visual_fixture_registry.v2`; missing fixtures block affected rows instead of using generic TODO text. |
