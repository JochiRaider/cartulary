...
- Frontend accessibility row evidence MUST use `make browser-e2e-a11y` and the retained `cartulary.frontend_accessibility_summary.v2` artifact unless a live frontend phase map explicitly routes an implemented row through `make browser-e2e-a11y-preflight` with required v4 frontend row accounting. Accessibility rows remain blocked from frontend phase completion until the mapped target exists, emits its normalized summary artifact, and maps each implemented `FE-A11Y-*` row to executed scenarios. Blocked future-row smoke MAY run through `make browser-e2e-a11y-preflight` and `cartulary.frontend_accessibility_preflight_summary.v1`, but that preflight summary alone does not complete blocked rows or replace row-owned accounting.
...
Release-readiness aggregation may carry frontend rows forward as release-gate inputs, but it does not change the row evidence class. Visual, accessibility, support, and design-direction rows remain non-product-conformance evidence in `cartulary.release_readiness_evidence.v1`, and no frontend release-readiness artifact is Core 05 publication evidence unless an owner document later activates a claim-bearing publication route.
...
The target MUST retain exactly one normalized accessibility evidence artifact named `frontend-accessibility-summary.json` with `schema_id="cartulary.frontend_accessibility_summary.v2"`. The artifact represents implemented accessibility rows only. Raw Playwright output, DOM snapshots, screenshots, traces, rule-engine output, and blocked-row preflight smoke are diagnostic inputs; they MUST NOT replace the normalized summary artifact.

The testing harness NLSpec owns `make browser-e2e-a11y` target mechanics and the normalized `cartulary.frontend_accessibility_summary.v2` schema. This guide maps implemented frontend accessibility rows to that target; it MUST NOT define an incompatible schema shape. As a derived restatement, the current harness schema includes these arrays:
...
`make browser-e2e-a11y-preflight` is an explicit helper target for blocked future-row smoke and for frontend rows whose live phase map explicitly requires preflight v4 row accounting. Its `cartulary.frontend_accessibility_preflight_summary.v1` artifact may list blocked rows, explicitly mapped implemented preflight rows, and smoke scenarios, but the summary alone is not row-completion evidence.
...
| `make browser-e2e-a11y` | Deterministic implemented-row browser accessibility scenarios and `cartulary.frontend_accessibility_summary.v2` evidence. |
| `make browser-e2e-a11y-preflight` | Explicit blocked-row browser accessibility smoke and `cartulary.frontend_accessibility_preflight_summary.v1` evidence. |
...
| FE-H-11 | Keyboard and focus traversal | P1 through P11 | Core 03 Sections 4.1, 18, and 19; UI/UX guide Section 14 | `make browser-e2e-support`; `make browser-e2e-a11y`; `make browser-e2e-a11y-preflight` | Keyboard access, visible focus, focus restoration, edit entry/exit, menus, switchers, inspector controls, conflict resolver, and `Esc` priority are covered. |
| FE-H-12 | Accessibility names, ARIA, and state communication | P1 through P11 | UI/UX guide Section 14; `docs/design.md` Accessibility Direction | `make browser-e2e-a11y`; `make browser-e2e-a11y-preflight` | Active tab/surface/menu/group/conflict/save/presence/evidence states are named and communicated without color alone. |
...
1. Every phase row executes in its intended layer: unit rows in frontend unit/typecheck targets, integration rows in frontend integration-style or browser-backed targets, browser rows in Playwright/browser targets, visual rows in `make browser-e2e-visual`, implemented accessibility rows in `make browser-e2e-a11y` or an explicitly mapped preflight row-accounting target, blocked accessibility smoke in `make browser-e2e-a11y-preflight`, and support rows in their named support targets.
...
7. Rows with unresolved owner references, non-empty `blockers[]`, `claim_status="blocked"`, or `claim_status="stale"` are not counted as authoritative completion criteria.
...
12. Accessibility row evidence exists for every implemented phase row that introduces user-visible UI and is represented in `cartulary.frontend_accessibility_summary.v2`, or in explicitly mapped `cartulary.frontend_row_accounting.v4` plus `cartulary.frontend_accessibility_preflight_summary.v1` when the live phase map requires preflight closure; blocked rows MAY appear only in preflight evidence.
...
Frontend verification contract. `tools/frontend_phase_registry.json` MUST declare `schema_id="cartulary.frontend_phase_registry.v4"`, `schema_version=4`, `phase_namespace="frontend"`, `guide_path`, `guide_digest`, and `phases[]`. Valid phase IDs are the append-only contiguous declared range beginning at `FE-P0`; the current registry declares `FE-P0` through `FE-P11`. Valid `status` values are `planned`, `active`, and `retired`, and `status` represents execution eligibility only. Each phase item MUST include `phase_id`, `status`, `row_rollup_state`, `base_phase_join`, `manifest_path`, `manifest_digest`, `ledger_path`, `ledger_digest`, `owner_refs[]`, `depends_on[]`, `activation_blockers[]`, and `evidence_freshness_digest`. `base_phase_join` is either a base `phase<N>` identifier or `null`; it is the only owner for frontend/base phase browser selection and duration joins, and it MUST NOT be inferred from the `FE-P<N>` numeric suffix.

Frontend verification contract. A frontend phase MAY move to `status="active"` only when its frontend map, generated ledger, public targets, target schedule metadata, row evidence, evidence-class owner metadata, and evidence freshness are promoted in the same change. An active phase MUST have `row_rollup_state="active_green"` and no `activation_blockers[]`. A planned phase MAY be explainable by task guidance but MUST NOT be executable as a whole phase by `phase-slice` or `service-backed-slice`. Explicit `PHASE_NAMESPACE=frontend PHASE=FE-P<N> ROWS=<FE-row-id,...>` slices MAY execute implemented or stale rows from planned phases without activating the whole phase. Direct standalone frontend-aware targets MAY still account for implemented or stale rows from planned phases so partially implemented phase rows can collect current row-owned evidence without activating the whole phase.

Frontend verification contract. Each `tools/frontend_phase_maps/fe_p*_test_map.json` file MUST declare `schema_id="cartulary.frontend_phase_test_map.v3"`, `schema_version=3`, `phase_namespace="frontend"`, matching `phase_id`, `guide_digest`, and `rows[]`. Each row MUST include `phase_id`, closed `id`, closed `layer`, closed `evidence_class`, `default_check_required`, structured `owner_refs[]`, `core_req_ids[]`, `core_ac_ids[]`, `support_or_design_ac_ids[]`, structured `targets[]`, `scenario_titles[]`, closed `claim_status`, structured `claim`, `blockers[]`, and `out_of_scope[]`. Maps MUST NOT contain `null`, unknown object members, duplicate row IDs, duplicate object members, or lifecycle contradictions. `FE-V-*` and `FE-A11Y-*` rows MUST remain `design_direction` or `implementation_support` unless they use an explicit Core 05 `claim_publication_boundary` route; they MUST NOT use `product_conformance`. `default_check_required` means the row belongs in the local default-check projection; explicit browser, visual, accessibility, support, and claim-publication evidence remains reachable through its direct target even when this field is `false`.

Frontend verification contract. Generated frontend ledgers under `docs/testing/frontend_phase_coverage_ledgers/` MUST render registry metadata, row metadata, target mapping, evidence class, owner refs, Core IDs, support/design AC IDs, blocked status, scenario titles, and source-map claims. The maps are the source of truth; ledgers are rendered companions and historical plans are non-closure evidence.

Frontend verification contract. Frontend-aware targets that map one or more frontend rows MUST retain `<target>/frontend-row-accounting.json` with schema ID `cartulary.frontend_row_accounting.v4` whenever the target maps frontend rows or an explicit slice scope disables frontend row accounting. That artifact is the durable row-to-scenario closure record for target runs and MUST include accounting scope, command ID, map and registry digests, guide digest, run root, scenario results, row results, required-target closure, and stale or not-closed reasons. Standalone broad frontend-aware targets MUST use `accounting_scope.mode="active_target"` and enforce implemented or stale rows mapped to the target from active and planned frontend phases while excluding planned rows whose `claim_status` is `blocked`, `not_implemented`, or `retired`. Whole `PHASE_NAMESPACE=frontend PHASE=FE-P<N>` slices MUST use `accounting_scope.mode="selected_rows"` and enforce selected active frontend rows through `FE-P<N>` only. Explicit `PHASE_NAMESPACE=frontend PHASE=FE-P<N> ROWS=<FE-row-id,...>` slices MUST use `accounting_scope.mode="selected_rows"` and enforce only the selected implemented or stale frontend row IDs through `FE-P<N>`, including rows from planned phases. Base `PHASE=phaseN` slices MUST use `accounting_scope.mode="disabled"` for frontend-aware child summaries and MUST NOT fail on non-selected `FE-*` rows. Target and tool-run summaries MUST reference `frontend-row-accounting.json` through artifact refs rather than duplicating row accounting under `extensions["cartulary.frontend_row_accounting"]`. Base browser `manifest-selected-tests.json` files remain base phase inventory and MUST NOT be used as frontend row closure evidence. Product-conformance and design-direction rows MUST close from exact `scenario_titles[]` in the intended target artifact. An implemented `implementation_support` row with intentionally empty `scenario_titles[]` MAY close from a passing mapped target when the support assertion is target-level rather than scenario-level. A target that exits successfully while an implemented frontend row inside its accounting scope is not closed MUST fail target-summary generation with `failure_class="harness"` and `failure_reason="frontend_row_accounting"`; blocked rows remain excluded from completion and may stay missing without failing the target. `release-readiness-evidence` consumes only v4 row-accounting artifacts from the current run root and derives explicit release-gate, product-conformance, and claim-publication effects from the row class. Retained v1/v2/v3 artifacts are diagnostic only and MUST NOT close v4 rows or satisfy release-readiness aggregation. Frontend row accounting remains harness/frontend verification evidence and MUST NOT move into `/apps/web`, `/packages/grid-adapter`, or product runtime architecture unless a later owner document creates that runtime contract.
...
| FE-GUIDE-AC-019 | `make browser-e2e-a11y` emits `cartulary.frontend_accessibility_summary.v2` with implemented row, scenario, keyboard, state-communication, contrast, violation, and artifact-ref arrays. |
...
| FE-GUIDE-AC-022 | `make check` includes frontend map validation and frontend ledger drift; `make release-check` additionally runs mapped frontend support, visual, accessibility, and accessibility-preflight readiness targets before `release-readiness-evidence`. |
...
| FE-GUIDE-AC-024 | Frontend phase registry validation uses `cartulary.frontend_phase_registry.v4`, permits append-only contiguous phase growth, separates execution `status` from `row_rollup_state`, and requires explicit `base_phase_join`. |
| FE-GUIDE-AC-025 | Frontend phase map validation uses `cartulary.frontend_phase_test_map.v3` with closed owner-reference objects, closed target-reference objects, structured claims, explicit blockers, no `null`, and no unknown members. |
| FE-GUIDE-AC-026 | Frontend row accounting validation uses `cartulary.frontend_row_accounting.v4` and includes accounting scope, command ID, registry/map/guide digests, run root, scenario results, row results, required-target closure, and stale or not-closed reasons. |
...
| FE-GUIDE-AC-028 | `make browser-e2e-a11y` is the default implemented accessibility closure target; `make browser-e2e-a11y-preflight` is limited to blocked future-row smoke unless the live frontend phase map explicitly requires preflight v4 row accounting. |
