...
`make browser-e2e-a11y-preflight` is an explicit helper target for blocked future-row smoke and for frontend rows whose live phase map explicitly requires preflight v3 row accounting. Its `cartulary.frontend_accessibility_preflight_summary.v1` artifact may list blocked rows, explicitly mapped implemented preflight rows, and smoke scenarios, but the summary alone is not row-completion evidence.
...
Frontend verification contract. `tools/frontend_phase_registry.json` MUST declare `schema_id="cartulary.frontend_phase_registry.v3"`, `schema_version=3`, `phase_namespace="frontend"`, `guide_path`, `guide_digest`, and `phases[]`. Valid phase IDs are the append-only contiguous declared range beginning at `FE-P0`; the current registry declares `FE-P0` through `FE-P11`. Valid `status` values are `planned`, `active`, and `retired`, and `status` represents execution eligibility only. Each phase item MUST include `phase_id`, `status`, `row_rollup_state`, `manifest_path`, `manifest_digest`, `ledger_path`, `ledger_digest`, `owner_refs[]`, `depends_on[]`, `activation_blockers[]`, and `evidence_freshness_digest`.
...
| FE-GUIDE-AC-024 | Frontend phase registry validation uses `cartulary.frontend_phase_registry.v3`, permits append-only contiguous phase growth, and separates execution `status` from `row_rollup_state`. |
...
| FE-GUIDE-AC-028 | `make browser-e2e-a11y` is the default implemented accessibility closure target; `make browser-e2e-a11y-preflight` is limited to blocked future-row smoke unless the live frontend phase map explicitly requires preflight v3 row accounting. |
