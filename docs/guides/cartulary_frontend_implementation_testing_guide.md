...
Frontend verification contract. Frontend-aware targets that map one or more frontend rows MUST retain `<target>/frontend-row-accounting.json` with schema ID `cartulary.frontend_row_accounting.v2`. That artifact is the durable row-to-scenario closure record for target runs and MUST include command ID, map and registry digests, guide digest, run root, scenario results, row results, required-target closure, and stale or not-closed reasons. Target and tool-run summary extensions under `cartulary.frontend_row_accounting` are compatibility copies only; new audit workflows SHOULD prefer the schema-owned artifact. Base browser `manifest-selected-tests.json` files remain base phase inventory and MUST NOT be used as frontend row closure evidence. Product-conformance and design-direction rows MUST close from exact `scenario_titles[]` in the intended target artifact. An implemented `implementation_support` row with intentionally empty `scenario_titles[]` MAY close from a passing mapped target when the support assertion is target-level rather than scenario-level. A target that exits successfully while an implemented frontend row is not closed MUST fail target-summary generation with `failure_class="harness"` and `failure_reason="frontend_row_accounting"`; blocked rows remain excluded from completion and may stay missing without failing the target. Retained v1 artifacts are diagnostic only and MUST NOT close v2/v3 rows.
...
| FE-GUIDE-AC-026 | Frontend row accounting validation uses `cartulary.frontend_row_accounting.v2` and includes command ID, registry/map/guide digests, run root, scenario results, row results, required-target closure, and stale or not-closed reasons. |
...
| FE-GUIDE-AC-036 | Frontend row closure fails target-summary generation with `failure_reason="frontend_row_accounting"` when an implemented required row is missing, stale, or not closed. |
