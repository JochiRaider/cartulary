...
- Frontend accessibility row evidence MUST use `make browser-e2e-a11y`, retained `cartulary.frontend_accessibility_summary.v2` artifacts, and v4 frontend row accounting. Accessibility rows remain incomplete until the mapped target exists, emits its normalized summary artifact, and maps each implemented `FE-A11Y-*` row to executed scenarios.
...
The target MUST retain exactly one normalized accessibility evidence artifact named `frontend-accessibility-summary.json` with `schema_id="cartulary.frontend_accessibility_summary.v2"`. The artifact represents implemented accessibility rows only. Raw Playwright output, DOM snapshots, screenshots, traces, and rule-engine output are diagnostic inputs; they MUST NOT replace the normalized summary artifact.
...
12. Accessibility row evidence exists for every implemented phase row that introduces user-visible UI and is represented in `cartulary.frontend_accessibility_summary.v2` plus `cartulary.frontend_row_accounting.v4`.
