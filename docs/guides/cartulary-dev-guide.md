...
| Paste                     | Workbook clipboard owner, grid adapter, and sync engine | Enabled only where active selection contains mutation-capable writable cells | Creates or patches through public write contracts                                   | The workbook owner decodes scalar or rectangular CSV/TSV input; the adapter plans visible semantic targets without parsing; group rows, read-only cells, and presentation rows MUST NOT enqueue mutations. |
...
The adapter is the event owner for committed-cell navigation, range extension, page and edge movement, keyboard editor entry, editor cancel/commit transitions, and grid exit. Its private presentation model is the only translation from stable semantic anchors to visible semantic positions and RDG positions, including grouped and collapsed rows. Application code MUST call `GridHandle.moveFocus`, `GridHandle.planPasteTargets`, `focusAnchor`, or `scrollToAnchor` instead of rebuilding visible coordinate maps. Application code MAY own keyboard behavior inside an explicit collection/action control, menu, inspector, conflict resolver, or evidence preview, but that handler MUST stop the consumed event so RDG cannot dispatch the same command. Workbook code MUST NOT recover cells by querying RDG DOM coordinates or class names.
...
Every production workbook grid MUST keep RDG row and column virtualization enabled, independent of result size. No small-result threshold or surface exception is permitted. The root component binds virtualization on structurally; no prop, process-global setter, environment variable, storage value, or test setup may alter that production binding. A package-private immutable non-virtualized binding exists only for explicitly classified package-local DOM-unit tests and MUST NOT be exported from the root facade or `./test-support`.

Application tests whose postcondition is workbook behavior SHOULD explicitly mock the adapter with `@cartulary/grid-adapter/test-support`. That lightweight fake shares deterministic semantic policies and callback contracts with production but is not evidence for RDG lifecycle, virtualization, frozen columns, scrolling, offscreen focus, or vendor event choreography. Those postconditions require package-local production-binding tests or live browser evidence. Measurement evidence remains informative unless Core 05 separately authorizes a claim.
...
If the repository exposes a root `Makefile`, it SHOULD remain the stable human-facing task surface. `make help` is the compact workflow surface, `make task-guide` is the module-owner decision surface, and `make help-all` is the exhaustive public workflow-tiered catalog.

| Command              | Minimum responsibility                                                                                       |
| -------------------- | ------------------------------------------------------------------------------------------------------------ |
| `make help`          | Print the compact workflow task surface without bootstrapping local toolchains                               |
| `make task-guide ROLE=module-author OWNER=module.incidents` | Recommend focused, generated, broad, and release gates from one owner catalog snapshot |
| `make test-slice OWNER=module.incidents` | Run every active executable row owned by the selected owner |
| `make service-backed-test-slice OWNER=module.incidents` | Run every selected owner row whose runtime profile requires managed services |
| `make help-all`      | Print the exhaustive public workflow task surface plus the `owner -> family -> row -> work unit -> artifact` reading model without bootstrapping local toolchains |
...
| `make backend-store` | Run service-backed store-domain backend evidence while preserving semantic owner-row identity              |
...
| `make explain-test-owner OWNER=module.incidents` | Explain one owner's manifest, families, row inventory, evidence distribution, profiles, and narrow commands |
...
- `investigate a run`: owner guidance, task-surface, target-plan, owner, target, run-summary, and fixture-cost inspection without rerunning tests.
- `evidence maintenance`: catalog, generated topology, benchmark claims, and Go, browser, and scheduler work-unit duration-baseline maintenance.
...
Owner-aware targets require an exact active `OWNER`. Optional `ROWS` values are exact comma-separated row IDs owned by that owner. Missing, unknown, duplicate, cross-owner, or zero-row selections fail before setup; runner type does not create a separate frontend ownership namespace.
...
For retained-run duration maintenance, first run `make check` and use that successful full warm check root as `RESULTS_DIR=<root>` for `make agent-finalize`. Do not pass owner-slice, service-backed-only, browser-only, or failed check roots to `agent-finalize`; even a successful `check-service-backed` run root is partial finalizer input. Run `make agent-finalize` without `RESULTS_DIR` when only non-retained maintenance is intended.

The check scheduler and service-backed scheduler use work-unit progressive-disclosure output, and run summaries separately report work units, summary targets, evidence targets, and helper units. Scheduled helper steps write under their own target-owned artifact directory and remain helper support evidence rather than evidence targets unless a schedule explicitly lists them in `produces_summary_targets`. Default `summary` stdout treats successful output as a bounded operator and coding-agent summary: targets print compact `[RESULT]` and `[ARTIFACTS]` lines with status, duration, counts, slowest work, `run_root`, and relative summary JSON path; the check scheduler `[SUMMARY]` line also reports `total_wall_time` for the full scheduler run. Successful child summaries, child stdout/stderr, package-manager banners, scheduler logs, and raw tool output stay in artifacts. Successful default stderr must be empty. Live scheduler progress is bounded `[PROGRESS] target=<target> completed=<done>/<total> running=<ids> blocked=<count> slowest=<id>:<ms> blocker=<id-or-none>` text emitted only by interval or material state changes, and it does not repeat artifact paths. Scheduler runs retain the compact operator stream in `progress-summary.log`, and scheduler summaries retain capped progress snapshots plus slowest-running observations so long runs can be inspected without parsing JSONL. Failure output surfaces `failure_class`, `reason=<failure_reason>`, a direct headline, compact artifact refs, rerun command, and investigation command so product regressions, service/runtime problems, harness failures, timing drift, and missing artifacts can be separated before reading logs. Per-shard or per-work-unit `[STEP]` lines, successful child logs, helper-step stdout and stderr logs, raw progress key/value fields, raw resource claims, active resource totals, resource limits, skip reasons, complete nested scheduler progress, and per-transition telemetry remain available in target-owned artifacts, through `make explain-target TARGET=<target> DETAIL=artifacts`, through `make explain-run RESULTS_DIR=<root|run-dir> [RUN_ID=<id>] [TARGET=<target>] [DETAIL=summary|children|logs|progress]`, and in `CARTULARY_OUTPUT_MODE=verbose` or `CARTULARY_OUTPUT_MODE=debug`. `VERBOSE=1` is a compatibility alias for verbose mode. `CARTULARY_OUTPUT_MODE=machine` emits canonical JSON and must not stream child logs or scheduler progress prose.

During `make test-service-backed` and `make check-service-backed`, service-backed backend and browser work is placed from the owner catalog and authored execution topology. Logical resources, duration weights, fixture profiles, runtime profiles, and reset boundaries remain explicit scheduler inputs. Owner and row identities survive Go batching and Playwright sharding unchanged. Refresh duration baselines only from representative successful owner-based runs; changes to owner-slice planning, scheduler behavior, harness checks, or fast-harness membership require focused harness smoke and baseline validation. Direct service-backed targets may run stateful, measurement, visual, and accessibility leaf work according to their catalog evidence classes, while default local `make check` selects only rows marked `default_check=true`. Summary artifacts keep wall, critical-path, executed, derived, and teardown accounting distinct, and lifecycle failures remain target failures.
...
Browser verification MUST keep isolated ordering and reset policy in the topology-derived `tools/browser_e2e_batch_manifest.json`. Webserver-backed functional browser evidence MUST be selected from active owner rows and split into duration-balanced Playwright work using `tools/browser_e2e_duration_baselines.json` without inferring ownership from historical phase numbers or spec filenames. Frontend package scripts expose stable task aliases, not delivery-phase aliases. The default developer gate schedules lower-layer-gap webserver-backed browser projection evidence plus full-target-equivalent stateful browser evidence directly inside the check scheduler with explicit `browser_stage_*` lane claims; ordinary measurement, visual, and accessibility work remains explicit/CI evidence so browser inventory growth remains resource-visible without making the local correctness gate timing-bound.

The owner catalog has no generic policy-exception layer. An authorized verification skip is explicit, owner-reviewed, reason-coded, and expiring; task-surface runners do not accept ad hoc environment-variable allowances for empty selections.

Backend verification distinguishes semantic evidence class from execution dependency. `backend-unit` remains pure backend work with no `pgtest`, `s3test`, or real runtime startup. `backend-store`, `backend-integration`, `backend-process`, and browser suites retain their runner-specific execution responsibilities, while catalog owner/family/row IDs remain stable across batching and service placement.

The backend Go target plan is inspectable without executing tests. `make target-plan` renders the human execution plan, and `make target-plan-json` emits the deterministic owner/family/row projection consumed by task-surface checks. `tools/test_catalog_owner.json` and its family manifests are the only executable ownership source. `make help-all`, `make task-guide`, `make explain-test-owner`, and `make explain-target` present task evidence through one concept hierarchy: `owner -> family -> row -> scheduler work unit -> artifact`. `make explain-target TARGET=<target> [DETAIL=summary|rows|artifacts]` focuses on one backend, frontend, browser, aggregate, check, or release target. `make fixture-report RESULTS_DIR=<root|run-dir>` inspects existing fixture diagnostics without rerunning tests. `make explain-run RESULTS_DIR=<root|run-dir>` inspects current-schema run summaries, aggregate children, scheduler logs, and work-unit logs without rerunning tests.
...
For flyaway or disconnected deployments, backup artifacts and restore-verification extracts that carry incident data MUST remain on encrypted storage. The current filesystem-backed backup-storage implementation uses authenticated application-level encrypted artifact envelopes under `roots.backup_storage` and requires `CARTULARY_RECOVERY_MASTER_KEY`; raw unencrypted legacy backup artifacts are nonconformant and should be recreated rather than migrated. `roots.backup_storage` is the authoritative deployment-local root for those artifacts. `roots.export_outputs` and `roots.temporary_work` are not substitute backup roots.
