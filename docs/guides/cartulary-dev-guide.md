...
`make check` is the required developer gate. It MUST include contract-generation drift detection, migration verification, an authored-frontend Biome pass, and a failure condition when any authoritative phase-manifest row is absent from actual execution. Only toolchain drift and frontend install are early setup blockers; parallel-safe static validation, Biome, generated-artifact drift, harness smoke, and product checks run together after those blockers pass. When browser suites depend on the real Playwright web-server bootstrap, each verification stage must run its compatible browser suites under one owned shared stack rather than parallelizing multiple independent startup attempts. `make ci` composes the same execution-truth guarantee into the provider-neutral CI surface. `make release-check` is the release verification gate and MUST fail if the configured license report or SBOM artifact is missing or empty.

During `make test-service-backed` and `make check-service-backed`, `browser-e2e-webserver-backed` reuses the active `cartulary-test-services` Postgres and MinIO stack, cloning the migrated suite template database and creating a browser-scoped object bucket through that harness. Service-backed aggregate targets are placed by `tools/service_backed_schedule_manifest.json` through the weighted scheduler in `scripts/run-service-backed-schedule.mjs`; target weights, schedule-level `resource_limits`, and child-level `resource_claims` describe capacity policy while the single `cartulary-test-services` wrapper remains the service owner. For v2 schedules, resource capacity is the active scheduler cap; `SERVICE_BACKED_JOBS` is retained only as a legacy compatibility fallback for manifests without resource limits. Postgres and MinIO are shared services with isolated logical fixtures rather than broad exclusive scheduler locks, while the backend, process, browser, and browser-webserver lanes constrain the work that has real fixture contention. Service-backed aggregate target summaries are finalized after the wrapper has completed teardown, so browser-fixture leak checks, owned-stack fixture reclamation, stale-fixture janitor work, and service termination are included in target timing. Owned service-backed browser fixtures are reclaimed by terminating the ephemeral Postgres and MinIO stack after the leak check; explicit DB-drop and bucket-cleanup spans remain visible for stale, pass-through, and prepare-failure cleanup paths. The final `make test` and `make check` summaries report `backend-service-backed`, `browser-webserver-backed`, and `browser-isolated` duration groups separately, so backend service work, browser web-server lifecycle work, and reset-bounded isolated browser work remain independently attributable. Summary artifacts use explicit accounting fields: wall and critical-path wall time describe elapsed execution, executed duration describes work actually run in the current target, logical duration includes reused phase evidence, reused and derived durations remain separate, and teardown duration is reported independently as disjoint bucket wall time when lifecycle spans overlap. Hidden lifecycle timing failures, including teardown spans emitted by browser and test-service cleanup, are promoted to non-test target and run failures. Direct standalone browser E2E targets continue to use the development Compose stack through the same browser batch manifest.
...
Browser verification MUST also distinguish shared-stack functional browser work from reset-bounded isolated browser classes. Stateful browser suites, fixture-sensitive measurement suites, and visual suites run after the shared service-backed stage through one owned isolated browser batch; ordinary web-server-backed browser suites belong to one owned shared-stack orchestration path and may run alongside backend service-backed work when the harness provides isolated database, bucket, runtime-root, and port allocation.
