...
- `/packages/test-utils` owns route-free browser choreography through the exact `@cartulary/test-utils/grid`, `@cartulary/test-utils/accessibility`, and `@cartulary/test-utils/visual` package subpaths. It has no root export and MUST NOT own Cartulary routes, domain payloads, authentication, test-control tokens, extension profiles, app registries, or product semantics.
...
| `/packages/test-utils`     | Route-free browser setup, actions, focused observers, and diagnostics exposed only through the exact subpaths listed below; product accessibility and behavior assertions remain with their live owners                                                                        |

The supported test-utils package surface is exact:

| Subpath | Current capability |
| --- | --- |
| `@cartulary/test-utils/grid` | Playwright-independent structural browser capabilities; grid sort, filter, grouping, paste, edit-commit, virtual-target setup, focus continuity, mounted-row, viewport, group-row, and marker observations. |
| `@cartulary/test-utils/accessibility` | Focus-continuity and marker-anchor observers only. It is not a generic accessibility assertion API. |
| `@cartulary/test-utils/visual` | Grid scrolling and virtual-target setup plus the read-only marker-anchor observer for visual evidence. |
