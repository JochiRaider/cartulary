...
`make check` is the required developer gate. It MUST include contract-generation drift detection, migration verification, an authored-frontend Biome pass, and a failure condition when any authoritative phase-manifest row is absent from actual execution. Only toolchain drift and frontend install are early setup blockers; after setup, `tools/check_schedule_manifest.json` schedules the readiness gate and independent check work by declared resources. Once build artifacts and service images are ready, `check-service-backed` reserves the parent scheduler's full CPU and IO capacity, forwards those limits to the service-backed scheduler, and starts backend, duration-balanced webserver-backed browser functional shards, and the isolated browser batch inside that reservation while local product checks, static validation, generated-artifact drift detection, frontend unit work, harness smoke, and Go duration-baseline drift remain separate check-scheduler work units. The isolated browser batch uses one owned shared stack through a service-backed scheduler browser stage rather than a final top-level check work unit. Suite-service release checks stay synchronous, while Docker container termination is delegated to a detached lease-backed reaper and is reported as janitorial work rather than target critical-path teardown. `make ci` composes the same execution-truth guarantee into the provider-neutral CI surface. `make release-check` is the release verification gate and MUST fail if the configured license report or SBOM artifact is missing or empty.
