...
`make check` is the required developer gate. It MUST include contract-generation drift detection, migration verification, an authored-frontend Biome pass, and a failure condition when any authoritative phase-manifest row is absent from actual execution. Only toolchain drift and frontend install are early setup blockers; after setup, `tools/check_schedule_manifest.json` schedules the readiness gate and independent check work by declared resources. Once build artifacts and service images are ready, `check-service-backed` starts backend, webserver-backed browser, and isolated browser work through the service-backed scheduler while local product checks, static validation, generated-artifact drift detection, and harness smoke run as separate check-scheduler work units. The isolated browser batch still uses one owned shared stack, but it is no longer a fixed serial tail after service-backed teardown. Suite-service release checks stay synchronous, while Docker container termination is delegated to a detached lease-backed reaper and is reported as janitorial work rather than target critical-path teardown. `make ci` composes the same execution-truth guarantee into the provider-neutral CI surface. `make release-check` is the release verification gate and MUST fail if the configured license report or SBOM artifact is missing or empty.

During `make test-service-backed` and `make check-service-backed`, service-backed backend work and browser evidence are placed by `tools/service_backed_schedule_manifest.json` through the weighted scheduler in `scripts/run-service-backed-schedule.mjs`; v5 schedules declare source class, expand Go-backed backend targets into `scripts/lib/go-shard-plan.mjs` shards, and use schedule-level `resource_limits` as the concurrency authority. Go shard concurrency is split into `go_cpu` and `go_io` resources derived from committed shard timing and fixture metadata, with optional local or CI overrides, while `postgres`, `minio`, `process`, and `browser` remain separate resources. Explicit Make-target work such as `backend-process` remains available for process evidence, while direct aggregate targets such as `backend-integration` and `backend-store` remain standalone developer entrypoints. `make test` still runs the isolated aggregate `browser-e2e` batch after `test-service-backed`; `make check` schedules that same `browser-e2e` isolated batch inside `check-service-backed` so it can start when the webserver-backed browser slot is free. `tools/browser_e2e_batch_manifest.json` owns the `isolated` stage and reset boundaries for stateful, measurement, and visual groups only. The final summaries report `backend-service-backed` and aggregate `browser` duration groups separately. Summary artifacts use explicit accounting fields: wall and critical-path wall time describe elapsed execution, executed duration describes work actually run in the current target, logical duration includes reused phase evidence, reused and derived durations remain separate, and teardown duration is reported independently as disjoint bucket wall time when lifecycle spans overlap. Hidden lifecycle timing failures, including teardown spans emitted by browser and test-service cleanup, are promoted to non-test target and run failures. Direct standalone browser E2E targets continue to use the development Compose stack through the same browser batch manifest.
...
Browser verification MUST keep isolated ordering and reset policy in `tools/browser_e2e_batch_manifest.json`. The default developer gate schedules webserver-backed browser evidence and the isolated `browser-e2e` aggregate through the service-backed scheduler with `browser` capacity set to one, so stateful, fixture-sensitive measurement, and visual browser suites run through one owned stack as soon as the webserver-backed browser slot is free.
