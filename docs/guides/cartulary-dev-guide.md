...
`make check` is the required developer gate. It MUST include contract-generation drift detection, migration verification, an authored-frontend Biome pass, and a failure condition when any authoritative phase-manifest row is absent from actual execution. Only toolchain drift and frontend install are early setup blockers; after setup, `tools/check_schedule_manifest.json` schedules the readiness gate and independent check work by declared resources from `tools/scheduler_resource_registry.json`. Once build artifacts and service images are ready, `check-service-backed` claims the exclusive `service_stack` lane plus a bounded `host_cpu` and `host_io` budget, forwards those bounded limits through `check_host_to_service_backed_go` to service-backed `go_cpu` and `go_io`, and starts backend, duration-balanced webserver-backed browser functional shards, and the isolated browser batch while cheap independent check work can run beside it. Static validation leaves, generated-artifact drift detection, migration verification, backend lint, pure backend unit evidence, frontend type-checking, frontend unit work, harness smoke, and Go duration-baseline coverage remain separate check-scheduler work units so failures name the concrete owner. Duration-baseline drift checks remain downstream of `check-service-backed` because they consume its timing artifacts. The isolated browser batch uses one owned shared stack through a service-backed scheduler browser stage rather than a final top-level check work unit. Suite-service release checks stay synchronous, while Docker container termination is delegated to a detached lease-backed reaper and is reported as janitorial work rather than target critical-path teardown. `make ci` composes the same execution-truth guarantee into the provider-neutral CI surface. `make release-check` is the release verification gate and MUST fail if the configured license report or SBOM artifact is missing or empty.

The check scheduler and service-backed scheduler use work-unit progressive-disclosure output, and run summaries separately report work units, summary targets, evidence targets, and helper units. Default quiet stdout treats successful output as an operator summary: aggregate evidence targets print one compact result line with slowest-child and failure hints, successful child summaries and successful scheduler logs stay in artifacts, and live scheduler progress is printed as `[PROGRESS]` text only after the progress interval or when blocked work needs explanation. Those progress lines report the completed work count, nested scheduler count when present, currently running work, blocker, next unblocker, slowest running work, and scheduler artifact path without exposing raw key/value telemetry. Failure output surfaces `failure_class=test|infra|helper|timing|artifact` plus a direct headline so product regressions, service/runtime problems, harness failures, timing drift, and missing artifacts can be separated before reading logs. Per-shard or per-work-unit `[STEP]` lines, successful child logs, raw progress key/value fields, raw resource claims, active resource totals, resource limits, skip reasons, and per-transition telemetry remain available in scheduler JSON artifacts, through `make explain-run RESULTS_DIR=<root|run-dir> [RUN_ID=<id>] [TARGET=<target>] [DETAIL=summary|children|logs]`, in `CARTULARY_OUTPUT_MODE=machine`, and in `VERBOSE=1` or `CI_VERBOSE=1` output rather than quiet stdout.

During `make test-service-backed` and `make check-service-backed`, service-backed backend work, webserver-backed browser evidence, and isolated browser evidence are placed by `tools/service_backed_schedule_manifest.json` through the weighted scheduler in `scripts/run-service-backed-schedule.mjs`; v8 schedules declare source class, expand Go-backed backend targets into `scripts/lib/go-shard-plan.mjs` shards, and use schedule-level `resource_limits` as the concurrency authority. Go shard concurrency is split into `go_cpu` and `go_io` resources derived from committed shard timing and fixture metadata, with optional local or CI overrides, while `postgres`, `minio`, `process`, `browser_stack`, and per-stage `browser_stage_*` lanes remain separate resources. Go duration baselines use explicit v4 timing components: top-level test elapsed time, target-scoped package overhead, target command overhead, and raw aggregate wall time. `make go-test-duration-baseline-coverage` verifies that the current Go shard plan has committed timing components for every service-backed test, target package, command overhead, and raw aggregate before the run-time drift check consumes observed artifacts. Explicit Make-target work such as `backend-process` remains available for process evidence, while direct aggregate targets such as `backend-integration` and `backend-store` remain standalone developer entrypoints. Webserver-backed browser functional evidence is selected from the authoritative phase manifests and split by `scripts/lib/browser-shard-plan.mjs` using committed spec weights in `tools/browser_e2e_duration_baselines.json`; shard reports are merged before phase summaries are emitted. The isolated aggregate `browser-e2e` batch is scheduler-visible service-backed work for `make test` and `make check`, while public direct `make browser-e2e` remains a standalone helper. `tools/browser_e2e_batch_manifest.json` owns webserver-backed functional shard mode plus the `isolated` stage and reset boundaries for stateful, measurement, and visual groups. The final summaries report `backend-service-backed` and aggregate `browser` duration groups separately. Summary artifacts use explicit accounting fields: wall and critical-path wall time describe elapsed execution, executed duration describes work actually run in the current target, logical duration includes reused phase evidence, reused and derived durations remain separate, and teardown duration is reported independently as disjoint bucket wall time when lifecycle spans overlap. Hidden lifecycle timing failures, including teardown spans emitted by browser and test-service cleanup, are promoted to non-test target and run failures. Direct standalone browser E2E targets continue to use the development Compose stack through the same browser batch manifest.
