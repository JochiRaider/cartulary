...
The check scheduler and service-backed scheduler use target-level progressive-disclosure output. Default stdout prints concise start, progress, and summary lines that show completed, running, pending, blocked or skipped, active work groups, the blocking dependency or resource, the most likely unblocker, the slowest currently running work, and the scheduler artifact path. Per-shard or per-work-unit `[STEP]` lines, raw resource claims, active resource totals, resource limits, skip reasons, and per-transition telemetry remain available in scheduler JSON artifacts and in `VERBOSE=1` or `CI_VERBOSE=1` output rather than quiet stdout.
