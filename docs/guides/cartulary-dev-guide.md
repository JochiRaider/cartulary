...
- `/packages/test-utils` owns route-free browser choreography through the exact `@cartulary/test-utils/grid` package subpath. It has no root export and MUST NOT own Cartulary routes, domain payloads, authentication, test-control tokens, extension profiles, app registries, or product semantics.
...
/
  ...
  /internal
    ...
    /modules
      ...
      /projections                   # Workbook projection boundary; no root Go package
        /adapters                    # Sole production constructor; imported by projectionassembly only
        /providercontract            # Immutable descriptors and semantic query intent
        /testsupport                 # Named typed test capabilities only
        /internal                    # Private runtime, storage, and query compilation
...
| `projections`    | All production `*_grid_projection` SQL, typed projection writers/readers, private query compilation and paging, rebuild mechanics, and immutable descriptor assembly | Source authority, authorization, source/history mutation, Collaboration publication, and Reporting semantics |
...
- All production reads, writes, deletes, locks, cleanup, and row counts against
  descriptor-owned projection tables stay inside the compiler-contained
  Projections storage or query packages. Source owners supply typed source data
  and semantic intent, not table SQL.
- Projection writers participating in a source mutation use the caller-owned
  transaction and never begin, commit, roll back, authorize, or publish
...
| `/packages/test-utils`     | Route-free browser setup, actions, focused observers, and diagnostics exposed only through the exact subpath listed below; product accessibility and behavior assertions remain with their live owners                                                                         |

The supported test-utils package surface is exact:

| Subpath | Current capability |
| --- | --- |
| `@cartulary/test-utils/grid` | Playwright-independent semantic grid sort, filter, grouping, paste, virtual-target setup, focus continuity, mounted-row, viewport, group-row, and marker choreography. Browser capability primitives and structural types remain private. |

There is no `@cartulary/test-utils` root export. Product-owned keyboard,
accessible-name, ARIA, contrast, and state assertions remain direct live-owner
evidence unless a reusable helper satisfies the frontend guide's complete
...
`make check` is the required developer gate. The owner catalog assigns every active row one `minimum_tier`; `check` selects `fast` and `standard` rows, while static, boundary, generated-drift, security, and evidence checks come from the aggregate policy. `test-fast`, `test`, `ci`, and `release-check` select the monotonic tier closures defined by the Testing Harness NLSpec. Owner slices are independent of tiers: omitting `ROWS` selects the complete active owner inventory.

All graph-backed public targets compile through the same work-graph owner, graph compiler, scheduler, capability snapshot, fixture broker, and canonical event writer. The scheduler uses critical-path rank, resource-fitting backfill, monotonic aging, and stable work-unit IDs. `CARTULARY_HARNESS_CAPACITY_OVERRIDE` is the only scheduler capacity override; it must name a schema-valid JSON declaration file. Inline JSON is unsupported. `PLAYWRIGHT_WORKERS` and `VITEST_MAX_WORKERS` remain runner-specific inputs, and their resource effects are represented in the affected work-unit claims.

Service-backed rows declare an explicit fixture capability rather than inheriting a clone-oriented fixture profile. The run-scoped broker owns managed resources, replenishes quarantined dedicated databases asynchronously, and keeps borrowed services borrowed. Go work is grouped only when its runner, environment, fixture, and isolation semantics are compatible, then distributed deterministically from available Go capacity. Browser groups, resets, readiness, finalizers, and summaries are individual graph units; stateful groups preserve affinity and reset order while independent lanes can overlap.

Aggregates are graph unions rather than nested Make or scheduler phases. A semantic unit selected by more than one target executes once per run and contributes to each applicable target projection. Registered cache policies are `none`, `same_run`, or `content_addressed`; graph commands support normal, cold, and off modes, with the selected mode recorded in `run-manifest.json`. Stateful, lifecycle, migration, measurement, drift, publication, and other registered non-cacheable work always executes.

Canonical retained output is `run-manifest.json`, `unit-events.ndjson`, `run-summary.json`, and `target-summaries/<target>.json`. These files are the timing, failure, resource, cache, critical-path, and target-projection authority. Historical scheduler, wrapper, shard, and specialized duration artifacts are archival only and are not current inputs. Use `make explain-target`, `make explain-run`, and the canonical observability commands to inspect a run.

For performance maintenance, collect one forced-cold observation, discard one warm-up, and retain five warm observations per measured target, adding a matched sixth only under the adopted instability rule. `harness-public-target-duration-baselines` is the sole v3 baseline publisher. The retained specialized baseline target names now expose read-only canonical views; they no longer own separate baseline files or timing authorities. Publish a baseline only after the complete functional and performance acceptance ladders pass.

`make agent-finalize` renders generated state once into scratch, validates it once, and publishes atomically only when refresh is required. Pass `RESULTS_DIR` only for a qualifying successful full warm `check` root; omit it when retained-run maintenance is not available. `make ci` and `make release-check` use the same graph and evidence contracts, with release-only policy work added by the release tier.
...
When one graph work unit fails, already-started independent siblings may continue until they drain. That drain behavior is expected orchestration output and is not, by itself, a second verification failure.
...
Browser verification keeps semantic group identity, isolated ordering, reset policy, and evidence ownership in the v8 browser manifest. The graph compiler creates one unit for every group, reset, readiness action, and finalizer. Broker-issued browser-stack leases provide unique database, object-store, port, and process identities; measurement quietness and visual write exclusion are explicit resource constraints. Frontend package scripts expose stable semantic task aliases, not delivery-phase aliases.
...
| Notes                   | `artifact` filtered to `artifact_type='note'`                   | `artifacts`   | `artifact_grid_projection`; `cartulary.view.notes.v1`                                     | workbook shell, grid adapter consumer, Notes columns               | Built-in tab. Notes are one artifact-backed surface, not the artifact family as a whole.                                                                                                                                                               |
...
| Assessments             | `assessment`                                                    | `assessments` | `assessment_grid_projection`; `cartulary.view.assessments.v1`                             | workbook shell, system-view registration, assessment columns       | Contract-backed system view.                                                                                                                                                                                                                           |
| Task Requests           | `task_request`                                                  | `tasksdecisions` | `task_request_grid_projection`; `cartulary.view.task_requests.v1`                       | workbook shell, system-view registration, queue-oriented columns   | Contract-backed system view.                                                                                                                                                                                                                           |
| Decisions               | `decision`                                                      | `tasksdecisions` | `decision_grid_projection`; `cartulary.view.decisions.v1`                               | workbook shell, system-view registration, decision columns         | Contract-backed system view.                                                                                                                                                                                                                           |
| Parties                 | `party`                                                         | `parties`     | `party_grid_projection`; `cartulary.view.parties.v1`                                      | workbook shell, system-view registration, party columns            | Contract-backed system view. Party identity is incident-scoped coordination identity, not deployment-local account identity.                                                                                                                           |
| Communications Log      | `artifact` filtered to `artifact_type='comm_log'`               | `artifacts`   | `artifact_grid_projection` filtered to `comm_log`; `cartulary.view.comm_log.v1`           | workbook shell, coordination-surface components                    | Workbook-native coordination surface. Canonical base surface is the standardized `view_schema_id`; an additional `scope='system'` saved view over the same schema MAY exist as an additive preset only and MUST NOT replace the required base surface. |
| Handoff                 | `artifact` filtered to `artifact_type='handoff'`                | `artifacts`   | `artifact_grid_projection` filtered to `handoff`; `cartulary.view.handoff.v1`             | workbook shell, coordination-surface components                    | Workbook-native coordination surface. Canonical base surface is the standardized `view_schema_id`; an additional `scope='system'` saved view over the same schema MAY exist as an additive preset only and MUST NOT replace the required base surface. |
| Status Review           | `artifact` filtered to `artifact_type='status_review'`          | `artifacts`   | `artifact_grid_projection` filtered to `status_review`; `cartulary.view.status_review.v1` | workbook shell, coordination-surface components                    | Workbook-native coordination surface. Canonical base surface is the standardized `view_schema_id`; an additional `scope='system'` saved view over the same schema MAY exist as an additive preset only and MUST NOT replace the required base surface. |
| Lesson                  | `artifact` filtered to `artifact_type='lesson'`                 | `artifacts`   | `artifact_grid_projection` filtered to `lesson`; `cartulary.view.lesson.v1`               | workbook shell, coordination-surface components                    | Workbook-native coordination surface. Canonical base surface is the standardized `view_schema_id`; an additional `scope='system'` saved view over the same schema MAY exist as an additive preset only and MUST NOT replace the required base surface. |
...
If the implementation exposes standardized optional artifact-backed surfaces in the current profile, the `artifacts` module also owns `cartulary.view.findings.v1`, `cartulary.view.investigative_queries.v1`, and `cartulary.view.forensic_keywords.v1` over filtered artifact-backed or otherwise owner-defined storage. Those surfaces remain optional and MUST use the standardized `view_schema_id` values when exposed.

The backend-owner column identifies authoritative source semantics. Physical
access to every projection table in this matrix belongs to Projections. Owner
facades contribute immutable descriptors, semantic query intent, typed source
readers, and caller-transaction writers; they do not expose projection-table
SQL. Only `internal/app/projectionassembly` constructs the Projections adapter,
and consumers receive narrow Workbook, Recovery, Revisions, or source-owner
ports rather than a runtime catalog or store.
...
| `operator restore latest` | Requires an absolute `--target-config-file` and exact `--confirm-backup-set-id`; after response loss, retry with the prior exact `--operation-id` so durable terminal evidence can be replayed. |
