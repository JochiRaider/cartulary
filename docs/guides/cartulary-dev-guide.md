...
`make check` is the required developer gate. It MUST include contract-generation drift detection, migration verification, an authored-frontend Biome pass, and a failure condition when any authoritative phase-manifest row is absent from actual execution. Only toolchain drift and frontend install are early setup blockers; after setup, `tools/check_schedule_manifest.json` schedules the readiness gate and independent check work by declared resources. Once build artifacts and service images are ready, `check-service-backed` starts backend and duration-balanced webserver-backed browser functional shards while local product checks, static validation, generated-artifact drift detection, frontend unit work, harness smoke, and Go duration-baseline drift run as separate check-scheduler work units. The isolated browser batch uses one owned shared stack and is the final top-level check work unit after the rest of the gate quiesces. Suite-service release checks stay synchronous, while Docker container termination is delegated to a detached lease-backed reaper and is reported as janitorial work rather than target critical-path teardown. `make ci` composes the same execution-truth guarantee into the provider-neutral CI surface. `make release-check` is the release verification gate and MUST fail if the configured license report or SBOM artifact is missing or empty.

The check scheduler and service-backed scheduler use progressive-disclosure output. Default stdout prints concise start, progress, and summary lines that show completed, running, pending, blocked or skipped, next, and slowest-work state without raw resource maps. Full resource claims, active resource totals, resource limits, skip reasons, and per-transition telemetry remain available in scheduler JSON artifacts and in `VERBOSE=1` or `CI_VERBOSE=1` output.

During `make test-service-backed` and `make check-service-backed`, service-backed backend work and webserver-backed browser evidence are placed by `tools/service_backed_schedule_manifest.json` through the weighted scheduler in `scripts/run-service-backed-schedule.mjs`; v6 schedules declare source class, expand Go-backed backend targets into `scripts/lib/go-shard-plan.mjs` shards, and use schedule-level `resource_limits` as the concurrency authority. Go shard concurrency is split into `go_cpu` and `go_io` resources derived from committed shard timing and fixture metadata, with optional local or CI overrides, while `postgres`, `minio`, `process`, and `browser_stack` remain separate resources. Go duration baselines use explicit v4 timing components: top-level test elapsed time, target-scoped package overhead, target command overhead, and raw aggregate wall time. Explicit Make-target work such as `backend-process` remains available for process evidence, while direct aggregate targets such as `backend-integration` and `backend-store` remain standalone developer entrypoints. Webserver-backed browser functional evidence is selected from the authoritative phase manifests and split by `scripts/lib/browser-shard-plan.mjs` using committed spec weights in `tools/browser_e2e_duration_baselines.json`; shard reports are merged before phase summaries are emitted. `make test` still runs the isolated aggregate `browser-e2e` batch after `test-service-backed`; `make check` schedules that same `browser-e2e` isolated batch as the final top-level check work unit after service-backed and heavy local check work complete. `tools/browser_e2e_batch_manifest.json` owns webserver-backed functional shard mode plus the `isolated` stage and reset boundaries for stateful, measurement, and visual groups. The final summaries report `backend-service-backed` and aggregate `browser` duration groups separately. Summary artifacts use explicit accounting fields: wall and critical-path wall time describe elapsed execution, executed duration describes work actually run in the current target, logical duration includes reused phase evidence, reused and derived durations remain separate, and teardown duration is reported independently as disjoint bucket wall time when lifecycle spans overlap. Hidden lifecycle timing failures, including teardown spans emitted by browser and test-service cleanup, are promoted to non-test target and run failures. Direct standalone browser E2E targets continue to use the development Compose stack through the same browser batch manifest.
...
Browser verification MUST keep isolated ordering and reset policy in `tools/browser_e2e_batch_manifest.json`. Webserver-backed functional browser evidence MUST be split into duration-balanced Playwright spec shards using `tools/browser_e2e_duration_baselines.json` rather than by phase number. The default developer gate schedules webserver-backed browser evidence through the service-backed scheduler, then schedules the isolated `browser-e2e` aggregate as the final top-level check work unit with `service_stack` capacity so stateful, fixture-sensitive measurement, and visual browser suites run through one owned stack after heavy gate work has quiesced.
