...
`/apps/web/e2e` remains the Playwright discovery and evidence facade. Phase-named spec paths and exact titles may remain when harness accounting or scheduling owns them, but shared support belongs under semantic `support/<owner>` modules. Root fixtures and specs compose those owners; support modules MUST NOT use phase-shaped helper names, wildcard barrels, app-internal workbook registries, protected generated roots, or grid-vendor imports. Public JSON, privileged test-control, and incident-socket transport are separate facades. Workbook-surface identity and required/optional status flow from `/contracts/view-schemas/index.json` through `/packages/view-contracts` and the E2E contract adapter.
...
/
  ...
  /internal
    ...
    /modules
      ...
      /incidents                     # Incidents, lifecycle, memberships, incident access
...
| `/tools/task_surface.generated.mk`       | Generated                           | `tools/task_surface_manifest.json` as public-target owner; execution topology only for scheduler wiring inputs where declared | MUST NOT be hand-edited          |
...
- `make generated-artifact-policy-check` verifies that checked-in generated source artifacts carry `DO NOT EDIT` markers and that authored lint wrappers do not accidentally include generated paths.
...
`make generate-drift` owns regeneration-equivalence: it fails when current generated outputs differ from outputs regenerated from the current authoritative inputs. `make generated-artifact-policy-check` owns generated-source policy: it fails when declared generated source artifacts are missing generated markers, when undeclared file types appear under generated roots, or when authored lint scopes include generated paths.

At minimum, generated-artifact failure means one or more of the following:

- `make generate` changes tracked generated files,
- `make generated-artifact-policy-check` detects an unmarked or unsupported generated source artifact,
...
- `make generated-artifact-policy-check` is part of developer verification and must remain cheaper than full regeneration drift.
...
| `incidents`      | Incident CRUD, lifecycle transitions, memberships, visible-incident access, and lifecycle guards                                                             | Password/MFA logic, blob lifecycle, saved-view ownership, workbook startup/preference ownership |
| `savedviews`     | Saved-view configuration lifecycle, scope visibility, duplication, and saved-view route persistence                                                           | Incident lifecycle, workbook source-row mutation                         |
...
| `entities`       | Hosts, identities, exact-match reusable identifiers, aliases, explicit host/identity merge policy, and entity mention lifecycle boundaries                    | Indicators, assessments, parties, task requests, decisions, artifact-backed coordination surfaces |
| `indicators`     | Canonical indicators, indicator observations, indicator lifecycle intervals, and indicator workbook/import facades                                            | Host/identity merge policy, party records, assessments                   |
| `assessments`    | Assessment records, assessment workbook/import facades, and assessment merge-repoint policy                                                                   | Indicator observation lifecycle, host/identity source records            |
| `parties`        | Party records, exact-match party reuse, party workbook/import facades, and party projection provider inputs                                                  | Deployment users, host/identity records, task/decision lifecycle         |
| `tasksdecisions` | Task request and decision records, task/decision workbook/import facades, and task/decision projection provider inputs                                       | Parties, evidence blobs, artifact-backed coordination text surfaces      |
...
Workbook surfaces and phase evidence rows are not module-boundary authority. This guide follows the current implementation-support package layout; future splits remain owner-spec work, not evidence inferred from test locations.
...
| `inspector drawer`      | Config-derived row-context panels and feature groups for enrichment, relationships, history, rollback, evidence inspection, party linking, mention resolution, merge, supersede, and create-related workflows. | MUST render only declared `inspector_config_v1` panels and `feature_group_key` entries for the active `view_schema_id`; MUST remain non-blocking relative to the grid; MUST NOT persist open state, active panel, previews, local forms, confirmations, rollback previews, or merge plans in saved views. |
| `view-contract adapter` | Parses generated view schema and inspector config artifacts.                                                                        | MUST reject unknown feature keys, route owner tokens, result behaviors, and disabled-state tokens before rendering feature controls.                                |
...
| `accessibility harness` | Reusable keyboard, focus, accessible-name, ARIA, contrast, and state-communication evidence choreography                            | MUST keep helper choreography in `/packages/test-utils`; runtime rendering remains owned by `/apps/web` and `/packages/ui` through stable selectors and design tokens |
...
Selector and test-id builders in `/packages/ui-contracts` are the shared frontend automation contract. Builder inputs with owner-defined syntax, including `view_schema_id`, `field_key`, and closed-vocabulary action or status tokens, MUST fail closed when the value is empty, whitespace-only, malformed, or outside the declared vocabulary. Opaque selector inputs such as `record_id`, item refs, `history_item_ref`, and `history_entry_ref` MUST be rejected when empty or whitespace-only, but MUST otherwise remain syntax-opaque unless the applicable owner spec defines a stricter grammar. Variable selector segments MUST use lossless encoding rather than lossy replacement so distinct stable IDs cannot collapse to the same `data-testid`. Runtime code and tests MUST consume these builders instead of duplicating selector template strings in `/apps/web` or E2E helpers.
...
| Bottom create draft       | Workbook owner through the grid adapter      | Present only when owner-authorized creation is available                       | Draft state only; committed creation uses the public owner route                     | The adapter MAY privately use RDG's `bottomSummaryRows` renderer channel, but exports no summary or aggregation API. The draft has no `record_id`, grouping bucket, record selection, or existing-row mutation target.                    |
...
The adapter is the event owner for committed-cell navigation, range extension, page and edge movement, keyboard editor entry, editor cancel/commit transitions, and grid exit. Its private flattened-position map is the only translation from semantic `record_id + field_key` anchors to RDG positions, including grouped and collapsed rows. Application code MAY own keyboard behavior inside an explicit collection/action control, menu, inspector, conflict resolver, or evidence preview, but that handler MUST stop the consumed event so RDG cannot dispatch the same command. Focus restoration MUST call the semantic `GridHandle`; workbook code MUST NOT recover cells by querying RDG DOM coordinates or class names.

Editor validation, conflict, stale-target, and rejected-mutation outcomes use one adapter-owned assertive announcement. Loading, refresh, bulk-selection count, and range dimensions use polite announcements. Owners MUST avoid rendering a second live region for the same outcome. RTL remains disabled until localization authority adopts direction metadata and mirrored behavior.
...
Semantic state styling MUST follow `docs/design.md` §8.2: conflict, invalid, and pending treatments override lower-priority state while co-displaying active focus; bulk selection remains a row-level cue; read-only/derived treatment applies only without a higher-priority state; stale is an independent row-block cue; and saved has no required persistent success decoration. Every color treatment MUST retain its declared marker, shape, pattern, focus outline, text, or accessible description.
...
The query refresh path, sparse-patch application path, local pending-edit path, and collaboration event reducer MUST preserve unchanged row object references when the row's public data has not changed. A full query response MAY rebuild the containing array, but rows whose authoritative `record_id` and `row_version` are unchanged MUST retain their prior object references. State restoration remains keyed by `record_id`.
...
Every production workbook grid MUST keep RDG row and column virtualization enabled, independent of result size. No small-result threshold or surface exception is permitted. Disabling virtualization is available only through the grid package's test-support-only diagnostic seam for isolated non-layout fixtures; production and live browser evidence MUST NOT use that seam.
...
| `make phase-slice PHASE=phase2` | Run the selected phase's manifest-owned evidence rows through the phase scheduler |
| `make service-backed-slice PHASE=phase2` | Run only the selected phase's service-backed manifest rows through the phase scheduler, reporting an explicit no-op when none exists |
...
| `make explain-phase PHASE=phase2` | Explain one phase's manifest scope, owner refs, execution map, target coverage, scheduler work units, and artifact locations |
| `make explain-phase PHASE_NAMESPACE=frontend PHASE=FE-P3` | Explain one frontend phase's registry metadata, frontend map rows, evidence classes, target mapping, and ledger path |
| `make explain-target TARGET=backend-store DETAIL=rows` | Explain one backend, frontend, browser, aggregate, check, or release target, including scheduler paths and artifacts, without running tests |
...
| `make lint`          | Run backend gofmt/vet/Staticcheck, authored frontend Biome, scripts Biome, structural Markdown lint, blocking ShellCheck, and frontend type checks |
...
| `make go-gosec-audit` | Run advisory Gosec audit profiles for goroutine context and scoped file/path rules outside default local check |
| `make lint-scripts`  | Run check-only Biome lint over JavaScript orchestration scripts                                               |
| `make lint-markdown` | Run structural Markdown lint over authored active docs without wrapping or prose reflow                        |
...
`make check` is the required developer gate. It MUST include contract-generation drift detection, generated-artifact policy lint, migration verification, frontend phase registry/map validation, frontend ledger drift, a Go gofmt/vet/Staticcheck pass, targeted Gosec for focused security-sensitive rules plus scoped blocking runtime file/path rules, an authored-frontend Biome pass, a check-only scripts Biome pass, structural Markdown lint over authored active docs, blocking ShellCheck over tracked shell scripts through the scheduled `lint-shell` work unit, and a failure condition when any authoritative phase-manifest row is absent from its declared evidence surface. Advisory Gosec audit profiles are explicit CI/release or direct-target evidence and MUST NOT be selected by default local `check`. Default local `check` also MUST NOT schedule deployable-shape, current full visual, accessibility, or measurement browser work by default; those remain explicit direct and CI/release evidence surfaces unless the harness NLSpec declares a bounded readiness projection. Operator binary builds are scheduled only as harness readiness for selected rows that declare the `operator` runtime binary.

The public `check` wrapper performs only minimal runner/bootstrap checks before the Node scheduler starts. `toolchain-drift` is the scheduler root setup gate; after it passes, the check scheduler fans out exact readiness work for codegen, Go lint, Govulncheck, targeted Gosec, ShellCheck, frontend install, server and migration build artifacts, and service images. `tools/execution_topology_manifest.json` expands reusable resource profiles plus per-target `check_schedule` dependency metadata into `tools/check_schedule_manifest.json`, which schedules readiness fanout and independent check work by declared resources from `tools/scheduler_resource_registry.json`. Public target identity, output class, artifact policy, and side-effect declarations remain owned by the testing harness public registry and `tools/task_surface_manifest.json`; execution topology supplies scheduler work-unit structure only. The scheduler resource registry owns capacity profiles, default limits, override environment names, and service-backed auto-limit policies; `make check` forwards `CHECK_HOST_CPU_JOBS` and `CHECK_HOST_IO_JOBS` only when explicitly set rather than injecting default CLI resource overrides. Scheduler ordering uses dependencies first, then `priority`, then advisory `weight_ms`; a ready higher-priority unit that is temporarily resource-blocked reserves the resources blocking it, while lower-priority or lower-weight work may backfill only through unrelated resources.

The concrete `build-server`, `build-migrate`, and `test-service-images` work units prepare only the artifacts their default local-check consumers require; migration verification depends on `build-migrate`, and `check-service-backed` depends on declared service-backed readiness units. Operator build and deployable-shape evidence belong to explicit build, CI, and release-shaped surfaces. `check-service-backed` expands into scheduler-visible service-backed leaves inside the top `check` DAG: a low-resource service-session startup unit claims `host_cpu:1`, `host_io:1`, and retains the exclusive `suite_service_stack` lease, while backend, process, webserver-backed browser projection, full-target-equivalent stateful browser work, and aggregate service-backed leaves run as ordinary sibling check work units with direct `host_cpu`, `host_io`, service, process, browser stack, and browser-stage claims. The check path does not forward bounded host claims into nested `go_cpu` or `go_io`; as independent top-level work drains, service-backed leaves can consume the newly available host capacity directly. Migration verification remains a separate check-scheduler work unit, but it applies migrations against scratch Postgres databases and therefore declares Postgres service requirements and claims the separate exclusive `migration_scratch_postgres` lane instead of serializing behind the suite service stack.

Static validation leaves, generated-artifact drift detection, generated-source policy lint, backend formatting/lint, targeted Go security static analysis, script lint, structural Markdown lint, blocking shell lint, pure backend unit evidence, frontend type-checking, frontend unit work, harness smoke, and Go duration-baseline coverage remain separate check-scheduler work units so failures name the concrete owner. Public `make lint-shell`, `make lint`, and scheduled `check` ShellCheck evidence are blocking; raw shellcheck helper scripts may still be used diagnostically outside canonical Make evidence. Duration-baseline drift and warm scheduler health checks run from explicit maintenance surfaces such as `agent-finalize RESULTS_DIR=<retained-run-root>` so the default local correctness gate is not itself a timing-drift gate. The isolated browser batch uses one owned shared stack through a service-backed scheduler browser stage rather than a final top-level check work unit. Suite-service release checks stay synchronous, while Docker container termination is delegated to a detached lease-backed reaper and is reported as janitorial work rather than target critical-path teardown. `make ci` composes the same execution-truth guarantee into the provider-neutral CI surface. `make release-check` is the release verification gate and MUST include explicit accessibility evidence where selected by release policy, MUST activate Core 05 review only when `claim_publication_intent="claim_bearing_publication"`, and MUST fail if the configured license report or SBOM artifact is missing or empty.

For retained-run duration maintenance, first run `make check` and use that successful full warm check root as `RESULTS_DIR=<root>` for `make agent-finalize`. Do not pass service-backed-only, phase-slice, browser-only, or failed check roots to `agent-finalize`; even a successful `check-service-backed` run root is partial finalizer input. Run `make agent-finalize` without `RESULTS_DIR` when only non-retained maintenance is intended.

The check scheduler and service-backed scheduler use work-unit progressive-disclosure output, and run summaries separately report work units, summary targets, evidence targets, and helper units. Scheduled helper phases write under their own target-owned artifact directory and remain helper support evidence rather than evidence targets unless a schedule explicitly lists them in `produces_summary_targets`. Default `summary` stdout treats successful output as a bounded operator and coding-agent summary: targets print compact `[RESULT]` and `[ARTIFACTS]` lines with status, duration, counts, slowest work, `run_root`, and relative summary JSON path; the check scheduler `[SUMMARY]` line also reports `total_wall_time` for the full scheduler run. Successful child summaries, child stdout/stderr, package-manager banners, scheduler logs, and raw tool output stay in artifacts. Successful default stderr must be empty. Live scheduler progress is bounded `[PROGRESS] target=<target> completed=<done>/<total> running=<ids> blocked=<count> slowest=<id>:<ms> blocker=<id-or-none>` text emitted only by interval or material state changes, and it does not repeat artifact paths. Scheduler runs retain the compact operator stream in `progress-summary.log`, and scheduler summaries retain capped progress snapshots plus slowest-running observations so long runs can be inspected without parsing JSONL. Failure output surfaces `failure_class`, `reason=<failure_reason>`, a direct headline, compact artifact refs, rerun command, and investigation command so product regressions, service/runtime problems, harness failures, timing drift, and missing artifacts can be separated before reading logs. Per-shard or per-work-unit `[STEP]` lines, successful child logs, helper phase stdout and stderr logs, raw progress key/value fields, raw resource claims, active resource totals, resource limits, skip reasons, complete nested scheduler progress, and per-transition telemetry remain available in target-owned artifacts, through `make explain-target TARGET=<target> DETAIL=artifacts`, through `make explain-run RESULTS_DIR=<root|run-dir> [RUN_ID=<id>] [TARGET=<target>] [DETAIL=summary|children|logs|progress]`, and in `CARTULARY_OUTPUT_MODE=verbose` or `CARTULARY_OUTPUT_MODE=debug`. `VERBOSE=1` is a compatibility alias for verbose mode. `CARTULARY_OUTPUT_MODE=machine` emits canonical JSON and must not stream child logs or scheduler progress prose.

During `make test-service-backed` and `make check-service-backed`, service-backed backend work, webserver-backed browser evidence, and isolated browser evidence are placed by `tools/scheduler_manifest.json` through `priority` and advisory `weight_ms` scheduling in `tools/harness/scheduler/service-backed-schedule-cli.mjs`; `cartulary.scheduler_manifest.v2` expands service-backed source metadata into Go-backed backend shards through `tools/harness/backend/go-shard-plan.mjs` and uses registry capacity profiles plus generated per-browser-stage lanes as the concurrency authority. Go shard concurrency is split into `go_cpu` and `go_io` resources derived from committed shard timing and fixture metadata, with optional local or CI overrides declared in `tools/scheduler_resource_registry.json`, while explicit scheduler work-unit weights come from `tools/service_backed_make_target_duration_baselines.json` generated from successful scheduler `finish` events keyed by scheduler kind, schedule target, work-unit ID, and aggregate target; temporary topology overrides must declare a reason and expiration. `postgres`, `object_store`, `process`, `browser_stack`, and per-stage `browser_stage_*` lanes remain separate resources; `browser_stack` auto-capacity is resolved from distinct claimed browser-stage lanes capped by the resolved scheduler CPU budget and declared `process` capacity, while explicit `scheduler_needs` on browser stages remains the owner for reset or ordering boundaries. Go duration baselines use explicit v4 timing components: top-level test elapsed time, target-scoped package overhead, target command overhead, and raw aggregate wall time. Refresh Go duration baselines from representative successful service-backed artifacts, preferably the same check-shaped scheduler run that will be used for drift verification; narrow or unusually warm runs should not lower command overhead unless `ALLOW_COMMAND_OVERHEAD_DECREASE=1` is intentionally set to accept a new orchestration floor. Harness smoke duration baselines in `tools/harness_smoke_duration_baselines.json` are fast-tier scheduler weights for maintenance feedback, not product behavior assertions. Any change to phase-slice planning, scheduler behavior, harness checks, or fast harness tier membership must be followed by a successful `make run-harness-smoke-fast` and `make harness-smoke-duration-baselines RESULTS_DIR=<run-dir>` using that run's artifacts, so future phase expansion increases or rebalances the committed weights instead of hiding coverage. `make go-test-duration-baseline-coverage` verifies that the current Go shard plan has committed timing components for every service-backed test, target package, command overhead, and raw aggregate before the run-time drift check consumes observed artifacts. Explicit Make-target work such as `backend-process` remains available for process evidence, while direct aggregate targets such as `backend-integration` and `backend-store` remain standalone developer entrypoints. Webserver-backed browser functional evidence is selected from the authoritative phase manifests and split by `tools/harness/browser/browser-shard-plan.mjs` using committed manifest-entry weights in `tools/browser_e2e_duration_baselines.json`; shard reports are merged before phase summaries are emitted. Direct service-backed targets may run isolated browser evidence as separate stateful, measurement, visual, and accessibility leaf work units. Default local `make check` imports only lower-layer-gap webserver-backed browser projection work and full-target-equivalent stateful browser work as flattened check-scheduler work; public direct browser targets and CI/release evidence retain ordinary measurement, visual, and accessibility evidence. `tools/browser_e2e_batch_manifest.json` is rendered from `tools/execution_topology_manifest.json` and carries webserver-backed functional shard mode plus direct aggregate reset boundaries for stateful, measurement, visual, and accessibility groups. The final summaries report `backend-service-backed` and aggregate `browser` duration groups separately. Summary artifacts use explicit accounting fields: wall and critical-path wall time describe elapsed execution, executed duration describes work actually run in the current target, logical duration includes reused phase evidence, reused and derived durations remain separate, and teardown duration is reported independently as disjoint bucket wall time when lifecycle spans overlap. Hidden lifecycle timing failures, including teardown spans emitted by browser and test-service cleanup, are promoted to non-test target and run failures. Direct standalone browser E2E targets continue to use the development Compose stack through the same browser batch manifest.

The supported authored-source formatter command is `make format`. It formats authored Go through the curated gofmt wrapper and authored frontend code through the curated frontend Biome wrapper. Frontend Biome enforcement SHOULD use that authored-source scope rather than `biome check .` so generated artifacts, runtime outputs such as `dist/`, `test-results/`, coverage artifacts, and installed dependencies are not treated as formatter owners. JavaScript orchestration scripts are owned by `make lint-scripts`; that target is lint-only and MUST keep Biome formatting and assist disabled until script formatting is intentionally adopted. Markdown linting is owned by `make lint-markdown`; it is structural and MUST NOT enforce line length, broad wrapping, or prose reflow.

Repo-local `tmp/` is reserved for reproducible tool caches, retained run artifacts, and operator-inspectable local outputs. Disposable harness fixtures, especially repo-shaped scratch trees used by smoke tests, MUST be created through `tools/harness/test-support/harness-scratch.sh` outside the checkout. `CARTULARY_HARNESS_SCRATCH_ROOT` MAY redirect that scratch root, but it MUST resolve outside the repository so parallel linters and future phase checks cannot index transient harness state as authored source.
...
- **generated-artifact policy drift** means declared generated source artifacts lack required markers, use undeclared file types, or leak into authored lint scopes,
...
Browser verification MUST keep isolated ordering and reset policy in the topology-derived `tools/browser_e2e_batch_manifest.json`. Webserver-backed functional browser evidence MUST be selected from active registry phase manifests and split into duration-balanced Playwright manifest-entry shards using `tools/browser_e2e_duration_baselines.json` rather than by phase number or spec file. Frontend package scripts should expose stable task aliases, not `test:e2e:phaseN` aliases. The default developer gate schedules lower-layer-gap webserver-backed browser projection evidence plus full-target-equivalent stateful browser evidence directly inside the check scheduler with explicit `browser_stage_*` lane claims; ordinary measurement, visual, and accessibility work remains explicit/CI evidence so browser phase growth remains resource-visible without making the local correctness gate timing-bound.
...
The backend Go target plan is inspectable without executing tests. `make target-plan` renders the human execution plan, and `make target-plan-json` emits the deterministic JSON consumed by task-surface checks. `tools/phase_registry.json` owns phase order, status, manifest paths, ledger paths, scope, and owner refs; only active registry entries are executable. `make help-all`, `make task-guide`, `make explain-phase`, and `make explain-target` present task evidence through one concept hierarchy: `phase -> target -> scheduler work unit -> artifact`. `tools/harness/planning/task-execution-map.mjs` is the read-only projection layer for that hierarchy, deriving public/support grouping from the phase maps and scheduler paths from the generated check, service-backed, and browser schedules. When `ROLE=feature-dev` and `PHASE=phaseN` are both set for an active phase, task guidance recommends the public `make phase-slice PHASE=phaseN` and `make service-backed-slice PHASE=phaseN` wrappers derived from that phase manifest. Those wrappers schedule only the selected manifest rows, not whole targets that happen to cover the phase, then list aggregate gates separately from general hygiene checks that do not claim selected-phase evidence. Public runnable targets appear under public evidence; `check_internal` targets and support-only rows appear under support/internal evidence and are not direct feature-dev recommendations. `make explain-phase PHASE=phaseN` focuses on the registry-owned phase metadata plus active manifest counts, execution dependencies, public/support target coverage, scheduler work-unit paths, and artifact locations. Planned phases are explainable but not executable. `make explain-target TARGET=<target> [DETAIL=summary|rows|artifacts]` focuses on one backend, frontend, browser, aggregate, check, or release target; row detail preserves the backend Go target-plan rows where applicable, and artifact detail includes target summaries, scheduler summaries, and target-owned helper phase summaries. `make fixture-report RESULTS_DIR=<root|run-dir>` inspects an existing results root or concrete run directory's fixture diagnostics and prints thresholded hotspots without rerunning tests. `make explain-run RESULTS_DIR=<root|run-dir>` inspects retained run summaries, aggregate children, helper phase summaries, scheduler logs, and helper phase logs without rerunning tests.
...
| ----------------------- | --------------------------------------------------------------- | ------------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Timeline                | `timeline_event`                                                | `timeline`    | `timeline_grid_projection`; `cartulary.view.timeline.v2`                                  | workbook shell, grid adapter consumer, Timeline columns            | Built-in tab. Zero-field create remains allowed only because the owner contract explicitly allows it.                                                                                                                                                  |
...
| Saved views             | `saved_view`                                                    | `savedviews`  | `/api/v1/incidents/{incident_id}/saved-views/*`                                           | workbook shell, saved-view selector, layout persistence            | Workbook configuration object, not a projection row. Scope controls discoverability and mutability of the configuration object only.                                                                                                                   |
| Workbook preferences    | `user_workbook_preferences`, `incident_workbook_preferences`    | `workbook/startup` | `/api/v1/incidents/{incident_id}/workbook-preferences/*`                             | workbook bootstrap and startup-surface selection                   | Route-backed configuration object, not a workbook projection. Incident create still bootstraps the initial rows transactionally.                                                                                                                        |
