...
`make check` is the required developer gate. It MUST include contract-generation drift detection, migration verification, an authored-frontend Biome pass, and a failure condition when any authoritative phase-manifest row is absent from actual execution. Only toolchain drift and frontend install are early setup blockers; after setup, build artifacts and service images are prepared as the readiness gate for service-backed work. Once that readiness gate passes, `check-service-backed` starts through the service-backed scheduler while independent local product checks, static validation, generated-artifact drift detection, and harness smoke continue concurrently. When browser suites depend on the real Playwright web-server bootstrap, each verification stage must run its compatible browser suites under one owned shared stack rather than parallelizing multiple independent startup attempts. `make ci` composes the same execution-truth guarantee into the provider-neutral CI surface. `make release-check` is the release verification gate and MUST fail if the configured license report or SBOM artifact is missing or empty.

During `make test-service-backed` and `make check-service-backed`, service-backed backend aggregate targets are placed by `tools/service_backed_schedule_manifest.json` through the weighted scheduler in `scripts/run-service-backed-schedule.mjs`; target weights, schedule-level `resource_limits`, and child-level `resource_claims` describe backend capacity policy while the single `cartulary-test-services` wrapper remains the service owner. Browser evidence is owned separately by `tools/browser_e2e_batch_manifest.json`: `make test` and `make check` run the aggregate `browser-e2e` target once, using the manifest `all` stage and its reset boundaries for webserver-backed, stateful, measurement, and visual groups. For v2 service-backed schedules, `--jobs` is the active global concurrency cap and `SERVICE_BACKED_JOBS` supplies that cap for the standard service-backed developer-gate schedules; schedule `resource_limits` then apply per-resource ceilings within the global cap. The final `make test` and `make check` summaries report `backend-service-backed` and aggregate `browser` duration groups separately. Summary artifacts use explicit accounting fields: wall and critical-path wall time describe elapsed execution, executed duration describes work actually run in the current target, logical duration includes reused phase evidence, reused and derived durations remain separate, and teardown duration is reported independently as disjoint bucket wall time when lifecycle spans overlap. Hidden lifecycle timing failures, including teardown spans emitted by browser and test-service cleanup, are promoted to non-test target and run failures. Direct standalone browser E2E targets continue to use the development Compose stack through the same browser batch manifest.
...
When one `check-pre-browser` child fails under parallel execution, GNU Make MAY still print `Waiting for unfinished jobs....` while already-started siblings drain. That line is expected orchestration output and is not, by itself, a second verification failure.
...
Browser verification MUST keep ordering and reset policy in `tools/browser_e2e_batch_manifest.json`. The default gate runs webserver-backed, stateful, fixture-sensitive measurement, and visual browser suites through one owned `browser-e2e` stack per verification run, with reset boundaries declared in the manifest rather than split across backend service-backed schedules.
