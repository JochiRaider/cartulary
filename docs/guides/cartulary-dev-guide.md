...
For retained-run duration maintenance, first run `make check` and use that successful full warm check root as `RESULTS_DIR=<root>` for `make agent-finalize`. Do not pass service-backed-only, phase-slice, browser-only, or failed check roots to `agent-finalize`; run `make agent-finalize` without `RESULTS_DIR` when only non-retained maintenance is intended.
