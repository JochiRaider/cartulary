...
`make check` is the required developer gate. It MUST include contract-generation drift detection, migration verification, a Go gofmt/vet/Staticcheck pass, targeted Gosec for focused security-sensitive rules, warning-only Gosec audit profiles for emerging goroutine and file/path risks, an authored-frontend Biome pass, a check-only scripts Biome pass, warning-only ShellCheck over tracked shell scripts, and a failure condition when any authoritative phase-manifest row is absent from actual execution. Only toolchain drift and frontend install are early setup blockers; after setup, `tools/check_schedule_manifest.json` schedules the readiness gate and independent check work by declared resources from `tools/scheduler_resource_registry.json`. Once build artifacts and service images are ready, `check-service-backed` claims the exclusive `service_stack` lane plus a bounded `host_cpu` and `host_io` budget, forwards those bounded limits through `check_host_to_service_backed_go` to service-backed `go_cpu` and `go_io`, and starts backend, duration-balanced webserver-backed browser functional shards, and the isolated browser batch while cheap independent check work can run beside it. Static validation leaves, generated-artifact drift detection, migration verification, backend formatting/lint, targeted Go security static analysis, warning-only Go security audit output, script lint, warning-only shell lint, pure backend unit evidence, frontend type-checking, frontend unit work, harness smoke, and Go duration-baseline coverage remain separate check-scheduler work units so failures name the concrete owner. Duration-baseline drift checks remain downstream of `check-service-backed` because they consume its timing artifacts, including service-backed make-target duration drift from scheduler events. The isolated browser batch uses one owned shared stack through a service-backed scheduler browser stage rather than a final top-level check work unit. Suite-service release checks stay synchronous, while Docker container termination is delegated to a detached lease-backed reaper and is reported as janitorial work rather than target critical-path teardown. `make ci` composes the same execution-truth guarantee into the provider-neutral CI surface. `make release-check` is the release verification gate and MUST fail if the configured license report or SBOM artifact is missing or empty.
