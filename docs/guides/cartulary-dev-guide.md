...
Browser verification MUST keep final isolated ordering and reset policy in `tools/browser_e2e_batch_manifest.json`. The default gate schedules webserver-backed browser evidence with service-backed work, then runs stateful, fixture-sensitive measurement, and visual browser suites through one owned `browser-e2e` stack per verification run, with reset boundaries declared in the manifest.
