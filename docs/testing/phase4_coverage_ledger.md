...
| `E-4-05` | `apps/web/e2e/phase4.spec.ts::E-4-05 creates append-only assessment history through the workbook UI` | `browser_functional` | The browser workbook creates append-only assessment rows through the Assessments surface with band-first confidence, rationale, support refs, default ordering, and state or confidence filters. | Assessment storage, projection determinism, idempotency, and validation remain backend evidence. |
| `E-4-06` | `apps/web/e2e/phase4.spec.ts::E-4-06 creates and edits Notes, Task Requests, and Decisions through generic workbook surfaces` | `browser_functional` | The browser workbook creates and edits Notes, Task Requests, and Decisions through their real generic workbook surfaces, including defaults, collections, and visible validation failure. | Detailed lifecycle transition and idempotency assertions remain backend integration evidence. |
