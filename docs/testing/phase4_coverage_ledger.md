...
- `tools/execution_topology_manifest.json` owns browser batch grouping and renders `tools/browser_e2e_batch_manifest.json` for Phase 4: `browser-e2e-webserver-backed` carries authoritative `E-4-*` rows through duration-balanced Playwright spec shards for `test` and `check`, while direct `browser-e2e-webserver-backed` and helper-only `browser-e2e-functional` select the same rows through manifest-driven `browser_functional` shard planning. The scheduler-visible `browser-e2e` aggregate is reserved for isolated stateful, measurement, and visual browser batches.
...
| Row | Evidence | Execution | Claim | Out of scope |
| --- | --- | --- | --- | --- |
| `E-4-01` | `apps/web/e2e/phase4.mentions.spec.ts::E-4-01 resolves and creates entities from Timeline mentions in the inspector` | `browser_functional` | The browser inspector resolves existing entities and creates new entities from Timeline mentions through the real end-user flow. | Lower-level route replay and store substrate assertions remain backend evidence. |
| `E-4-02` | `apps/web/e2e/phase4.mentions.spec.ts::E-4-02 dismisses and ordinarily restores a mention without relinking` | `browser_functional` | The browser inspector dismisses a mention and restores it to unresolved state without silently relinking it. | Mention substrate mutation details remain backend evidence. |
...
| `E-4-05` | `apps/web/e2e/phase4.workbook.spec.ts::E-4-05 creates append-only assessment history through the workbook UI` | `browser_functional` | The browser workbook creates append-only assessment rows through the Assessments surface with band-first confidence, rationale, support refs, default ordering, and state or confidence filters. | Assessment storage, projection determinism, idempotency, and validation remain backend evidence. |
| `E-4-06` | `apps/web/e2e/phase4.workbook.spec.ts::E-4-06 creates and edits Notes, Task Requests, and Decisions through generic workbook surfaces` | `browser_functional` | The browser workbook creates and edits Notes, Task Requests, and Decisions through their real generic workbook surfaces, including defaults, collections, and visible validation failure. | Detailed lifecycle transition and idempotency assertions remain backend integration evidence. |
