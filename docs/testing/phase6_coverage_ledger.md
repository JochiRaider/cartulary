...
- `internal/modules/workbook/phase6_shared_harness_test.go`, `internal/modules/collaboration/phase6_shared_harness_test.go`, and `internal/platform/ws/phase6_ws_test.go` are selected as Phase 6 support evidence for shared §14 inventory, socket lifecycle evidence indexing, and low-level socket behavior.
...
| `E-6-03` | `apps/web/e2e/phase6.session-recovery.spec.ts::E-6-03 preserves unsaved local work after socket revocation and re-authentication` | `browser_stateful` | Representative route-owned browser revocation sources, including admin revoke-all, current-session logout, concurrency-limit eviction, and shared-clock idle expiry, emit `session_revoked`, close the socket, and preserve unsaved local work for explicit recovery and FIFO replay after re-authentication. | Browser evidence proves UI preservation and replay after representative route-owned revocation sources. Low-level revocation-source correctness remains backend and integration evidence. |
...
| `E-6-05` | `apps/web/e2e/phase6.collaboration.spec.ts::E-6-05 replays queued unsent writes after re-authentication without silent reload restore` | `browser_functional` | Within one browser runtime, queued unsent writes survive transient transport loss, real HTTP auth failure, or session revocation; replay in FIFO order after recovery or re-authentication; halt on the first real same-field conflict without applying later queued writes; and are never silently restored after a full reload. | Queue size and coalescing unit semantics remain frontend-unit evidence. |

## Visual Regression

| Row | Evidence | Execution | Claim | Out of scope |
| --- | --- | --- | --- | --- |
| `V-6-GRID-01` | `apps/web/e2e/workbook.visual.spec.ts::V-6-GRID-01 regresses Phase 6 row-gutter and same-cell presence markers` | `browser_visual` | The visual harness asserts deterministic Timeline row-gutter and same-cell presence marker presentation keyed by `record_id` and `field_key` from real browser presence state against a stable screenshot baseline. | Socket payload canonicalization and timing remain backend and functional browser evidence. |
...
| Incident WebSocket lifecycle | `internal/testutil/phase6test.Phase6SocketEventInventory()` is inventory only; `internal/modules/collaboration/phase6_shared_harness_test.go` asserts that the required lifecycle obligations map to behavior-level tests, while lifecycle conformance proof lives in `internal/modules/collaboration/phase6_socket_test.go`, `internal/modules/collaboration/phase6_integration_test.go`, `internal/platform/ws/phase6_ws_test.go`, and `internal/testutil/incidentwstest`. |
| Grid and browser-command anchors | `packages/test-utils/src/index.ts`, `packages/test-utils/src/index.test.ts`, `packages/grid-adapter/src/index.test.tsx`, `packages/ui-contracts/src/index.test.ts`, `apps/web/e2e/phase6.collaboration.spec.ts`, and `apps/web/e2e/workbook.visual.spec.ts` support current Phase 6 sort/filter/group/scroll/autosave command anchors and geometric marker anchoring; column resize, drag fill, tree expand/collapse, and paste matrix helpers are forward-compatible unsupported shells until a later phase exposes those commands. |
...
- `internal/modules/workbook/phase6_shared_harness_test.go`, `internal/modules/collaboration/phase6_shared_harness_test.go`, and `internal/platform/ws/phase6_ws_test.go` are Phase 6 support evidence for shared harness inventory, socket lifecycle evidence indexing, and low-level socket behavior; inventory-only rows must not claim `U-6-*` or `I-6-*` identifiers.
