...
| Mutation attribution, secret-safe payloads, and bootstrap auth-binding shape | `internal/testutil/crosscutting` plus `internal/modules/auth/testsupport/phase0test.RequireBootstrapUserLocalAuthOnly` cover startup audit attribution, secret-safe payloads, and bootstrap-created-user auth-binding shape. |
