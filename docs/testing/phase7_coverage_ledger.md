...
| `U-7-03` | `internal/modules/revisions/phase7_delete_restore_test.go::TestPhase7_SoftDeleteRoutePreconditions_U_7_03` | `backend_store` | Sprint 1 records U-7-03 as a later-sprint scope sentinel only; first-class record soft-delete route behavior is not claimed by Sprint 1. | Soft-delete `row_version`, role gates, route-owned failures, restore consequences, and destructive lock precedence remain later Phase 7 work. |
| `U-7-04` | `internal/modules/revisions/phase7_delete_restore_test.go::TestPhase7_RestoreTombstonePreconditions_U_7_04` | `backend_store` | Sprint 1 records U-7-04 as a later-sprint scope sentinel only; restore tombstone route behavior is not claimed by Sprint 1. | Restore `row_version`, role gates, active-state projection, append-only consequences, and browser-visible collaboration behavior remain later Phase 7 work. |
...
| Row | Evidence | Execution | Claim | Out of scope |
| --- | --- | --- | --- | --- |
| `I-7-01` | `internal/modules/revisions/phase7_integration_test.go::TestPhase7_DeleteRestoreRollbackAtomicConsequences_I_7_01` | `backend_integration` | Sprint 1 records I-7-01 as a later-sprint scope sentinel only; delete, restore, rollback, projection, history, and collaboration atomicity are not claimed by Sprint 1. | Mutation-route atomic consequences remain later Phase 7 work; Sprint 1 direct evidence stays on record-history reads. |
...
| `I-7-03` | `internal/modules/revisions/phase7_integration_test.go::TestPhase7_StaleRestoreRollbackFailsClosed_I_7_03` | `backend_integration` | Sprint 1 records I-7-03 as a later-sprint scope sentinel only; stale restore and rollback precondition behavior is not claimed by Sprint 1. | Stale restore or rollback failure behavior and lock acquisition precedence remain later Phase 7 work. |
...
| `E-7-03` | `apps/web/e2e/phase7.history.spec.ts::E-7-03 soft-deletes and restores a row with tombstone concurrency` | `browser_functional` | Sprint 1 records E-7-03 as a later-sprint scope sentinel only; browser delete and restore behavior is not claimed by Sprint 1. | Tombstone concurrency, collaboration remove or invalidate events, and destructive lock precedence remain later Phase 7 work. |
