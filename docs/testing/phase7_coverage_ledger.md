...
- Phase 7 is active so public phase wrappers execute the Phase 7 manifest. Sprint 3 covers rollback route admission, rollback reason registries, destructive-operation lock precedence for rollback, single-entry `history_entry` rollback, executable whole-`change_set` rollback for reversible non-merge substrate, `entry_requires_change_set`, and direct attached-evidence association rollback.
...
- Adjunct mutation-target-family history completeness remains incremental; Sprint 3 claims single-entry and whole-change-set rollback only where reversible before/after mutation data is available, and does not claim merge-specific change-set rollback.
...
- Retained-history restart or incident-closure evidence remains limited to existing explicit tests and is not expanded by Sprint 3 rollback closure work.
...
| `U-7-05` | `internal/modules/revisions/phase7_rollback_test.go::TestPhase7_RollbackSelectorUnion_U_7_05` | `backend_store` | Sprint 3 covers rollback route admission, derived rollback reason registries, the closed target selector union, strict request validation, durable `history_entry_ref` resolution, route-scoped idempotency, append-only single-entry rollback, executable whole-change-set rollback for reversible non-merge substrate, `entry_requires_change_set`, and executable-only `available_rollback_actions[]` metadata. | Whole-row restore, retained-history restart or closure expansion, merge-specific rollback, merge-lock integration, reviewer workbook UI, browser reviewer workflows, and typed tag rollback remain later owner-controlled work. |
| `U-7-06` | `internal/modules/revisions/phase7_locks_test.go::TestPhase7_DestructiveOperationLocks_U_7_06` | `backend_store` | Sprint 3 covers rollback destructive-operation lock precedence for single-entry and whole-change-set rollback: protected-set locks are acquired before stale row-version and rollback target precondition evaluation, and released requests fall through to ordinary downstream errors. | Merge-lock integration and browser-visible destructive-operation workflow handling remain later Phase 7 work. |
| `U-7-07` | `internal/modules/revisions/phase7_history_test.go::TestPhase7_RetainedHistoryInvariants_U_7_07` | `backend_store` | Existing retained-history invariants for extant records remain fully paginatable and preserve issued `history_entry_ref` values across the explicit lifecycle states covered by this test. | Sprint 3 does not expand retained-history restart or incident-closure scope; the current profile also defines no public history-purge route or retention-horizon setting for extant incidents. |

## Integration

| Row | Evidence | Execution | Claim | Out of scope |
| --- | --- | --- | --- | --- |
| `I-7-01` | `internal/modules/revisions/phase7_integration_test.go::TestPhase7_DeleteRestoreRollbackAtomicConsequences_I_7_01` | `backend_integration` | Sprint 3 covers delete, restore, single-entry rollback, direct attached-evidence create/delete association rollback, and executable whole-change-set rollback for reversible non-merge substrate across source rows, projections, history rows, record revisions, affected_record_ids, and ordinary record_changed collaboration events. | Whole-row restore, merge-specific rollback, merge-lock integration, reviewer workbook UI, browser workflows, and typed tag rollback remain unclaimed. |
...
| `I-7-03` | `internal/modules/revisions/phase7_integration_test.go::TestPhase7_StaleRestoreRollbackFailsClosed_I_7_03` | `backend_integration` | Sprint 3 covers stale restore plus stale or non-reversible single-entry and whole-change-set rollback failure as closed transactions: no envelope/source state, projection, history, idempotency, or collaboration event changes. | Whole-row restore stale-precondition behavior remains later Phase 7 work. |
| `I-7-04` | `internal/modules/revisions/phase7_integration_test.go::TestPhase7_RetainedHistoryAcrossRestartAndClosure_I_7_04` | `backend_integration` | Existing retained-history evidence for an extant record remains fully paginatable and stable across the explicit restart, incident-closure, and later-change sequence covered by this test, with prior `history_entry_ref` values preserved. | Sprint 3 does not expand retained-history restart or incident-closure scope; incident portability selector preservation across deployments is outside Phase 7. |
