...
func TestPhase10_BackupStorageRootBinding_U_10_05(t *testing.T) {
	t.Run("requires backup storage for every supported deployment profile", func(t *testing.T) {
		for _, profile := range phase10SupportedDeploymentProfiles() {
			t.Run(profile, func(t *testing.T) {
				cfg := phase10DeploymentProfileConfig(t, profile)
	...
	t.Run("accepts filesystem root backup storage for every supported deployment profile", func(t *testing.T) {
		for _, profile := range phase10SupportedDeploymentProfiles() {
			t.Run(profile, func(t *testing.T) {
				if _, err := Validate(phase10DeploymentProfileConfig(t, profile)); err != nil {
	...
	t.Run("rejects unknown backup storage binding kind", func(t *testing.T) {
		cfg := phase10DeploymentProfileConfig(t, "on_prem")
		cfg.Roots.BackupStorage = RootBinding{
	...
	t.Run("rejects export and temporary-work roots as backup storage", func(t *testing.T) {
		cases := []struct {
			...
			{name: "export_outputs", path: phase10DeploymentProfileConfig(t, "disconnected").Roots.ExportOutputs.Path},
			{name: "temporary_work", path: phase10DeploymentProfileConfig(t, "disconnected").Roots.TemporaryWork.Path},
		}

		for _, tc := range cases {
			t.Run(tc.name, func(t *testing.T) {
				cfg := phase10DeploymentProfileConfig(t, "disconnected")
...
func phase10DeploymentProfileConfig(t testing.TB, profile string) Config {
	t.Helper()

	return phase0DeploymentProfileConfig(t, profile)
}

func phase10SupportedDeploymentProfiles() []string {
