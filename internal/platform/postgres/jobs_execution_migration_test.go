...
func TestJobsExecutionMigration59FreshAndGuarded_Integration(t *testing.T) {
	...
	db := harness.MigrationDatabaseT(t, "jobs-execution-fresh", "up-to", "59")
	var columns int
...
func TestJobsExecutionMigration59RejectsUnsafeIdentityBeforeMutation_Integration(t *testing.T) {
	harness := pgtest.Start(t)
	db := harness.MigrationDatabaseT(t, "jobs-execution-reject", "up-to", "58")
...
`); err != nil {
	...
	_, err := postgres.Migrate(ctx, db, dbmigrations.Source(), "up-to", "59")
