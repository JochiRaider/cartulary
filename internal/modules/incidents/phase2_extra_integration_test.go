...
func TestPhase2_IncidentCreateTreatsComposedAndDecomposedIncidentKeysAsEqual_I_2_14(t *testing.T) {
	...
	firstCreate := phase2test.DoJSON(
		...
		map[string]any{
			"client_txn_id": "txn-nfc-incident-key-first",
			"incident_key":  "IR-E\u0301-900",
			"title":         "Decomposed Incident",
		},
		phase2test.WithCookies(adminLogin.SessionCookie, adminLogin.CSRFCookie),
		phase2test.WithHeader(authn.CSRFHeaderName, adminLogin.CSRFCookie.Value),
	)
	firstBody := httptestx.RequireSuccessEnvelope(t, firstCreate, http.StatusCreated)["data"].(map[string]any)
	if firstBody["incident_key"] != "IR-É-900" {
		t.Fatalf("unexpected normalized incident key in create response: %#v", firstBody)
	}

	phase2test.RequireErrorContract(t, "incident_key_conflict", http.StatusConflict)
	duplicate := phase2test.DoJSON(
		t,
		http.MethodPost,
		harness.Server.HTTP.URL+"/api/v1/incidents",
		map[string]any{
			"client_txn_id": "txn-nfc-incident-key-duplicate",
			"incident_key":  "IR-É-900",
			"title":         "Composed Incident",
		},
		phase2test.WithCookies(adminLogin.SessionCookie, adminLogin.CSRFCookie),
		phase2test.WithHeader(authn.CSRFHeaderName, adminLogin.CSRFCookie.Value),
	)
	httptestx.RequireErrorEnvelope(t, duplicate, http.StatusConflict, "incident_key_conflict")
