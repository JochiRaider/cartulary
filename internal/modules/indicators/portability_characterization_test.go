...
func TestIndicatorPortableRowsCharacterization_Integration(t *testing.T) {
	...
	if _, err := application.CreateIndicatorObservation(ctx, actor.ID, manualObservationParams(
	...
	if _, err := application.AppendIndicatorLifecycleInterval(ctx, actor.ID, lifecycleAppendParams(
	...
	for _, path := range descriptor.Paths {
		if !slices.Equal(path.Versions, []int{3}) {
			t.Fatalf("%s versions = %v, want [3]", path.LogicalPath, path.Versions)
		}
	}
}
