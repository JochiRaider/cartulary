...
func TestIndicatorCreateAdmissionAndHashCompatibility(t *testing.T) {
	t.Parallel()
	left, apiErr := decodeIndicatorCreate(strings.NewReader(`{
	...
	right, apiErr := decodeIndicatorCreate(strings.NewReader(`{"indicator.display_value":"203[.]0[.]113[.]7","indicator.value_kind":"atomic","client_txn_id":"txn-indicator","indicator.indicator_type":"ipv4_addr"}`))
	if apiErr != nil {
		t.Fatalf("decode reordered indicator create: %v", apiErr)
	}
	leftHash := indicatorCreateCommandHash(left)
	if !bytes.Equal(leftHash, indicatorCreateCommandHash(right)) {
	...
	if bytes.Equal(leftHash, indicatorCreateCommandHash(differentTxn)) {
...
func TestIndicatorCreateAdmissionRejectsBeforeOwnerExecution(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			_, apiErr := decodeIndicatorCreate(strings.NewReader(test.body))
