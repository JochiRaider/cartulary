...
func TestPartyLinkHelperFieldsPreserveTextIndependently_Integration(t *testing.T) {
	...
	beforeTaskClearBothChangeSets := PartiesQueryCount(t, harness, `SELECT count(*) FROM change_sets WHERE incident_id = $1`, incidentID)
	beforeTaskClearBothMutations := PartiesQueryCount(t, harness, `SELECT count(*) FROM change_set_mutations WHERE target_id = $1`, refOnlyTaskID.String())
	beforeTaskClearBothRevisions := PartiesQueryCount(t, harness, `SELECT count(*) FROM record_revisions WHERE record_id = $1`, refOnlyTaskID)
	...
	if got, want := PartiesQueryCount(t, harness, `SELECT count(*) FROM change_sets WHERE incident_id = $1`, incidentID), beforeTaskClearBothChangeSets+1; got != want {
		t.Fatalf("task requester clear both change set count: got %d want %d", got, want)
	}
	if got, want := PartiesQueryCount(t, harness, `SELECT count(*) FROM change_set_mutations WHERE target_id = $1`, refOnlyTaskID.String()), beforeTaskClearBothMutations+1; got != want {
		t.Fatalf("task requester clear both mutation count: got %d want %d", got, want)
	}
	if got, want := PartiesQueryCount(t, harness, `SELECT count(*) FROM record_revisions WHERE record_id = $1`, refOnlyTaskID), beforeTaskClearBothRevisions+1; got != want {
