...
func TestOptionalStandardizedSurfacesCatalogBehavior_Unit(t *testing.T) {
	...
	support := mustCreateRow(t, store, actor, incident.ID, artifacts.NotesViewSchemaID, "txn-workbook_interaction-optional-support-note", map[string]workbookroutetest.ValueChange{
		"note.title": Text("Supporting note"),
	}, nil, Time(0))
	contradiction := mustCreateRow(t, store, actor, incident.ID, artifacts.NotesViewSchemaID, "txn-workbook_interaction-optional-contradiction-note", map[string]workbookroutetest.ValueChange{
		"note.title": Text("Contradictory note"),
	}, nil, Time(0))

	finding := mustCreateRow(t, store, actor, incident.ID, artifacts.FindingsViewSchemaID, "txn-workbook_interaction-optional-finding", map[string]workbookroutetest.ValueChange{
	...
	closed := mustPatch(t, store, actor, finding.RecordID, artifacts.FindingsViewSchemaID, 1, "txn-workbook_interaction-optional-finding-close",
	...
	reopened := mustPatch(t, store, actor, finding.RecordID, artifacts.FindingsViewSchemaID, 2, "txn-workbook_interaction-optional-finding-reopen",
	...
	_, err = Patch(store, actor, finding.RecordID, artifacts.FindingsViewSchemaID, 3, "txn-workbook_interaction-optional-finding-invalid-confidence",
	...
	investigativeQuery := mustCreateRow(t, store, actor, incident.ID, artifacts.InvestigativeQueriesViewSchemaID, "txn-workbook_interaction-optional-query", map[string]workbookroutetest.ValueChange{
	...
	forensicKeyword := mustCreateRow(t, store, actor, incident.ID, artifacts.ForensicKeywordsViewSchemaID, "txn-workbook_interaction-optional-keyword", map[string]workbookroutetest.ValueChange{
	...
	keywordPatched := mustPatch(t, store, actor, forensicKeyword.RecordID, artifacts.ForensicKeywordsViewSchemaID, 1, "txn-workbook_interaction-optional-keyword-patch",
	...
	_, err = Patch(store, actor, forensicKeyword.RecordID, artifacts.ForensicKeywordsViewSchemaID, 2, "txn-workbook_interaction-optional-keyword-invalid-mode",
...
func TestOptionalStandardizedSurfacesProjectionQueryBehavior_Unit(t *testing.T) {
	...
	finding := mustCreateRow(t, store, actor, incident.ID, artifacts.FindingsViewSchemaID, "txn-workbook_interaction-optional-query-finding-high", map[string]workbookroutetest.ValueChange{
		"finding.statement":        Text("Confirmed malware execution"),
		"finding.kind":             Text("hypothesis"),
		"finding.confidence_score": optionalNumber(91),
	}, nil, Time(0))
	mustCreateRow(t, store, actor, incident.ID, artifacts.FindingsViewSchemaID, "txn-workbook_interaction-optional-query-finding-low", map[string]workbookroutetest.ValueChange{
		"finding.statement":        Text("Possible benign admin action"),
		"finding.kind":             Text("finding"),
		"finding.confidence_score": optionalNumber(12),
	}, nil, Time(0))
	requireProjectedRow(t, store, incident.ID, artifacts.FindingsViewSchemaID, viewschema.QueryMeta{
	...
	query := mustCreateRow(t, store, actor, incident.ID, artifacts.InvestigativeQueriesViewSchemaID, "txn-workbook_interaction-optional-query-kusto", map[string]workbookroutetest.ValueChange{
		"investigative_query.platform":   Text("Kusto"),
		"investigative_query.purpose":    Text("Endpoint triage"),
		"investigative_query.query_text": Text("DeviceProcessEvents | take 20"),
	}, nil, Time(0))
	mustCreateRow(t, store, actor, incident.ID, artifacts.InvestigativeQueriesViewSchemaID, "txn-workbook_interaction-optional-query-splunk", map[string]workbookroutetest.ValueChange{
	...
	requireProjectedRow(t, store, incident.ID, artifacts.InvestigativeQueriesViewSchemaID, viewschema.QueryMeta{
	...
	keyword := mustCreateRow(t, store, actor, incident.ID, artifacts.ForensicKeywordsViewSchemaID, "txn-workbook_interaction-optional-query-keyword-regex", map[string]workbookroutetest.ValueChange{
	...
	mustCreateRow(t, store, actor, incident.ID, artifacts.ForensicKeywordsViewSchemaID, "txn-workbook_interaction-optional-query-keyword-literal", map[string]workbookroutetest.ValueChange{
		"forensic_keyword.pattern": Text("cmd.exe"),
		"forensic_keyword.reason":  Text("Command shell"),
	}, nil, Time(0))
	requireProjectedRow(t, store, incident.ID, artifacts.ForensicKeywordsViewSchemaID, viewschema.QueryMeta{
...
func TestFindingsConfidenceBandBoundaries_Unit(t *testing.T) {
	...
	for index, tc := range []struct {
		...
		result := mustCreateRow(t, store, actor, incident.ID, artifacts.FindingsViewSchemaID, "txn-workbook_interaction-optional-boundary-"+tc.key, values, nil, Time(time.Duration(index)*time.Minute))
...
func mustPatch(t testing.TB, store *workbook.WorkbookContributionCatalog, actor authn.UserRecord, recordID uuid.UUID, viewSchemaID string, baseRowVersion int64, clientTxnID string, changes ...workbookroutetest.PatchChange) workbook.MutationResult {
	t.Helper()
...
func Patch(store *workbook.WorkbookContributionCatalog, actor authn.UserRecord, recordID uuid.UUID, viewSchemaID string, baseRowVersion int64, clientTxnID string, changes ...workbookroutetest.PatchChange) (workbook.MutationResult, error) {
...
func ValueChange(fieldKey string, value workbookroutetest.ValueChange) workbookroutetest.PatchChange {
	return workbookroutetest.PatchChange{FieldKey: fieldKey, Value: &value}
}

func CollectionChange(fieldKey string, value workbookroutetest.CollectionActionPayload) workbookroutetest.PatchChange {
	return workbookroutetest.PatchChange{FieldKey: fieldKey, Collection: &value}
}

func Collection(actions ...workbookroutetest.CollectionAction) workbookroutetest.CollectionActionPayload {
	return workbookroutetest.CollectionActionPayload{Actions: actions}
}

func addOptionalSurfaceRecordRef(recordID uuid.UUID) workbookroutetest.CollectionAction {
	return workbookroutetest.CollectionAction{Op: "add_record_ref", LinkedRecordID: &recordID}
}
...
func optionalNumber(value int64) workbookroutetest.ValueChange {
	return workbookroutetest.ValueChange{Kind: "number", Number: &value}
}

func optionalBool(value bool) workbookroutetest.ValueChange {
	return workbookroutetest.ValueChange{Kind: "bool", Bool: &value}
}
