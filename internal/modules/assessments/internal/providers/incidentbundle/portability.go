...
func exportIncidentBundleFiles(
	...
	file, err := incidentportability.ExportNDJSON(
		ctx,
		q,
		incidentID,
		assessmentsBundlePath,
		`SELECT to_jsonb(t) FROM assessments t WHERE incident_id = $1 ORDER BY record_id`,
	)
	if err != nil {
		return nil, err
	}
...
func importIncidentBundleFilesTx(
	...
	return incidentportability.ImportFixedBundleFileNDJSON(ctx, tx, incidentportability.FixedImportSpec{
		LogicalBundlePath: assessmentsBundlePath,
