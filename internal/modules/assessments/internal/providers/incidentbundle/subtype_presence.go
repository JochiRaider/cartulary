...
func SubtypeContribution() subtypepresence.Contribution {
	return subtypepresence.Contribution{
		FamilyID: "assessments",
		Source:   subtypeSource{},
	}
}

func (subtypeSource) SupportedRecordTypes() []subtypepresence.RecordType {
	return []subtypepresence.RecordType{subtypepresence.RecordTypeAssessment}
}

func (subtypeSource) ListSubtypeBindingsTx(
	ctx context.Context,
	tx pgx.Tx,
	incidentID uuid.UUID,
) ([]subtypepresence.Binding, error) {
	rows, err := tx.Query(ctx, `
SELECT record_id, incident_id, 'assessment'::text
  FROM assessments
 WHERE incident_id = $1
 ORDER BY record_id
`, incidentID)
