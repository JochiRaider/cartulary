...
func testRecordChangeIntentBuildsSortedCompactPatch(t *testing.T) {
	...
	catalog, err := NewPublicationCatalog([]PublicationContribution{{
		ContributionID: "test.record-change", SourceOwnerID: "test",
		AffectedViews: []ViewPublicationContribution{
			{ViewSchemaID: "cartulary.view.notes.v1", RecordTypes: []string{"test"}, PublicFieldKeys: []string{"note.body", "note.title"}, PatchFieldKeys: []string{"note.body", "note.title"}},
			{ViewSchemaID: "cartulary.view.hosts.v1", RecordTypes: []string{"test"}, PublicFieldKeys: []string{"host.evidence_count"}, PatchFieldKeys: []string{"host.evidence_count"}},
			{ViewSchemaID: "cartulary.view.timeline.v2", RecordTypes: []string{"test"}, PublicFieldKeys: []string{"timeline.evidence_count"}, PatchFieldKeys: []string{"timeline.evidence_count"}},
		},
	}}, []CanonicalPublicationView{
		{ViewSchemaID: "cartulary.view.notes.v1", SourceOwnerID: "test", RecordTypes: []string{"test"}},
		{ViewSchemaID: "cartulary.view.hosts.v1", SourceOwnerID: "test", RecordTypes: []string{"test"}},
		{ViewSchemaID: "cartulary.view.timeline.v2", SourceOwnerID: "test", RecordTypes: []string{"test"}},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := appender.recordChangedIntent(input); err != nil {
		t.Fatal(err)
	}
	...
	if got, want := changedKeys, []string{"note.body", "note.title"}; len(got) != len(want) || got[0] != want[0] || got[1] != want[1] {
		t.Fatalf("changed keys = %#v want %#v", got, want)
	}
	affectedViews := payload["affected_views"].([]map[string]any)
	decodedPatch := affectedViews[0]["patch_cells"].(map[string]any)
	...
	if _, err := appender.recordChangedIntent(multiInput); err != nil {
		t.Fatal(err)
	}
	multiPayload, err := appender.validatedRecordChangePayload(multiInput)
	if err != nil {
		t.Fatal(err)
	}
	multiViews := multiPayload["affected_views"].([]map[string]any)
	if len(multiViews) != 2 ||
		multiViews[0]["view_schema_id"] != "cartulary.view.hosts.v1" ||
		multiViews[1]["view_schema_id"] != "cartulary.view.timeline.v2" {
		t.Fatalf("multi-view intent order = %#v", multiViews)
	}
	streamSeq := int64(11)
	parsed, err := collabtestprotocol.RecordChangeFromSequencedMessage(collabprotocol.Message{
		Type:       "record_changed",
		IncidentID: "",
		EventID:    uuid.NewString(),
		EmittedAt:  time.Date(2026, time.July, 26, 12, 1, 1, 0, time.UTC).Format(time.RFC3339Nano),
		StreamSeq:  &streamSeq,
		Payload:    collabtestprotocol.RawPayload(multiPayload),
	})
