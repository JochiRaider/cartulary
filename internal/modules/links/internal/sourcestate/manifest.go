		...
		paths: []pathSpec{
			{
				...
				versions: []int{4}, stableIdentity: []string{"record_link_id"}, relation: relationRecordLinks,
			...
			},
			{
				...
				versions: []int{4}, stableIdentity: []string{"normalized_tag_name", "tag_name"},
			...
			},
			{
				...
				versions: []int{4}, stableIdentity: []string{"record_tag_id"}, relation: relationRecordTags,
...
func validateManifest(input manifestInput) (manifest, error) {
	...
	for index, path := range input.paths {
		if path.kind != expectedPaths[index] || !safeLogicalPath(path.logicalPath) ||
			...
			!slices.Equal(path.versions, []int{4}) || !validUniqueIdentifiers(path.stableIdentity) {
			return manifest{}, errInvalidManifest
		}
