...
func TestNetworkFlowV6StateJobsAndReportingProjection_Unit(t *testing.T) {
	...
	for _, rawFact := range coreJobs["facts"].([]any) {
		if rawFact.(map[string]any)["profile_id"] == "network_flow_activity" {
			t.Fatal("Core must not duplicate Network Flow-owned job or worker registrations")
		}
	}
	jobsDocument := readGeneratedExtensionObject(t, "contracts/extensions/fragments/network_flow_activity.profile.json")
	...
	for _, rawFact := range jobsDocument["facts"].([]any) {
		...
		case "job_kind":
			if graphJob != nil {
				t.Fatal("duplicate saved-graph job registration")
			}
			graphJob = fact["job_kind_contract"].(map[string]any)
		case "worker_kind":
			if graphWorker != nil {
				t.Fatal("duplicate saved-graph worker registration")
			}
			graphWorker = fact
		}
	}
	...
	if graphWorker == nil || graphWorker["worker_kind"] != "network_flow_activity.graph_view_worker_v1" ||
		graphWorker["max_active_attempts_per_process"] != float64(1) {
		t.Fatalf("Network Flow graph materialization worker contract drifted: %#v", graphWorker)
	}
	workerRuntime := readGeneratedExtensionObject(t, "contracts/extensions/generated/worker-runtime-contracts/network_flow_activity/network_flow_activity.graph_view_worker_v1.json")
	if workerRuntime["schema_id"] != "cartulary.extension_worker_runtime_contract.v1" ||
		workerRuntime["max_active_attempts_per_process"] != float64(1) {
		t.Fatalf("Network Flow generated worker runtime contract drifted: %#v", workerRuntime)
	}
	...
	if participant["participant_id"] != "network_flow_activity.reporting_graph_source_v1" || len(operations) != 1 ||
		operations[0].(map[string]any)["output_schema_id"] != "cartulary.network_flow.reporting_graph_source.v2" {
...
func TestExtensionBehaviorRouting_Static(t *testing.T) {
	...
	jobRows := map[string][]string{
		"module.extensions.unit.admission_invocation_v6": {
			"TestAdmissionAlgorithmResultAndDeadline_Unit",
		},
		"module.extensions.integration.admission_preflight_v6": {
			"TestClaimAdmissionPreflightBeforeState_ServiceBacked",
		},
