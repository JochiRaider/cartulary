...
func TestTaskRequestLifecycleDecisionLinksAndProjection_Unit(t *testing.T) {
	...
	}, []byte("txn-workbook_interaction-task-decision-task-minimum-fail"), "req-workbook_interaction-task-decision-task-minimum-fail", Time(0))
	...
	decision := mustCreateDecision(t, store, actor, incident.ID, "txn-workbook_interaction-task-decision-task-decision", "approved", "Task-linked decision")
	support := mustCreateEvidence(t, store, actor, incident.ID, "txn-workbook_interaction-task-decision-task-support", "Task support record")
	dueAt := Time(24 * time.Hour)
	task := mustCreateTask(t, store, actor, incident.ID, "txn-workbook_interaction-task-decision-task-create", map[string]workbook.ValueChange{
	...
	queueTask := mustCreateTask(t, store, actor, incident.ID, "txn-workbook_interaction-task-decision-task-queue-default", map[string]workbook.ValueChange{
	...
	queueTask = mustPatch(t, store, actor, queueTaskID, workbook.TaskRequestsViewSchemaID, 1, "txn-workbook_interaction-task-decision-task-queue-edit",
	...
	urgentTask := mustCreateTask(t, store, actor, incident.ID, "txn-workbook_interaction-task-decision-task-queue-urgent", map[string]workbook.ValueChange{
	...
	queueTask = mustPatch(t, store, actor, queueTaskID, workbook.TaskRequestsViewSchemaID, 2, "txn-workbook_interaction-task-decision-task-queue-clear-due",
	...
	_, err = Patch(store, actor, queueTaskID, workbook.TaskRequestsViewSchemaID, 3, "txn-workbook_interaction-task-decision-task-invalid-priority",
	...
	task = mustPatch(t, store, actor, taskID, workbook.TaskRequestsViewSchemaID, 1, "txn-workbook_interaction-task-decision-task-in-progress",
		ValueChange("task.status", workbook.ValueChange{Kind: "text", Text: stringPtr("in_progress")}))
	task = mustPatch(t, store, actor, taskID, workbook.TaskRequestsViewSchemaID, 2, "txn-workbook_interaction-task-decision-task-blocked",
	...
	task = mustPatch(t, store, actor, taskID, workbook.TaskRequestsViewSchemaID, 3, "txn-workbook_interaction-task-decision-task-unblocked",
		ValueChange("task.status", workbook.ValueChange{Kind: "text", Text: stringPtr("in_progress")}))
	requireCellValue(t, task.Payload["row"].(map[string]any), "task.blocked_reason", nil)
	task = mustPatch(t, store, actor, taskID, workbook.TaskRequestsViewSchemaID, 4, "txn-workbook_interaction-task-decision-task-done",
	...
	task = mustPatch(t, store, actor, taskID, workbook.TaskRequestsViewSchemaID, 5, "txn-workbook_interaction-task-decision-task-reopen",
	...
	cleared := mustPatch(t, store, actor, taskID, workbook.TaskRequestsViewSchemaID, 6, "txn-workbook_interaction-task-decision-task-clear-decision",
	...
	reset := mustPatch(t, store, actor, taskID, workbook.TaskRequestsViewSchemaID, 7, "txn-workbook_interaction-task-decision-task-reset-decision",
	...
	for _, invalid := range []struct {
		...
		_, err = Patch(store, actor, taskID, workbook.TaskRequestsViewSchemaID, 8, "txn-workbook_interaction-task-decision-task-invalid-decision-"+invalid.name,
	...
	done := mustPatch(t, store, actor, taskID, workbook.TaskRequestsViewSchemaID, 8, "txn-workbook_interaction-task-decision-task-done-again",
		ValueChange("task.status", workbook.ValueChange{Kind: "text", Text: stringPtr("done")}))
	requireCellValue(t, done.Payload["row"].(map[string]any), "task.status", "done")
	_, err = Patch(store, actor, taskID, workbook.TaskRequestsViewSchemaID, 9, "txn-workbook_interaction-task-decision-task-done-canceled",
		ValueChange("task.status", workbook.ValueChange{Kind: "text", Text: stringPtr("canceled")}))
	requireLifecycle(t, err)

	canceled := mustCreateTask(t, store, actor, incident.ID, "txn-workbook_interaction-task-decision-task-canceled", map[string]workbook.ValueChange{
	...
	_, err = Patch(store, actor, canceled.RecordID, workbook.TaskRequestsViewSchemaID, 1, "txn-workbook_interaction-task-decision-task-canceled-done",
		ValueChange("task.status", workbook.ValueChange{Kind: "text", Text: stringPtr("done")}))
	requireLifecycle(t, err)

	ownerless := mustCreateTask(t, store, actor, incident.ID, "txn-workbook_interaction-task-decision-task-ownerless", map[string]workbook.ValueChange{
		"task.title":     {Kind: "text", Text: stringPtr("Owner guard")},
		"task.task_kind": {Kind: "text", Text: stringPtr("request")},
	}, nil)
	_, err = Patch(store, actor, ownerless.RecordID, workbook.TaskRequestsViewSchemaID, 1, "txn-workbook_interaction-task-decision-task-ownerless-open",
...
func TestTaskLifecycleGuardFailures_Unit(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		_, err := store.CreateWorkbookRow(ctx, actor, incident.ID, workbook.CreateRequest{
			...
			Values:       tc.values,
		}, []byte("txn-workbook_interaction-task-decision-task-guard-"+tc.name), "req-workbook_interaction-task-decision-task-guard-"+tc.name, Time(0))
		requireLifecycle(t, err)
		if got := countRecords(t, harness.DB, incident.ID); got != beforeRecords {
	...
	for _, tc := range []struct {
		...
		task := mustCreateTask(t, store, actor, incident.ID, "txn-workbook_interaction-task-decision-task-guard-base-"+tc.name, map[string]workbook.ValueChange{
			"task.title":     {Kind: "text", Text: stringPtr("Guard base " + tc.name)},
			"task.task_kind": {Kind: "text", Text: stringPtr("request")},
		}, nil)
		before := TaskSnapshot(t, harness.DB, task.RecordID)
		_, err := Patch(store, actor, task.RecordID, workbook.TaskRequestsViewSchemaID, before.RowVersion, "txn-workbook_interaction-task-decision-task-guard-"+tc.name, tc.changes...)
		requireLifecycle(t, err)
...
func TestDecisionLifecycleSupersessionAndConsistency_Unit(t *testing.T) {
	...
	}, []byte("txn-workbook_interaction-task-decision-decision-minimum-fail"), "req-workbook_interaction-task-decision-decision-minimum-fail", Time(0))
	...
	}, []byte("txn-workbook_interaction-task-decision-decision-create-superseded"), "req-workbook_interaction-task-decision-decision-create-superseded", Time(0))
	requireLifecycle(t, err)

	support := mustCreateEvidence(t, store, actor, incident.ID, "txn-workbook_interaction-task-decision-decision-support", "Decision support record")
	affectedOne := mustCreateEvidence(t, store, actor, incident.ID, "txn-workbook_interaction-task-decision-decision-affected-one", "Decision affected record one")
	affectedTwo := mustCreateEvidence(t, store, actor, incident.ID, "txn-workbook_interaction-task-decision-decision-affected-two", "Decision affected record two")
	relationshipDecision := mustCreateDecisionWithCollections(t, store, actor, incident.ID, "txn-workbook_interaction-task-decision-decision-relationships", "proposed", "Relationship decision", map[string]workbook.CollectionActionPayload{
	...
	relationshipDecision = mustPatch(t, store, actor, relationshipDecision.RecordID, workbook.DecisionsViewSchemaID, 1, "txn-workbook_interaction-task-decision-decision-affected-add",
	...
	relationshipDecision = mustPatch(t, store, actor, relationshipDecision.RecordID, workbook.DecisionsViewSchemaID, 2, "txn-workbook_interaction-task-decision-decision-affected-remove",
	...
	target := mustCreateDecision(t, store, actor, incident.ID, "txn-workbook_interaction-task-decision-decision-target", "proposed", "Target decision")
	source := mustCreateDecision(t, store, actor, incident.ID, "txn-workbook_interaction-task-decision-decision-source", "approved", "Superseding decision")
	executed := mustCreateDecision(t, store, actor, incident.ID, "txn-workbook_interaction-task-decision-decision-executed", "approved", "Executed decision")
	executedRow := mustPatch(t, store, actor, executed, workbook.DecisionsViewSchemaID, 1, "txn-workbook_interaction-task-decision-decision-approved-executed",
	...
	_, err = Patch(store, actor, target, workbook.DecisionsViewSchemaID, 1, "txn-workbook_interaction-task-decision-decision-direct-superseded",
		ValueChange("decision.status", workbook.ValueChange{Kind: "text", Text: stringPtr("superseded")}))
	requireLifecycle(t, err)
	rejected := mustCreateDecision(t, store, actor, incident.ID, "txn-workbook_interaction-task-decision-decision-rejected", "rejected", "Rejected decision")
	_, err = Patch(store, actor, rejected, workbook.DecisionsViewSchemaID, 1, "txn-workbook_interaction-task-decision-decision-rejected-proposed",
		ValueChange("decision.status", workbook.ValueChange{Kind: "text", Text: stringPtr("proposed")}))
	requireLifecycle(t, err)
	_, err = Patch(store, actor, executed, workbook.DecisionsViewSchemaID, 2, "txn-workbook_interaction-task-decision-decision-executed-approved",
	...
	executedSource := mustCreateDecision(t, store, actor, incident.ID, "txn-workbook_interaction-task-decision-decision-executed-source", "approved", "Executed target replacement")
	...
	badSource := mustCreateDecision(t, store, actor, incident.ID, "txn-workbook_interaction-task-decision-decision-bad-source", "proposed", "Inconsistent source")
	badTarget := mustCreateDecision(t, store, actor, incident.ID, "txn-workbook_interaction-task-decision-decision-bad-target", "proposed", "Inconsistent target")
...
`, incident.ID, badSource, badTarget, actor.ID, Time(4*time.Hour)); err != nil {
	...
	_, err = Patch(store, actor, badSource, workbook.DecisionsViewSchemaID, 1, "txn-workbook_interaction-task-decision-decision-inconsistent-fail",
		ValueChange("decision.status", workbook.ValueChange{Kind: "text", Text: stringPtr("approved")}))
	requireLifecycle(t, err)
	beforeInconsistentVersion := RecordVersion(t, harness.DB, badSource)
	_, err = Patch(store, actor, badSource, workbook.DecisionsViewSchemaID, 1, "txn-workbook_interaction-task-decision-decision-inconsistent-rationale-fail",
	...
	_, err = Patch(store, actor, badSource, workbook.DecisionsViewSchemaID, 1, "txn-workbook_interaction-task-decision-decision-inconsistent-support-fail",
	...
	_, err = Patch(store, actor, badSource, workbook.DecisionsViewSchemaID, 1, "txn-workbook_interaction-task-decision-decision-inconsistent-affected-fail",
		CollectionChange("decision.affected_record_ids", Collection(addOptionalSurfaceRecordRef(affectedTwo))))
	requireLifecycle(t, err)
...
func TestSupersedeDecisionRejectsInconsistentSourceOrTarget_Unit(t *testing.T) {
	...
	validTarget := mustCreateDecision(t, store, actor, incident.ID, "txn-workbook_interaction-task-decision-decision-source-fail-target", "proposed", "Source fail target")
	...
	validSource := mustCreateDecision(t, store, actor, incident.ID, "txn-workbook_interaction-task-decision-decision-target-fail-source", "approved", "Valid source")
	inconsistentTarget := mustCreateDecision(t, store, actor, incident.ID, "txn-workbook_interaction-task-decision-decision-inconsistent-target", "proposed", "Inconsistent target")
	targetExistingSource := mustCreateDecision(t, store, actor, incident.ID, "txn-workbook_interaction-task-decision-decision-inconsistent-target-existing-source", "approved", "Existing superseding source")
...
func TestDecisionTerminalTransitionMatrix_Unit(t *testing.T) {
	...
	for _, from := range []string{"rejected", "executed", "superseded"} {
		for _, to := range []string{"proposed", "approved", "rejected", "executed", "superseded"} {
			name := from + "-to-" + to
			decisionID := mustCreateDecisionInTerminalState(t, store, actor, incident.ID, "txn-workbook_interaction-task-decision-decision-terminal-base-"+name, from)
			...
			_, err := Patch(store, actor, decisionID, workbook.DecisionsViewSchemaID, before.RowVersion, "txn-workbook_interaction-task-decision-decision-terminal-"+name, changes...)
