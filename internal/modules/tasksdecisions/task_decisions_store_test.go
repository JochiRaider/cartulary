...
func TestTaskRequestLifecycleDecisionLinksAndProjection_Unit(t *testing.T) {
	...
	incident := recordstoretest.CreateIncidentInStore(t, harness.DB, actor, "txn-phase9-sprint6-task-incident", "IR-S6-TASK", "Workbook inspector Sprint 6 task requests")
	...
	}, []byte("txn-phase9-sprint6-task-minimum-fail"), "req-phase9-sprint6-task-minimum-fail", Time(0))
	...
	decision := mustCreateDecision(t, store, actor, incident.ID, "txn-phase9-sprint6-task-decision", "approved", "Task-linked decision")
	support := mustCreateEvidence(t, store, actor, incident.ID, "txn-phase9-sprint6-task-support", "Task support record")
	dueAt := Time(24 * time.Hour)
	task := mustCreateTask(t, store, actor, incident.ID, "txn-phase9-sprint6-task-create", map[string]workbook.ValueChange{
	...
	queueTask := mustCreateTask(t, store, actor, incident.ID, "txn-phase9-sprint6-task-queue-default", map[string]workbook.ValueChange{
	...
	queueTask = mustPatch(t, store, actor, queueTaskID, workbook.TaskRequestsViewSchemaID, 1, "txn-phase9-sprint6-task-queue-edit",
	...
	urgentTask := mustCreateTask(t, store, actor, incident.ID, "txn-phase9-sprint6-task-queue-urgent", map[string]workbook.ValueChange{
	...
	queueTask = mustPatch(t, store, actor, queueTaskID, workbook.TaskRequestsViewSchemaID, 2, "txn-phase9-sprint6-task-queue-clear-due",
	...
	_, err = Patch(store, actor, queueTaskID, workbook.TaskRequestsViewSchemaID, 3, "txn-phase9-sprint6-task-invalid-priority",
	...
	task = mustPatch(t, store, actor, taskID, workbook.TaskRequestsViewSchemaID, 1, "txn-phase9-sprint6-task-in-progress",
		ValueChange("task.status", workbook.ValueChange{Kind: "text", Text: stringPtr("in_progress")}))
	task = mustPatch(t, store, actor, taskID, workbook.TaskRequestsViewSchemaID, 2, "txn-phase9-sprint6-task-blocked",
	...
	task = mustPatch(t, store, actor, taskID, workbook.TaskRequestsViewSchemaID, 3, "txn-phase9-sprint6-task-unblocked",
		ValueChange("task.status", workbook.ValueChange{Kind: "text", Text: stringPtr("in_progress")}))
	requireCellValue(t, task.Payload["row"].(map[string]any), "task.blocked_reason", nil)
	task = mustPatch(t, store, actor, taskID, workbook.TaskRequestsViewSchemaID, 4, "txn-phase9-sprint6-task-done",
	...
	task = mustPatch(t, store, actor, taskID, workbook.TaskRequestsViewSchemaID, 5, "txn-phase9-sprint6-task-reopen",
	...
	cleared := mustPatch(t, store, actor, taskID, workbook.TaskRequestsViewSchemaID, 6, "txn-phase9-sprint6-task-clear-decision",
	...
	reset := mustPatch(t, store, actor, taskID, workbook.TaskRequestsViewSchemaID, 7, "txn-phase9-sprint6-task-reset-decision",
	...
	requireManualReferenceLink(t, harness.DB, taskID, decision, "task.decision_record_id", "references_record")
	otherIncident := recordstoretest.CreateIncidentInStore(t, harness.DB, actor, "txn-phase9-sprint6-task-other-incident", "IR-S6-TASK-OTHER", "Workbook inspector Sprint 6 task requests other")
	foreignDecision := mustCreateDecision(t, store, actor, otherIncident.ID, "txn-phase9-sprint6-task-foreign-decision", "approved", "Foreign decision")
	deletedDecision := mustCreateDecision(t, store, actor, incident.ID, "txn-phase9-sprint6-task-deleted-decision", "approved", "Deleted decision")
	...
	for _, invalid := range []struct {
		...
		_, err = Patch(store, actor, taskID, workbook.TaskRequestsViewSchemaID, 8, "txn-phase9-sprint6-task-invalid-decision-"+invalid.name,
	...
	done := mustPatch(t, store, actor, taskID, workbook.TaskRequestsViewSchemaID, 8, "txn-phase9-sprint6-task-done-again",
		ValueChange("task.status", workbook.ValueChange{Kind: "text", Text: stringPtr("done")}))
	requireCellValue(t, done.Payload["row"].(map[string]any), "task.status", "done")
	_, err = Patch(store, actor, taskID, workbook.TaskRequestsViewSchemaID, 9, "txn-phase9-sprint6-task-done-canceled",
		ValueChange("task.status", workbook.ValueChange{Kind: "text", Text: stringPtr("canceled")}))
	requireLifecycle(t, err)

	canceled := mustCreateTask(t, store, actor, incident.ID, "txn-phase9-sprint6-task-canceled", map[string]workbook.ValueChange{
	...
	_, err = Patch(store, actor, canceled.RecordID, workbook.TaskRequestsViewSchemaID, 1, "txn-phase9-sprint6-task-canceled-done",
		ValueChange("task.status", workbook.ValueChange{Kind: "text", Text: stringPtr("done")}))
	requireLifecycle(t, err)

	ownerless := mustCreateTask(t, store, actor, incident.ID, "txn-phase9-sprint6-task-ownerless", map[string]workbook.ValueChange{
		"task.title":     {Kind: "text", Text: stringPtr("Owner guard")},
		"task.task_kind": {Kind: "text", Text: stringPtr("request")},
	}, nil)
	_, err = Patch(store, actor, ownerless.RecordID, workbook.TaskRequestsViewSchemaID, 1, "txn-phase9-sprint6-task-ownerless-open",
...
func TestTaskLifecycleGuardFailures_Unit(t *testing.T) {
	...
	incident := recordstoretest.CreateIncidentInStore(t, harness.DB, actor, "txn-phase9-sprint6-task-guard-incident", "IR-S6-TASK-GUARDS", "Workbook inspector Sprint 6 task guard failures")
	...
	for _, tc := range []struct {
		...
		_, err := store.CreateWorkbookRow(ctx, actor, incident.ID, workbook.CreateRequest{
			...
			Values:       tc.values,
		}, []byte("txn-phase9-sprint6-task-guard-"+tc.name), "req-phase9-sprint6-task-guard-"+tc.name, Time(0))
		requireLifecycle(t, err)
		if got := countRecords(t, harness.DB, incident.ID); got != beforeRecords {
	...
	for _, tc := range []struct {
		...
		task := mustCreateTask(t, store, actor, incident.ID, "txn-phase9-sprint6-task-guard-base-"+tc.name, map[string]workbook.ValueChange{
			"task.title":     {Kind: "text", Text: stringPtr("Guard base " + tc.name)},
			"task.task_kind": {Kind: "text", Text: stringPtr("request")},
		}, nil)
		before := TaskSnapshot(t, harness.DB, task.RecordID)
		_, err := Patch(store, actor, task.RecordID, workbook.TaskRequestsViewSchemaID, before.RowVersion, "txn-phase9-sprint6-task-guard-"+tc.name, tc.changes...)
		requireLifecycle(t, err)
...
func TestDecisionLifecycleSupersessionAndConsistency_Unit(t *testing.T) {
	...
	incident := recordstoretest.CreateIncidentInStore(t, harness.DB, actor, "txn-phase9-sprint6-decision-incident", "IR-S6-DECISION", "Workbook inspector Sprint 6 decisions")
	...
	}, []byte("txn-phase9-sprint6-decision-minimum-fail"), "req-phase9-sprint6-decision-minimum-fail", Time(0))
	...
	}, []byte("txn-phase9-sprint6-decision-create-superseded"), "req-phase9-sprint6-decision-create-superseded", Time(0))
	requireLifecycle(t, err)

	support := mustCreateEvidence(t, store, actor, incident.ID, "txn-phase9-sprint6-decision-support", "Decision support record")
	affectedOne := mustCreateEvidence(t, store, actor, incident.ID, "txn-phase9-sprint6-decision-affected-one", "Decision affected record one")
	affectedTwo := mustCreateEvidence(t, store, actor, incident.ID, "txn-phase9-sprint6-decision-affected-two", "Decision affected record two")
	relationshipDecision := mustCreateDecisionWithCollections(t, store, actor, incident.ID, "txn-phase9-sprint6-decision-relationships", "proposed", "Relationship decision", map[string]workbook.CollectionActionPayload{
	...
	relationshipDecision = mustPatch(t, store, actor, relationshipDecision.RecordID, workbook.DecisionsViewSchemaID, 1, "txn-phase9-sprint6-decision-affected-add",
	...
	relationshipDecision = mustPatch(t, store, actor, relationshipDecision.RecordID, workbook.DecisionsViewSchemaID, 2, "txn-phase9-sprint6-decision-affected-remove",
	...
	target := mustCreateDecision(t, store, actor, incident.ID, "txn-phase9-sprint6-decision-target", "proposed", "Target decision")
	source := mustCreateDecision(t, store, actor, incident.ID, "txn-phase9-sprint6-decision-source", "approved", "Superseding decision")
	executed := mustCreateDecision(t, store, actor, incident.ID, "txn-phase9-sprint6-decision-executed", "approved", "Executed decision")
	executedRow := mustPatch(t, store, actor, executed, workbook.DecisionsViewSchemaID, 1, "txn-phase9-sprint6-decision-approved-executed",
	...
	_, err = Patch(store, actor, target, workbook.DecisionsViewSchemaID, 1, "txn-phase9-sprint6-decision-direct-superseded",
		ValueChange("decision.status", workbook.ValueChange{Kind: "text", Text: stringPtr("superseded")}))
	requireLifecycle(t, err)
	rejected := mustCreateDecision(t, store, actor, incident.ID, "txn-phase9-sprint6-decision-rejected", "rejected", "Rejected decision")
	_, err = Patch(store, actor, rejected, workbook.DecisionsViewSchemaID, 1, "txn-phase9-sprint6-decision-rejected-proposed",
		ValueChange("decision.status", workbook.ValueChange{Kind: "text", Text: stringPtr("proposed")}))
	requireLifecycle(t, err)
	_, err = Patch(store, actor, executed, workbook.DecisionsViewSchemaID, 2, "txn-phase9-sprint6-decision-executed-approved",
	...
	result, err := store.SupersedeDecision(ctx, actor, target, request, timeline.TimelineActionRequestHash(request.BaseRowVersion, request.ClientTxnID, &request.Reason, request.ReplacementRecordID), "req-phase9-sprint6-decision-supersede", Time(time.Hour))
	if err != nil {
	...
	replay, err := store.SupersedeDecision(ctx, actor, target, request, timeline.TimelineActionRequestHash(request.BaseRowVersion, request.ClientTxnID, &request.Reason, request.ReplacementRecordID), "req-phase9-sprint6-decision-supersede-replay", Time(2*time.Hour))
	if err != nil {
	...
	executedTarget := mustCreateDecision(t, store, actor, incident.ID, "txn-phase9-sprint6-decision-executed-target", "executed", "Executed target")
	executedSource := mustCreateDecision(t, store, actor, incident.ID, "txn-phase9-sprint6-decision-executed-source", "approved", "Executed target replacement")
	...
	if _, err := store.SupersedeDecision(ctx, actor, executedTarget, executedRequest, timeline.TimelineActionRequestHash(executedRequest.BaseRowVersion, executedRequest.ClientTxnID, &executedRequest.Reason, executedRequest.ReplacementRecordID), "req-phase9-sprint6-decision-supersede-executed", Time(3*time.Hour)); err != nil {
	...
	badSource := mustCreateDecision(t, store, actor, incident.ID, "txn-phase9-sprint6-decision-bad-source", "proposed", "Inconsistent source")
	badTarget := mustCreateDecision(t, store, actor, incident.ID, "txn-phase9-sprint6-decision-bad-target", "proposed", "Inconsistent target")
...
`, incident.ID, badSource, badTarget, actor.ID, Time(4*time.Hour)); err != nil {
	...
	_, err = Patch(store, actor, badSource, workbook.DecisionsViewSchemaID, 1, "txn-phase9-sprint6-decision-inconsistent-fail",
		ValueChange("decision.status", workbook.ValueChange{Kind: "text", Text: stringPtr("approved")}))
	requireLifecycle(t, err)
	beforeInconsistentVersion := RecordVersion(t, harness.DB, badSource)
	_, err = Patch(store, actor, badSource, workbook.DecisionsViewSchemaID, 1, "txn-phase9-sprint6-decision-inconsistent-rationale-fail",
	...
	_, err = Patch(store, actor, badSource, workbook.DecisionsViewSchemaID, 1, "txn-phase9-sprint6-decision-inconsistent-support-fail",
	...
	_, err = Patch(store, actor, badSource, workbook.DecisionsViewSchemaID, 1, "txn-phase9-sprint6-decision-inconsistent-affected-fail",
		CollectionChange("decision.affected_record_ids", Collection(addOptionalSurfaceRecordRef(affectedTwo))))
	requireLifecycle(t, err)
...
func TestSupersedeDecisionRejectsInconsistentSourceOrTarget_Unit(t *testing.T) {
	...
	incident := recordstoretest.CreateIncidentInStore(t, harness.DB, actor, "txn-phase9-sprint6-decision-supersede-inconsistent-incident", "IR-S6-DECISION-SUPERSEDE-INCONSISTENT", "Workbook inspector Sprint 6 decision supersede inconsistent")

	inconsistentSource := mustCreateDecision(t, store, actor, incident.ID, "txn-phase9-sprint6-decision-inconsistent-source", "proposed", "Inconsistent source")
	sourceExistingTarget := mustCreateDecision(t, store, actor, incident.ID, "txn-phase9-sprint6-decision-inconsistent-source-existing-target", "proposed", "Existing target")
	validTarget := mustCreateDecision(t, store, actor, incident.ID, "txn-phase9-sprint6-decision-source-fail-target", "proposed", "Source fail target")
	...
	_, err := store.SupersedeDecision(ctx, actor, validTarget, sourceRequest, timeline.TimelineActionRequestHash(sourceRequest.BaseRowVersion, sourceRequest.ClientTxnID, &sourceRequest.Reason, sourceRequest.ReplacementRecordID), "req-phase9-sprint6-decision-inconsistent-source-route", Time(2*time.Hour))
	requireLifecycle(t, err)
	...
	validSource := mustCreateDecision(t, store, actor, incident.ID, "txn-phase9-sprint6-decision-target-fail-source", "approved", "Valid source")
	inconsistentTarget := mustCreateDecision(t, store, actor, incident.ID, "txn-phase9-sprint6-decision-inconsistent-target", "proposed", "Inconsistent target")
	targetExistingSource := mustCreateDecision(t, store, actor, incident.ID, "txn-phase9-sprint6-decision-inconsistent-target-existing-source", "approved", "Existing superseding source")
	...
	_, err = store.SupersedeDecision(ctx, actor, inconsistentTarget, targetRequest, timeline.TimelineActionRequestHash(targetRequest.BaseRowVersion, targetRequest.ClientTxnID, &targetRequest.Reason, targetRequest.ReplacementRecordID), "req-phase9-sprint6-decision-inconsistent-target-route", Time(4*time.Hour))
	requireLifecycle(t, err)
...
func TestDecisionTerminalTransitionMatrix_Unit(t *testing.T) {
	...
	incident := recordstoretest.CreateIncidentInStore(t, harness.DB, actor, "txn-phase9-sprint6-decision-terminal-incident", "IR-S6-DECISION-TERMINAL", "Workbook inspector Sprint 6 decision terminal matrix")

	for _, from := range []string{"rejected", "executed", "superseded"} {
		for _, to := range []string{"proposed", "approved", "rejected", "executed", "superseded"} {
			name := from + "-to-" + to
			decisionID := mustCreateDecisionInTerminalState(t, store, actor, incident.ID, "txn-phase9-sprint6-decision-terminal-base-"+name, from)
			...
			_, err := Patch(store, actor, decisionID, workbook.DecisionsViewSchemaID, before.RowVersion, "txn-phase9-sprint6-decision-terminal-"+name, changes...)
