...
func NewIncidentBundleSourcePort() sourceport.Port {
	descriptor := sourceport.Descriptor{
		...
		Paths: []sourceport.Path{
			{LogicalPath: timelineBundleProfilesPath, ContentRole: "source_rows", Versions: []int{3, 4}, StableIdentity: []string{"incident_id"}, StableIdentityInvariantID: "timeline.source_identity_admitted"},
			{LogicalPath: timelineBundleRecordsPath, ContentRole: "source_rows", Versions: []int{3, 4}, StableIdentity: []string{"record_id"}, StableIdentityInvariantID: "timeline.source_identity_admitted"},
			{LogicalPath: timelineBundleProvenancePath, ContentRole: "source_rows", Versions: []int{3, 4}, StableIdentity: []string{"record_id", "source_row_ordinal", "source_column_ordinal", "source_identity_sha256"}, StableIdentityInvariantID: "timeline.source_identity_admitted"},
		},
		InvariantIDs: []string{
    ...
    SELECT 1
     ...
     WHERE event.incident_id = $1
       AND (record.record_id IS NULL OR record.incident_id <> $1 OR record.record_type <> 'timeline_event')
			...
			if invalid {
				return descriptor.DeclaredFailure("timeline.envelope_type_scope")
			}
			return nil
		},
	})
