...
// RootStorage realizes the Incident Bundles storage port with admitted,
...
var _ incidentbundles.BundleStorage = (*RootStorage)(nil)

// NewRootStorage opens the temporary and published-bundle root capabilities.
// The caller owns the returned storage and must close it.
func NewRootStorage(temporaryRoot string, exportRoot string) (*RootStorage, error) {
	...
	return &RootStorage{temporary: temporary, exports: exports}, nil
}
...
func (storage *RootStorage) Stage(ctx context.Context, fileSHA string, data []byte) (incidentbundles.BundleStagingRef, error) {
	...
	if err := storage.temporary.CreateExclusive(ctx, rootReference, incidentBundleBytesWriter(data)); err != nil {
		return incidentbundles.BundleStagingRef{}, err
	}
	return reference, nil
}

func (storage *RootStorage) Publish(ctx context.Context, bundleID string, data []byte) (incidentbundles.BundleStorageRef, error) {
	...
	if err := storage.exports.CreateExclusive(ctx, rootReference, incidentBundleBytesWriter(data)); err != nil {
		return incidentbundles.BundleStorageRef{}, err
	}
	return reference, nil
}

func (storage *RootStorage) ReadStaged(reference incidentbundles.BundleStagingRef, maxBytes int64) ([]byte, error) {
	if storage == nil || storage.temporary == nil {
...
func (storage *RootStorage) RemoveStaged(reference incidentbundles.BundleStagingRef) error {
	if storage == nil || storage.temporary == nil {
...
func (storage *RootStorage) RemovePublished(reference incidentbundles.BundleStorageRef) error {
	if storage == nil || storage.exports == nil {
...
func incidentBundleBytesWriter(data []byte) rootedfs.WriteFunc {
