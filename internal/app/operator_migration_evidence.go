...
const defaultMigrationEvidenceManifestPath = "tools/migration_history_manifest.json"
...
func (runner operatorRunner) runMigrationEvidenceCapture(ctx context.Context, parsed operatorCLIResult) error {
	...
	result, err := buildOperatorMigrationEvidence(ctx, cfg, pool, collectedAt.UTC(), parsed.manifestPath, dbmigrations.Files)
	if err != nil {
		return err
	}
...
func buildOperatorMigrationEvidence(ctx context.Context, cfg config.Config, pool postgres.DB, collectedAt time.Time, manifestPath string, sourceFS fs.FS) (OperatorMigrationEvidenceResult, error) {
	...
	return OperatorMigrationEvidenceResult{
		SchemaID:          OperatorMigrationEvidenceSchemaID,
