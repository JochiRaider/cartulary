...
assert_count "$(line_count '^TEST_SUMMARY_GROUPS :=')" "1" "test summary group declaration"
...
assert_count "$(line_count '^HARNESS_SMOKE_FAST_TARGETS :=')" "1" "harness smoke fast target list declaration"
assert_count "$(line_count '^HARNESS_SMOKE_EXTENDED_TARGETS :=')" "1" "harness smoke extended target list declaration"
assert_count "$(line_count '^HARNESS_SMOKE_LIFECYCLE_TARGETS :=')" "1" "harness smoke lifecycle target list declaration"
assert_count "$(line_count '^HARNESS_SMOKE_FULL_TARGETS :=')" "1" "harness smoke full target list declaration"
assert_count "$(line_count '^HARNESS_SMOKE_TARGETS :=')" "1" "harness smoke target list declaration"
assert_count "$(line_count '^HARNESS_SMOKE_FAST_GROUPS :=')" "1" "harness smoke fast group declaration"
assert_count "$(line_count '^HARNESS_SMOKE_EXTENDED_GROUPS :=')" "1" "harness smoke extended group declaration"
assert_count "$(line_count '^HARNESS_SMOKE_FULL_GROUPS :=')" "1" "harness smoke full group declaration"
...
assert_contains "${test_block}" '--summary-groups "$(TEST_SUMMARY_GROUPS)"' "make test summary group list"
...
assert_contains "${check_block}" "--step check-setup-blockers --parallel-step check-parallel:\$(CHECK_JOBS) --step check-service-backed --step browser-e2e" "make check sequence"
...
assert_contains "${run_harness_smoke_fast_block}" '--summary-targets "$(HARNESS_SMOKE_FAST_TARGETS)"' "run-harness-smoke-fast summary list"
assert_contains "${run_harness_smoke_fast_block}" '--summary-groups "$(HARNESS_SMOKE_FAST_GROUPS)"' "run-harness-smoke-fast summary group list"
...
assert_contains "${run_harness_smoke_extended_block}" '--summary-targets "$(HARNESS_SMOKE_EXTENDED_TARGETS)"' "run-harness-smoke-extended summary list"
assert_contains "${run_harness_smoke_extended_block}" '--summary-groups "$(HARNESS_SMOKE_EXTENDED_GROUPS)"' "run-harness-smoke-extended summary group list"
...
assert_contains "${run_harness_smoke_full_block}" '--summary-targets "$(HARNESS_SMOKE_FULL_TARGETS)"' "run-harness-smoke-full summary list"
assert_contains "${run_harness_smoke_full_block}" '--summary-groups "$(HARNESS_SMOKE_FULL_GROUPS)"' "run-harness-smoke-full summary group list"
...
assert_contains "${run_phase_smoke_block}" '--summary-targets "$(HARNESS_SMOKE_FULL_TARGETS)"' "run-phase-smoke summary list"
assert_contains "${run_phase_smoke_block}" '--summary-groups "$(HARNESS_SMOKE_FULL_GROUPS)"' "run-phase-smoke summary group list"
...
assert_contains "${run_phase_smoke_all_block}" "run-harness-smoke-full-all" "run-phase-smoke-all compatibility aggregate"
...
assert_not_contains "${check_harness_smoke_block}" "run-phase-smoke" "check-harness-smoke avoids full compatibility target"
...
assert_contains "${makefile_content}" "TEST_SERVICE_BACKED_CHILD_TARGETS := backend-integration,backend-integration-support,backend-store,backend-process" "test service-backed child list"
...
assert_contains "${makefile_content}" "CHECK_SERVICE_BACKED_CHILD_TARGETS := backend-integration,backend-integration-support,backend-store,backend-process" "check service-backed child list"
...
assert_contains "${makefile_content}" "HARNESS_SMOKE_EXTENDED_TARGETS := harness-smoke-run-make-sequence,harness-smoke-run-go-target" "harness smoke extended target list"
...
assert_contains "${makefile_content}" 'HARNESS_SMOKE_FULL_TARGETS := $(HARNESS_SMOKE_FAST_TARGETS),$(HARNESS_SMOKE_EXTENDED_TARGETS),$(HARNESS_SMOKE_LIFECYCLE_TARGETS)' "harness smoke full target list"
assert_contains "${makefile_content}" 'HARNESS_SMOKE_TARGETS := $(HARNESS_SMOKE_FULL_TARGETS)' "harness smoke compatibility target list"
assert_contains "${makefile_content}" 'HARNESS_SMOKE_GROUPS := $(HARNESS_SMOKE_FULL_GROUPS)' "harness smoke compatibility group list"
assert_contains "${makefile_content}" "TEST_SUMMARY_GROUPS := backend-service-backed=backend-integration,backend-integration-support,backend-store,backend-process;browser=browser-e2e-webserver-backed,browser-e2e-stateful,browser-e2e-measurement,browser-e2e-visual" "test summary group list"
assert_contains "${makefile_content}" "CHECK_SUMMARY_GROUPS := backend-service-backed=backend-integration,backend-integration-support,backend-store,backend-process;browser=browser-e2e-webserver-backed,browser-e2e-stateful,browser-e2e-measurement,browser-e2e-visual" "check summary group list"
assert_contains "${test_service_backed_block}" '$(RUN_SERVICE_BACKED_SCHEDULE_SCRIPT) --target test-service-backed --jobs $(TEST_SERVICE_BACKED_JOBS) --manifest "$(SERVICE_BACKED_SCHEDULE_MANIFEST)" --defer-summary' "test service-backed scheduler invocation"
assert_contains "${test_service_backed_block}" '$(TEST_OUTPUT_SCRIPT) target-summary test-service-backed $$requested --children "$(TEST_SERVICE_BACKED_CHILD_TARGETS)"' "test service-backed post-wrapper summary"
assert_contains "${test_fast_service_backed_block}" '$(RUN_SERVICE_BACKED_SCHEDULE_SCRIPT) --target test-fast-service-backed --jobs $(SERVICE_BACKED_JOBS) --manifest "$(SERVICE_BACKED_SCHEDULE_MANIFEST)" --defer-summary' "test-fast service-backed scheduler invocation"
assert_contains "${test_fast_service_backed_block}" '$(TEST_OUTPUT_SCRIPT) target-summary test-fast-service-backed $$requested --children "$(TEST_FAST_SERVICE_BACKED_CHILD_TARGETS)"' "test-fast service-backed post-wrapper summary"
assert_contains "${check_service_backed_block}" '$(RUN_SERVICE_BACKED_SCHEDULE_SCRIPT) --target check-service-backed --jobs $(SERVICE_BACKED_JOBS) --manifest "$(SERVICE_BACKED_SCHEDULE_MANIFEST)" --defer-summary' "check service-backed scheduler invocation"
assert_contains "${check_service_backed_block}" '$(TEST_OUTPUT_SCRIPT) target-summary check-service-backed $$requested --children "$(CHECK_SERVICE_BACKED_CHILD_TARGETS)"' "check service-backed post-wrapper summary"
