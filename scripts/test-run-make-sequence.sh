...
assert_contains "${check_block}" '$(MAKE) --silent --no-print-directory frontend-install' "make check cold-start frontend install bootstrap"
...
assert_contains "${target_plan_node_tool_line}" '$(call RUN_MAKE_NODE_TOOL,target-plan,)' "target-plan node-tool empty env"
