...
service_schedule_target_content="$(cat "${ROOT_DIR}/scripts/run-service-backed-schedule-target.sh")"
...
assert_contains "${check_block}" '--resource-limit io=$(CHECK_IO_JOBS)' "make check scheduler io resource"
...
assert_contains "${service_schedule_target_content}" 'target-summary "$target" "$requested" --projection "$projection"' "service-backed post-wrapper summary"
