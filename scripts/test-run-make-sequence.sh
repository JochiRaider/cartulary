...
assert_contains "${test_block}" "--parallel-step test-local:3 --step test-service-backed --step test-isolated" "make test sequence"
assert_not_contains "${test_block}" "--step test-fast --step browser-e2e" "make test old serialized sequence"
...
assert_contains "${makefile_content}" "TEST_SUMMARY_TARGETS := test-service-backed,test-isolated,backend-unit,frontend-typecheck,frontend-unit,backend-store,backend-integration,backend-integration-support,backend-process,browser-e2e-webserver-backed,browser-e2e-stateful,browser-e2e-measurement,browser-e2e-visual" "test summary target list"
...
assert_contains "${makefile_content}" "TEST_SUMMARY_GROUPS := backend-service-backed=backend-integration,backend-integration-support,backend-store,backend-process;browser-webserver-backed=browser-e2e-webserver-backed;browser-isolated=browser-e2e-stateful,browser-e2e-measurement,browser-e2e-visual" "test summary group list"
assert_contains "${makefile_content}" "CHECK_SUMMARY_GROUPS := backend-service-backed=backend-integration,backend-integration-support,backend-store,backend-process;browser-webserver-backed=browser-e2e-webserver-backed;browser-isolated=browser-e2e-stateful,browser-e2e-measurement,browser-e2e-visual" "check summary group list"
