...
function main() {
  ...
  writeMarkdownReport(toolingPlanPath, [
    ...
    "",
    "- `make sbom` and `make license-report` generate `.cartulary/release-artifacts/sbom/<run-id>/` through `scripts/generate-sbom-license-evidence.mjs`, then validate canonical artifacts.",
  ...
  const validator = path.join(repoRoot, "scripts", "validate-cyclonedx.mjs");
  ...
  const readinessLines = [
    ...
    "- Release verification checks canonical non-empty SBOM/license artifacts through `scripts/check-release-artifact.sh`.",
    "",
    "## Unresolved Issues",
    "",
