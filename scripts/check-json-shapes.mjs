...
} from "./lib/service-backed-schedule-topology.mjs";
import {
...
const frontendBoundaryKeys = new Set(["schema_id", "scan_roots", "scan_excludes", "rules"]);
...
const generatedArtifactEntryKeys = new Set(["path", "allowed_extensions", "required_marker"]);
const lintScopeCheckKeys = new Set([
  "shell_sources",
  "biome",
  "frontend_import_boundaries",
]);
const lintShellSourceKeys = new Set(["path", "must_contain", "must_not_contain"]);
...
const schedulerCapacityKeys = new Set(["default_limit", "auto_policy", "override_env"]);
const schedulerCapacityProfileKeys = new Set(["name", "scheduler", "resources"]);
...
function usage() {
  throw new Error("usage: check-json-shapes.mjs [--root <path>] [--kind <kind> --file <path>]");
}
...
function parseArgs(argv) {
  ...
  if (options.file) {
    options.file = path.isAbsolute(options.file) ? options.file : path.join(options.root, options.file);
  }
  return options;
}
...
function validatePhaseRegistryShape(file) {
  ...
  const entries = requireObjectArray(registry.phases, `${file}.phases`, { nonEmpty: true });
  const phases = [];
  const orders = [];
  for (const [index, entry] of entries.entries()) {
    ...
    phases.push(requireString(entry.phase, `${label}.phase`, { pattern: phaseNamePattern }));
    ...
    requireRepoRelativePath(entry.manifest_path, `${label}.manifest_path`, { extension: ".json" });
    requireRepoRelativePath(entry.ledger_path, `${label}.ledger_path`, { extension: ".md" });
...
function validatePhaseMapShape(file) {
  ...
  requireStringArray(manifest.expected_ids, `${file}.expected_ids`, { nonEmpty: true });
  if (manifest.forbidden_id_files !== undefined) {
    requireStringArray(manifest.forbidden_id_files, `${file}.forbidden_id_files`);
  }
  for (const section of ["unit", "integration", "e2e"]) {
    const entries = requireObjectArray(manifest[section] ?? [], `${file}.${section}`);
    const ids = [];
    for (const [index, entry] of entries.entries()) {
      const label = `${file}.${section}[${index + 1}]`;
      assertObjectKeys(entry, phaseMapEntryKeys, label);
      ids.push(requireString(entry.id, `${label}.id`));
      requireEnum(entry.coverage, `${label}.coverage`, new Set(["authoritative", "supplemental"]));
      requireEnum(entry.runner, `${label}.runner`, new Set(["go_test", "playwright", "vitest"]));
...
function validatePhasePolicyExceptionsShape(file) {
  ...
  for (const [index, entry] of entries.entries()) {
    ...
    assertObjectKeys(
      entry,
      new Set(["id", "type", "owner", "reason", "expires_before_phase", "expires_on", "selection"]),
      label,
    );
    ids.push(requireString(entry.id, `${label}.id`));
...
function validateExecutionTopologyShape(file) {
  ...
  assertUnique(
    dependencies.map((entry, index) =>
      requireString(entry.id, `${file}.execution_dependencies[${index + 1}].id`, {
        pattern: snakeIDPattern,
      }),
    ),
    `${file}.execution_dependencies.id`,
  );
  const taskSurface = requireObject(topology.task_surface, `${file}.task_surface`);
  const targets = requireObjectArray(taskSurface.targets, `${file}.task_surface.targets`, {
    nonEmpty: true,
  });
  assertUnique(
    targets.map((entry, index) =>
      requireString(entry.name, `${file}.task_surface.targets[${index + 1}].name`, {
    ...
    for (const [unitIndex, unit] of requireObjectArray(
      ...
      if (unit.env !== undefined) {
        for (const name of Object.keys(requireObject(unit.env, `${unitLabel}.env`))) {
          requireString(name, `${unitLabel}.env key`, { pattern: envNamePattern });
        }
      }
    }
  }
...
function validateServiceBackedScheduleShape(file) {
  ...
  for (const [scheduleIndex, schedule] of requireObjectArray(
    ...
    for (const [sourceIndex, source] of requireObjectArray(
      ...
      requireEnum(source.type, `${sourceLabel}.type`, new Set(["go_shards", "make_target"]));
      requireEnum(source.class, `${sourceLabel}.class`, new Set(["backend", "browser"]));
      requireString(source.target, `${sourceLabel}.target`, { pattern: makeTargetPattern });
...
function validateBrowserBatchShape(file) {
  ...
  for (const [stageIndex, stage] of requireObjectArray(
    ...
    requireString(stage.target, `${label}.target`, { pattern: makeTargetPattern });
    for (const [groupIndex, group] of requireObjectArray(stage.groups, `${label}.groups`, {
      nonEmpty: true,
    }).entries()) {
      ...
      requireString(group.target, `${groupLabel}.target`, { pattern: makeTargetPattern });
    }
  }
}
...
function validateGeneratedArtifactPolicyShape(file) {
  ...
  requireStringArray(policy.ignored_sentinel_filenames ?? [], `${file}.ignored_sentinel_filenames`);
  for (const [index, root] of requireObjectArray(policy.generated_roots ?? [], `${file}.generated_roots`).entries()) {
    ...
    requireStringArray(root.allowed_extensions, `${label}.allowed_extensions`, { nonEmpty: true });
  ...
  for (const [index, generatedFile] of requireObjectArray(
    ...
    requireStringArray(generatedFile.allowed_extensions, `${label}.allowed_extensions`, {
      nonEmpty: true,
    });
    requireString(generatedFile.required_marker, `${label}.required_marker`);
  }
  const lintScope = requireObject(policy.lint_scope_checks, `${file}.lint_scope_checks`);
  ...
  for (const [index, source] of requireObjectArray(
    ...
    requireStringArray(source.must_contain, `${label}.must_contain`, { nonEmpty: true });
    requireStringArray(source.must_not_contain ?? [], `${label}.must_not_contain`);
  }
  const biome = requireObject(lintScope.biome, `${file}.lint_scope_checks.biome`);
  ...
  requireStringArray(biome.required_files_includes, `${file}.lint_scope_checks.biome.required_files_includes`);
  requireStringArray(biome.forbidden_files_includes, `${file}.lint_scope_checks.biome.forbidden_files_includes`);
  requireStringArray(biome.required_override_includes, `${file}.lint_scope_checks.biome.required_override_includes`);
  ...
  requireRepoRelativePath(frontendBoundaries.path, `${file}.lint_scope_checks.frontend_import_boundaries.path`);
  requireStringArray(
...
function validateFrontendImportBoundariesShape(file) {
  ...
  requireStringArray(config.scan_roots, `${file}.scan_roots`, { nonEmpty: true });
...
function validateSchedulerResourceRegistryShape(file) {
  ...
  for (const [index, resource] of requireObjectArray(registry.resources, `${file}.resources`, {
    nonEmpty: true,
  }).entries()) {
    ...
    requireStringArray(resource.schedulers, `${label}.schedulers`, { nonEmpty: true });
    requireInteger(resource.display_order, `${label}.display_order`, { min: 0 });
    if (resource.capacity !== undefined) {
      ...
      if (capacity.default_limit !== undefined) {
        requirePositiveInteger(capacity.default_limit, `${label}.capacity.default_limit`);
      }
      if (capacity.auto_policy !== undefined) {
        requireString(capacity.auto_policy, `${label}.capacity.auto_policy`);
      }
      if ((capacity.default_limit === undefined) === (capacity.auto_policy === undefined)) {
      ...
      if (capacity.override_env !== undefined) {
        requireString(capacity.override_env, `${label}.capacity.override_env`, { pattern: envNamePattern });
      }
    }
  }
  for (const [index, template] of requireObjectArray(registry.templates, `${file}.templates`, {
    nonEmpty: true,
  }).entries()) {
    ...
    requireStringArray(template.schedulers, `${label}.schedulers`, { nonEmpty: true });
    requireInteger(template.display_order, `${label}.display_order`, { min: 0 });
  }
  for (const [index, profile] of requireObjectArray(
    ...
    requireStringArray(profile.resources, `${label}.resources`, { nonEmpty: true });
  }
  for (const [index, profile] of requireObjectArray(
    ...
    for (const [mappingIndex, mapping] of requireObjectArray(
      ...
      requireString(mapping.env_variable, `${mappingLabel}.env_variable`, { pattern: envNamePattern });
    }
  }
}
...
function validateBootstrapAdminShape(file) {
  ...
  if (manifest.bootstrap_schema_id !== bootstrapAdminSchemaID) {
    throw new Error(`${file}.bootstrap_schema_id must be ${bootstrapAdminSchemaID}`);
  }
  requireString(manifest.bootstrap_artifact_id, `${file}.bootstrap_artifact_id`);
  requireString(manifest.email, `${file}.email`, { pattern: /^[^\s@]+@[^\s@]+\.[^\s@]+$/ });
...
function validateDurationBaselineShape(file) {
  ...
  requirePositiveInteger(baseline.default_make_target_weight_ms, `${file}.default_make_target_weight_ms`);
...
function validateAll(root) {
  ...
  validatePhasePolicyExceptionsShape(repoFile(root, "tools/phase_policy_exceptions.json"));
  loadPhasePolicyExceptions(root);

  validateExecutionTopologyShape(repoFile(root, "tools/execution_topology_manifest.json"));
  ...
  assertGeneratedJSONFresh(root, topology.generatedOutputs.task_surface_manifest, taskSurface);
  assertGeneratedJSONFresh(root, topology.generatedOutputs.check_schedule_manifest, checkSchedule);
  assertGeneratedJSONFresh(root, topology.generatedOutputs.browser_e2e_batch_manifest, browserBatch);
  ...
  validateCheckScheduleShape(repoFile(root, "tools/check_schedule_manifest.json"));
  validateServiceBackedScheduleShape(repoFile(root, "tools/service_backed_schedule_manifest.json"));
  validateServiceBackedScheduleTopology({
    scheduleManifestPath: repoFile(root, "tools/service_backed_schedule_manifest.json"),
    topologyPath: repoFile(root, "tools/execution_topology_manifest.json"),
  });
  validateBrowserBatchShape(repoFile(root, "tools/browser_e2e_batch_manifest.json"));
  loadBrowserBatchManifest(repoFile(root, "tools/browser_e2e_batch_manifest.json"));

  validateGeneratedArtifactPolicyShape(repoFile(root, "tools/generated_artifact_policy.json"));
  validateFrontendImportBoundariesShape(repoFile(root, "tools/frontend_import_boundaries.json"));
  validateSchedulerResourceRegistryShape(repoFile(root, "tools/scheduler_resource_registry.json"));
  loadSchedulerResourceRegistry(repoFile(root, "tools/scheduler_resource_registry.json"));
  validateDurationBaselineShape(repoFile(root, "tools/service_backed_make_target_duration_baselines.json"));
  validateBootstrapAdminShape(repoFile(root, "configs/dev/bootstrap-admin.json"));
