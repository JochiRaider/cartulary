...
run_step() {
  ...
  case "${kind}" in
    step)
      target="${rest}"
      CARTULARY_TEST_TARGET="${target}" "${MAKE_BIN}" --no-print-directory "${target}"
      ;;
    parallel)
      target="${rest%%:*}"
      jobs="${rest#*:}"
      ...
      CARTULARY_TEST_TARGET="${target}" "${MAKE_BIN}" --no-print-directory --output-sync=target "-j${jobs}" "${target}"
      ;;
    *)
