...
const manifest = JSON.parse(fs.readFileSync(path.join(root, "tools", "phase0_test_map.json"), "utf8"));
const sections = [
  ["unit", "backend_unit"],
...
for (const [section, dependency] of sections) {
  for (const entry of manifest[section] ?? []) {
    ...
    if (entry.execution_dependency !== dependency) {
      console.error(
        `phase0 authoritative ${section} row ${entry.id} must declare execution_dependency=${dependency}`,
      );
      process.exit(1);
    }
  }
...
EOF
then
  fail "Phase 0 authoritative manifest rows must carry the canonical execution_dependency for their layer"
fi
