...
function validateBrowserGroup(
  ...
  if (group.aggregate_target.trim() !== source.target.trim()) {
    throw new Error(
      `${groupLabel}.aggregate_target must match ${source.target}`,
    );
  }
  ...
  if (!validBrowserGroupKinds.has(group.kind.trim())) {
    throw new Error(
      `${groupLabel}.kind must be one of ${Array.from(validBrowserGroupKinds).join(", ")}`,
    );
  }
  if (group.kind.trim() === "functional_shard") {
    if (
      typeof group.shard_name !== "string" ||
      group.shard_name.trim() === ""
    ) {
      throw new Error(
        `${groupLabel}.shard_name must be a non-empty string for functional_shard`,
      );
    }
    if (!Number.isInteger(group.shard_index) || group.shard_index < 0) {
      throw new Error(
        `${groupLabel}.shard_index must be a non-negative integer for functional_shard`,
      );
    }
    if (!Number.isInteger(group.shard_count) || group.shard_count < 1) {
      throw new Error(
        `${groupLabel}.shard_count must be a positive integer for functional_shard`,
      );
    }
    if (group.shard_index >= group.shard_count) {
      throw new Error(
        `${groupLabel}.shard_index must be less than shard_count`,
      );
    }
    if (!Array.isArray(group.entry_ids) || group.entry_ids.length === 0) {
      throw new Error(
        `${groupLabel}.entry_ids must be non-empty for functional_shard`,
      );
    }
  }
  return {
    ...
    executionDependency:
      typeof group.execution_dependency === "string"
        ? group.execution_dependency.trim()
        : "",
    shardName:
      typeof group.shard_name === "string" ? group.shard_name.trim() : "",
    ...
    phases: Array.isArray(group.phases)
      ? group.phases.filter((entry) => typeof entry === "string")
      : [],
    entryIDs: Array.isArray(group.entry_ids)
      ? group.entry_ids.filter((entry) => typeof entry === "string")
      : [],
...
function validateSource(
  ...
  if (!validSourceTypes.has(source.type)) {
    throw new Error(
      `${label} must declare type go_shards, make_target, or browser_stage`,
    );
  }
  ...
  if (
    source.resource_tags !== undefined ||
    source.exclusive_tags !== undefined
  ) {
    throw new Error(
      `${label} must not declare legacy resource_tags or exclusive_tags`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `${label} ${target} priority must be a non-negative integer`,
    );
  }
  ...
  if (source.type === "go_shards") {
    if (source.class !== "backend") {
      throw new Error(
        `${label} ${target} go_shards sources must declare class backend`,
      );
    }
    for (const resource of [goCPUResource, goIOResource]) {
      if (!resourceLimits.has(resource)) {
        throw new Error(
          `${label} ${target} go_shards sources require resource_limits.${resource}`,
        );
      }
    }
    return {
  ...
  if (source.type === "browser_stage") {
    if (source.class !== "browser") {
      throw new Error(
        `${label} ${target} browser_stage sources must declare class browser`,
      );
    }
    const groups = Array.isArray(source.groups)
      ? source.groups.map((group, groupIndex) =>
          validateBrowserGroup(
            source,
            group,
            groupIndex,
            label,
    ...
    if (groups.length === 0) {
      throw new Error(
        `${label} ${target} browser_stage sources must declare groups[]`,
      );
    }
    return {
      type: source.type,
...
function _findSchedule(manifest, target, browserStages, overrides) {
  ...
  if (
    !Array.isArray(schedule.work_unit_sources) ||
    schedule.work_unit_sources.length === 0
  ) {
    throw new Error(
      `schedule ${target} must declare at least one work_unit_sources entry`,
    );
  }
  if (schedule.children !== undefined) {
    throw new Error(
      `schedule ${target} must use work_unit_sources, not legacy children`,
    );
  }
  ...
  const resourceLimits = provisionalResourceLimitsForClaims(
  ...
  return {
    ...
    dependencyCount: sources.reduce(
      (sum, source) => sum + source.needs.length,
      0,
    ),
  };
}
...
function validateMeasurementIsolation(sources, scheduleLabel) {
  ...
  const measurement = browserSources.find(
    (source) => source.browserStage === "measurement",
  );
  if (!measurement) {
    return;
  }
...
function _expandSchedule(schedule) {
  ...
  const resolvedLimits = resolveResourceLimits(
    schedule.resourceLimits,
    schedule.resourceLimitSources,
    countedWorkUnits,
  );
  const finalWorkUnits = workUnits.map((unit) => {
    if (
      !["make_target", "browser_stage_session", "browser_group"].includes(
        unit.kind,
      ) ||
      !unit.rawResourceClaims
    ) {
      return unit;
    }
...
function resolveResourceLimits(
  ...
  return resolveAutoResourceLimits(
    resourceLimits,
    resourceLimitSources,
    "service-backed schedule",
    {
