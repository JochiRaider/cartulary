...
func main() {
	...
	if err := writeRecoveryGenerationRegistryGo(root, families); err != nil {
		fatal(err)
	}
...
func collectArtifacts(root, familyDir string) ([]artifact, error) {
	...
	if err := filepath.WalkDir(baseDir, func(path string, entry os.DirEntry, err error) error {
		...
		if entry.IsDir() {
			if familyDir == "graph-projection" && entry.Name() == "fixtures" {
...
type recoveryGenerationRegistry struct {
	SchemaID    string                       `json:"schema_id"`
	Generations []recoveryGenerationContract `json:"generations"`
}

type recoveryGenerationContract struct {
	GenerationID   string                           `json:"generation_id"`
	CaptureCurrent bool                             `json:"capture_current"`
	Catalog        recoveryGenerationCatalogBinding `json:"catalog"`
	CodecRegistry  recoveryGenerationCodecBinding   `json:"codec_registry"`
	Graph          recoveryGenerationGraphBinding   `json:"graph"`
}

type recoveryGenerationArtifactBinding struct {
	ArtifactPath    string `json:"artifact_path"`
	SchemaID        string `json:"schema_id"`
	CanonicalSHA256 string `json:"canonical_sha256"`
}

type recoveryGenerationCatalogBinding struct {
	ArtifactPath        string `json:"artifact_path"`
	SchemaID            string `json:"schema_id"`
	CanonicalSHA256     string `json:"canonical_sha256"`
	CatalogDigestSHA256 string `json:"catalog_digest_sha256"`
	ContributionCount   int    `json:"contribution_count"`
	AuthoredTableCount  int    `json:"authored_table_count"`
	RequiredTableCount  int    `json:"required_table_count"`
	ObjectFamilyCount   int    `json:"object_family_count"`
}

type recoveryGenerationCodecBinding struct {
	SchemaIDs    []string `json:"schema_ids"`
	DigestSHA256 string   `json:"digest_sha256"`
}

type recoveryGenerationGraphBinding struct {
	AlgorithmID           string                            `json:"algorithm_id"`
	SourceRegistry        recoveryGenerationArtifactBinding `json:"source_registry"`
	ImplementationBinding recoveryGenerationArtifactBinding `json:"implementation_binding"`
}

func writeRecoveryGenerationRegistryGo(root string, families []family) error {
	const registryPath = "contracts/recovery/fixtures/recovery-generation-registry.v1.json"
	...
	for _, current := range families {
		...
		for _, currentArtifact := range current.Artifacts {
			artifacts[strings.TrimPrefix(currentArtifact.Path, "contracts/recovery/")] = currentArtifact
			if currentArtifact.Path == registryPath {
				registryArtifact = currentArtifact
			}
		}
	}
	if registryArtifact.Path == "" {
		return fmt.Errorf("missing Recovery generation registry artifact")
	}
	var registry recoveryGenerationRegistry
	if err := json.Unmarshal([]byte(registryArtifact.JSON), &registry); err != nil {
		return fmt.Errorf("decode Recovery generation registry: %w", err)
	}

	var buffer bytes.Buffer
	buffer.WriteString("// Code generated by tools/contractgen; DO NOT EDIT.\n\n")
	buffer.WriteString("package contractrecovery\n\n")
	buffer.WriteString("type RecoveryGeneration struct {\n")
	buffer.WriteString("\tGenerationID string\n\tCaptureCurrent bool\n")
	buffer.WriteString("\tCatalogJSON string\n\tCatalogCanonicalSHA256 string\n\tCatalogDigestSHA256 string\n")
	buffer.WriteString("\tContributionCount int\n\tAuthoredTableCount int\n\tRequiredTableCount int\n\tObjectFamilyCount int\n")
	buffer.WriteString("\tCodecSchemaIDs []string\n\tCodecRegistrySHA256 string\n")
	buffer.WriteString("\tGraphAlgorithmID string\n")
	buffer.WriteString("\tGraphSourceRegistrySchemaID string\n\tGraphSourceRegistryJSON string\n\tGraphSourceRegistrySHA256 string\n")
	buffer.WriteString("\tGraphImplementationBindingSchemaID string\n\tGraphImplementationBindingJSON string\n\tGraphImplementationBindingSHA256 string\n")
	buffer.WriteString("}\n\n")
	fmt.Fprintf(&buffer, "const RecoveryGenerationRegistrySchemaID = %s\n", strconv.Quote(registry.SchemaID))
	fmt.Fprintf(&buffer, "const RecoveryGenerationRegistryJSON = %s\n", strconv.Quote(registryArtifact.JSON))
	fmt.Fprintf(&buffer, "const RecoveryGenerationRegistrySHA256 = %s\n\n", strconv.Quote(registryArtifact.SHA256))
	buffer.WriteString("var RecoveryGenerations = []RecoveryGeneration{\n")
	for _, generation := range registry.Generations {
		catalogArtifact, catalogOK := artifacts[generation.Catalog.ArtifactPath]
		registryArtifact, registryOK := artifacts[generation.Graph.SourceRegistry.ArtifactPath]
		bindingArtifact, bindingOK := artifacts[generation.Graph.ImplementationBinding.ArtifactPath]
		if !catalogOK || !registryOK || !bindingOK {
			return fmt.Errorf("recovery generation %s references an unavailable artifact", generation.GenerationID)
		}
		fmt.Fprintf(&buffer, "\t{GenerationID: %s, CaptureCurrent: %t,\n", strconv.Quote(generation.GenerationID), generation.CaptureCurrent)
		fmt.Fprintf(&buffer, "\t\tCatalogJSON: %s, CatalogCanonicalSHA256: %s, CatalogDigestSHA256: %s,\n", strconv.Quote(catalogArtifact.JSON), strconv.Quote(generation.Catalog.CanonicalSHA256), strconv.Quote(generation.Catalog.CatalogDigestSHA256))
		fmt.Fprintf(&buffer, "\t\tContributionCount: %d, AuthoredTableCount: %d, RequiredTableCount: %d, ObjectFamilyCount: %d,\n", generation.Catalog.ContributionCount, generation.Catalog.AuthoredTableCount, generation.Catalog.RequiredTableCount, generation.Catalog.ObjectFamilyCount)
		fmt.Fprintf(&buffer, "\t\tCodecSchemaIDs: %#v, CodecRegistrySHA256: %s,\n", generation.CodecRegistry.SchemaIDs, strconv.Quote(generation.CodecRegistry.DigestSHA256))
		fmt.Fprintf(&buffer, "\t\tGraphAlgorithmID: %s,\n", strconv.Quote(generation.Graph.AlgorithmID))
		fmt.Fprintf(&buffer, "\t\tGraphSourceRegistrySchemaID: %s, GraphSourceRegistryJSON: %s, GraphSourceRegistrySHA256: %s,\n", strconv.Quote(generation.Graph.SourceRegistry.SchemaID), strconv.Quote(registryArtifact.JSON), strconv.Quote(generation.Graph.SourceRegistry.CanonicalSHA256))
		fmt.Fprintf(&buffer, "\t\tGraphImplementationBindingSchemaID: %s, GraphImplementationBindingJSON: %s, GraphImplementationBindingSHA256: %s,\n", strconv.Quote(generation.Graph.ImplementationBinding.SchemaID), strconv.Quote(bindingArtifact.JSON), strconv.Quote(generation.Graph.ImplementationBinding.CanonicalSHA256))
		buffer.WriteString("\t},\n")
	}
	buffer.WriteString("}\n")
	formatted, err := format.Source(buffer.Bytes())
	if err != nil {
		return fmt.Errorf("format generated Recovery generation registry: %w", err)
	}
	return stageGeneratedFile(
		filepath.Join(root, "internal", "gen", "contractrecovery", "recovery_generation_registry_gen.go"),
		formatted,
		0o644,
	)
}
