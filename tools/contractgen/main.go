...
func writeTypeScript(root string, plan generationPlan) error {
	artifactSource := "// Code generated by tools/contractgen; DO NOT EDIT.\n\n" +
		...
		"}\n"
	if err := stageGeneratedFile(filepath.Join(root, filepath.FromSlash(plan.TypeScriptArtifactSupportOutput)), []byte(artifactSource), 0o644); err != nil {
		return err
	}
	writtenOutputs := map[string]struct{}{plan.TypeScriptArtifactSupportOutput: {}}
	var exports bytes.Buffer
	exports.WriteString("// Code generated by tools/contractgen; DO NOT EDIT.\n\n")
	if plan.TypeScriptBarrelOutput != "" {
		exports.WriteString("export type { Artifact } from ")
		exports.WriteString(marshalJSONString(typeScriptModuleSpecifier(plan.TypeScriptBarrelOutput, plan.TypeScriptArtifactSupportOutput)))
		exports.WriteString(";\n")
	}
	for _, current := range plan.Families {
		for _, projection := range current.TypeScriptProjections {
			...
			case artifactCollectionProjection:
				source, err = artifactCollectionSource(plan.TypeScriptArtifactSupportOutput, current, projection)
			case jsonValueProjection:
				source, err = jsonValueSource(current, projection)
			default:
				err = fmt.Errorf("unsupported TypeScript projection kind %s", projection.Kind)
			}
			if err != nil {
				return err
			}
			...
			if plan.TypeScriptBarrelOutput != "" {
				...
				exports.WriteString(marshalJSONString(typeScriptModuleSpecifier(plan.TypeScriptBarrelOutput, projection.OutputPath)))
				exports.WriteString(";\n")
			}
		}
	}
	if plan.TypeScriptBarrelOutput != "" {
		if err := stageGeneratedFile(filepath.Join(root, filepath.FromSlash(plan.TypeScriptBarrelOutput)), exports.Bytes(), 0o644); err != nil {
			return err
		}
		writtenOutputs[plan.TypeScriptBarrelOutput] = struct{}{}
	}
...
func artifactCollectionSource(artifactSupportOutput string, current family, projection typeScriptProjection) ([]byte, error) {
	selected, err := selectTypeScriptArtifacts(current, projection.ArtifactPrefixes)
	if err != nil {
		return nil, err
	}
	var buffer bytes.Buffer
	buffer.WriteString("// Code generated by tools/contractgen; DO NOT EDIT.\n\n")
	moduleSpecifier := typeScriptModuleSpecifier(projection.OutputPath, artifactSupportOutput)
	buffer.WriteString("import type { Artifact } from ")
	buffer.WriteString(marshalJSONString(moduleSpecifier))
	buffer.WriteString(";\n")
	buffer.WriteString("import { indexArtifacts } from ")
	buffer.WriteString(marshalJSONString(moduleSpecifier))
	buffer.WriteString(";\n\n")
	buffer.WriteString("export const ")
	buffer.WriteString(projection.Identifier)
	buffer.WriteString(": readonly Artifact[] = [\n")
	for _, currentArtifact := range selected {
		buffer.WriteString("  {\n")
		buffer.WriteString("    path: ")
	...
	buffer.WriteString("];\n\nexport const ")
	buffer.WriteString(projection.IndexIdentifier)
	buffer.WriteString(" = indexArtifacts(")
	buffer.WriteString(projection.Identifier)
	buffer.WriteString(");\n")
...
func selectTypeScriptArtifacts(current family, prefixes []string) ([]artifact, error) {
	selected := make([]artifact, 0, len(current.Artifacts))
	matched := make([]bool, len(prefixes))
	for _, currentArtifact := range current.Artifacts {
		for index, prefix := range prefixes {
			matches := currentArtifact.Path == prefix
			if strings.HasSuffix(prefix, "/") {
				matches = strings.HasPrefix(currentArtifact.Path, prefix)
			}
			if matches {
	...
	for index, ok := range matched {
		if !ok {
			return nil, fmt.Errorf("contract family %s TypeScript artifact selector %s matches no artifact", current.Dir, prefixes[index])
		}
	}
	return selected, nil
}

func typeScriptModuleSpecifier(fromOutput, toOutput string) string {
	fromDir := filepath.Dir(filepath.FromSlash(fromOutput))
	relative, err := filepath.Rel(fromDir, filepath.FromSlash(toOutput))
	if err != nil {
		fatal(fmt.Errorf("relativize TypeScript generated output: %w", err))
	}
	module := filepath.ToSlash(strings.TrimSuffix(relative, ".ts")) + ".js"
