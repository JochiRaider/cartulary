...
func validateContractInput(familyDir, relativePath string, value any) error {
	...
	case "tasksdecisions":
		return validateTasksDecisionsContractInput(relativePath, value)
	case "database-migrations":
		return validateDatabaseMigrationsContractInput(relativePath, value)
	default:
		return nil
	}
}
...
func validateContractFamily(root, familyDir string) error {
	...
	if familyDir == "tasksdecisions" {
		return validateTasksDecisionsSourceCatalogFamily(root)
	}
	if familyDir == "database-migrations" {
		return validateDatabaseMigrationsContractFamily(root)
	}
...
func validateExtensionInputCatalog(value any) error {
	...
	if schemaID, err := requiredString(object, "schema_id", "contracts/extensions/index.json"); err != nil {
		return err
	} else if schemaID != "cartulary.extension_authored_input_catalog.v3" {
		return fmt.Errorf("contracts/extensions/index.json.schema_id must be cartulary.extension_authored_input_catalog.v3")
	}
...
func validateExtensionContractFamily(root string) error {
	...
	if err := validateExtensionOperationalBindings(indexedObjects); err != nil {
		return err
	}
	return nil
}
