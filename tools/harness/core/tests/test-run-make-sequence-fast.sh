...
const requiredScratchHelpers = new Map([
  ["tools/harness/core/tests/test-public-make-wrapper-smoke.sh", ["cartulary_harness_mktemp_dir \"public-make-wrapper.XXXXXX\""]],
...
assert_contains "$(cat "${ROOT_DIR}/tools/task_surface_manifest.json")" "tools/harness/core/tests/test-run-make-sequence-fast.sh" "fast make-sequence smoke backing script"
