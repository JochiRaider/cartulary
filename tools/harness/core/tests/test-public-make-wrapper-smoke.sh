...
govuln_db_status="$(
  run_make_capture "${govuln_db_stdout}" "${govuln_db_stderr}" env -i PATH="${PATH}" HOME="${HOME:-}" GOVULNCHECK_DB=/tmp/cartulary-vulndb CARTULARY_MAKE_ORIGIN_GOVULNCHECK_DB="command line" "${NODE_BIN:-node}" "${ROOT_DIR}/tools/harness/core/harness-contract-cli.mjs" preflight go-vulncheck
)"
