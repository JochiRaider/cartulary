  ...
  cat >"$target_dir/target-summary.json" <<JSON
{
  ...
  "own": {
    ...
    "accounting_modes": { "actual": ${steps}, "reused": 0, "derived": 0 },
    "counts": {
      "steps": ${steps},
      "tests": ${tests},
      "failed": 0,
  ...
  "children": {
    ...
    "counts": { "steps": 0, "tests": 0, "failed": 0, "authoritative": 0, "support": 0, "unmapped": 0, "non_test": 0, "authoritative_failed": 0, "support_failed": 0, "unmapped_failed": 0, "non_test_failed": 0, "packages": 0 },
    "fixture": { "target": "${target}", "total_count": 0, "total_duration_ms": 0, "by_package": [], "by_test": [], "by_strategy": [], "slowest": [] }
  },
  "totals": {
    "target": "${target}",
...
short_failure_summary="$short_failure_results/short-failure/adhoc/short-failure/step-summary.json"
...
assert_contains "$browser_start_failure_output" "failure_class=infra" "browser start step failure class"
assert_contains "$browser_start_failure_output" "reason=service_start_error" "browser start step failure reason"
browser_start_failure_step_summary="$browser_start_failure_results/browser-start-failure/browser-e2e-webserver-backed/browser-e2e-startup-frontend-ready/step-summary.json"
assert_equals "$(json_field "$browser_start_failure_step_summary" "failure_class")" "infra" "browser start step summary class"
assert_equals "$(json_field "$browser_start_failure_step_summary" "failure_reason")" "service_start_error" "browser start step summary reason"
set +e
browser_start_failure_target_output="$(
  CARTULARY_OUTPUT_MODE=quiet \
  CARTULARY_TEST_RESULTS_DIR="$browser_start_failure_results" \
  CARTULARY_TEST_RUN_ID="browser-start-failure" \
    "$ROOT_DIR/tools/harness/output/test-output.sh" target-summary browser-e2e-webserver-backed fail \
    2>&1
)"
...
assert_contains "$browser_resource_conflict_output" "failure_class=infra" "browser resource conflict step failure class"
assert_contains "$browser_resource_conflict_output" "reason=resource_conflict" "browser resource conflict step failure reason"
browser_resource_conflict_step_summary="$browser_resource_conflict_results/browser-resource-conflict/browser-e2e-webserver-backed/browser-e2e-startup-frontend-ready/step-summary.json"
assert_equals "$(json_field "$browser_resource_conflict_step_summary" "failure_class")" "infra" "browser resource conflict step summary class"
assert_equals "$(json_field "$browser_resource_conflict_step_summary" "failure_reason")" "resource_conflict" "browser resource conflict step summary reason"
set +e
...
assert_contains "$listener_conflict_output" "failure_class=infra" "listener conflict step failure class"
assert_contains "$listener_conflict_output" "reason=resource_conflict" "listener conflict step failure reason"
listener_conflict_step_summary="$listener_conflict_results/listener-conflict/browser-e2e-stateful/browser-e2e-startup-services/step-summary.json"
...
shell_progress_failure_output="$(
  ...
  CARTULARY_TEST_RUN_ID="shell-progress-failure" \
    "$HELPER" "shell progress failure" -- bash -lc 'printf "%s\n" "[TARGET] start browser-e2e-webserver-backed service_backed=1 expected_steps=0 expected_tests=0" "real-shell-failure" >&2; exit 9' \
    2>&1
)"
...
shellcheck_stdout_failure_output="$(
  ...
  CARTULARY_TEST_RUN_ID="shellcheck-stdout-failure" \
    "$HELPER" "shellcheck stdout failure" -- bash -lc 'printf "%s\n" "tools/harness/readiness/bootstrap-node-runtime.sh" "In tools/harness/generated-artifacts/tests/test-json-shapes.sh line 200:" "               ^-- SC2016 (info): Expressions don'\''t expand in single quotes, use double quotes for that."; exit 11' \
    2>&1
)"
...
shellcheck_message="ShellCheck SC2016 at tools/harness/generated-artifacts/tests/test-json-shapes.sh:200: Expressions don't expand in single quotes, use double quotes for that."
...
shellcheck_diagnostic_failure_summary="$shellcheck_diagnostic_failure_results/shellcheck-diagnostic-failure/lint-shell/lint-shell/step-summary.json"
...
biome_diagnostic_failure_summary="$biome_diagnostic_failure_results/biome-diagnostic-failure/lint-biome/lint-biome/step-summary.json"
...
cat >"$CARTULARY_STEP_ARTIFACT_DIR/govulncheck-findings.json" <<'JSON'
{
...
set -e
assert_equals "$govulncheck_security_status" "1" "Govulncheck security step status"
...
govulncheck_security_summary="$govulncheck_security_results/govulncheck-security-failure/go-vulncheck/go-vulncheck/step-summary.json"
...
set -e
assert_equals "$govulncheck_malformed_status" "1" "Govulncheck malformed step status"
assert_contains "$govulncheck_malformed_output" "reason=artifact_error" "Govulncheck malformed failure reason"
govulncheck_malformed_summary="$govulncheck_malformed_results/govulncheck-malformed-failure/go-vulncheck/go-vulncheck/step-summary.json"
...
set -e
assert_equals "$gosec_security_status" "1" "Gosec security step status"
assert_contains "$gosec_security_output" "reason=security_finding" "Gosec security failure reason"
gosec_security_summary="$gosec_security_results/gosec-security-failure/go-gosec-targeted/go-gosec-targeted/step-summary.json"
...
CARTULARY_TEST_RUN_ID="single-span" \
  "$ROOT_DIR/tools/harness/output/test-output.sh" target-summary short-target pass >/dev/null 2>&1
...
infra_timing_output="$(
  ...
  CARTULARY_TEST_RUN_ID="infra-timing" \
    "$ROOT_DIR/tools/harness/output/test-output.sh" target-summary infra-target pass \
    2>&1
)"
...
retry_timing_output="$(
  ...
  CARTULARY_TEST_RUN_ID="retry-timing" \
    "$ROOT_DIR/tools/harness/output/test-output.sh" target-summary retry-target pass \
    2>&1
)"
...
CARTULARY_TEST_RUN_ID="skipped-after-failure" \
  "$ROOT_DIR/tools/harness/output/test-output.sh" target-summary failed-check fail >/dev/null 2>&1
set +e
skipped_after_failure_output="$(
  CARTULARY_OUTPUT_MODE=verbose \
  CARTULARY_TEST_RESULTS_DIR="$skipped_after_failure_results" \
  CARTULARY_TEST_RUN_ID="skipped-after-failure" \
    "$ROOT_DIR/tools/harness/output/test-output.sh" run-summary "skipped after failure" fail 0 1 failed-check \
...
child_target_output="$(
  ...
  CARTULARY_TEST_RESULTS_DIR="$child_summary_results" \
  CARTULARY_TEST_RUN_ID="child-summary" \
    "$ROOT_DIR/tools/harness/output/test-output.sh" target-summary parent-target pass --children child-a,child-b \
    2>&1
)"
...
explain_run_summary="$(
  "$ROOT_DIR/tools/harness/diagnostics/explain-run-cli.mjs" --results-dir "$child_summary_results" --run-id child-summary --target parent-target \
    2>&1
)"
...
explain_run_children="$(
  "$ROOT_DIR/tools/harness/diagnostics/explain-run-cli.mjs" --results-dir "$child_summary_results/child-summary" --target parent-target --detail children \
    2>&1
)"
...
explain_run_logs_output="$(
  "$ROOT_DIR/tools/harness/diagnostics/explain-run-cli.mjs" --results-dir "$child_summary_results/child-summary" --detail logs \
    2>&1
)"
...
explain_run_progress_output="$(
  "$ROOT_DIR/tools/harness/diagnostics/explain-run-cli.mjs" --results-dir "$child_summary_results/child-summary" --detail progress \
    2>&1
)"
...
explain_missing_finalize="$(
  "$ROOT_DIR/tools/harness/diagnostics/explain-run-cli.mjs" --results-dir "$missing_finalize_results/tool-run" \
    2>&1
)"
...
if ! wait "$nested_pid_a"; then
  fail "nested step artifact owner a failed"
fi
if ! wait "$nested_pid_b"; then
  fail "nested step artifact owner b failed"
fi
[[ -f "$nested_artifacts_results/nested-artifacts/browser-e2e-webserver-backed/frontend-toolchain/step-summary.json" ]] || fail "nested step artifacts missing webserver-backed owner"
...
for step_owner in step-a step-b; do
  ...
  CARTULARY_STEP_COMMAND_ARGV='[":"]' \
  CARTULARY_STEP_START_TIME="2026-01-01T00:00:00.000Z" \
  CARTULARY_STEP_END_TIME="2026-01-01T00:00:00.001Z" \
  CARTULARY_STEP_LOGICAL_DURATION_MS="1" \
  CARTULARY_STEP_EXECUTED_DURATION_MS="1" \
  CARTULARY_STEP_WALL_DURATION_MS="1" \
  CARTULARY_STEP_EXIT_STATUS="0" \
  CARTULARY_STEP_STDOUT_LOG="$shared_stdout" \
  CARTULARY_STEP_STDERR_LOG="$shared_stderr" \
...
fixture_report_output="$(
  "$ROOT_DIR/tools/harness/diagnostics/fixture-report-cli.mjs" --results-dir "$fixture_results" --threshold-ms 30000 --top 2 \
    2>&1
)"
...
"$ROOT_DIR/tools/harness/diagnostics/fixture-report-cli.mjs" --results-dir "$fixture_results" --run-id fixture-run --threshold-ms 30000 --json >"$fixture_report_json"
...
CARTULARY_TEST_RESULTS_DIR="$fixture_results" \
CARTULARY_TEST_RUN_ID="fixture-aggregate-run" \
  "$ROOT_DIR/tools/harness/output/test-output.sh" target-summary fixture-parent pass --children fixture-child >/dev/null 2>&1
CARTULARY_TEST_RESULTS_DIR="$fixture_results" \
CARTULARY_TEST_RUN_ID="fixture-aggregate-run" \
  "$ROOT_DIR/tools/harness/output/test-output.sh" run-summary check pass 1 1 - fixture-parent >/dev/null 2>&1
fixture_report_run_label_output="$(
  "$ROOT_DIR/tools/harness/diagnostics/fixture-report-cli.mjs" --results-dir "$fixture_results" --run-id fixture-aggregate-run --target check --threshold-ms 1 \
    2>&1
)"
...
fixture_report_aggregate_target_output="$(
  "$ROOT_DIR/tools/harness/diagnostics/fixture-report-cli.mjs" --results-dir "$fixture_results" --run-id fixture-aggregate-run --target fixture-parent --threshold-ms 1 \
    2>&1
)"
...
child_run_output="$(
  ...
  CARTULARY_TEST_RESULTS_DIR="$child_summary_results" \
  CARTULARY_TEST_RUN_ID="child-summary" \
    "$ROOT_DIR/tools/harness/output/test-output.sh" run-summary "child run" pass 1 1 - \
...
CARTULARY_TEST_RUN_ID="shared-execution" \
  "$ROOT_DIR/tools/harness/output/test-output.sh" shared-execution \
...
shared_execution_output="$(
  ...
  CARTULARY_TEST_RESULTS_DIR="$shared_execution_results" \
  CARTULARY_TEST_RUN_ID="shared-execution" \
    "$ROOT_DIR/tools/harness/output/test-output.sh" run-summary "shared execution run" pass 2 2 - target-fast target-slow \
    2>&1
)"
...
missing_group_output="$(
  ...
  CARTULARY_TEST_RESULTS_DIR="$child_summary_results" \
  CARTULARY_TEST_RUN_ID="child-summary" \
    "$ROOT_DIR/tools/harness/output/test-output.sh" run-summary "child run missing group" pass 1 1 - \
...
missing_child_output="$(
  ...
  CARTULARY_TEST_RUN_ID="missing-child" \
    "$ROOT_DIR/tools/harness/output/test-output.sh" target-summary parent-with-missing pass --children missing-child \
    2>&1
)"
...
cat >"$status_only_child_run/status-only-child/target-summary.json" <<'JSON'
{
  ...
  "counts": { "steps": 1, "tests": 1, "failed": 0, "authoritative": 1, "support": 0, "unmapped": 0, "non_test": 0, "authoritative_failed": 0, "support_failed": 0, "unmapped_failed": 0, "non_test_failed": 0, "packages": 1 },
  "failure_class": null,
  "failure_reason": null,
  "failure_classes": { "product": 0, "security": 0, "config": 0, "infra": 0, "harness": 0, "artifact": 0, "timing": 0, "interrupted": 0, "unknown": 0 },
...
status_only_child_output="$(
  ...
  CARTULARY_TEST_RUN_ID="status-only-child" \
    "$ROOT_DIR/tools/harness/output/test-output.sh" target-summary parent-with-status-only-child fail --children status-only-child \
    2>&1
)"
...
cat >"$skipped_child_run/failed-backend/target-summary.json" <<'JSON'
{
  ...
  "teardown_duration_ms": 0,
  "accounting_modes": { "actual": 1, "reused": 0, "derived": 0 },
  "counts": { "steps": 1, "tests": 1, "failed": 1, "authoritative": 1, "support": 0, "unmapped": 0, "non_test": 0, "authoritative_failed": 1, "support_failed": 0, "unmapped_failed": 0, "non_test_failed": 0, "packages": 1 },
...
skipped_child_output="$(
  ...
  CARTULARY_TEST_RUN_ID="skipped-child" \
    "$ROOT_DIR/tools/harness/output/test-output.sh" target-summary parent-with-skipped fail --children failed-backend,skipped-browser \
    2>&1
)"
...
scheduler_skipped_only_output="$(
  ...
  CARTULARY_TEST_RESULTS_DIR="$skipped_child_results" \
  CARTULARY_TEST_RUN_ID="skipped-child" \
    "$ROOT_DIR/tools/harness/output/test-output.sh" target-summary scheduler-skipped-only fail \
    2>&1
)"
...
explicit_skipped_child_output="$(
  ...
  CARTULARY_TEST_RESULTS_DIR="$skipped_child_results" \
  CARTULARY_TEST_RUN_ID="skipped-child" \
    "$ROOT_DIR/tools/harness/output/test-output.sh" target-summary parent-with-explicit-skipped fail \
...
imported_skipped_child_output="$(
  ...
  CARTULARY_TEST_RESULTS_DIR="$skipped_child_results" \
  CARTULARY_TEST_RUN_ID="skipped-child" \
    "$ROOT_DIR/tools/harness/output/test-output.sh" target-summary parent-with-imported-skipped fail \
...
scheduler_imported_skipped_child_output="$(
  ...
  CARTULARY_TEST_RESULTS_DIR="$skipped_child_results" \
  CARTULARY_TEST_RUN_ID="skipped-child" \
    "$ROOT_DIR/tools/harness/output/test-output.sh" target-summary parent-with-scheduler-imported-skipped fail \
