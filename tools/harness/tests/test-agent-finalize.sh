...
assert_equals "$(json_field "$results_summary" 'value.actions.map((action) => action.action_id)')" $'scheduler_drift_validation\nschema_shape_validation\nduration_baseline_coverage\nduration_baseline_refresh\ngenerated_structure_refresh\nduration_baseline_drift_validation' "RESULTS_DIR action selection validates authored closure before refresh"
assert_equals "$(json_field "$results_summary" 'value.actions.find((action) => action.action_id === "duration_baseline_refresh").substeps.map((substep) => substep.id)')" $'go-test-duration-baselines\nbrowser-e2e-duration-baselines\nservice-backed-make-target-duration-baselines\nharness-smoke-duration-baselines' "RESULTS_DIR baseline refresh owns only baseline writers"
assert_equals "$(grep -c '^generate$' "$results_log")" "1" "RESULTS_DIR invokes generate once"
assert_equals "$(grep -c '^generate-drift$' "$results_log")" "1" "RESULTS_DIR invokes generate-drift once"
...
assert_equals "$(json_field "$failure_summary" 'value.actions.flatMap((action) => action.substeps).filter((step) => step.status === "skipped").length')" "9" "child failure skipped substeps"
...
assert_contains "$machine_stdout" '"schema_id":"cartulary.tool_run_summary.v5"' "machine tool summary"
