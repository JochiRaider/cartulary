...
assert_contains "${makefile_content}" '$(FRONTEND_NODE_MODULES_DIRS) $(CURDIR)/.pnpm-store' "distclean removes dependency roots with pnpm store"
...
assert_contains "${generated_make}" 'RUN_MAKE_NODE_TOOL = env $(TASK_SURFACE_PUBLIC_INPUT_STRIP_ENV) CARTULARY_HARNESS_IDENTITY_PREPARED=1 CARTULARY_TEST_TARGET="$(1)" CARTULARY_MAKE_INPUT_SOURCES="$(call TASK_SURFACE_INPUT_SOURCES,$(TASK_SURFACE_PUBLIC_INPUT_NAMES))" NODE_BIN="$(NODE_BIN)" $(2) ./tools/harness/execution/run-make-node-tool.sh $(1)' "generated Make node-tool macro"
...
assert_contains "${generated_make}" 'RUN_MAKE_NODE_TOOL = env $(TASK_SURFACE_PUBLIC_INPUT_STRIP_ENV) CARTULARY_HARNESS_IDENTITY_PREPARED=1 CARTULARY_TEST_TARGET="$(1)"' "generated Make node tools reuse the preflighted public identity"
...
assert_contains "${generated_make}" 'RUN_STEP_SCRIPT="$(RUN_STEP_SCRIPT)"' "generated summary target run step"
...
task_guide_node_tool_line="$(printf '%s\n' "${task_guide_block}" | sed -n '/RUN_MAKE_NODE_TOOL/p')"
target_plan_node_tool_line="$(printf '%s\n' "${target_plan_block}" | sed -n '/RUN_MAKE_NODE_TOOL/p')"
fixture_report_node_tool_line="$(printf '%s\n' "${fixture_report_block}" | sed -n '/RUN_MAKE_NODE_TOOL/p')"
go_test_duration_baseline_drift_node_tool_line="$(printf '%s\n' "${go_test_duration_baseline_drift_block}" | sed -n '/RUN_MAKE_NODE_TOOL/p')"
browser_e2e_duration_baselines_node_tool_line="$(printf '%s\n' "${browser_e2e_duration_baselines_block}" | sed -n '/RUN_MAKE_NODE_TOOL/p')"
...
assert_contains "${check_frontend_install_block}" 'check-frontend-install: frontend-install' "check frontend install helper dependency"
...
assert_contains "${browser_e2e_duration_baselines_node_tool_line}" '$(call RUN_MAKE_NODE_TOOL,browser-e2e-duration-baselines,RESULTS_DIR="$(RESULTS_DIR)" BROWSER_E2E_DURATION_BASELINE="$(BROWSER_E2E_DURATION_BASELINE)")' "browser duration refresh node-tool explicit baseline env"
...
for target in test-fast test ci release-check run-harness-smoke-fast run-harness-smoke-execution run-harness-smoke-extended run-harness-smoke-full; do
  ...
  if [[ "${target}" == run-harness-smoke-* ]]; then
    ...
    assert_contains "${make_dry_run_output}" "tools/harness/execution/run-make-sequence.sh --sequence ${target}" "make -n ${target} helper command"
  fi
