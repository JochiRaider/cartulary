...
export function createBaseStepContext(runner) {
  const stepDir = requiredEnv("CARTULARY_STEP_DIR");
  ensureDir(stepDir);
  const accountingMode =
    optionalEnv("CARTULARY_REPORT_SLICE") === "1"
      ? normalizeAccountingMode(
          optionalEnv("CARTULARY_STEP_ACCOUNTING_MODE", "actual"),
        )
      : "actual";
  const countingMode = normalizeStepCountingMode(
    optionalEnv("CARTULARY_STEP_COUNTING_MODE", "counted"),
  );
  const logicalDurationMs = clampDurationMs(
    parseInteger("CARTULARY_STEP_LOGICAL_DURATION_MS", 0),
  );
  const executedDurationMs = clampDurationMs(
    parseInteger(
      "CARTULARY_STEP_EXECUTED_DURATION_MS",
      accountingMode === "actual" ? logicalDurationMs : 0,
    ),
  );
  return {
    label: requiredEnv("CARTULARY_STEP_LABEL"),
    stepDir,
    target: optionalEnv("CARTULARY_TEST_TARGET", "adhoc"),
    command: requiredEnv("CARTULARY_STEP_COMMAND"),
    commandArgv: optionalJSONEnv("CARTULARY_STEP_COMMAND_ARGV", null),
    runner,
    timingBucket: normalizeTimingBucket(
      optionalEnv("CARTULARY_STEP_TIMING_BUCKET"),
      runner,
    ),
    startTime: requiredEnv("CARTULARY_STEP_START_TIME"),
    endTime: requiredEnv("CARTULARY_STEP_END_TIME"),
    accountingMode,
    countingMode,
    executedDurationMs,
    logicalDurationMs,
    ...
    wallDurationMs: clampDurationMs(
      parseInteger("CARTULARY_STEP_WALL_DURATION_MS", logicalDurationMs),
    ),
    exitStatus: parseInteger("CARTULARY_STEP_EXIT_STATUS", 0),
  };
}

export function writeStepArtifacts(context, details) {
  const playwrightTimingPath = details.playwrightTiming
    ? path.join(context.stepDir, "playwright-timing.json")
    : "";
  ...
  for (const [key, value] of Object.entries({
    ...
    shellcheck_inventory: existsSync(
      path.join(context.stepDir, "shellcheck-inventory.txt"),
    )
      ? path.join(context.stepDir, "shellcheck-inventory.txt")
      : "",
    security_profiles: existsSync(
      path.join(context.stepDir, "security-profiles.jsonl"),
    )
      ? path.join(context.stepDir, "security-profiles.jsonl")
      : "",
  ...
  writeJson(path.join(context.stepDir, "meta.json"), meta);

  if (details.manifestSummary) {
    writeJson(
      path.join(context.stepDir, "manifest-summary.json"),
      details.manifestSummary,
    );
  }
  if (details.manifestMismatch) {
    writeJson(
      path.join(context.stepDir, "manifest-mismatch.json"),
      details.manifestMismatch,
    );
  }

  const summary = {
    schema_id: stepSummarySchemaID,
    label: context.label,
    target: context.target,
    runner: context.runner,
    status: details.status,
  ...
  writeValidatedJson(
    path.join(context.stepDir, "step-summary.json"),
    stepSummarySchemaID,
    summary,
  );
  ...
  const toolSummary = buildToolRunSummary({
    ...
    summaryArtifacts: [
      ...
      details.manifestSummary
        ? fileArtifactRef(
            "manifest_summary",
            relToRepo(path.join(context.stepDir, "manifest-summary.json")),
          )
        : null,
      details.manifestMismatch
        ? fileArtifactRef(
            "manifest_mismatch",
            relToRepo(path.join(context.stepDir, "manifest-mismatch.json")),
          )
        : null,
      finalizeSummaryRel
