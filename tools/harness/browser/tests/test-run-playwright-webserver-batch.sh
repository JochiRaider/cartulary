...
HELPER="$ROOT_DIR/tools/harness/browser/run-playwright-webserver-batch.sh"
cleanup_paths=()
...
const frontendPhaseMap = JSON.parse(
  fs.readFileSync(path.resolve(process.cwd(), "tools/frontend_phase_maps/fe_p1_test_map.json"), "utf8"),
);
const expectedScenarioCount = frontendPhaseMap.rows
  ?.find((row) => row.id === "FE-E-P1-01")
  ?.scenario_titles
  ?.length;
...
if (!toolSummary.summary_artifacts?.some((entry) =>
  ...
  entry.format === "json" &&
  entry.path.endsWith("frontend-row-accounting.json") &&
