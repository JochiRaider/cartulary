...
assert_file_contains "$ROOT_DIR/tools/harness/browser/start-web-e2e.sh" 'CARTULARY_STEP_TIMING_BUCKET=service_wait run_step_command "browser-e2e startup services"' "browser lifecycle service timing bucket"
...
assert_file_contains "$ROOT_DIR/tools/harness/browser/start-web-e2e.sh" 'CARTULARY_STEP_TIMING_BUCKET=migration run_step_command "browser-e2e startup database"' "browser lifecycle migration timing bucket"
...
assert_file_contains "$ROOT_DIR/tools/harness/browser/start-web-e2e.sh" 'run_step_command "browser-e2e validate frontend preview artifact" require_frontend_preview_artifacts' "browser lifecycle validates built preview artifact"
assert_file_contains "$ROOT_DIR/tools/harness/browser/start-web-e2e.sh" 'run_step_command "browser-e2e startup frontend ready" start_frontend_preview_ready_with_retry' "browser lifecycle proves frontend before backend startup"
assert_file_contains "$ROOT_DIR/tools/harness/browser/start-web-e2e.sh" 'run_step_command "browser-e2e verify frontend ready" browser_verify_frontend_ready' "browser lifecycle rechecks frontend after backend readiness"
