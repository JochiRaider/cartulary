...
export function renderTaskSurfaceMakeRuntime(manifest) {
  ...
  const literalInputs = literalMakeInputNames(manifest);
  const lines = [
    "# Code generated by tools/harness/generated-artifacts/render-task-surface-make.mjs; DO NOT EDIT.",
    "",
    ...
    `TASK_SURFACE_LITERAL_INPUT_NAMES := ${literalInputs.join(" ")}`,
...
function renderMakeRecipe(recipe, manifest) {
  ...
  if (recipe.type === "work_graph") {
    ...
    return [
      ...
      ...publicPrelude,
      ...prerequisitePrelude,
      `\t$(Q)env ${envStripArgsForTarget(entry, manifest)} $(TASK_SURFACE_MACHINE_STATE_ENV) ${forwarded.join(" ")} MAKE="$(MAKE)" NODE_BIN="$(NODE_BIN)" TEST_SERVICES_BIN="$(TEST_SERVICES_BIN)" $(NODE_BIN) ./${workGraphRunnerScript} ${selectionArgs}`,
    ];
  }
...
function requiresLiteralMakeTransport(input) {
  return (
    ...
    Array.isArray(input?.allowed_sources) &&
    input.allowed_sources.length === 1 &&
    input.allowed_sources[0] === "make_command_line"
  );
}

function literalMakeInputNames(manifest) {
  const names = new Set();
  for (const entry of targetEntries(manifest ?? { targets: [] })) {
    if (entry.target_class !== "public") {
      continue;
    ...
    for (const input of entry.input_contract?.inputs ?? []) {
      if (requiresLiteralMakeTransport(input)) {
        names.add(input.name);
      }
    }
  }
...
function runtimeForwardedInputEnvEntries(entry = null) {
  const inputMap = new Map(
    (entry?.input_contract?.inputs ?? []).map((input) => [input.name, input]),
  );
  ...
  return [
    ...names.map((name) =>
      requiresLiteralMakeTransport(inputMap.get(name))
...
function literalHarnessInputEnvEntries(entry = null) {
  const runtimeNames = runtimeForwardedInputNames(entry);
  const names = (entry?.input_contract?.inputs ?? [])
    .filter(
      (input) =>
        requiresLiteralMakeTransport(input) && !runtimeNames.has(input.name),
    )
    .map((input) => input.name)
    .sort((left, right) => left.localeCompare(right));
  return [
    ...names.map(
      (name) => `${name}=$(call task_surface_shell_quote_raw,${name})`,
    ),
    ...(names.length > 0
      ? [
          `CARTULARY_MAKE_INPUT_SOURCES="$(call TASK_SURFACE_INPUT_SOURCES,${names.join(" ")})"`,
...
function renderStepCommandRecipe(recipe, entry = null, manifest = null) {
  ...
  if (recipe.mode === "run_step") {
    const runnerEnv = [
      ...forwardedEnv,
      ...literalHarnessInputEnvEntries(entry),
    ];
