...
expected_support="$(( base_support + 2 + frontend_support ))"
expected_derived="$(( phase_count + 1 ))"
expected_total="$(( expected_authoritative + expected_support ))"
...
assert_equals "$(json_field "$success_summary" "own.counts.unowned_regression")" "0" "success unowned regression count"
...
assert_equals "$(json_field "$residual_summary" "own.counts.unowned_regression_failed")" "0" "residual unowned regression failure count"
assert_equals "$(json_field "$residual_summary" "own.counts.support_failed")" "1" "residual support failure count"
