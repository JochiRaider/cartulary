...
TEST_OUTPUT_HELPER="${RUN_STEP_REPO_ROOT}/tools/harness/output/test-output.sh"
...
step_elapsed_ms() {
  ...
  step_clamp_duration_ms "$((end_ms - start_ms))"
}
...
emit_target_timing_span() {
  ...
  duration_ms="$(step_clamp_duration_ms "$5")"
...
run_timing_span() {
  ...
  end_time="$(step_now_utc)"
  end_ms="$(step_now_monotonic_ms)"
  duration_ms="$(step_elapsed_ms "${start_ms}" "${end_ms}")"
  span_status="pass"
  if [[ "${status}" -ne 0 ]]; then
    span_status="fail"
  fi
...
step_capture_start() {
  if [[ "$#" -ne 1 ]]; then
    echo "step_capture_start requires <prefix>" >&2
    return 2
  fi

  local prefix="$1"
  printf -v "${prefix}_START_TIME" '%s' "$(step_now_utc)"
  printf -v "${prefix}_START_MONOTONIC_MS" '%s' "$(step_now_monotonic_ms)"
}

step_capture_finish() {
  if [[ "$#" -ne 1 ]]; then
    echo "step_capture_finish requires <prefix>" >&2
    return 2
  fi
  ...
  end_time="$(step_now_utc)"
  end_monotonic="$(step_now_monotonic_ms)"
  duration_ms="$(step_elapsed_ms "${!start_monotonic_var:-0}" "${end_monotonic}")"
...
run_vitest_command_with_watchdog() {
  if [[ "$#" -lt 6 ]]; then
    echo "run_vitest_command_with_watchdog requires <label> <step-dir> <stdout-log> <stderr-log> <output-mode> <command...>" >&2
    return 2
  fi
  ...
  step_secure_mkdir "$(dirname "$stdout_log")" "$(dirname "$stderr_log")" "$step_dir"
  CARTULARY_VITEST_WATCHDOG_LOG="${step_dir}/watchdog.json"
  ...
  _cartulary_vitest_wait_for_interrupted_child() {
    ...
    grace_start_ms="$(step_now_monotonic_ms)"
    while kill -0 "$child_pid" >/dev/null 2>&1; do
      local grace_now_ms
      grace_now_ms="$(step_now_monotonic_ms)"
  ...
  while kill -0 "$child_pid" >/dev/null 2>&1; do
    ...
    local now_ms
    now_ms="$(step_now_monotonic_ms)"
    if (( now_ms - start_ms >= timeout_seconds * 1000 )); then
      trap - INT TERM
      local timed_out_at
      local killed_at
      timed_out_at="$(step_now_utc)"
...
step_redact_stream() {
  ...
  "${node_bin}" "${RUN_STEP_REPO_ROOT}/tools/harness/contract/harness-contract-cli.mjs" redact
}

step_redact_file() {
  if [[ "$#" -ne 1 ]]; then
    echo "step_redact_file requires <file>" >&2
    return 2
  fi
  local file="$1"
  if [[ ! -e "$file" ]]; then
...
resolve_results_root() {
  if [[ -n "${CARTULARY_TEST_RESULTS_DIR:-}" ]]; then
    if [[ "${CARTULARY_TEST_RESULTS_DIR}" = /* ]]; then
      ...
      printf '%s\n' "${RUN_STEP_REPO_ROOT}/${CARTULARY_TEST_RESULTS_DIR}"
    fi
    return
  fi
  printf '%s\n' "${RUN_STEP_REPO_ROOT}/.cartulary/test-results"
}
...
ensure_target_artifact_dir() {
  ...
  target="$(resolve_test_target)"
  step_secure_mkdir "${results_root}/${run_id}" "${results_root}/${run_id}/${target}"
...
prepare_target_support_dir() {
  ...
  step_secure_mkdir "${target_dir}/${name}"
  printf '%s\n' "${target_dir}/${name}"
}
...
prepare_shared_artifact_dir() {
  ...
  results_root="$(resolve_results_root)"
  run_id="$(resolve_test_run_id)"
  step_secure_mkdir "${results_root}/${run_id}" "${results_root}/${run_id}/_shared" "${results_root}/${run_id}/_shared/${name}"
  printf '%s\n' "${results_root}/${run_id}/_shared/${name}"
}

prepare_step_artifact_dir() {
  local step="$1"
  local target_dir
  local slug
  target_dir="$(ensure_target_artifact_dir)"
...
emit_report_step_summary() {
  if [[ "$#" -ne 8 ]]; then
    echo "emit_report_step_summary requires <helper-command> <label> <command-text> <start-time> <end-time> <duration-ms> <wall-duration-ms> <exit-status>" >&2
    return 2
  fi
  ...
  duration_ms="$(step_clamp_duration_ms "$6")"
  wall_duration_ms="$(step_clamp_duration_ms "$7")"
  ...
  CARTULARY_STEP_EXIT_STATUS="$exit_status" \
  CARTULARY_STEP_TIMING_BUCKET="${CARTULARY_STEP_TIMING_BUCKET:-}" \
  CARTULARY_STEP_INTERRUPT_SIGNAL="${CARTULARY_STEP_INTERRUPT_SIGNAL:-}" \
...
run_step_command() {
  if [[ "$#" -lt 2 ]]; then
    echo "run_step_command requires <label> <command...>" >&2
    return 2
  fi
  ...
  CARTULARY_STEP_DIR="$step_dir" \
  CARTULARY_STEP_COMMAND="$command_text" \
  CARTULARY_STEP_COMMAND_ARGV="$command_argv_json" \
  CARTULARY_STEP_START_TIME="$start_time" \
  CARTULARY_STEP_END_TIME="$end_time" \
  CARTULARY_STEP_LOGICAL_DURATION_MS="$duration_ms" \
  CARTULARY_STEP_EXECUTED_DURATION_MS="$duration_ms" \
  CARTULARY_STEP_WALL_DURATION_MS="${STEP_WALL_DURATION_MS}" \
  CARTULARY_STEP_EXIT_STATUS="$status" \
  CARTULARY_STEP_TIMING_BUCKET="${CARTULARY_STEP_TIMING_BUCKET:-}" \
  CARTULARY_STEP_STDOUT_LOG="$stdout_log" \
  CARTULARY_STEP_STDERR_LOG="$stderr_log" \
    NODE_BIN="${NODE_BIN:-}" "${TEST_OUTPUT_HELPER}" shell-step
  helper_status=$?
  set -e
