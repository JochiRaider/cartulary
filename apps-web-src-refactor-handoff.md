...
| Clipboard utility | Future `workbookClipboard.ts`; duplicate `parseClipboardTableForDimensions`; Timeline and Shell paste callers | String parsing helpers only. | Grid target resolution, mutation dispatch, API calls, DOM clipboard access. | Plain text clipboard payload. | String matrix and row/column dimensions. | Empty text returns one row with one empty cell. | No frontend utility row/column maximum is enforced; parser returns computed dimensions for any payload it can parse. Malformed unclosed quote parses to one final cell, preserving delimiters and newlines as literal content until EOF. Any accepted paste-size ceiling belongs to the server/API contract, not this utility. | `workbookClipboard.test.ts` plus phase 9 paste target tests. |
| Value formatting utility | Future `workbookValueFormat.ts`; duplicate `stringifyGridValue` | Primitive conversion and collection display extraction. | Locale policy, UI markup, mutation payloads, validation. | Unknown cell value. | Display-safe string. | `null`, `undefined`, missing cell, unsupported object, unsupported array, Date object, and plain object without `items[]` return empty string. | Non-finite numbers follow the existing `number` rule and return `String(value)`, including `Infinity`, `-Infinity`, and `NaN`. | `workbookValueFormat.test.ts` and entity/timeline display tests. |
...
| Reference option model | Future `workbookReferenceOptions.ts`; `referenceOptionsForField` | View field contract, loaded same-incident option lists. | Network loading, component rendering, cross-incident lookup. | Field contract and normalized option buckets. | Ordered reference option list and boolean use-reference decision. | Missing option bucket is empty list. | Normalized inputs are required to contain only visible, active, same-incident option rows. Poisoned options are invalid loader output, not values this selector normalizes; a detecting loader must fail closed for the option load rather than returning a repaired bucket. | `workbookReferenceOptions.test.ts` and generic reference flow tests. |
...
| Timeline paste | Existing `WorkbookShell.phase9.sentinel.test.tsx` covers scalar versus tabular dispatch and stable paste anchors; `workbookClipboard.test.ts` covers parser edge behavior. | Characterized: malformed unclosed quote returns one final literal cell; frontend utility enforces no row/column maximum; accepted paste-size limits are server/API owner if added later. |
...
| Mention states | Existing `WorkbookShell.phase4.support.test.tsx`, `WorkbookShell.phase5.mentionChips.test.ts`, and `workbookMentionChips` model coverage. | Characterized for model/hook boundaries; full panel extraction is closed by the view-model/intent boundary below, with dismissed-mention state and messages still owned by the parent model. |
...
| Row history/rollback/destructive controls | Existing `WorkbookShell.phase7.test.tsx` covers history rendering, rollback targets, delete, restore, and continuity. | Characterized before history panel movement; full panel extraction is closed by the precomputed legal-action/view-model boundary below. |
...
| Date object | Empty string. | Characterized by `workbookValueFormat.test.ts`; Date remains an unsupported object shape and this generic utility does not add locale or ISO formatting. |
| Non-finite number | `String(value)`, including `Infinity`, `-Infinity`, and `NaN`. | Characterized by `workbookValueFormat.test.ts`; public JSON/wire validation remains outside this display utility. |
...
| Malformed unclosed quote | Parse through EOF as quoted content; delimiters and newlines encountered before EOF remain literal cell content, and the final row/cell is emitted. | Characterized by `workbookClipboard.test.ts`; no parser repair or rejection is added. |
| Maximum accepted row or column count | No frontend utility-level maximum; return the parsed matrix and computed dimensions. Any accepted paste-size ceiling must be owned by the server/API contract or another Core-backed caller boundary before client rejection is added. | Characterized by `workbookClipboard.test.ts`; no owner-derived utility limit is recorded. |
...
| Invisible, deleted, wrong-type, malformed, or foreign-incident reference option appears while building normalized input | Invalid option-load boundary: fail closed for that option load and expose unavailable reference options instead of returning a partially repaired bucket. `referenceOptionsForField` remains a pure bucket selector and must not filter, relabel, resolve, or normalize poisoned inputs. | Core 01 and Core 04 own same-incident, active-target, visibility, wrong-type, and mutation rejection rules; this handoff owns only the frontend normalized-input contract. |

### Timeline Panel Extraction Closure

`TimelineHistoryPanel` full extraction is closed by a precomputed view-model and intent-only boundary. The panel receives display-ready history rows, destructive/rollback preview state, messages, and a legal-action list already derived from `/history` metadata. It may render only actions present in that legal-action list.

The parent hook/model owns rollback target construction, `base_row_version`, `client_txn_id`, route paths, `waitForCommittedRecordIdle`, save-state updates, refresh sequencing, and selected/focused row continuity. The panel emits intents only, such as preview destructive action, preview rollback, confirm pending action, cancel pending action, refresh, and row-history selection. It must not receive route payload builders, raw fetch functions, mutation state setters, or row-version resolver internals.

`TimelineMentionsPanel` full extraction is closed by the same model/intent split. The panel receives grouped mentions, selected mention state, resolve-target options, role/action availability, selected target id, and display-ready messages. It emits select, resolve, create, dismiss, restore, target-change, and missing-target intents only.

The parent hook/model owns latest-row lookup, `entity_mention_id`, mention row version, route calls, dismissed-mention persistence, inspector messages, and entity refresh. Missing-target handling remains an emitted intent so the parent can preserve the current inspector message behavior without coupling the panel to `TimelineWorkbookInspector` internals.

### Workbook Surface Extraction Closure

Full extraction of `EntityWorkbookSurface`, `GenericWorkbookSurface`, and `AssessmentWorkbookSurface` requires runtime hooks before moving the components: `useEntitySurfaceRuntime`, `useGenericSurfaceRuntime`, and `useAssessmentSurfaceRuntime`.

Extracted surfaces receive only `{ model, commands }` props. They must not receive `apiBase`, raw `fetchJSON`, direct refresh closures, React query setters, saved-view mutation callbacks, or route payload builders. The `model` side contains rows, grids, drafts, messages, reference/support options, action availability, and query-control display state. The `commands` side exposes typed UI intents for query controls, grid actions, draft edits, paste, create/update, merge, evidence handles, task lifecycle, decision supersede, and assessment create.

The shell/runtime remains owner of active surface identity, `sheet_ref`, saved-view state, query state transitions, and query-control intent handling. The runtime hooks own route payload construction, refresh sequencing, evidence handles, reference-option loading, support-row loading, paste submission, create/edit submit, merge, task lifecycle, decision supersede, and assessment create.

`useEntitySurfaceRuntime` owns entity rows, entity reference options, paste/edit/merge commands, evidence-handle commands, and task lifecycle commands. `useGenericSurfaceRuntime` owns contract-shaped generic rows, create/edit drafts, generic reference-option loading, reference field command shaping, and generic mutation submit. `useAssessmentSurfaceRuntime` owns assessment rows, support-row loading, confidence/create draft state, decision supersede commands, and assessment create submission.

### Non-Normative Research Rationale For Closure

Research reports R01 through R09 are rationale only and do not create Core conformance behavior. Core 01 through Core 04 remain the authority for product, security, visibility, mutation, startup, and workbook semantics.

R04 and R05 support keeping workbook hot-path interactions direct and adjacent to the grid. R08 and R09 support treating grid libraries as controlled adapters rather than owners of workbook semantics. R01 and R03 support avoiding controller-heavy coupling. R06 and R07 support workbook-shaped structured state with companion artifacts. R02 supports lightweight coordination surfaces outside routine row-edit ceremony.
