...
CREATE TABLE public.extension_migration_ledger (
    ...
    CONSTRAINT extension_migration_ledger_metadata_fk
        FOREIGN KEY (profile_id) REFERENCES public.extension_state_metadata(profile_id)
        DEFERRABLE INITIALLY DEFERRED
);

-- Network Flow is the only extension-versioned state owner in this adoption.
-- Its authoritative v1 tables predate the generic metadata table, so the owner
-- migration establishes the exact known lineage/version for both empty and
-- populated deployments instead of inferring presence from coordination rows.
INSERT INTO public.extension_state_metadata (
    profile_id, migration_lineage_id, state_version, last_migration_id,
    metadata_version, created_at, updated_at
) VALUES (
    'network_flow_activity', 'network_flow_activity.state_v1', 1, NULL,
...
CREATE TABLE public.extension_staged_object_references (
    staging_id text PRIMARY KEY REFERENCES public.extension_staged_objects(staging_id),
...
CREATE INDEX jobs_extension_nonterminal_idx
    ON public.jobs (extension_owner_profile_id, submitted_at, job_id)
    WHERE extension_owner_profile_id IS NOT NULL
      AND status IN ('queued', 'running', 'cancel_requested');

CREATE TABLE public.extension_job_commit_proofs (
    job_id uuid PRIMARY KEY REFERENCES public.jobs(job_id) ON DELETE RESTRICT,
    owner_profile_id text NOT NULL,
    operation_kind text NOT NULL,
    final_commit_id text NOT NULL UNIQUE,
...
CREATE TABLE public.extension_job_cancellation_observations (
    ...
    job_id uuid NOT NULL UNIQUE REFERENCES public.jobs(job_id) ON DELETE RESTRICT,
...
-- +goose Down
-- Extension coordination history and proof tables are intentionally forward-only.
-- A production down migration could erase migration/proof facts or make committed
-- authoritative references uninterpretable, so the required Goose marker is a no-op.
SELECT 1;
