...
ALTER TABLE ONLY public.network_flow_tables
    ADD CONSTRAINT network_flow_tables_incident_id_fkey FOREIGN KEY (incident_id) REFERENCES public.incidents(id) ON DELETE CASCADE,
    ADD CONSTRAINT network_flow_tables_created_by_user_id_fkey FOREIGN KEY (created_by_user_id) REFERENCES public.users(id),
    ADD CONSTRAINT network_flow_tables_source_import_session_id_fkey FOREIGN KEY (source_import_session_id) REFERENCES public.import_sessions(import_session_id),
    ADD CONSTRAINT network_flow_tables_source_import_unit_id_fkey FOREIGN KEY (source_import_unit_id) REFERENCES public.import_units(import_unit_id),
...
ALTER TABLE ONLY public.network_flow_rows
    ADD CONSTRAINT network_flow_rows_table_id_fkey FOREIGN KEY (network_flow_table_id) REFERENCES public.network_flow_tables(network_flow_table_id) ON DELETE CASCADE,
    ADD CONSTRAINT network_flow_rows_incident_id_fkey FOREIGN KEY (incident_id) REFERENCES public.incidents(id) ON DELETE CASCADE,
    ADD CONSTRAINT network_flow_rows_created_by_user_id_fkey FOREIGN KEY (created_by_user_id) REFERENCES public.users(id),
    ADD CONSTRAINT network_flow_rows_table_source_row_key UNIQUE (network_flow_table_id, source_row_number);

CREATE INDEX network_flow_rows_table_order_idx
    ON public.network_flow_rows USING btree (network_flow_table_id, source_row_number, network_flow_row_id);
...
ALTER TABLE ONLY public.network_flow_rejected_row_diagnostics
    ADD CONSTRAINT network_flow_rejected_row_diagnostics_table_id_fkey FOREIGN KEY (network_flow_table_id) REFERENCES public.network_flow_tables(network_flow_table_id) ON DELETE CASCADE,
    ADD CONSTRAINT network_flow_rejected_row_diagnostics_incident_id_fkey FOREIGN KEY (incident_id) REFERENCES public.incidents(id) ON DELETE CASCADE;

CREATE INDEX network_flow_rejected_row_diagnostics_table_order_idx
    ON public.network_flow_rejected_row_diagnostics USING btree (network_flow_table_id, source_row_number, source_column_ordinal, field_key, error_code, diagnostic_id);
...
-- +goose Down
DROP TRIGGER IF EXISTS network_flow_rejected_row_diagnostics_immutable_update ON public.network_flow_rejected_row_diagnostics;
DROP TRIGGER IF EXISTS network_flow_rows_immutable_update ON public.network_flow_rows;
DROP FUNCTION IF EXISTS public.network_flow_reject_immutable_update();

DROP INDEX IF EXISTS public.network_flow_rejected_row_diagnostics_table_order_idx;
DROP TABLE IF EXISTS public.network_flow_rejected_row_diagnostics;

DROP INDEX IF EXISTS public.network_flow_rows_table_order_idx;
DROP TABLE IF EXISTS public.network_flow_rows;

DROP INDEX IF EXISTS public.network_flow_tables_retained_count_idx;
DROP INDEX IF EXISTS public.network_flow_tables_active_order_idx;
DROP INDEX IF EXISTS public.network_flow_tables_active_display_name_uidx;
DROP TABLE IF EXISTS public.network_flow_tables;
