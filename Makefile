...
GO_TMP_DIR ?= $(CARTULARY_MACHINE_CACHE_DIR)/go/tmp
GO_RUN_ENV := GOTOOLCHAIN=$(GO_TOOLCHAIN) GOCACHE=$(GO_CACHE_DIR) GOMODCACHE=$(GO_MOD_CACHE_DIR) GOTMPDIR=$(GO_TMP_DIR)
...
BROWSER_E2E_OWNED_STACK_ENV := PATH="$(NODE_RUNTIME_DIR)/bin:$$PATH" NODE_RUNTIME_DIR=$(NODE_RUNTIME_DIR) NODE_BIN=$(NODE_BIN) PNPM=$(PNPM) CARTULARY_SERVER_HARNESS_BIN=$(SERVER_HARNESS_BIN) CARTULARY_MIGRATE_BIN=$(MIGRATE_BIN) CARTULARY_TEST_SERVICES_BIN=$(TEST_SERVICES_BIN)
Q := @
comma := ,
RUN_STEP_SCRIPT := $(CURDIR)/tools/harness/execution/run-step.sh
...
CACHE_ARTIFACT_SCRIPT := $(CURDIR)/tools/harness/readiness/cache-artifact.sh
LIST_BUILD_INPUTS_IMPL := tools/harness/readiness/list-build-inputs.sh
BOOTSTRAP_NODE_RUNTIME_IMPL := tools/harness/readiness/bootstrap-node-runtime.sh
BOOTSTRAP_GO_TOOL_IMPL := tools/harness/readiness/bootstrap-go-tool.sh
GO_TOOLCHAIN_READINESS_IMPL := tools/harness/readiness/go-toolchain-readiness.sh
BOOTSTRAP_SHELLCHECK_IMPL := tools/harness/readiness/bootstrap-shellcheck.sh
FRONTEND_TOOLCHAIN_IMPL := tools/harness/readiness/frontend-toolchain.sh
FRONTEND_INSTALL_IMPL := tools/harness/readiness/frontend-install.sh
...
BUILD_WEB_ARTIFACT_IMPL := tools/harness/readiness/build-web-artifact.sh
...
export CARTULARY_OUTPUT_MODE VERBOSE CI_VERBOSE CI CARTULARY_TEST_RESULTS_DIR CARTULARY_TEST_RUN_ID CARTULARY_TEST_INVENTORY CARTULARY_MACHINE_CACHE_DIR GO_CACHE_DIR GO_MOD_CACHE_DIR GO_TMP_DIR GOTOOLCHAIN GO_TOOLCHAIN GO_TOOLCHAIN_READY
...
$(NODE_BIN): FORCE tools/harness/readiness/bootstrap-node-runtime.sh $(BOOTSTRAP_NODE_RUNTIME_IMPL) Makefile
	$(Q)NODE_VERSION="$(NODE_VERSION)" NODE_RUNTIME_DIR="$(NODE_RUNTIME_DIR)" ./tools/harness/readiness/bootstrap-node-runtime.sh
...
$(FRONTEND_INSTALL_STAMP): FORCE $(FRONTEND_INSTALL_INPUTS) $(FRONTEND_TOOLCHAIN_STAMP) tools/harness/readiness/frontend-install.sh $(FRONTEND_INSTALL_IMPL) $(CACHE_ARTIFACT_SCRIPT)
	$(Q)CARTULARY_TEST_TARGET="frontend-install" $(CACHE_ARTIFACT_SCRIPT) --scope readiness --profile frontend-install --cache-dir "$(CARTULARY_READINESS_CACHE_DIR)" --disable-env CARTULARY_READINESS_DISABLE_CACHE --force-env CARTULARY_FORCE_REINSTALL $(foreach input,$(FRONTEND_INSTALL_INPUTS),--input "$(input)") --input .npmrc --input tools/harness/readiness/frontend-install.sh --input $(FRONTEND_INSTALL_IMPL) --input tools/harness/readiness/cache-artifact.sh --output "$(FRONTEND_INSTALL_STAMP)" --key "node_version=$(NODE_VERSION)" --key "pnpm_version=$(PNPM_VERSION)" --key "pnpm_install_flags=$(PNPM_INSTALL_FLAGS)" -- env PATH="$(NODE_RUNTIME_DIR)/bin:$$PATH" COREPACK_HOME="$(NODE_RUNTIME_DIR)/corepack" CI=true FRONTEND_INSTALL_STAMP="$(FRONTEND_INSTALL_STAMP)" PNPM="$(PNPM)" PNPM_INSTALL_FLAGS="$(PNPM_INSTALL_FLAGS)" NODE_BIN="$(NODE_BIN)" NODE_VERSION="$(NODE_VERSION)" PNPM_VERSION="$(PNPM_VERSION)" bash ./tools/harness/readiness/frontend-install.sh
...
$(GOOSE_BIN): FORCE go-toolchain-readiness Makefile tools/harness/readiness/bootstrap-go-tool.sh $(BOOTSTRAP_GO_TOOL_IMPL) $(GO_TOOLCHAIN_READINESS_IMPL) $(CACHE_ARTIFACT_SCRIPT)
...
$(SYFT_BIN): FORCE go-toolchain-readiness Makefile tools/harness/readiness/bootstrap-go-tool.sh $(BOOTSTRAP_GO_TOOL_IMPL) $(GO_TOOLCHAIN_READINESS_IMPL) $(CACHE_ARTIFACT_SCRIPT)
	$(Q)$(CACHE_ARTIFACT_SCRIPT) --scope readiness --profile go-tool-syft --cache-dir "$(CARTULARY_READINESS_CACHE_DIR)" --disable-env CARTULARY_READINESS_DISABLE_CACHE --force-env CARTULARY_FORCE_REINSTALL --input Makefile --input tools/harness/readiness/bootstrap-go-tool.sh --input $(BOOTSTRAP_GO_TOOL_IMPL) --input tools/harness/readiness/cache-artifact.sh --input "$(GO)" --output "$(SYFT_BIN)" --key "tool=$(SYFT_TOOL)" --key "binary=syft" -- env GO="$(GO)" TOOLBIN_DIR="$(TOOLBIN_DIR)" TOOL_OUTPUT="$(SYFT_BIN)" TOOL_MODULE="$(SYFT_TOOL)" TOOL_BINARY_NAME="syft" TOOL_LABEL="bootstrap syft tool" GO_CACHE_DIR="$(GO_CACHE_DIR)" GO_MOD_CACHE_DIR="$(GO_MOD_CACHE_DIR)" GO_TMP_DIR="$(GO_TMP_DIR)" RUN_STEP_SCRIPT="$(RUN_STEP_SCRIPT)" ./tools/harness/readiness/bootstrap-go-tool.sh

$(SHELLCHECK_BIN): FORCE tools/harness/readiness/bootstrap-shellcheck.sh $(BOOTSTRAP_SHELLCHECK_IMPL) Makefile $(CACHE_ARTIFACT_SCRIPT)
	$(Q)$(CACHE_ARTIFACT_SCRIPT) --scope readiness --profile shellcheck-tool --cache-dir "$(CARTULARY_READINESS_CACHE_DIR)" --disable-env CARTULARY_READINESS_DISABLE_CACHE --force-env CARTULARY_FORCE_REINSTALL --input Makefile --input tools/harness/readiness/bootstrap-shellcheck.sh --input $(BOOTSTRAP_SHELLCHECK_IMPL) --input tools/harness/readiness/cache-artifact.sh --output "$(SHELLCHECK_BIN)" --key "shellcheck_version=$(SHELLCHECK_VERSION)" -- env SHELLCHECK_VERSION="$(SHELLCHECK_VERSION)" TOOLBIN_DIR="$(TOOLBIN_DIR)" SHELLCHECK_BIN="$(SHELLCHECK_BIN)" CARTULARY_SHELLCHECK_ARCHIVE_DIR="$(SHELLCHECK_ARCHIVE_DIR)" ./tools/harness/readiness/bootstrap-shellcheck.sh

$(TEST_SERVICES_BIN): FORCE go-toolchain-readiness $$(TEST_SERVICES_BUILD_INPUTS) tools/harness/backend/build-go-artifact.sh $(BUILD_GO_ARTIFACT_IMPL) $(CACHE_ARTIFACT_SCRIPT)
	$(Q)$(CACHE_ARTIFACT_SCRIPT) --scope build-artifact --profile testservices-build --key "go_toolchain=$(GO_TOOLCHAIN)" --cache-dir "$(CARTULARY_BUILD_CACHE_DIR)" --disable-env CARTULARY_BUILD_CACHE_DISABLE --force-env CARTULARY_FORCE_REBUILD $(foreach input,$(TEST_SERVICES_BUILD_INPUTS),--input "$(input)") --input tools/harness/readiness/list-build-inputs.sh --input $(LIST_BUILD_INPUTS_IMPL) --input tools/harness/backend/build-go-artifact.sh --input $(BUILD_GO_ARTIFACT_IMPL) --input tools/harness/readiness/cache-artifact.sh --input "$(GO)" --output "$(TEST_SERVICES_BIN)" --key "go=$${GO:-$(GO)}" --key "GOOS=$${GOOS:-}" --key "GOARCH=$${GOARCH:-}" --key "CGO_ENABLED=$${CGO_ENABLED:-}" --key "GOFLAGS=$${GOFLAGS:-}" -- env GO="$(GO)" BUILD_OUTPUT="$(TEST_SERVICES_BIN)" BUILD_PACKAGE="./tools/testservices" BUILD_LABEL="build testservices" CARTULARY_TEST_TARGET="testservices-build" GO_CACHE_DIR="$(GO_CACHE_DIR)" GO_MOD_CACHE_DIR="$(GO_MOD_CACHE_DIR)" GO_TMP_DIR="$(GO_TMP_DIR)" RUN_STEP_SCRIPT="$(RUN_STEP_SCRIPT)" ./tools/harness/backend/build-go-artifact.sh

$(PLAYWRIGHT_INSTALL_STAMP): FORCE $(FRONTEND_INSTALL_STAMP) $(FRONTEND_TOOLCHAIN_STAMP) tools/harness/browser/playwright-install.sh $(PLAYWRIGHT_INSTALL_IMPL) $(CACHE_ARTIFACT_SCRIPT)
	$(Q)$(CACHE_ARTIFACT_SCRIPT) --scope readiness --profile playwright-install --cache-dir "$(CARTULARY_READINESS_CACHE_DIR)" --disable-env CARTULARY_READINESS_DISABLE_CACHE --force-env CARTULARY_FORCE_REINSTALL --input package.json --input pnpm-lock.yaml --input apps/web/package.json --input tools/harness/browser/playwright-install.sh --input $(PLAYWRIGHT_INSTALL_IMPL) --input tools/harness/readiness/cache-artifact.sh --output "$(PLAYWRIGHT_INSTALL_STAMP)" --key "node_version=$(NODE_VERSION)" --key "pnpm_version=$(PNPM_VERSION)" -- env PLAYWRIGHT_INSTALL_STAMP="$(PLAYWRIGHT_INSTALL_STAMP)" RUN_STEP_SCRIPT="$(RUN_STEP_SCRIPT)" NODE_RUNTIME_DIR="$(NODE_RUNTIME_DIR)" NODE_BIN="$(NODE_BIN)" PNPM="$(PNPM)" NODE_VERSION="$(NODE_VERSION)" PNPM_VERSION="$(PNPM_VERSION)" ./tools/harness/browser/playwright-install.sh

$(EMBEDDED_WEB_ASSET_STAMP) $(EMBEDDED_WEB_ASSET_ARCHIVE) $(EMBEDDED_CLIENT_ASSET_MANIFEST) $(EMBEDDED_CLIENT_SUPPORT_REGISTRY) $(EMBEDDED_WEB_ASSET_READY_STAMP) &: FORCE go-toolchain-readiness $(WEB_DIST_INDEX) $(EXTENSION_CLIENT_SUPPORT_SOURCE) tools/harness/readiness/embed-web-assets.sh tools/embedwebassets/main.go go.mod $(CACHE_ARTIFACT_SCRIPT)
	$(Q)$(CACHE_ARTIFACT_SCRIPT) --scope build-artifact --profile embedded-web-assets --key "go_toolchain=$(GO_TOOLCHAIN)" --cache-dir "$(CARTULARY_BUILD_CACHE_DIR)" --disable-env CARTULARY_BUILD_CACHE_DISABLE --force-env CARTULARY_FORCE_REBUILD --input-dir "$(CURDIR)/apps/web/dist" --input "$(EXTENSION_CLIENT_SUPPORT_SOURCE)" --input tools/harness/readiness/embed-web-assets.sh --input tools/embedwebassets/main.go --input go.mod --input tools/harness/readiness/cache-artifact.sh --input "$(GO)" --output "$(EMBEDDED_WEB_ASSET_STAMP)" --output "$(EMBEDDED_WEB_ASSET_ARCHIVE)" --output "$(EMBEDDED_CLIENT_ASSET_MANIFEST)" --output "$(EMBEDDED_CLIENT_SUPPORT_REGISTRY)" --output "$(EMBEDDED_WEB_ASSET_READY_STAMP)" --output-dir "$(EMBEDDED_WEB_ASSET_DIR)" --key "source=$(WEB_DIST_INDEX)" --key "go=$${GO:-$(GO)}" -- env GO="$(GO)" GO_CACHE_DIR="$(GO_CACHE_DIR)" GO_MOD_CACHE_DIR="$(GO_MOD_CACHE_DIR)" GO_TMP_DIR="$(GO_TMP_DIR)" WEB_DIST_INDEX="$(WEB_DIST_INDEX)" EMBEDDED_WEB_ASSET_DIR="$(EMBEDDED_WEB_ASSET_DIR)" EMBEDDED_WEB_ASSET_ARCHIVE="$(EMBEDDED_WEB_ASSET_ARCHIVE)" EMBEDDED_CLIENT_ASSET_MANIFEST="$(EMBEDDED_CLIENT_ASSET_MANIFEST)" EMBEDDED_CLIENT_SUPPORT_REGISTRY="$(EMBEDDED_CLIENT_SUPPORT_REGISTRY)" EXTENSION_CLIENT_SUPPORT_SOURCE="$(EXTENSION_CLIENT_SUPPORT_SOURCE)" EMBEDDED_WEB_ASSET_STAMP="$(EMBEDDED_WEB_ASSET_STAMP)" EMBEDDED_WEB_ASSET_READY_STAMP="$(EMBEDDED_WEB_ASSET_READY_STAMP)" ./tools/harness/readiness/embed-web-assets.sh

$(SERVER_BIN): FORCE go-toolchain-readiness $$(SERVER_BUILD_INPUTS) $(EMBEDDED_WEB_ASSET_STAMP) $(EMBEDDED_WEB_ASSET_ARCHIVE) $(EMBEDDED_WEB_ASSET_READY_STAMP) tools/harness/backend/build-go-artifact.sh $(BUILD_GO_ARTIFACT_IMPL) $(CACHE_ARTIFACT_SCRIPT)
	$(Q)CARTULARY_TEST_TARGET="build-server" $(CACHE_ARTIFACT_SCRIPT) --scope build-artifact --profile build-server --cache-dir "$(CARTULARY_BUILD_CACHE_DIR)" --disable-env CARTULARY_BUILD_CACHE_DISABLE --force-env CARTULARY_FORCE_REBUILD $(foreach input,$(SERVER_BUILD_INPUTS),--input "$(input)") --input-dir "$(EMBEDDED_WEB_ASSET_DIR)" --input tools/harness/readiness/list-build-inputs.sh --input $(LIST_BUILD_INPUTS_IMPL) --input tools/harness/backend/build-go-artifact.sh --input $(BUILD_GO_ARTIFACT_IMPL) --input tools/harness/readiness/cache-artifact.sh --input "$(GO)" --output "$(SERVER_BIN)" --key "go=$${GO:-$(GO)}" --key "GOOS=$${GOOS:-}" --key "GOARCH=$${GOARCH:-}" --key "CGO_ENABLED=$${CGO_ENABLED:-}" --key "GOFLAGS=$${GOFLAGS:-}" -- env GO="$(GO)" BUILD_OUTPUT="$(SERVER_BIN)" BUILD_PACKAGE="./cmd/server" BUILD_LABEL="build server" CARTULARY_TEST_TARGET="build-server" GO_CACHE_DIR="$(GO_CACHE_DIR)" GO_MOD_CACHE_DIR="$(GO_MOD_CACHE_DIR)" GO_TMP_DIR="$(GO_TMP_DIR)" RUN_STEP_SCRIPT="$(RUN_STEP_SCRIPT)" ./tools/harness/backend/build-go-artifact.sh

$(SERVER_HARNESS_BIN): FORCE go-toolchain-readiness $$(SERVER_BUILD_INPUTS) $(EMBEDDED_WEB_ASSET_STAMP) $(EMBEDDED_WEB_ASSET_ARCHIVE) $(EMBEDDED_WEB_ASSET_READY_STAMP) tools/harness/backend/build-go-artifact.sh $(BUILD_GO_ARTIFACT_IMPL) $(CACHE_ARTIFACT_SCRIPT)
	$(Q)CARTULARY_TEST_TARGET="build-server-harness" $(CACHE_ARTIFACT_SCRIPT) --scope build-artifact --profile build-server-harness --cache-dir "$(CARTULARY_BUILD_CACHE_DIR)" --disable-env CARTULARY_BUILD_CACHE_DISABLE --force-env CARTULARY_FORCE_REBUILD $(foreach input,$(SERVER_BUILD_INPUTS),--input "$(input)") --input-dir "$(EMBEDDED_WEB_ASSET_DIR)" --input tools/harness/readiness/list-build-inputs.sh --input $(LIST_BUILD_INPUTS_IMPL) --input tools/harness/backend/build-go-artifact.sh --input $(BUILD_GO_ARTIFACT_IMPL) --input tools/harness/readiness/cache-artifact.sh --input "$(GO)" --output "$(SERVER_HARNESS_BIN)" --key "go=$${GO:-$(GO)}" --key "GOOS=$${GOOS:-}" --key "GOARCH=$${GOARCH:-}" --key "CGO_ENABLED=$${CGO_ENABLED:-}" --key "build_tags=cartulary_harness" -- env GO="$(GO)" GO_BUILD_TAGS="cartulary_harness" BUILD_OUTPUT="$(SERVER_HARNESS_BIN)" BUILD_PACKAGE="./cmd/server" BUILD_LABEL="build server harness profile" CARTULARY_TEST_TARGET="build-server-harness" GO_CACHE_DIR="$(GO_CACHE_DIR)" GO_MOD_CACHE_DIR="$(GO_MOD_CACHE_DIR)" GO_TMP_DIR="$(GO_TMP_DIR)" RUN_STEP_SCRIPT="$(RUN_STEP_SCRIPT)" ./tools/harness/backend/build-go-artifact.sh

$(MIGRATE_BIN): FORCE go-toolchain-readiness $$(MIGRATE_BUILD_INPUTS) tools/harness/backend/build-go-artifact.sh $(BUILD_GO_ARTIFACT_IMPL) $(CACHE_ARTIFACT_SCRIPT)
	$(Q)CARTULARY_TEST_TARGET="build-migrate" $(CACHE_ARTIFACT_SCRIPT) --scope build-artifact --profile build-migrate --cache-dir "$(CARTULARY_BUILD_CACHE_DIR)" --disable-env CARTULARY_BUILD_CACHE_DISABLE --force-env CARTULARY_FORCE_REBUILD $(foreach input,$(MIGRATE_BUILD_INPUTS),--input "$(input)") --input tools/harness/readiness/list-build-inputs.sh --input $(LIST_BUILD_INPUTS_IMPL) --input tools/harness/backend/build-go-artifact.sh --input $(BUILD_GO_ARTIFACT_IMPL) --input tools/harness/readiness/cache-artifact.sh --input "$(GO)" --output "$(MIGRATE_BIN)" --key "go=$${GO:-$(GO)}" --key "GOOS=$${GOOS:-}" --key "GOARCH=$${GOARCH:-}" --key "CGO_ENABLED=$${CGO_ENABLED:-}" --key "GOFLAGS=$${GOFLAGS:-}" -- env GO="$(GO)" BUILD_OUTPUT="$(MIGRATE_BIN)" BUILD_PACKAGE="./cmd/migrate" BUILD_LABEL="build migrate" CARTULARY_TEST_TARGET="build-migrate" GO_CACHE_DIR="$(GO_CACHE_DIR)" GO_MOD_CACHE_DIR="$(GO_MOD_CACHE_DIR)" GO_TMP_DIR="$(GO_TMP_DIR)" RUN_STEP_SCRIPT="$(RUN_STEP_SCRIPT)" ./tools/harness/backend/build-go-artifact.sh

$(OPERATOR_BIN): FORCE go-toolchain-readiness $$(OPERATOR_BUILD_INPUTS) $(EMBEDDED_WEB_ASSET_STAMP) $(EMBEDDED_WEB_ASSET_ARCHIVE) $(EMBEDDED_WEB_ASSET_READY_STAMP) tools/harness/backend/build-go-artifact.sh $(BUILD_GO_ARTIFACT_IMPL) $(CACHE_ARTIFACT_SCRIPT)
	$(Q)CARTULARY_TEST_TARGET="build-operator" $(CACHE_ARTIFACT_SCRIPT) --scope build-artifact --profile build-operator --cache-dir "$(CARTULARY_BUILD_CACHE_DIR)" --disable-env CARTULARY_BUILD_CACHE_DISABLE --force-env CARTULARY_FORCE_REBUILD $(foreach input,$(OPERATOR_BUILD_INPUTS),--input "$(input)") --input-dir "$(EMBEDDED_WEB_ASSET_DIR)" --input tools/harness/readiness/list-build-inputs.sh --input $(LIST_BUILD_INPUTS_IMPL) --input tools/harness/backend/build-go-artifact.sh --input $(BUILD_GO_ARTIFACT_IMPL) --input tools/harness/readiness/cache-artifact.sh --input "$(GO)" --output "$(OPERATOR_BIN)" --key "go=$${GO:-$(GO)}" --key "GOOS=$${GOOS:-}" --key "GOARCH=$${GOARCH:-}" --key "CGO_ENABLED=$${CGO_ENABLED:-}" --key "GOFLAGS=$${GOFLAGS:-}" -- env GO="$(GO)" BUILD_OUTPUT="$(OPERATOR_BIN)" BUILD_PACKAGE="./cmd/operator" BUILD_LABEL="build operator" CARTULARY_TEST_TARGET="build-operator" GO_CACHE_DIR="$(GO_CACHE_DIR)" GO_MOD_CACHE_DIR="$(GO_MOD_CACHE_DIR)" GO_TMP_DIR="$(GO_TMP_DIR)" RUN_STEP_SCRIPT="$(RUN_STEP_SCRIPT)" ./tools/harness/backend/build-go-artifact.sh

$(WEB_DIST_INDEX): FORCE $$(WEB_BUILD_INPUTS) $(FRONTEND_INSTALL_STAMP) tools/harness/readiness/build-web-artifact.sh $(BUILD_WEB_ARTIFACT_IMPL) $(CACHE_ARTIFACT_SCRIPT) | $(NODE_BIN)
	$(Q)CARTULARY_TEST_TARGET="build-web" $(CACHE_ARTIFACT_SCRIPT) --scope build-artifact --profile build-web --cache-dir "$(CARTULARY_BUILD_CACHE_DIR)" --disable-env CARTULARY_BUILD_CACHE_DISABLE --force-env CARTULARY_FORCE_REBUILD $(foreach input,$(WEB_BUILD_INPUTS),--input "$(input)") --input tools/harness/readiness/list-build-inputs.sh --input $(LIST_BUILD_INPUTS_IMPL) --input tools/harness/readiness/build-web-artifact.sh --input $(BUILD_WEB_ARTIFACT_IMPL) --input tools/harness/readiness/cache-artifact.sh --output-dir "$(CURDIR)/apps/web/dist" --key "node_version=$(NODE_VERSION)" --key "pnpm_version=$(PNPM_VERSION)" --key "vite_flags=$(VITE_BUILD_FLAGS)" -- env RUN_STEP_SCRIPT="$(RUN_STEP_SCRIPT)" NODE_RUNTIME_DIR="$(NODE_RUNTIME_DIR)" PNPM="$(PNPM)" VITE_BUILD_FLAGS="$(VITE_BUILD_FLAGS)" CARTULARY_TEST_TARGET="build-web" ./tools/harness/readiness/build-web-artifact.sh
...
$(CURDIR)/apps/web/dist-measurement/index.html: FORCE $$(WEB_BUILD_INPUTS) $(FRONTEND_INSTALL_STAMP) tools/harness/readiness/build-web-artifact.sh $(BUILD_WEB_ARTIFACT_IMPL) $(CACHE_ARTIFACT_SCRIPT) | $(NODE_BIN)
	$(Q)CARTULARY_TEST_TARGET="build-web-measurement" $(CACHE_ARTIFACT_SCRIPT) --scope build-artifact --profile build-web-measurement --cache-dir "$(CARTULARY_BUILD_CACHE_DIR)" --disable-env CARTULARY_BUILD_CACHE_DISABLE --force-env CARTULARY_FORCE_REBUILD $(foreach input,$(WEB_BUILD_INPUTS),--input "$(input)") --input tools/harness/readiness/list-build-inputs.sh --input $(LIST_BUILD_INPUTS_IMPL) --input tools/harness/readiness/build-web-artifact.sh --input $(BUILD_WEB_ARTIFACT_IMPL) --input tools/harness/readiness/cache-artifact.sh --output-dir "$(CURDIR)/apps/web/dist-measurement" --key "node_version=$(NODE_VERSION)" --key "pnpm_version=$(PNPM_VERSION)" --key "vite_flags=$(VITE_BUILD_FLAGS) --mode measurement --outDir dist-measurement" -- env RUN_STEP_SCRIPT="$(RUN_STEP_SCRIPT)" NODE_RUNTIME_DIR="$(NODE_RUNTIME_DIR)" PNPM="$(PNPM)" VITE_BUILD_FLAGS="$(VITE_BUILD_FLAGS) --mode measurement --outDir dist-measurement" CARTULARY_TEST_TARGET="build-web-measurement" ./tools/harness/readiness/build-web-artifact.sh
