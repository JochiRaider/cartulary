...
# Graph producers consume completed predecessor outputs. Standalone file bindings
# retain their Make prerequisites; public build targets enter the canonical graph.
ifeq ($(CARTULARY_HARNESS_GRAPH_CHILD),1)
FRONTEND_BUILD_PREREQUISITE :=
EMBEDDED_BUILD_PREREQUISITES :=
else
FRONTEND_BUILD_PREREQUISITE := $(WEB_DIST_INDEX)
EMBEDDED_BUILD_PREREQUISITES := $(EMBEDDED_WEB_ASSET_STAMP) $(EMBEDDED_WEB_ASSET_ARCHIVE) $(EMBEDDED_WEB_ASSET_READY_STAMP)
endif

$(EMBEDDED_WEB_ASSET_STAMP) $(EMBEDDED_WEB_ASSET_ARCHIVE) $(EMBEDDED_CLIENT_ASSET_MANIFEST) $(EMBEDDED_CLIENT_SUPPORT_REGISTRY) $(EMBEDDED_WEB_ASSET_READY_STAMP) &: FORCE go-toolchain-readiness $(FRONTEND_BUILD_PREREQUISITE) $(EXTENSION_CLIENT_SUPPORT_SOURCE) tools/harness/readiness/embed-web-assets.sh tools/embedwebassets/main.go go.mod $(CACHE_ARTIFACT_SCRIPT)
	$(Q)source_dir="$(CURDIR)/apps/web/dist"; if [ -n "$${CARTULARY_HARNESS_SUITE_RUNTIME_ROOT:-}" ]; then source_dir="$$("$(NODE_BIN)" tools/harness/readiness/frontend-artifact.mjs resolve build-web)" || exit $$?; fi; $(CACHE_ARTIFACT_SCRIPT) --scope build-artifact --profile embedded-web-assets --key "go_toolchain=$(GO_TOOLCHAIN)" --cache-dir "$(CARTULARY_BUILD_CACHE_DIR)" --disable-env CARTULARY_BUILD_CACHE_DISABLE --force-env CARTULARY_FORCE_REBUILD --input-dir "$$source_dir" --input "$(EXTENSION_CLIENT_SUPPORT_SOURCE)" --input tools/harness/readiness/embed-web-assets.sh --input tools/embedwebassets/main.go --input go.mod --input tools/harness/readiness/cache-artifact.sh --input "$(GO)" --output "$(EMBEDDED_WEB_ASSET_STAMP)" --output "$(EMBEDDED_WEB_ASSET_ARCHIVE)" --output "$(EMBEDDED_CLIENT_ASSET_MANIFEST)" --output "$(EMBEDDED_CLIENT_SUPPORT_REGISTRY)" --output "$(EMBEDDED_WEB_ASSET_READY_STAMP)" --output-dir "$(EMBEDDED_WEB_ASSET_DIR)" --key "source=$(WEB_DIST_INDEX)" --key "go=$${GO:-$(GO)}" -- env GO="$(GO)" GO_CACHE_DIR="$(GO_CACHE_DIR)" GO_MOD_CACHE_DIR="$(GO_MOD_CACHE_DIR)" GO_TMP_DIR="$(GO_TMP_DIR)" WEB_DIST_INDEX="$$source_dir/index.html" EMBEDDED_WEB_ASSET_DIR="$(EMBEDDED_WEB_ASSET_DIR)" EMBEDDED_WEB_ASSET_ARCHIVE="$(EMBEDDED_WEB_ASSET_ARCHIVE)" EMBEDDED_CLIENT_ASSET_MANIFEST="$(EMBEDDED_CLIENT_ASSET_MANIFEST)" EMBEDDED_CLIENT_SUPPORT_REGISTRY="$(EMBEDDED_CLIENT_SUPPORT_REGISTRY)" EXTENSION_CLIENT_SUPPORT_SOURCE="$(EXTENSION_CLIENT_SUPPORT_SOURCE)" EMBEDDED_WEB_ASSET_STAMP="$(EMBEDDED_WEB_ASSET_STAMP)" EMBEDDED_WEB_ASSET_READY_STAMP="$(EMBEDDED_WEB_ASSET_READY_STAMP)" ./tools/harness/readiness/embed-web-assets.sh

$(SERVER_BIN): FORCE go-toolchain-readiness $$(SERVER_BUILD_INPUTS) $(EMBEDDED_BUILD_PREREQUISITES) tools/harness/backend/build-go-artifact.sh $(BUILD_GO_ARTIFACT_IMPL) $(CACHE_ARTIFACT_SCRIPT)
...
$(SERVER_HARNESS_BIN): FORCE go-toolchain-readiness $$(SERVER_BUILD_INPUTS) $(EMBEDDED_BUILD_PREREQUISITES) tools/harness/backend/build-go-artifact.sh $(BUILD_GO_ARTIFACT_IMPL) $(CACHE_ARTIFACT_SCRIPT)
...
$(OPERATOR_BIN): FORCE go-toolchain-readiness $$(OPERATOR_BUILD_INPUTS) $(EMBEDDED_BUILD_PREREQUISITES) tools/harness/backend/build-go-artifact.sh $(BUILD_GO_ARTIFACT_IMPL) $(CACHE_ARTIFACT_SCRIPT)
...
$(WEB_DIST_INDEX): FORCE $$(WEB_BUILD_INPUTS) $(FRONTEND_INSTALL_STAMP) tools/harness/readiness/build-web-artifact.sh $(BUILD_WEB_ARTIFACT_IMPL) $(CACHE_ARTIFACT_SCRIPT) | $(NODE_BIN)
	$(Q)env RUN_STEP_SCRIPT="$(RUN_STEP_SCRIPT)" NODE_RUNTIME_DIR="$(NODE_RUNTIME_DIR)" PNPM="$(PNPM)" VITE_BUILD_FLAGS="$(VITE_BUILD_FLAGS)" CARTULARY_TEST_TARGET="build-web" ./tools/harness/readiness/build-web-artifact.sh
