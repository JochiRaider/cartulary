...
FRONTEND_TOOLCHAIN_IMPL := tools/harness/frontend/frontend-toolchain.sh
FRONTEND_INSTALL_IMPL := tools/harness/frontend/frontend-install.sh
...
$(EMBEDDED_WEB_ASSET_STAMP) $(EMBEDDED_WEB_ASSET_ARCHIVE) $(EMBEDDED_WEB_ASSET_READY_STAMP) &: FORCE $(WEB_DIST_INDEX) tools/harness/frontend/embed-web-assets.sh tools/embedwebassets/main.go go.mod $(CACHE_ARTIFACT_SCRIPT)
	$(Q)$(CACHE_ARTIFACT_SCRIPT) --schema-id cartulary.cache.build_artifact.v1 --scope build-artifact --profile embedded-web-assets --cache-dir "$(CARTULARY_BUILD_CACHE_DIR)" --disable-env CARTULARY_BUILD_CACHE_DISABLE --force-env CARTULARY_FORCE_REBUILD --input-dir "$(CURDIR)/apps/web/dist" --input tools/harness/frontend/embed-web-assets.sh --input tools/embedwebassets/main.go --input go.mod --input tools/harness/readiness/cache-artifact.sh --input "$(GO)" --output "$(EMBEDDED_WEB_ASSET_STAMP)" --output "$(EMBEDDED_WEB_ASSET_ARCHIVE)" --output "$(EMBEDDED_WEB_ASSET_READY_STAMP)" --output-dir "$(EMBEDDED_WEB_ASSET_DIR)" --key "source=$(WEB_DIST_INDEX)" --key "go=$${GO:-$(GO)}" -- env GO="$(GO)" WEB_DIST_INDEX="$(WEB_DIST_INDEX)" EMBEDDED_WEB_ASSET_DIR="$(EMBEDDED_WEB_ASSET_DIR)" EMBEDDED_WEB_ASSET_ARCHIVE="$(EMBEDDED_WEB_ASSET_ARCHIVE)" EMBEDDED_WEB_ASSET_STAMP="$(EMBEDDED_WEB_ASSET_STAMP)" EMBEDDED_WEB_ASSET_READY_STAMP="$(EMBEDDED_WEB_ASSET_READY_STAMP)" ./tools/harness/frontend/embed-web-assets.sh
...
$(WEB_DIST_INDEX): FORCE $$(WEB_BUILD_INPUTS) $(FRONTEND_INSTALL_STAMP) tools/harness/frontend/build-web-artifact.sh $(BUILD_WEB_ARTIFACT_IMPL) $(CACHE_ARTIFACT_SCRIPT) | $(NODE_BIN)
	$(Q)CARTULARY_TEST_TARGET="build-web" $(CACHE_ARTIFACT_SCRIPT) --schema-id cartulary.cache.build_artifact.v1 --scope build-artifact --profile build-web --cache-dir "$(CARTULARY_BUILD_CACHE_DIR)" --disable-env CARTULARY_BUILD_CACHE_DISABLE --force-env CARTULARY_FORCE_REBUILD $(foreach input,$(WEB_BUILD_INPUTS),--input "$(input)") --input tools/harness/readiness/list-build-inputs.sh --input $(LIST_BUILD_INPUTS_IMPL) --input tools/harness/frontend/build-web-artifact.sh --input $(BUILD_WEB_ARTIFACT_IMPL) --input tools/harness/readiness/cache-artifact.sh --output-dir "$(CURDIR)/apps/web/dist" --key "node_version=$(NODE_VERSION)" --key "pnpm_version=$(PNPM_VERSION)" --key "vite_flags=$(VITE_BUILD_FLAGS)" -- env RUN_PHASE_SCRIPT="$(RUN_PHASE_SCRIPT)" NODE_RUNTIME_DIR="$(NODE_RUNTIME_DIR)" PNPM="$(PNPM)" VITE_BUILD_FLAGS="$(VITE_BUILD_FLAGS)" CARTULARY_TEST_TARGET="build-web" ./tools/harness/frontend/build-web-artifact.sh
