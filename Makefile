...
$(EMBEDDED_WEB_ASSET_STAMP) $(EMBEDDED_WEB_ASSET_ARCHIVE) $(EMBEDDED_WEB_ASSET_READY_STAMP) &: FORCE $(WEB_DIST_INDEX) tools/harness/readiness/embed-web-assets.sh tools/embedwebassets/main.go go.mod $(CACHE_ARTIFACT_SCRIPT)
	$(Q)$(CACHE_ARTIFACT_SCRIPT) --schema-id cartulary.cache.build_artifact.v1 --scope build-artifact --profile embedded-web-assets --cache-dir "$(CARTULARY_BUILD_CACHE_DIR)" --disable-env CARTULARY_BUILD_CACHE_DISABLE --force-env CARTULARY_FORCE_REBUILD --input-dir "$(CURDIR)/apps/web/dist" --input tools/harness/readiness/embed-web-assets.sh --input tools/embedwebassets/main.go --input go.mod --input tools/harness/readiness/cache-artifact.sh --input "$(GO)" --output "$(EMBEDDED_WEB_ASSET_STAMP)" --output "$(EMBEDDED_WEB_ASSET_ARCHIVE)" --output "$(EMBEDDED_WEB_ASSET_READY_STAMP)" --output-dir "$(EMBEDDED_WEB_ASSET_DIR)" --key "source=$(WEB_DIST_INDEX)" --key "go=$${GO:-$(GO)}" -- env GO="$(GO)" WEB_DIST_INDEX="$(WEB_DIST_INDEX)" EMBEDDED_WEB_ASSET_DIR="$(EMBEDDED_WEB_ASSET_DIR)" EMBEDDED_WEB_ASSET_ARCHIVE="$(EMBEDDED_WEB_ASSET_ARCHIVE)" EMBEDDED_WEB_ASSET_STAMP="$(EMBEDDED_WEB_ASSET_STAMP)" EMBEDDED_WEB_ASSET_READY_STAMP="$(EMBEDDED_WEB_ASSET_READY_STAMP)" ./tools/harness/readiness/embed-web-assets.sh
