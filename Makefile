...
$(NODE_BIN): FORCE scripts/bootstrap-node-runtime.sh $(BOOTSTRAP_NODE_RUNTIME_IMPL) Makefile
	$(Q)NODE_VERSION="$(NODE_VERSION)" NODE_RUNTIME_DIR="$(NODE_RUNTIME_DIR)" ./scripts/bootstrap-node-runtime.sh

$(FRONTEND_TOOLCHAIN_STAMP): FORCE $(NODE_BIN) Makefile scripts/frontend-toolchain.sh $(FRONTEND_TOOLCHAIN_IMPL) $(CACHE_ARTIFACT_SCRIPT)
	$(Q)$(CACHE_ARTIFACT_SCRIPT) --schema-id cartulary.cache.readiness.v1 --scope readiness --profile frontend-toolchain --cache-dir "$(CARTULARY_READINESS_CACHE_DIR)" --disable-env CARTULARY_READINESS_DISABLE_CACHE --force-env CARTULARY_FORCE_REINSTALL --input Makefile --input scripts/frontend-toolchain.sh --input $(FRONTEND_TOOLCHAIN_IMPL) --input scripts/cache-artifact.sh --output "$(FRONTEND_TOOLCHAIN_STAMP)" --key "node_version=$(NODE_VERSION)" --key "pnpm_version=$(PNPM_VERSION)" --key "node_bin=$(NODE_BIN)" --key "pnpm=$(PNPM)" -- env NODE_RUNTIME_DIR="$(NODE_RUNTIME_DIR)" NODE_BIN="$(NODE_BIN)" PNPM="$(PNPM)" NODE_VERSION="$(NODE_VERSION)" PNPM_VERSION="$(PNPM_VERSION)" FRONTEND_TOOLCHAIN_STAMP="$(FRONTEND_TOOLCHAIN_STAMP)" ./scripts/frontend-toolchain.sh

$(FRONTEND_INSTALL_STAMP): FORCE $(FRONTEND_INSTALL_INPUTS) $(FRONTEND_TOOLCHAIN_STAMP) scripts/frontend-install.sh $(FRONTEND_INSTALL_IMPL) $(CACHE_ARTIFACT_SCRIPT)
	$(Q)CARTULARY_TEST_TARGET="frontend-install" $(CACHE_ARTIFACT_SCRIPT) --schema-id cartulary.cache.readiness.v1 --scope readiness --profile frontend-install --cache-dir "$(CARTULARY_READINESS_CACHE_DIR)" --disable-env CARTULARY_READINESS_DISABLE_CACHE --force-env CARTULARY_FORCE_REINSTALL $(foreach input,$(FRONTEND_INSTALL_INPUTS),--input "$(input)") --input .npmrc --input scripts/frontend-install.sh --input $(FRONTEND_INSTALL_IMPL) --input scripts/cache-artifact.sh --output "$(FRONTEND_INSTALL_STAMP)" --key "node_version=$(NODE_VERSION)" --key "pnpm_version=$(PNPM_VERSION)" --key "pnpm_install_flags=$(PNPM_INSTALL_FLAGS)" -- env PATH="$(NODE_RUNTIME_DIR)/bin:$$PATH" COREPACK_HOME="$(NODE_RUNTIME_DIR)/corepack" CI=true FRONTEND_INSTALL_STAMP="$(FRONTEND_INSTALL_STAMP)" RUN_PHASE_SCRIPT="$(RUN_PHASE_SCRIPT)" PNPM="$(PNPM)" PNPM_INSTALL_FLAGS="$(PNPM_INSTALL_FLAGS)" NODE_BIN="$(NODE_BIN)" NODE_VERSION="$(NODE_VERSION)" PNPM_VERSION="$(PNPM_VERSION)" bash ./scripts/frontend-install.sh

$(SQLC_BIN): FORCE Makefile scripts/bootstrap-go-tool.sh $(BOOTSTRAP_GO_TOOL_IMPL) $(CACHE_ARTIFACT_SCRIPT)
	$(Q)$(CACHE_ARTIFACT_SCRIPT) --schema-id cartulary.cache.readiness.v1 --scope readiness --profile go-tool-sqlc --cache-dir "$(CARTULARY_READINESS_CACHE_DIR)" --disable-env CARTULARY_READINESS_DISABLE_CACHE --force-env CARTULARY_FORCE_REINSTALL --input Makefile --input scripts/bootstrap-go-tool.sh --input $(BOOTSTRAP_GO_TOOL_IMPL) --input scripts/cache-artifact.sh --input "$(GO)" --output "$(SQLC_BIN)" --key "tool=$(SQLC_TOOL)" --key "binary=sqlc" -- env GO="$(GO)" TOOLBIN_DIR="$(TOOLBIN_DIR)" TOOL_OUTPUT="$(SQLC_BIN)" TOOL_MODULE="$(SQLC_TOOL)" TOOL_BINARY_NAME="sqlc" TOOL_LABEL="bootstrap sqlc tool" GO_CACHE_DIR="$(GO_CACHE_DIR)" GO_MOD_CACHE_DIR="$(GO_MOD_CACHE_DIR)" RUN_PHASE_SCRIPT="$(RUN_PHASE_SCRIPT)" ./scripts/bootstrap-go-tool.sh

$(GOOSE_BIN): FORCE Makefile scripts/bootstrap-go-tool.sh $(BOOTSTRAP_GO_TOOL_IMPL) $(CACHE_ARTIFACT_SCRIPT)
	$(Q)$(CACHE_ARTIFACT_SCRIPT) --schema-id cartulary.cache.readiness.v1 --scope readiness --profile go-tool-goose --cache-dir "$(CARTULARY_READINESS_CACHE_DIR)" --disable-env CARTULARY_READINESS_DISABLE_CACHE --force-env CARTULARY_FORCE_REINSTALL --input Makefile --input scripts/bootstrap-go-tool.sh --input $(BOOTSTRAP_GO_TOOL_IMPL) --input scripts/cache-artifact.sh --input "$(GO)" --output "$(GOOSE_BIN)" --key "tool=$(GOOSE_TOOL)" --key "binary=goose" -- env GO="$(GO)" TOOLBIN_DIR="$(TOOLBIN_DIR)" TOOL_OUTPUT="$(GOOSE_BIN)" TOOL_MODULE="$(GOOSE_TOOL)" TOOL_BINARY_NAME="goose" TOOL_LABEL="bootstrap goose tool" GO_CACHE_DIR="$(GO_CACHE_DIR)" GO_MOD_CACHE_DIR="$(GO_MOD_CACHE_DIR)" RUN_PHASE_SCRIPT="$(RUN_PHASE_SCRIPT)" ./scripts/bootstrap-go-tool.sh

$(STATICCHECK_BIN): FORCE Makefile scripts/bootstrap-go-tool.sh $(BOOTSTRAP_GO_TOOL_IMPL) $(CACHE_ARTIFACT_SCRIPT)
	$(Q)$(CACHE_ARTIFACT_SCRIPT) --schema-id cartulary.cache.readiness.v1 --scope readiness --profile go-tool-staticcheck --cache-dir "$(CARTULARY_READINESS_CACHE_DIR)" --disable-env CARTULARY_READINESS_DISABLE_CACHE --force-env CARTULARY_FORCE_REINSTALL --input Makefile --input scripts/bootstrap-go-tool.sh --input $(BOOTSTRAP_GO_TOOL_IMPL) --input scripts/cache-artifact.sh --input "$(GO)" --output "$(STATICCHECK_BIN)" --key "tool=$(STATICCHECK_TOOL)" --key "binary=staticcheck" -- env GO="$(GO)" TOOLBIN_DIR="$(TOOLBIN_DIR)" TOOL_OUTPUT="$(STATICCHECK_BIN)" TOOL_MODULE="$(STATICCHECK_TOOL)" TOOL_BINARY_NAME="staticcheck" TOOL_LABEL="bootstrap staticcheck tool" GO_CACHE_DIR="$(GO_CACHE_DIR)" GO_MOD_CACHE_DIR="$(GO_MOD_CACHE_DIR)" RUN_PHASE_SCRIPT="$(RUN_PHASE_SCRIPT)" ./scripts/bootstrap-go-tool.sh

$(GOVULNCHECK_BIN): FORCE Makefile scripts/bootstrap-go-tool.sh $(BOOTSTRAP_GO_TOOL_IMPL) $(CACHE_ARTIFACT_SCRIPT)
	$(Q)$(CACHE_ARTIFACT_SCRIPT) --schema-id cartulary.cache.readiness.v1 --scope readiness --profile go-tool-govulncheck --cache-dir "$(CARTULARY_READINESS_CACHE_DIR)" --disable-env CARTULARY_READINESS_DISABLE_CACHE --force-env CARTULARY_FORCE_REINSTALL --input Makefile --input scripts/bootstrap-go-tool.sh --input $(BOOTSTRAP_GO_TOOL_IMPL) --input scripts/cache-artifact.sh --input "$(GO)" --output "$(GOVULNCHECK_BIN)" --key "tool=$(GOVULNCHECK_TOOL)" --key "binary=govulncheck" -- env GO="$(GO)" TOOLBIN_DIR="$(TOOLBIN_DIR)" TOOL_OUTPUT="$(GOVULNCHECK_BIN)" TOOL_MODULE="$(GOVULNCHECK_TOOL)" TOOL_BINARY_NAME="govulncheck" TOOL_LABEL="bootstrap govulncheck tool" GO_CACHE_DIR="$(GO_CACHE_DIR)" GO_MOD_CACHE_DIR="$(GO_MOD_CACHE_DIR)" RUN_PHASE_SCRIPT="$(RUN_PHASE_SCRIPT)" ./scripts/bootstrap-go-tool.sh

$(GOSEC_BIN): FORCE Makefile scripts/bootstrap-go-tool.sh $(BOOTSTRAP_GO_TOOL_IMPL) $(CACHE_ARTIFACT_SCRIPT)
	$(Q)$(CACHE_ARTIFACT_SCRIPT) --schema-id cartulary.cache.readiness.v1 --scope readiness --profile go-tool-gosec --cache-dir "$(CARTULARY_READINESS_CACHE_DIR)" --disable-env CARTULARY_READINESS_DISABLE_CACHE --force-env CARTULARY_FORCE_REINSTALL --input Makefile --input scripts/bootstrap-go-tool.sh --input $(BOOTSTRAP_GO_TOOL_IMPL) --input scripts/cache-artifact.sh --input "$(GO)" --output "$(GOSEC_BIN)" --key "tool=$(GOSEC_TOOL)" --key "binary=gosec" -- env GO="$(GO)" TOOLBIN_DIR="$(TOOLBIN_DIR)" TOOL_OUTPUT="$(GOSEC_BIN)" TOOL_MODULE="$(GOSEC_TOOL)" TOOL_BINARY_NAME="gosec" TOOL_LABEL="bootstrap gosec tool" GO_CACHE_DIR="$(GO_CACHE_DIR)" GO_MOD_CACHE_DIR="$(GO_MOD_CACHE_DIR)" RUN_PHASE_SCRIPT="$(RUN_PHASE_SCRIPT)" ./scripts/bootstrap-go-tool.sh

$(CYCLONEDX_GOMOD_BIN): FORCE Makefile scripts/bootstrap-go-tool.sh $(BOOTSTRAP_GO_TOOL_IMPL) $(CACHE_ARTIFACT_SCRIPT)
	$(Q)$(CACHE_ARTIFACT_SCRIPT) --schema-id cartulary.cache.readiness.v1 --scope readiness --profile go-tool-cyclonedx-gomod --cache-dir "$(CARTULARY_READINESS_CACHE_DIR)" --disable-env CARTULARY_READINESS_DISABLE_CACHE --force-env CARTULARY_FORCE_REINSTALL --input Makefile --input scripts/bootstrap-go-tool.sh --input $(BOOTSTRAP_GO_TOOL_IMPL) --input scripts/cache-artifact.sh --input "$(GO)" --output "$(CYCLONEDX_GOMOD_BIN)" --key "tool=$(CYCLONEDX_GOMOD_TOOL)" --key "binary=cyclonedx-gomod" -- env GO="$(GO)" TOOLBIN_DIR="$(TOOLBIN_DIR)" TOOL_OUTPUT="$(CYCLONEDX_GOMOD_BIN)" TOOL_MODULE="$(CYCLONEDX_GOMOD_TOOL)" TOOL_BINARY_NAME="cyclonedx-gomod" TOOL_LABEL="bootstrap cyclonedx-gomod tool" GO_CACHE_DIR="$(GO_CACHE_DIR)" GO_MOD_CACHE_DIR="$(GO_MOD_CACHE_DIR)" RUN_PHASE_SCRIPT="$(RUN_PHASE_SCRIPT)" ./scripts/bootstrap-go-tool.sh

$(SYFT_BIN): FORCE Makefile scripts/bootstrap-go-tool.sh $(BOOTSTRAP_GO_TOOL_IMPL) $(CACHE_ARTIFACT_SCRIPT)
	$(Q)$(CACHE_ARTIFACT_SCRIPT) --schema-id cartulary.cache.readiness.v1 --scope readiness --profile go-tool-syft --cache-dir "$(CARTULARY_READINESS_CACHE_DIR)" --disable-env CARTULARY_READINESS_DISABLE_CACHE --force-env CARTULARY_FORCE_REINSTALL --input Makefile --input scripts/bootstrap-go-tool.sh --input $(BOOTSTRAP_GO_TOOL_IMPL) --input scripts/cache-artifact.sh --input "$(GO)" --output "$(SYFT_BIN)" --key "tool=$(SYFT_TOOL)" --key "binary=syft" -- env GO="$(GO)" TOOLBIN_DIR="$(TOOLBIN_DIR)" TOOL_OUTPUT="$(SYFT_BIN)" TOOL_MODULE="$(SYFT_TOOL)" TOOL_BINARY_NAME="syft" TOOL_LABEL="bootstrap syft tool" GO_CACHE_DIR="$(GO_CACHE_DIR)" GO_MOD_CACHE_DIR="$(GO_MOD_CACHE_DIR)" RUN_PHASE_SCRIPT="$(RUN_PHASE_SCRIPT)" ./scripts/bootstrap-go-tool.sh

$(SHELLCHECK_BIN): FORCE scripts/bootstrap-shellcheck.sh $(BOOTSTRAP_SHELLCHECK_IMPL) Makefile $(CACHE_ARTIFACT_SCRIPT)
	$(Q)$(CACHE_ARTIFACT_SCRIPT) --schema-id cartulary.cache.readiness.v1 --scope readiness --profile shellcheck-tool --cache-dir "$(CARTULARY_READINESS_CACHE_DIR)" --disable-env CARTULARY_READINESS_DISABLE_CACHE --force-env CARTULARY_FORCE_REINSTALL --input Makefile --input scripts/bootstrap-shellcheck.sh --input $(BOOTSTRAP_SHELLCHECK_IMPL) --input scripts/cache-artifact.sh --output "$(SHELLCHECK_BIN)" --key "shellcheck_version=$(SHELLCHECK_VERSION)" -- env SHELLCHECK_VERSION="$(SHELLCHECK_VERSION)" TOOLBIN_DIR="$(TOOLBIN_DIR)" SHELLCHECK_BIN="$(SHELLCHECK_BIN)" CARTULARY_SHELLCHECK_ARCHIVE_DIR="$(SHELLCHECK_ARCHIVE_DIR)" ./scripts/bootstrap-shellcheck.sh

$(TEST_SERVICES_BIN): FORCE $$(TEST_SERVICES_BUILD_INPUTS) scripts/build-go-artifact.sh $(BUILD_GO_ARTIFACT_IMPL) $(CACHE_ARTIFACT_SCRIPT)
	$(Q)$(CACHE_ARTIFACT_SCRIPT) --schema-id cartulary.cache.build_artifact.v1 --scope build-artifact --profile testservices-build --cache-dir "$(CARTULARY_BUILD_CACHE_DIR)" --disable-env CARTULARY_BUILD_CACHE_DISABLE --force-env CARTULARY_FORCE_REBUILD $(foreach input,$(TEST_SERVICES_BUILD_INPUTS),--input "$(input)") --input scripts/list-build-inputs.sh --input $(LIST_BUILD_INPUTS_IMPL) --input scripts/build-go-artifact.sh --input $(BUILD_GO_ARTIFACT_IMPL) --input scripts/cache-artifact.sh --input "$(GO)" --output "$(TEST_SERVICES_BIN)" --key "go=$${GO:-$(GO)}" --key "GOOS=$${GOOS:-}" --key "GOARCH=$${GOARCH:-}" --key "CGO_ENABLED=$${CGO_ENABLED:-}" --key "GOFLAGS=$${GOFLAGS:-}" -- env GO="$(GO)" BUILD_OUTPUT="$(TEST_SERVICES_BIN)" BUILD_PACKAGE="./tools/testservices" BUILD_LABEL="build testservices" CARTULARY_TEST_TARGET="testservices-build" GO_CACHE_DIR="$(GO_CACHE_DIR)" GO_MOD_CACHE_DIR="$(GO_MOD_CACHE_DIR)" RUN_PHASE_SCRIPT="$(RUN_PHASE_SCRIPT)" ./scripts/build-go-artifact.sh

$(PLAYWRIGHT_INSTALL_STAMP): FORCE $(FRONTEND_INSTALL_STAMP) $(FRONTEND_TOOLCHAIN_STAMP) scripts/playwright-install.sh $(PLAYWRIGHT_INSTALL_IMPL) $(CACHE_ARTIFACT_SCRIPT)
	$(Q)$(CACHE_ARTIFACT_SCRIPT) --schema-id cartulary.cache.readiness.v1 --scope readiness --profile playwright-install --cache-dir "$(CARTULARY_READINESS_CACHE_DIR)" --disable-env CARTULARY_READINESS_DISABLE_CACHE --force-env CARTULARY_FORCE_REINSTALL --input package.json --input pnpm-lock.yaml --input apps/web/package.json --input scripts/playwright-install.sh --input $(PLAYWRIGHT_INSTALL_IMPL) --input scripts/cache-artifact.sh --output "$(PLAYWRIGHT_INSTALL_STAMP)" --key "node_version=$(NODE_VERSION)" --key "pnpm_version=$(PNPM_VERSION)" -- env PLAYWRIGHT_INSTALL_STAMP="$(PLAYWRIGHT_INSTALL_STAMP)" RUN_PHASE_SCRIPT="$(RUN_PHASE_SCRIPT)" NODE_RUNTIME_DIR="$(NODE_RUNTIME_DIR)" NODE_BIN="$(NODE_BIN)" PNPM="$(PNPM)" NODE_VERSION="$(NODE_VERSION)" PNPM_VERSION="$(PNPM_VERSION)" ./scripts/playwright-install.sh
...
$(SERVER_BIN): FORCE $$(SERVER_BUILD_INPUTS) $(EMBEDDED_WEB_ASSET_STAMP) $(EMBEDDED_WEB_ASSET_ARCHIVE) $(EMBEDDED_WEB_ASSET_READY_STAMP) scripts/build-go-artifact.sh $(BUILD_GO_ARTIFACT_IMPL) $(CACHE_ARTIFACT_SCRIPT)
	$(Q)CARTULARY_TEST_TARGET="build-server" $(CACHE_ARTIFACT_SCRIPT) --schema-id cartulary.cache.build_artifact.v1 --scope build-artifact --profile build-server --cache-dir "$(CARTULARY_BUILD_CACHE_DIR)" --disable-env CARTULARY_BUILD_CACHE_DISABLE --force-env CARTULARY_FORCE_REBUILD $(foreach input,$(SERVER_BUILD_INPUTS),--input "$(input)") --input-dir "$(EMBEDDED_WEB_ASSET_DIR)" --input scripts/list-build-inputs.sh --input $(LIST_BUILD_INPUTS_IMPL) --input scripts/build-go-artifact.sh --input $(BUILD_GO_ARTIFACT_IMPL) --input scripts/cache-artifact.sh --input "$(GO)" --output "$(SERVER_BIN)" --key "go=$${GO:-$(GO)}" --key "GOOS=$${GOOS:-}" --key "GOARCH=$${GOARCH:-}" --key "CGO_ENABLED=$${CGO_ENABLED:-}" --key "GOFLAGS=$${GOFLAGS:-}" -- env GO="$(GO)" BUILD_OUTPUT="$(SERVER_BIN)" BUILD_PACKAGE="./cmd/server" BUILD_LABEL="build server" CARTULARY_TEST_TARGET="build-server" GO_CACHE_DIR="$(GO_CACHE_DIR)" GO_MOD_CACHE_DIR="$(GO_MOD_CACHE_DIR)" RUN_PHASE_SCRIPT="$(RUN_PHASE_SCRIPT)" ./scripts/build-go-artifact.sh

$(MIGRATE_BIN): FORCE $$(MIGRATE_BUILD_INPUTS) scripts/build-go-artifact.sh $(BUILD_GO_ARTIFACT_IMPL) $(CACHE_ARTIFACT_SCRIPT)
	$(Q)CARTULARY_TEST_TARGET="build-migrate" $(CACHE_ARTIFACT_SCRIPT) --schema-id cartulary.cache.build_artifact.v1 --scope build-artifact --profile build-migrate --cache-dir "$(CARTULARY_BUILD_CACHE_DIR)" --disable-env CARTULARY_BUILD_CACHE_DISABLE --force-env CARTULARY_FORCE_REBUILD $(foreach input,$(MIGRATE_BUILD_INPUTS),--input "$(input)") --input scripts/list-build-inputs.sh --input $(LIST_BUILD_INPUTS_IMPL) --input scripts/build-go-artifact.sh --input $(BUILD_GO_ARTIFACT_IMPL) --input scripts/cache-artifact.sh --input "$(GO)" --output "$(MIGRATE_BIN)" --key "go=$${GO:-$(GO)}" --key "GOOS=$${GOOS:-}" --key "GOARCH=$${GOARCH:-}" --key "CGO_ENABLED=$${CGO_ENABLED:-}" --key "GOFLAGS=$${GOFLAGS:-}" -- env GO="$(GO)" BUILD_OUTPUT="$(MIGRATE_BIN)" BUILD_PACKAGE="./cmd/migrate" BUILD_LABEL="build migrate" CARTULARY_TEST_TARGET="build-migrate" GO_CACHE_DIR="$(GO_CACHE_DIR)" GO_MOD_CACHE_DIR="$(GO_MOD_CACHE_DIR)" RUN_PHASE_SCRIPT="$(RUN_PHASE_SCRIPT)" ./scripts/build-go-artifact.sh

$(OPERATOR_BIN): FORCE $$(OPERATOR_BUILD_INPUTS) scripts/build-go-artifact.sh $(BUILD_GO_ARTIFACT_IMPL) $(CACHE_ARTIFACT_SCRIPT)
	$(Q)CARTULARY_TEST_TARGET="build-operator" $(CACHE_ARTIFACT_SCRIPT) --schema-id cartulary.cache.build_artifact.v1 --scope build-artifact --profile build-operator --cache-dir "$(CARTULARY_BUILD_CACHE_DIR)" --disable-env CARTULARY_BUILD_CACHE_DISABLE --force-env CARTULARY_FORCE_REBUILD $(foreach input,$(OPERATOR_BUILD_INPUTS),--input "$(input)") --input scripts/list-build-inputs.sh --input $(LIST_BUILD_INPUTS_IMPL) --input scripts/build-go-artifact.sh --input $(BUILD_GO_ARTIFACT_IMPL) --input scripts/cache-artifact.sh --input "$(GO)" --output "$(OPERATOR_BIN)" --key "go=$${GO:-$(GO)}" --key "GOOS=$${GOOS:-}" --key "GOARCH=$${GOARCH:-}" --key "CGO_ENABLED=$${CGO_ENABLED:-}" --key "GOFLAGS=$${GOFLAGS:-}" -- env GO="$(GO)" BUILD_OUTPUT="$(OPERATOR_BIN)" BUILD_PACKAGE="./cmd/operator" BUILD_LABEL="build operator" CARTULARY_TEST_TARGET="build-operator" GO_CACHE_DIR="$(GO_CACHE_DIR)" GO_MOD_CACHE_DIR="$(GO_MOD_CACHE_DIR)" RUN_PHASE_SCRIPT="$(RUN_PHASE_SCRIPT)" ./scripts/build-go-artifact.sh

$(WEB_DIST_INDEX): FORCE $$(WEB_BUILD_INPUTS) $(FRONTEND_INSTALL_STAMP) scripts/build-web-artifact.sh $(BUILD_WEB_ARTIFACT_IMPL) $(CACHE_ARTIFACT_SCRIPT) | $(NODE_BIN)
	$(Q)CARTULARY_TEST_TARGET="build-web" $(CACHE_ARTIFACT_SCRIPT) --schema-id cartulary.cache.build_artifact.v1 --scope build-artifact --profile build-web --cache-dir "$(CARTULARY_BUILD_CACHE_DIR)" --disable-env CARTULARY_BUILD_CACHE_DISABLE --force-env CARTULARY_FORCE_REBUILD $(foreach input,$(WEB_BUILD_INPUTS),--input "$(input)") --input scripts/list-build-inputs.sh --input $(LIST_BUILD_INPUTS_IMPL) --input scripts/build-web-artifact.sh --input $(BUILD_WEB_ARTIFACT_IMPL) --input scripts/cache-artifact.sh --output-dir "$(CURDIR)/apps/web/dist" --key "node_version=$(NODE_VERSION)" --key "pnpm_version=$(PNPM_VERSION)" --key "vite_flags=$(VITE_BUILD_FLAGS)" -- env RUN_PHASE_SCRIPT="$(RUN_PHASE_SCRIPT)" NODE_RUNTIME_DIR="$(NODE_RUNTIME_DIR)" PNPM="$(PNPM)" VITE_BUILD_FLAGS="$(VITE_BUILD_FLAGS)" CARTULARY_TEST_TARGET="build-web" ./scripts/build-web-artifact.sh
